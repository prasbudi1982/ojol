  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    try{
      ...
      if(cur && cur.driver_id === currentProfile.id){
        // Order sudah di-assign ke saya -> kembalikan ke searching, kosongkan driver_id, info ke penumpang
        console.log('Reject assigned order', orderId);
        const { error } = await supabase.from('orders').update({ status: 'searching', driver_id: null, accepted_at: null }).eq('id', orderId);
        if(error) throw error;
        alert('✅ Order ditolak - dikembalikan ke pencarian driver lain. Penumpang akan diinfo.');
      } else {
        // Order broadcast (driver_id null) -> sembunyikan lokal + tandai rejected untuk driver ini
        console.log('Reject broadcast order', orderId);
        try{
        ...
        // Coba update status tetap searching biar realtime trigger ke penumpang kalau perlu (optional)
        // Tidak update driver_id karena null, tapi tetap refresh
        // Untuk broadcast, kita tidak ubah DB, hanya hide lokal
      }
      await tracking.loadDriverOrders(currentProfile);
    }catch(e){ 
