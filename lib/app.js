...
async function render(){
  ...
  if(hash==='#/login') html=session? viewHome(currentProfile) : viewLogin();
    ...
    if((currentProfile?.role||'').toLowerCase()!=='admin'){
      html='<div class="card"><h3>🔒 Akses Ditolak</h3><p class="muted">Hanya admin. SQL: update users set role=\'admin\' where email=\'emailmu@gmail.com\'</p><p class="muted">Role kamu: '+(currentProfile?.role||'-')+'</p></div>';
    } else {
