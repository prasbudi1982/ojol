...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnToggleDisc') && false){ // disabled - handled inline in views.js
    e.preventDefault();
    const btn = e.target.closest('#btnToggleDisc');
    const shortEl = document.getElementById('discShort');
    const fullEl = document.getElementById('discFull');
    ...
    if(isExpanded){
      ...
      btn.innerHTML = '📖 Baca selengkapnya <span style="font-size:10px">▼</span>';
    } else {
      shortEl.style.display = 'none';
      fullEl.style.display = 'block';
      btn.setAttribute('data-expanded','true');
      btn.innerHTML = '🔼 Tutup <span style="font-size:10px">▲</span>';
    }
    return;
  }
