...
// ====== POPUP NOTIFIKASI PWA STYLE - ALWAYS ACTIVE ======
function createNotifPopup() {
  if(document.getElementById('notifPermissionBanner')) return null;
  const popup = document.createElement('div');
  popup.id = 'notifPermissionBanner';
  popup.innerHTML = `
    <div style="position:fixed;bottom:20px;left:12px;right:12px;z-index:99999;background:var(--card);border:1px solid var(--border);border-radius:20px;box-shadow:0 12px 32px rgba(0,0,0,.35);padding:16px;max-width:420px;margin:0 auto;transform:translateY(150%);transition:transform 0.4s cubic-bezier(0.34,1.56,0.64,1);">
      <div style="display:flex;gap:12px;align-items:flex-start">
        <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,var(--primary) 0%, #16a34a 100%);display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0">🔔</div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:800;font-size:14px;color:var(--text);line-height:1.2">Aktifkan Notifikasi</div>
          <div style="font-size:12px;color:var(--muted);margin-top:4px;line-height:1.4">Dapat order masuk walau HP terkunci. Driver & warung wajib aktifkan.</div>
          <div style="display:flex;gap:8px;margin-top:12px">
            <button id="btnPopupEnableNotif" style="flex:1;background:var(--primary);color:white;border:none;padding:10px 14px;border-radius:12px;font-weight:800;font-size:13px;cursor:pointer">🔔 Aktifkan</button>
            <button id="btnPopupCloseNotif" style="background:var(--card2);border:1px solid var(--border);color:var(--text);padding:10px 14px;border-radius:12px;font-weight:700;font-size:13px;cursor:pointer">Nanti</button>
          </div>
          <div style="font-size:10px;color:var(--muted);margin-top:8px;text-align:center">Popup ini muncul sekali sampai diaktifkan</div>
        </div>
      </div>
    </div>
  `;
  document.body.appendChild(popup);
  setTimeout(()=> { const inner = popup.firstElementChild; if(inner) inner.style.transform = 'translateY(0)'; }, 100);
  return popup;
}
function hideNotifPopup(permanent=false) {
  const banner = document.getElementById('notifPermissionBanner');
  if(!banner) return;
  const inner = banner.firstElementChild;
  if(inner) inner.style.transform = 'translateY(150%)';
  setTimeout(()=> { try{ banner.remove(); }catch(e){} }, 400);
  if(permanent){ localStorage.setItem('notif-popup-dismissed', Date.now().toString()); }
}
async function checkAndShowNotifPopup(currentProfile) {
  try{
    if(!('Notification' in window)) return false;
    const perm = Notification.permission;
    if(perm === 'granted'){
      // Auto aktifkan realtime tanpa popup
      if(currentProfile && push.forceEnablePush){
        push.forceEnablePush(currentProfile).catch(()=>{});
      } else if(currentProfile){
        try{
          if(push.listenRealtimeOrders) push.listenRealtimeOrders(currentProfile);
          if(push.listenRealtimeFood2Arah) push.listenRealtimeFood2Arah(currentProfile);
          if(push.listenRealtimeOjol2Arah) push.listenRealtimeOjol2Arah(currentProfile);
        }catch(e){}
      }
      return false;
    }
    if(perm === 'denied'){
      const last = parseInt(localStorage.getItem('notif-denied-popup')||'0');
      if(Date.now() - last > 7*24*60*60*1000){
        const p = createNotifPopup();
        if(p){
          p.querySelector('#btnPopupEnableNotif').textContent = '⚙️ Buka Settings';
          p.querySelector('#btnPopupEnableNotif').onclick = ()=>{
            alert('Notifikasi diblokir. Cara aktifkan:\n1. Chrome > ⋮ > Settings\n2. Site Settings > Notifications\n3. Cari ojol-suruh.pages.dev > Allow\n4. Reload');
            hideNotifPopup();
            localStorage.setItem('notif-denied-popup', Date.now().toString());
          };
        }
      }
      return true;
    }
    const dismissed = localStorage.getItem('notif-popup-dismissed');
    const autoActive = localStorage.getItem('push-auto-active');
    if(!dismissed && !autoActive){
      const popup = createNotifPopup();
      if(!popup) return false;
      const btnEnable = document.getElementById('btnPopupEnableNotif');
      const btnClose = document.getElementById('btnPopupCloseNotif');
      if(btnEnable){
        btnEnable.onclick = async ()=>{
          btnEnable.textContent = '⏳ Meminta izin...';
          ...
          try{
            ...
            if(result === 'granted'){
              if(currentProfile){
                try{
                  if(push.subscribeUser) await push.subscribeUser(currentProfile.id);
                  if(push.forceEnablePush) await push.forceEnablePush(currentProfile);
                }catch(e){}
              }
              if(push.showLocalNotification){
                push.showLocalNotification('✅ Notifikasi Aktif!', 'Kamu akan dapat order masuk walau HP terkunci', '/#/');
              }
              localStorage.setItem('push-auto-active','1');
              localStorage.setItem('notif-popup-dismissed','granted');
              hideNotifPopup(true);
            } else {
              btnEnable.textContent = '❌ Ditolak';
              setTimeout(()=> hideNotifPopup(), 1500);
            }
          }catch(e){
            btnEnable.textContent = '❌ Gagal';
            btnEnable.disabled = false;
          }
        };
      }
      if(btnClose){
        btnClose.onclick = ()=>{ hideNotifPopup(true); };
      }
      return true;
    }
    if(dismissed){
      const lastDismiss = parseInt(dismissed);
      if(!isNaN(lastDismiss) && Date.now() - lastDismiss > 24*60*60*1000){
        if(Notification.permission !== 'granted'){
          localStorage.removeItem('notif-popup-dismissed');
          return await checkAndShowNotifPopup(currentProfile);
        }
      }
    }
    return false;
  }catch(e){ console.error('popup fail', e); return false; }
}
async function initAlwaysActivePush(profile){
  if(!profile) return;
  try{
    localStorage.setItem('push-auto-active','1');
    if(push.forceEnablePush){
      await push.forceEnablePush(profile);
    } else {
      try{
        if(push.listenRealtimeOrders) push.listenRealtimeOrders(profile);
        if(push.listenRealtimeFood2Arah) push.listenRealtimeFood2Arah(profile);
      }catch(e){}
    }
    setTimeout(()=>{
      const el = document.getElementById('pushStatusText');
      if(el && Notification.permission==='granted'){ el.textContent='✅ Aktif (Auto)'; el.style.color='#16a34a'; }
    },500);
  }catch(e){}
}
window.checkAndShowNotifPopup = checkAndShowNotifPopup;
window.createNotifPopup = createNotifPopup;
window.hideNotifPopup = hideNotifPopup;
window.initAlwaysActivePush = initAlwaysActivePush;
// ====== END POPUP ======
...
async function render(){
  ...
  if(session){ try{ currentProfile=await getProfile(); userBadge.textContent=`${currentProfile?.name||''} • ${currentProfile?.role}`; userBadge.classList.remove('hidden'); }catch(e){} 
      // ===== POPUP PAKSA NOTIFIKASI SAAT PERTAMA BUKA =====
      if(currentProfile){
        setTimeout(()=> checkAndShowNotifPopup(currentProfile), 1200);
        setTimeout(()=> initAlwaysActivePush(currentProfile), 1500);
      } }
