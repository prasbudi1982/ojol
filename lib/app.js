...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    try{
      ...
      // Jika merchant, upsert ke tabel stores biar lat/lng di stores sinkron (sesuai screenshot tabel stores)
      if(role==='merchant' || role==='warung'){
        try{
          ...
          const storePayload = {
            ...
            name: warungName,
            desa: desaMerchant,
            alamat: alamatMerchant,
            lat: parseFloat(merchantLatRaw),
            lng: parseFloat(merchantLngRaw),
            is_active: true
          };
          if(existingStore){
        ...
        }catch(eStore){ console.warn('stores sync failed', eStore.message); }
      }
