    ...
    if(!currentProfile){
      ...
      try{
        if(currentProfile.role==='driver'){
          const orders = await history.getDriverHistory(currentProfile.id, 30);
          html = history.viewHistoryDriver(orders);
        } else {
          const orders = await history.getPassengerHistory(currentProfile.id, 30);
  ...
  if(e.target.closest('#btnCompleteOrder')){
    ...
    if(oid){
      try{ 
        ...
        orderData = data;
        await supabase.from('orders').update({status:'completed'}).eq('id', oid); 
      }catch(e){}
    }
    tracking.clearActiveTracking();
    const card = document.getElementById('activeOrderCard');
    if(card) card.style.display='none';
    // Jika ada driver, buka modal rating
    if(orderData && orderData.driver_id){
      try{
        const { data: drv } = await supabase.from('users').select('name').eq('id', orderData.driver_id).single();
