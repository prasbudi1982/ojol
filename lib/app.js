...
import { viewDriver } from './app/tracking.js';
...
async function render(){
  ...
  const navHome=document.querySelector('a[href="#/"]'); const navPass=document.querySelector('a[href="#/passenger"]'); const navDriver=document.querySelector('a[href="#/driver"]'); const navProfile=document.querySelector('a[href="#/profile"]'); const navAdminChk=document.querySelector('a[href="#/admin"]'); const navAdminSet=document.querySelector('a[href="#/admin/settings"]');
  if(currentProfile){
    if(currentProfile.role==='admin'){ if(navHome) navHome.style.display='none'; if(navPass) navPass.style.display='none'; if(navDriver) navDriver.style.display='none'; if(navProfile) navProfile.style.display='none'; if(navAdminChk) navAdminChk.style.display=''; if(navAdminSet) navAdminSet.style.display=''; if(hash.startsWith('#/passenger')||hash.startsWith('#/driver')||hash.startsWith('#/profile')||hash==='#/'||hash===''){ location.hash='#/admin'; return; } }
    else if(currentProfile.role==='driver'){ if(navHome) navHome.style.display=''; if(navPass) navPass.style.display='none'; if(navDriver) navDriver.style.display=''; if(navProfile) navProfile.style.display=''; if(navAdminChk) navAdminChk.style.display='none'; if(navAdminSet) navAdminSet.style.display='none'; if(hash.startsWith('#/passenger')||hash.startsWith('#/admin')){ location.hash='#/'; return; } }
    else { if(navHome) navHome.style.display=''; if(navDriver) navDriver.style.display='none'; if(navPass) navPass.style.display=''; if(navProfile) navProfile.style.display=''; if(navAdminChk) navAdminChk.style.display='none'; if(navAdminSet) navAdminSet.style.display='none'; if(hash.startsWith('#/driver')||hash.startsWith('#/admin')){ location.hash='#/'; return; } }
  } else { if(navHome) navHome.style.display=''; if(navPass) navPass.style.display=''; if(navDriver) navDriver.style.display=''; if(navProfile) navProfile.style.display=''; }
  ...
  if(hash.startsWith('#/driver') && currentProfile){
    ...
    try{ tracking.initDriverPage(currentProfile); }catch(e){ console.warn('initDriverPage fail', e); }
    ...
    if(btnOn) btnOn.onclick=async ()=>{
      ...
      try{ tracking.initDriverPage(currentProfile); }catch(e){}
    ...
    if(btnOff) btnOff.onclick=async ()=>{ 
      ...
      try{ tracking.clearDriverOrdersSubscription(); }catch(e){}
    };
  }
...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnRefreshDriverOrders')){
    e.preventDefault();
    try{ await tracking.loadDriverOrders(currentProfile); }catch(e){ console.warn(e); }
    return;
  }
  if(e.target.closest('#btnDebugOrders')){
    e.preventDefault();
    try{
      const { data, error } = await supabase.from('orders').select('*').order('created_at',{ascending:false}).limit(10);
      console.log('DEBUG all orders', data, error);
      alert('Debug: ' + JSON.stringify(data?.slice(0,2)||[], null, 2).slice(0,500) + '\nError: ' + (error?.message||'none'));
      // Tampilkan di box juga
      const box = document.getElementById('driverOrders');
      if(box && data){
        box.innerHTML = '<div style="font-size:10px;white-space:pre-wrap;background:#111;color:#0f0;padding:8px;overflow:auto;max-height:300px">'+JSON.stringify(data, null, 2).slice(0,3000)+'</div><button id="btnRefreshDriverOrders" class="btn secondary" style="margin-top:8px">🔄 Refresh</button>';
      }
  ...
  if(e.target.closest('[data-driver-accept]')){
    ...
    try{
      ...
      await tracking.loadDriverOrders(currentProfile);
  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    try{
      // Kalau order untuk saya, ubah jadi searching lagi biar bisa diambil driver lain
      const { data: cur } = await supabase.from('orders').select('driver_id').eq('id', orderId).single();
      if(cur && cur.driver_id === currentProfile.id){
        await supabase.from('orders').update({ status: 'searching', driver_id: null }).eq('id', orderId);
      }
      await tracking.loadDriverOrders(currentProfile);
