...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    // merchant fields
    const warungName=document.getElementById('profileWarungName')?.value?.trim()||null;
    const desaMerchant=document.getElementById('profileDesaMerchant')?.value?.trim()||null;
    const alamatMerchant=document.getElementById('profileAlamatMerchant')?.value?.trim()||null;
    const merchantLatRaw=document.getElementById('profileMerchantLat')?.value?.trim()||'';
    const merchantLngRaw=document.getElementById('profileMerchantLng')?.value?.trim()||'';
    ...
    if(role==='merchant' || role==='warung'){
      ...
      if(!merchantLatRaw || !merchantLngRaw) return alert('Lokasi Maps wajib - tap Pilih Lokasi Warung di Peta');
      const la=parseFloat(merchantLatRaw), ln=parseFloat(merchantLngRaw);
      if(isNaN(la)||isNaN(ln)) return alert('Koordinat Maps tidak valid');
      try{ if(typeof isInSuruhBbox==='function' && !isInSuruhBbox(la,ln)) return alert('Lokasi Warung harus di dalam area '+getActiveKecamatanName()); }catch(e){}
    }
    st.textContent='Menyimpan...';
    try{
      // HINDARI kolom lat/lng karena tidak ada di schema, pakai merchant_lat/merchant_lng saja
      const patch={
        ...
        merchant_lat: (role==='merchant'||role==='warung')?parseFloat(merchantLatRaw):null,
        merchant_lng: (role==='merchant'||role==='warung')?parseFloat(merchantLngRaw):null,
      };
