...
import { viewAdminDashboard, viewAdminSettings } from './app/dashboard.js';
...
// ====== POPUP CEK NOTIFIKASI SAAT APP DIBUKA - FINAL (HANYA TAMBAHAN) ======
...
window.addEventListener('hashchange', ()=>{ if(nearbyChannel) supabase.removeChannel(nearbyChannel); 
  const h=location.hash||'#/'; if(!h.startsWith('#/passenger')){ /* keep tracking in bg */ } render(); setTimeout(()=>checkNotifOnAppOpen(), 800); });
