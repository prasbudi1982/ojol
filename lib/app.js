...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('[data-driver-accept]')){
    ...
    try{
      const res = await supabase.from('orders').update({ status: 'accepted', driver_id: currentProfile.id, accepted_at: new Date().toISOString() }).eq('id', orderId).select().single();
  ...
  if(e.target.closest('[data-driver-picked]')){
    ...
    try{
      await supabase.from('orders').update({ status: 'picked', picked_at: new Date().toISOString() }).eq('id', orderId);
