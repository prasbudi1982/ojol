...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    // === VALIDASI MERCHANT ===
    let warungName='', desaMerchant='', alamatMerchant='', merchantLat='', merchantLng='';
    if(role==='merchant' || role==='warung'){
      warungName = document.getElementById('profileWarungName')?.value?.trim()||'';
      desaMerchant = document.getElementById('profileDesaMerchant')?.value?.trim()||'';
      alamatMerchant = document.getElementById('profileAlamatMerchant')?.value?.trim()||'';
      merchantLat = document.getElementById('profileMerchantLat')?.value?.trim()||'';
      merchantLng = document.getElementById('profileMerchantLng')?.value?.trim()||'';
      if(!warungName) return alert('Nama Warung wajib untuk Merchant');
      if(!desaMerchant) return alert('Desa / Dusun wajib untuk Merchant');
      if(!alamatMerchant) return alert('Alamat Lengkap Warung wajib untuk Merchant');
      if(!merchantLat || !merchantLng) return alert('Lokasi Maps wajib - tap Pilih Lokasi Warung di Peta');
      const latNum = parseFloat(merchantLat), lngNum = parseFloat(merchantLng);
      if(isNaN(latNum) || isNaN(lngNum)) return alert('Koordinat Maps tidak valid');
      // validasi geofence Suruh kalau fungsi ada
      try{ if(typeof isInSuruhBbox==='function' && !isInSuruhBbox(latNum,lngNum)) return alert('Lokasi Warung harus di dalam area '+getActiveKecamatanName()); }catch(e){}
    }
    ...
    try{
      const patch={
        ...
        hp:hp,
        desa: (role==='passenger'?desa : (role==='merchant'||role==='warung'?desaMerchant:null)),
        alamat: (role==='passenger'?alamat : (role==='merchant'||role==='warung'?alamatMerchant:null)),
        ...
        warung_name: (role==='merchant'||role==='warung')?warungName:null,
        lat: (role==='merchant'||role==='warung')?parseFloat(merchantLat):null,
        lng: (role==='merchant'||role==='warung')?parseFloat(merchantLng):null,
        merchant_lat: (role==='merchant'||role==='warung')?parseFloat(merchantLat):null,
        merchant_lng: (role==='merchant'||role==='warung')?parseFloat(merchantLng):null,
      };
