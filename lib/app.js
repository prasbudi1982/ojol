...
// === PATCH 5 Driver Terdekat di Keranjang - Inject via DOM, tidak ubah storeViews.js ===
window._loadNearbyDriversForCart = async function(){
  var listEl = document.getElementById('cartNearbyDriversList');
  if(!listEl){
    // Jika belum ada container, buat dulu di bawah alamat
    var destBox = document.getElementById('cartLatLngDisplay');
    if(!destBox) return;
    var parent = destBox.closest('.card') || document.body;
    // Cari tempat untuk inject: setelah box alamat
    var section = document.getElementById('cartNearbyDriversSection');
    if(!section){
      section = document.createElement('div');
      section.id = 'cartNearbyDriversSection';
      section.style.cssText = 'margin-top:14px;border:1px solid #f59e0b;border-radius:14px;padding:12px;background:var(--card)';
      section.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center"><b style="font-size:13px">5 Driver Terdekat dari Warung (Aktif)</b><button id="btnRefreshDrivers" style="font-size:11px;background:var(--card2);border:1px solid var(--border);padding:4px 8px;border-radius:8px">Refresh</button></div><div class="muted" style="font-size:10px;margin-top:4px">Driver aktif 15 menit terakhir, urut dari warung. Pilih driver atau Broadcast.</div><div id="cartNearbyDriversList" style="margin-top:10px"><div class="muted" style="padding:12px;text-align:center;font-size:11px">Pilih alamat antar dulu</div></div><div style="display:flex;gap:8px;margin-top:10px"><button id="btnCheckoutBroadcast" class="btn" style="flex:1;background:var(--card2);border:1px solid var(--border);padding:10px;border-radius:10px;font-size:11px;font-weight:700">Cari Driver Lain (Broadcast)</button><button id="btnCheckout2" class="btn primary" style="flex:1;padding:12px;border-radius:12px;font-weight:800;font-size:12px">Pesan dan Broadcast</button></div>';
      // Insert after alamat box
      var addrBox = document.getElementById('cartLatLngDisplay');
      if(addrBox && addrBox.parentElement){
        addrBox.parentElement.parentElement.insertAdjacentElement('afterend', section);
      } else {
        parent.appendChild(section);
      }
      listEl = document.getElementById('cartNearbyDriversList');
    } else {
      listEl = document.getElementById('cartNearbyDriversList');
    }
  }
  try{
    ...
    for(var d=0; d<drivers.length; d++){
      ...
      html += '<div style="border:1px solid var(--border);border-radius:12px;padding:10px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;background:' + (d===0 ? '#f0fdf4' : 'var(--card2)') + '">'
        + '<div style="flex:1"><div style="font-weight:800;font-size:12px">' + (d===0 ? '⭐ ' : '') + (usr.name||'Driver') + '</div><div style="font-size:10px" class="muted">' + (usr.jenis_kendaraan||'') + ' ' + (usr.nopol||'') + ' - ' + dr.dist.toFixed(2) + ' km - ' + onlineAgo + ' mnt lalu</div></div>'
        + '<button onclick="window._checkoutFoodDirect(\'' + usr.id + '\')" style="background:#22c55e;color:#052e16;border:none;padding:8px 12px;border-radius:10px;font-weight:800;font-size:11px">Pilih</button></div>';
    }
...
window._checkoutFoodDirect = async function(preferredDriverId){
  ...
  try{
    ...
    else { var tf = await import('./app/trackingFood.js'); tf.openFoodTrackingDetailModal(od.id); }
  }catch(e){ alert('Gagal: '+e.message); }
};

// Auto inject saat di keranjang
(function(){
  function tryLoad(){
  ...
  document.addEventListener('click', function(e){
    ...
    if(e.target && e.target.id==='btnCheckout2'){ window._checkoutFood(); }
    if(e.target && e.target.closest && e.target.closest('#btnCheckout2')){ window._checkoutFood(); }
  });
})();
