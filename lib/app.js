...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    if(!confirm('Tolak order ini? Penumpang akan diinfo penolakan.')) return;
    const origText = btn.textContent;
    btn.disabled = true; btn.textContent = '⏳...';
    try{
      const { data: cur, error: curErr } = await supabase.from('orders').select('driver_id, status, passenger_id').eq('id', orderId).single();
      if(curErr) throw curErr;
      
      if(cur && cur.driver_id === currentProfile.id){
        console.log('Reject assigned order', orderId, '-> notify passenger');
        // Coba update - jika RLS blok driver_id null, coba dengan status searching saja, atau pakai RPC fallback
        let updError = null;
        try{
          const { error } = await supabase.from('orders').update({ status: 'searching', driver_id: null, accepted_at: null }).eq('id', orderId);
          if(error) throw error;
        }catch(e){
          updError = e;
          // Fallback: coba update status saja ke searching, biarkan trigger / RLS lain yang kosongkan driver_id, atau coba set driver_id ke null via 2 step
          console.warn('First reject update failed', e.message, 'try fallback');
          try{
            const { error: e2 } = await supabase.from('orders').update({ status: 'searching' }).eq('id', orderId);
            if(e2) throw e2;
            // Coba lagi null kan driver_id
            const { error: e3 } = await supabase.from('orders').update({ driver_id: null }).eq('id', orderId);
            if(e3) console.warn('null driver_id failed', e3.message);
          }catch(e2){
            throw e2;
          }
        }
        // Simpan ke rejected lokal agar tidak muncul lagi di driver ini
        try{
          const rejected = JSON.parse(localStorage.getItem('rejected_orders_'+currentProfile.id)||'[]');
          if(!rejected.includes(orderId)){ rejected.push(orderId); localStorage.setItem('rejected_orders_'+currentProfile.id, JSON.stringify(rejected)); }
        }catch(e){}
        alert('✅ Order ditolak - penumpang sudah diinfo: Driver menolak, mencari driver lain.');
      } else {
        // Order broadcast (driver_id null) -> sembunyikan lokal, tidak perlu info ke penumpang karena belum diterima
        console.log('Reject broadcast order', orderId, '- hide local only, tidak info penumpang karena belum di-assign');
        try{
          const rejected = JSON.parse(localStorage.getItem('rejected_orders_'+currentProfile.id)||'[]');
          if(!rejected.includes(orderId)){ rejected.push(orderId); localStorage.setItem('rejected_orders_'+currentProfile.id, JSON.stringify(rejected)); }
        }catch(e){}
