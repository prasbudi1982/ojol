  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    try{
      ...
      if(cur && cur.driver_id === currentProfile.id){
        console.log('Reject assigned order', orderId, '-> notify passenger via status searching (RLS safe)');
        // FIX RLS: driver TIDAK boleh set driver_id=null (WITH CHECK gagal). Cukup set status=searching, biarkan driver_id tetap.
        // Penumpang yang akan null-kan driver_id sendiri (passenger owns order, lolos RLS)
        ...
        alert('✅ Order ditolak - penumpang akan otomatis dapat info dan cari driver lain.');
      } else {
