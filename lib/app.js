...
// --- STORE: lazy only - FIX fallback untuk Pages ---
let lastStoreError = '';
let storeMods = null;
async function getStoreMods(){
  ...
  try{
    let idx, views;
    try{
      const [i1, v1] = await Promise.all([
        import('./app/store/index.js'),
        import('./app/storeViews.js')
      ]);
      idx = i1; views = v1;
    }catch(e1){
      const [i2, v2] = await Promise.all([
        import('./app/store/index.js'),
    ...
    window.cartStore = idx.cartStore;
...
async function render(){
  ...
  if(currentProfile){
    ...
    else { if(navHome) navHome.style.display=''; if(navDriver) navDriver.style.display='none'; if(navPass) navPass.style.display=''; if(navProfile) navProfile.style.display=''; if(navHist) navHist.style.display=''; if(navAdminChk) navAdminChk.style.display='none'; if(navAdminSet) navAdminSet.style.display='none'; const navFoodP = document.querySelector('a[href="#/store"]'); if(navFoodP) navFoodP.style.display=''; if(hash.startsWith('#/driver')||hash.startsWith('#/admin')||hash.startsWith('#/store/my')||hash.startsWith('#/store/products')||hash.startsWith('#/store/orders')){ location.hash='#/'; return; } }
  } else { if(navHome) navHome.style.display=''; if(navPass) navPass.style.display=''; if(navDriver) navDriver.style.display=''; if(navProfile) navProfile.style.display=''; if(navHist) navHist.style.display='none'; const navFoodG = document.querySelector('a[href="#/store"]'); if(navFoodG) navFoodG.style.display=''; }
  ...
  if(hash.startsWith('#/store')){
    ...
    if(!mods){
      const errTxt = lastStoreError || localStorage.getItem("last_store_error") || "File lib/app/store/index.js atau lib/app/storeViews.js tidak ditemukan / syntax error. Pastikan ada dan schema.sql sudah dijalankan.";

      html = `<div class="card"><h2>🍔 Food</h2><p class="muted">Modul Food belum terpasang atau tabel stores belum ada. Login Ojol tetap jalan.</p><a href="#/" class="btn primary">Home</a></div>`;
    } else {
  ...
  // Passenger hooks - Ojek + Food tracking
  if(hash.startsWith('#/passenger')){
    setTimeout(()=>{ tracking.loadActiveTracking(); trackingFood.loadFoodActiveTracking?.(); },800);
...
document.addEventListener('click', async (e)=>{
  ...
  // ===== FOOD: Peta, 5 driver terdekat seperti order.js, Batal & Selesai - TAMBAHAN OPSI A =====
  // Peta Food - Pilih di Peta
        ...
        window.mapModule.openMapPicker('dest', async (picked)=>{
          try{
            ...
            if(raw){
              ...
              const dt=document.getElementById('destText'); if(dt) dt.value=s.dest.text;
              setTimeout(()=>location.reload(),300);
            }
          }catch(err){}
        });
      }
    }catch(err){ alert('Peta error: '+err.message); }
    return;
  }
  if(e.target.closest('#btnPickWarungLoc')){
    ...
    if(window.mapModule && window.mapModule.openMapPicker){
      window.mapModule.openMapPicker('warung', (picked)=>{
        const latEl=document.getElementById('sLat'), lngEl=document.getElementById('sLng'), alamatEl=document.getElementById('sAlamat');
  ...
  if(e.target.closest('#btnUseMyLocation')){
    ...
    navigator.geolocation.getCurrentPosition((pos)=>{
      ...
      try{
        const raw=localStorage.getItem('ojol_cart_v2_food'); if(raw){ const s=JSON.parse(raw); s.dest={lat:lat,lng:lng,text:s.dest.text||''}; localStorage.setItem('ojol_cart_v2_food', JSON.stringify(s)); location.reload(); }
      }catch(err){ alert(err.message); }
    }, (err)=>alert('GPS error: '+err.message), { enableHighAccuracy:true, timeout:8000 });
    return;
  }
  ...
  if(e.target.closest('#btnCheckout')){
    ...
    try{
      ...
      if(!profile) return alert('Harus login');
      const state = mods.cartStore.getState();
      if(!state.items.length) return alert('Keranjang kosong');
      ...
      try{ const raw=localStorage.getItem('ojol_cart_v2_food'); if(raw){ const s=JSON.parse(raw); s.dest.text=destText; localStorage.setItem('ojol_cart_v2_food', JSON.stringify(s)); } }catch(e){}
      ...
      payload.status='searching_driver'; payload.dest_text=destText;
      const { data: orderData, error } = await supabase.from('food_orders').insert(payload).select().single();
      if(error) throw error;
      mods.cartStore._actions.clear();
      try{ localStorage.setItem('active_food_order_id', orderData.id); }catch(e){}
      trackingFood.startFoodTracking(orderData.id);
      // Show modal dengan 5 driver terdekat
      let old=document.getElementById('foodCheckoutDriversModal'); if(old) old.remove();
      const themeColor = (()=>{ try{ const s=JSON.parse(localStorage.getItem('app_settings')||'{}'); return s.primaryColor||'#16a34a'; }catch(e){ return '#16a34a'; } })();
      const div=document.createElement('div'); div.id='foodCheckoutDriversModal'; div.style.cssText='position:fixed;inset:0;background:rgba(15,23,42,0.85);backdrop-filter:blur(8px);z-index:9999;display:flex;align-items:flex-end;justify-content:center;padding:0';
      div.innerHTML='<div style="background:var(--card);width:100%;max-width:520px;max-height:90vh;overflow:auto;border-radius:20px 20px 0 0;border-top:2px solid ' + themeColor + '"><div style="padding:16px 16px 8px;position:sticky;top:0;background:var(--card);border-bottom:1px solid var(--border)"><div style="width:40px;height:4px;background:var(--border);border-radius:99px;margin:0 auto 12px"></div><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:800;font-size:16px">🍔 Pesanan #' + orderData.id.slice(0,6).toUpperCase() + '</div><div style="font-size:12px" class="muted">Rp ' + Number(orderData.total||0).toLocaleString() + ' • Cari driver terdekat</div><div style="font-size:11px" class="muted">⏱️ Timer 5 menit auto batal seperti Ojol</div></div><button id="btnCloseFoodCheckout" style="background:var(--card2);border:1px solid var(--border);padding:8px 12px;border-radius:10px">✕</button></div></div><div style="padding:16px"><div style="background:var(--card2);border:1px solid var(--border);border-radius:12px;padding:12px;margin-bottom:12px"><div style="font-size:12px;font-weight:700">📍 Tracking Aktif</div><div style="font-size:11px" class="muted">Status: <b id="foodCheckoutStatus">Mencari driver...</b></div><div style="margin-top:10px;display:flex;gap:8px"><button id="btnFoodCheckoutTracking" style="flex:1;background:' + themeColor + ';color:white;border:none;padding:10px;border-radius:10px;font-weight:700">📍 Detail Tracking</button><button id="btnFoodCheckoutCancel" style="flex:1;background:var(--card);border:1px solid #ef4444;color:#ef4444;padding:10px;border-radius:10px;font-weight:700">❌ Batalkan Pesanan</button></div><button id="btnFoodCheckoutComplete" style="width:100%;margin-top:8px;background:#0ea5e9;color:white;border:none;padding:10px;border-radius:10px;font-weight:700;display:none">✅ Selesai - Pesanan Diterima</button></div><div style="font-size:13px;font-weight:800;margin-bottom:8px">🏍️ 5 Driver Terdekat dari Warung (opsi cepat seperti order.js)</div><div id="foodNearbyDriversList"><div style="padding:20px;text-align:center" class="muted">🔍 Mencari driver terdekat dari warung...</div></div></div></div>';
      document.body.appendChild(div);
      document.getElementById('btnCloseFoodCheckout').onclick=()=>div.remove();
      document.getElementById('btnFoodCheckoutTracking').onclick=()=>trackingFood.openFoodTrackingDetailModal(orderData.id);
      document.getElementById('btnFoodCheckoutCancel').onclick=async()=>{ if(!confirm('Batalkan pesanan?')) return; try{ await supabase.from('food_orders').update({status:'cancelled'}).eq('id',orderData.id); }catch(e){} trackingFood.clearFoodTracking(); div.remove(); alert('❌ Dibatalkan'); };
      document.getElementById('btnFoodCheckoutComplete').onclick=async()=>{ if(!confirm('Selesaikan?')) return; try{ await supabase.from('food_orders').update({status:'completed'}).eq('id',orderData.id); }catch(e){} trackingFood.clearFoodTracking(); div.remove(); alert('✅ Selesai'); };
      // Load 5 driver terdekat - logic sama kayak searchNearby di order.js
      try{
        ...
        if(!locs || !locs.length){ if(listEl) listEl.innerHTML='<div class="muted" style="padding:12px;text-align:center">Tidak ada driver online. Menunggu driver lihat di dashboard driver.<br/>Timer 5 menit auto batal seperti Ojol.</div>'; if(statusEl) statusEl.textContent='Menunggu driver...'; }
        else{
          ...
          if(listEl){
            listEl.innerHTML=drivers.map(d=>{
              ...
              return '<div style="background:var(--card);border:1px solid var(--border);border-radius:16px;padding:12px;margin-bottom:8px;display:flex;gap:10px"><div style="flex:1"><div style="font-weight:800;font-size:13px">' + d.name + ' ' + vehIcon + ' <span style="background:#22c55e;color:#052e16;padding:2px 6px;border-radius:6px;font-size:10px">' + d.distance_km.toFixed(2) + ' km dari warung</span></div><div style="font-size:11px" class="muted">' + (d.nopol||'') + '</div></div><div style="display:flex;flex-direction:column;gap:6px"><button data-order-food-driver=\'' + d.driver_id + '\' data-order-id=\'' + orderData.id + '\' style="background:#22c55e;color:#052e16;border:none;padding:10px 12px;border-radius:10px;font-weight:800;font-size:12px">✅ Pilih Cepat</button></div></div>';
            }).join('');
          }
        }
      }catch(err){ console.error(err); }
      if(btn){ btn.disabled=false; btn.textContent='✅ Checkout - Cari Driver Terdekat'; }
    }catch(err){ alert('Checkout gagal: '+err.message); const btn=document.getElementById('btnCheckout'); if(btn){ btn.disabled=false; btn.textContent='✅ Checkout'; } }
    return;
  }
  // Food driver accept & tracking actions (batal & selesai)
  if(e.target.closest('[data-food-accept]')){
    e.preventDefault();
    const orderId=e.target.closest('[data-food-accept]').getAttribute('data-food-accept');
    try{ await supabase.from('food_orders').update({ driver_id: currentProfile.id, status: 'driver_assigned' }).eq('id', orderId).eq('status','searching_driver'); alert('✅ Diterima! Warung dapat notif masak.'); trackingFood.openFoodTrackingDetailModal(orderId); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('[data-food-picked]')){
    e.preventDefault();
    const orderId=e.target.closest('[data-food-picked]').getAttribute('data-food-picked');
    if(!confirm('Sudah ambil di warung? OTW antar?')) return;
    try{ await supabase.from('food_orders').update({ status:'picked' }).eq('id', orderId); trackingFood.openFoodTrackingDetailModal(orderId); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('[data-food-complete]')){
    e.preventDefault();
    const orderId=e.target.closest('[data-food-complete]').getAttribute('data-food-complete');
    if(!confirm('Selesaikan pesanan?')) return;
    try{ await supabase.from('food_orders').update({ status:'completed' }).eq('id', orderId); alert('✅ Selesai!'); trackingFood.clearFoodTracking(); const m=document.getElementById('foodTrackingDetailModal'); if(m) m.remove(); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('[data-order-food-driver]')){
    e.preventDefault();
    const driverId=e.target.closest('[data-order-food-driver]').getAttribute('data-order-food-driver');
    const orderId=e.target.closest('[data-order-food-driver]').getAttribute('data-order-id');
    if(!confirm('Pilih driver ini?')) return;
    try{ await supabase.from('food_orders').update({ driver_id: driverId, status:'driver_assigned' }).eq('id', orderId); alert('✅ Driver dipilih!'); trackingFood.startFoodTracking(orderId); trackingFood.openFoodTrackingDetailModal(orderId); }catch(err){ alert(err.message); }
    return;
  }
