...
import { viewLogin, viewHome, viewOnboarding, viewPassenger, viewDriver, viewProfile, viewAdminDashboard, viewAdminSettings } from './app/views.js';
...
async function render(){
  ...
  const navPass=document.querySelector('a[href="#/passenger"]'); const navDriver=document.querySelector('a[href="#/driver"]'); const navAdminChk=document.querySelector('a[href="#/admin"]'); const navAdminSet=document.querySelector('a[href="#/admin/settings"]');
  if(currentProfile){
    if(currentProfile.role==='admin'){ if(navPass) navPass.style.display='none'; if(navDriver) navDriver.style.display='none'; if(navAdminChk) navAdminChk.style.display=''; if(navAdminSet) navAdminSet.style.display=''; if(hash.startsWith('#/passenger')||hash.startsWith('#/driver')){ location.hash='#/admin'; return; } }
    else if(currentProfile.role==='driver'){ if(navPass) navPass.style.display='none'; if(navDriver) navDriver.style.display=''; if(navAdminChk) navAdminChk.style.display='none'; if(navAdminSet) navAdminSet.style.display='none'; if(hash.startsWith('#/passenger')||hash.startsWith('#/admin')){ location.hash='#/'; return; } }
    else { if(navDriver) navDriver.style.display='none'; if(navPass) navPass.style.display=''; if(navAdminChk) navAdminChk.style.display='none'; if(navAdminSet) navAdminSet.style.display='none'; if(hash.startsWith('#/driver')||hash.startsWith('#/admin')){ location.hash='#/'; return; } }
  ...
  if(hash.startsWith('#/admin')){
    if(!isAdmin){ html='<div class=\"card\" style=\"border:1px solid #ef4444\">⛔ Akses ditolak - hanya admin</div>'; }
      ...
      html = viewAdminSettings(settings);
    } else {
    ...
    setTimeout(async ()=>{
      try{
        ...
        if(kecBox){
          ...
          kecBox.innerHTML = Object.entries(KECAMATAN_DATA).map(([code, info])=>`<button data-kec="${code}" class="btn ${code===ACTIVE_KECAMATAN_CODE?'primary':'secondary'}" style="font-size:10px;padding:4px 8px">${info.name}</button>`).join('');
        }
  ...
  function renderAdminUserList(users){
    ...
    box.innerHTML = users.map(u=>{
      ...
      return `<div style="padding:8px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;${banned?'background:#fef2f2':''}">
        <div style="flex:1">
          <b>${u.name||'Tanpa Nama'}</b> <span style="font-size:10px;background:${'admin'===u.role?'#8b5cf6':u.role==='driver'?'#3b82f6':'#f59e0b'};color:white;padding:2px 6px;border-radius:8px">${u.role}</span> ${banned?'<span style="font-size:10px;background:#ef4444;color:white;padding:2px 6px;border-radius:8px">BANNED</span>':''}
          <div style="font-size:11px;color:#666">${u.email||''} • ${u.hp||''} • ${u.status||''}</div>
        </div>
  ...
  function renderAdminReports(reports){
    ...
    box.innerHTML = reports.map(r=>`
      <div style="padding:8px;border-bottom:1px solid #fecaca;background:white;border-radius:8px;margin-bottom:6px">
...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveSettings')){
    ...
    const st=document.getElementById('settingStatus'); if(st) st.textContent= ok ? '✅ Disimpan, refresh halaman' : '❌ Gagal';
    return;
  }
  if(e.target.closest('#btnResetSettings')){
    e.preventDefault(); localStorage.removeItem('app_settings'); alert('Reset, refresh halaman'); location.reload(); return;
  }
  if(e.target.closest('[data-kec]')){
    const code = e.target.closest('[data-kec]').getAttribute('data-kec');
    alert('Untuk ganti kecamatan, ubah di lib/app/config.js: ACTIVE_KECAMATAN_CODE = '+code+' Lalu deploy ulang.');
    return;
  }
