...
async function render(){
  ...
  else if(hash.startsWith('#/profile')){ if(currentProfile && (currentProfile.role==='merchant'||currentProfile.role==='warung')){ try{ const {data:s}=await supabase.from('stores').select('lat,lng,name,desa,alamat').eq('owner_id', currentProfile.id).maybeSingle(); if(s){ currentProfile._storeLat=s.lat; currentProfile._storeLng=s.lng; currentProfile._storeName=s.name; if(!currentProfile.desa) currentProfile.desa=s.desa; if(!currentProfile.alamat) currentProfile.alamat=s.alamat; } }catch(e){} } html=viewProfile(currentProfile); }
...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    if(role==='merchant' || role==='warung'){
      ...
      if(!wLat || !wLng) return alert('Lokasi Maps wajib');
    }
    st.textContent='Menyimpan...';
    try{
      const patch={ name, role, status:'active', hp, desa: role==='passenger'?desa : (role==='merchant'||role==='warung'?wDesa:desa), alamat: role==='passenger'?alamat : (role==='merchant'||role==='warung'?wAlamat:alamat), catatan: role==='passenger'?catatan:null, jenis_kendaraan: role==='driver'?jenisKendaraan:null, nopol: role==='driver'?nopol:null, tipe_sim: role==='driver'?tipeSim:null, tipe_motor: role==='driver'?tipe:null };
      const {data,error}=await supabase.from('users').update(patch).eq('id',currentProfile.id).select().single(); if(error) throw error;
      currentProfile=data;
      if(role==='merchant' || role==='warung'){
        try{
          const {data:exist}=await supabase.from('stores').select('id').eq('owner_id', currentProfile.id).maybeSingle();
          const payload={ owner_id: currentProfile.id, name: wName, desa: wDesa, alamat: wAlamat, lat: parseFloat(wLat), lng: parseFloat(wLng), is_active: true };
          if(exist) await supabase.from('stores').update(payload).eq('id', exist.id);
          else await supabase.from('stores').insert(payload);
        }catch(e){ console.warn('stores', e.message); }
      }
      userBadge.textContent=`${data.name} • ${data.role}`; st.textContent='✅ Profil disimpan'; setTimeout(()=>{ location.hash='#/'; }, 600);
