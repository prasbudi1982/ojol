...
function applyAdaptiveTheme(){
  try{
    const stored = localStorage.getItem('app_theme'); // 'auto' | 'light' | 'dark'
    ...
    try{
      ...
      if(meta){
        meta.setAttribute('content', effective==='light' ? '#ffffff' : '#0b0f14');
      }
    }catch(e){}
  }catch(e){}
}
...
function applyLiveTheme(){
  try{
    ...
    if(!saved){
      const s2 = {}; // still apply appName from default
    } else {
...
function initThemeListener(){
  try{
    // listen to HP theme change
    if(window.matchMedia){
      window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', ()=> {
        const stored = localStorage.getItem('app_theme')||'auto';
        if(stored==='auto') applyAdaptiveTheme();
      });
    }
  }catch(e){}
}
...
window.setAppTheme = function(mode){ localStorage.setItem('app_theme', mode); applyAdaptiveTheme(); location.reload(); };
