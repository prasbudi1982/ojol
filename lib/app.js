...
import { viewDriver } from './app/tracking.js?v=14-tolak-offline-per-order';
...
import * as tracking from './app/tracking.js?v=14-tolak-offline-per-order';
  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    try{
      ...
      if(cur && cur.driver_id === currentProfile.id){
        // Order sudah di-assign ke driver ini -> kembalikan ke searching dan lepas driver_id
        // Ini yang membuat driver non-aktif khusus untuk order ini, dan order kembali ke pool driver lain
        console.log('Reject assigned order', orderId, '-> set searching + driver_id=null + broadcast');
        try{
          const { error } = await supabase.from('orders').update({ status: 'searching', driver_id: null, accepted_at: null }).eq('id', orderId);
          if(error) throw error;
        }catch(e){
          console.warn('Update searching+null failed, coba status only', e.message);
          // Fallback: coba tanpa accepted_at jika kolom tidak ada
          try{ await supabase.from('orders').update({ status: 'searching', driver_id: null }).eq('id', orderId); }catch(e2){ console.warn(e2.message); }
        }
      } else {
        // Order broadcast (driver_id null) -> cukup sembunyikan lokal, tidak perlu update order
        console.log('Reject broadcast order', orderId, '- hide for this driver only');
      }
