...
import { viewLogin, viewHome, viewOnboarding, viewPassenger, viewDriver, viewProfile, viewReportModal, viewBannedInfo, viewAdminReports, viewAdminDashboard, viewAdminSettings } from './app/views.js';
import * as reportMod from './app/report.js';
import * as adminMod from './app/admin.js';
...
async function render(){
  ...
  const navPass=document.querySelector('a[href="#/passenger"]'); const navDriver=document.querySelector('a[href="#/driver"]'); const navAdmin=document.getElementById('navAdmin');
  console.log('[ROLE]', currentProfile?.role, 'admin?', (currentProfile?.role||'').toLowerCase()==='admin');
  if(currentProfile){
    const role = (currentProfile.role||'').toLowerCase().trim();
    if(role==='driver'){ 
      if(navPass) navPass.style.display='none'; 
      if(navDriver) navDriver.style.display='flex'; 
      if(navAdmin) navAdmin.style.display='none';
      if(hash.startsWith('#/passenger')){ location.hash='#/'; return; } 
    }
    else if(role==='admin'){
      // ADMIN lihat semua tab
      if(navPass) { navPass.style.display='flex'; navPass.style.visibility='visible'; }
      if(navDriver) { navDriver.style.display='flex'; navDriver.style.visibility='visible'; }
      if(navAdmin) { navAdmin.style.display='flex'; navAdmin.style.visibility='visible'; navAdmin.style.opacity='1'; }
      console.log('[ADMIN] All tabs forced visible');
    }
    else { 
      if(navDriver) navDriver.style.display='none'; 
      if(navPass) navPass.style.display='flex'; 
      if(navAdmin) navAdmin.style.display='none';
      if(hash.startsWith('#/driver')){ location.hash='#/'; return; } 
    }
  } else { 
    if(navPass) navPass.style.display='flex'; 
    if(navDriver) navDriver.style.display='flex'; 
  ...
  if(hash==='#/login') html=session? viewHome(currentProfile) : viewLogin();
    ...
    if((currentProfile?.role||'').toLowerCase()!=='admin'){
      html='<div class="card"><h3>🔒 Akses Ditolak</h3><p class="muted">Hanya admin. SQL: update users set role=\'admin\' where email=\'prasbudi.2008@gmail.com\'</p><p class="muted">Role kamu: '+(currentProfile?.role||'-')+'</p></div>';
      ...
      setTimeout(async ()=>{
        try{
          ...
          setTimeout(()=>{
            ...
            const updatePreview = ()=>{
              const baseM = parseInt(document.getElementById('setMotorBase')?.value||3000);
              const perM = parseInt(document.getElementById('setMotorPerKm')?.value||2500);
              const minM = parseInt(document.getElementById('setMotorMin')?.value||5000);
              const baseMb = parseInt(document.getElementById('setMobilBase')?.value||8000);
              const perMb = parseInt(document.getElementById('setMobilPerKm')?.value||5500);
              const minMb = parseInt(document.getElementById('setMobilMin')?.value||15000);
              const mult = parseFloat(document.getElementById('setPP')?.value||1.6);
              const calc = (base, per, min, km)=>{ let c=base+per*km; c=Math.max(c,min); return Math.round(c/500)*500; };
              if(preview) preview.textContent = `3km Motor Rp ${calc(baseM,perM,minM,3).toLocaleString('id-ID')} • PP Rp ${Math.round(calc(baseM,perM,minM,3)*mult/500)*500} | 3km Mobil Rp ${calc(baseMb,perMb,minMb,3).toLocaleString('id-ID')}`;
            };
            updatePreview();
            ['setMotorBase','setMotorPerKm','setMotorMin','setMobilBase','setMobilPerKm','setMobilMin','setPP'].forEach(id=>{ const el=document.getElementById(id); if(el) el.addEventListener('input', updatePreview); });
          }, 100);
        }catch(e){ routerEl.innerHTML = `<div class="card">❌ ${e.message}</div>`; }
      }, 100);
    } else if(hash.startsWith('#/admin/reports')){
      html='<div class="card"><p class="muted">⏳ Memuat laporan...</p></div>';
      setTimeout(async ()=>{
        try{
      ...
      setTimeout(async ()=>{
        try{
          const [stats, users, reports, banned, orders] = await Promise.all([
            ...
            adminMod.getAllUsers(100),
            reportMod.getReports(),
            reportMod.getBannedUsers(),
            adminMod.getRecentOrders(20)
          ]);
          routerEl.innerHTML = viewAdminDashboard(stats, users, reports, banned, orders);
        }catch(e){ routerEl.innerHTML = `<div class="card">❌ ${e.message}<br/><pre style="font-size:10px">${e.stack||''}</pre></div>`; }
      }, 100);
    }
  }
  ...
  if(hash.startsWith('#/driver') && currentProfile){
    ...
    if(btnOn) btnOn.onclick=async ()=>{
      ...
      const watchId=navigator.geolocation.watchPosition(async pos=>{
        ...
        if(!isInSuruhBbox(lat,lng)){ setDriverHighlight('luar Suruh - offline'); alert('⛔ Keluar dari Suruh. Auto offline.'); await supabase.from('users').update({status:'offline'}).eq('id',currentProfile.id); setDriverHighlight('offline'); if(window._watchId) navigator.geolocation.clearWatch(window._watchId); return; }
...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    try{
      // Jangan timpa admin jadi passenger/driver kalau email admin
    const isAdminEmail = ['prasbudi.2008@gmail.com'].includes((currentProfile.email||'').toLowerCase());
    if(isAdminEmail && role!=='admin'){
      console.log('[ADMIN] Email admin mencoba ganti role, force tetap admin');
      role='admin';
    }
    const patch={
  ...
  if(e.target.closest('#btnCloseReport') || e.target.closest('#btnCloseReport2')){
    e.preventDefault(); const m=document.getElementById('reportModal'); if(m) m.remove(); return;
  }
  if(e.target.closest('#btnSubmitReport')){
    ...
    await reportMod.submitReportFromUI(tid, tg, tem, tname); return;
  }
  if(e.target.closest('[data-ban-google]')){
    e.preventDefault();
    const b = e.target.closest('[data-ban-google]');
    const gId = b.getAttribute('data-ban-google'); const email = b.getAttribute('data-ban-email');
    const reportId = b.getAttribute('data-ban-report'); const reason = b.getAttribute('data-ban-reason')||'Melanggar';
    if(!confirm(`Banned permanen ${gId||email}?`)) return;
    try{ await reportMod.banUser({ googleId: gId||null, email: email||null, reason, reportId }); alert('Banned OK'); location.reload(); }catch(err){ alert(err.message); } return;
  }
  if(e.target.closest('[data-mark-reviewed]')){
    e.preventDefault(); const id = e.target.closest('[data-mark-reviewed]').getAttribute('data-mark-reviewed');
    try{ const { createClient } = await import('./supabase.js'); }catch{} // dummy
    try{ const { supabase } = await import('./supabase.js'); await supabase.from('reports').update({status:'reviewed'}).eq('id', id); alert('Reviewed'); location.reload(); }catch(err){ alert(err.message); } return;
  }
  if(e.target.closest('[data-unban-google]')){
    e.preventDefault(); const gId = e.target.closest('[data-unban-google]').getAttribute('data-unban-google');
    if(!confirm(`Unban ${gId}?`)) return; try{ await reportMod.unbanUser(gId); alert('Unbanned'); location.reload(); }catch(err){ alert(err.message); } return;
  }
  if(e.target.closest('#btnForceLogout')){ await supabase.auth.signOut(); localStorage.clear(); location.hash='#/login'; location.reload(); return; }

  if(e.target.closest('[data-change-role]')){
    e.preventDefault();
    const sel = e.target.closest('[data-change-role]');
    const userId = sel.getAttribute('data-change-role');
    const newRole = sel.value;
    if(!confirm(`Ganti role user ${userId.slice(0,8)} jadi ${newRole}?`)) return;
    try{ await adminMod.updateUserRole(userId, newRole); alert('Role updated jadi '+newRole); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('[data-ban-direct]')){
    e.preventDefault();
    const b = e.target.closest('[data-ban-direct]');
    const gId = b.getAttribute('data-ban-direct'); const email = b.getAttribute('data-ban-email-direct'); const name = b.getAttribute('data-ban-name');
    const reason = prompt(`Alasan banned ${name||email||gId}?`, 'Melanggar aturan')||'Melanggar aturan';
    if(!confirm(`Banned permanen ${name||email}?`)) return;
    try{ await reportMod.banUser({ googleId: gId||null, email: email||null, reason }); alert('Banned OK'); location.reload(); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('#btnSaveSettings')){
    ...
    try{
      ...
      await adminMod.saveAppSettings(newSettings);
      if(statusEl) statusEl.textContent='✅ Pengaturan disimpan (localStorage + supabase jika ada tabel app_settings). Reload app penumpang/driver biar efek.';
      alert('✅ Pengaturan disimpan! Tarif & coverage akan pakai localStorage. Buat tabel app_settings di Supabase biar sinkron semua device.');
