...
import './app/store/events.js'; // food module - isolasi

// ===== THEME SYNC - Sesuaikan dengan tema HP (light/dark) =====
function initSystemThemeSync(){
  const mql = window.matchMedia('(prefers-color-scheme: light)');
  const applySystemTheme = (isLight) => {
    // Update meta theme-color
    document.querySelectorAll('meta[name="theme-color"]').forEach(m=>{
      // jika ada media attribute, biarkan, tapi update yang tanpa media juga
      if(!m.getAttribute('media')){
        m.content = isLight ? '#ffffff' : '#0b0f14';
      }
    });
    // fallback: buat meta jika belum ada
    if(!document.querySelector('meta[name="theme-color"]:not([media])')){
      const meta = document.createElement('meta');
      meta.name = 'theme-color';
      meta.content = isLight ? '#ffffff' : '#0b0f14';
      document.head.appendChild(meta);
    }
    document.documentElement.setAttribute('data-theme', isLight ? 'light' : 'dark');
    // Jika ada custom primary dari admin, tetap pertahankan
    console.log('🎨 Tema HP:', isLight ? 'LIGHT' : 'DARK');
  };
...
function applyLiveTheme(){
  try{
    ...
    // Tema mengikuti HP, jadi tidak perlu set --bg di sini, CSS yang handle via prefers-color-scheme
  }catch(e){}
}
...
async function render(){
  ...
  const navMerchant = document.querySelectorAll('[data-nav-merchant]');
  const isMerchantRole = currentProfile && (currentProfile.role==='merchant' || currentProfile.role==='warung' || currentProfile.role==='owner');
  navMerchant.forEach(a=>{ a.style.display = isMerchantRole ? '' : 'none'; });
  // kalau merchant, sembunyikan tab Food umum biar fokus ke Warungku & Menu
  const navFood = document.querySelector('a[href="#/store"]');
  if(navFood && isMerchantRole){ navFood.style.display='none'; }
  ...
  if(currentProfile){
    if(currentProfile.role==='admin'){ if(navHome) navHome.style.display='none'; if(navPass) navPass.style.display='none'; if(navDriver) navDriver.style.display='none'; if(navProfile) navProfile.style.display='none'; if(navHist) navHist.style.display='none'; if(navAdminChk) navAdminChk.style.display=''; if(navAdminSet) navAdminSet.style.display=''; if(hash.startsWith('#/passenger')||hash.startsWith('#/driver')||hash.startsWith('#/profile')||hash==='#/'||hash===''||hash.startsWith('#/history')){ location.hash='#/admin'; return; } }
      ...
      const isMerchant = currentProfile.role==='merchant' || currentProfile.role==='warung' || currentProfile.role==='owner' || !!localStorage.getItem('is_merchant_'+currentProfile.id) || !!localStorage.getItem('merchant_store_id');
      if(navHome) navHome.style.display=''; if(navDriver) navDriver.style.display='none'; if(navPass) navPass.style.display=''; if(navProfile) navProfile.style.display=''; if(navHist) navHist.style.display=''; if(navAdminChk) navAdminChk.style.display='none'; if(navAdminSet) navAdminSet.style.display='none'; 
      // merchant tetap bisa order ojol + buka warung
      if(hash.startsWith('#/driver')||hash.startsWith('#/admin')){ location.hash='#/'; return; } 
    }
  ...
  if(isAdmin){ if(navHomeG) navHomeG.style.display='none'; if(navProfG) navProfG.style.display='none'; }
    ...
    let html='';
  // ===== FOOD STORE MODULE - ISOLASI, tab baru index.html #/store =====
  if(hash.startsWith('#/store')){
    try{
      // coba dua path (lib/app.js dan final_fixed/app.js punya struktur beda)
      let mod;
      try{ mod = await import('./app/store/index.js'); }
      catch(e1){ mod = await import('./store/index.js'); }
      html = await mod.router(hash, currentProfile);
      routerEl.innerHTML = html;
      document.querySelectorAll('.bottom-nav a').forEach(a=>a.classList.remove('active'));
      const navStore = document.querySelector('a[href="#/store"]');
      if(navStore) navStore.classList.add('active');
      const navHist2 = document.querySelector('a[href="#/history"]');
      const navStoreEl = document.querySelector('a[href="#/store"]');
      if(navStoreEl) navStoreEl.style.display='';
      setTimeout(()=>{ try{ document.dispatchEvent(new CustomEvent('store:rendered',{detail:{hash}})); }catch(e){} },100);
      return;
    }catch(e){
      console.error('store router fail', e);
      routerEl.innerHTML = `<div class="card">❌ Store error: ${e.message}<br/><pre style="font-size:10px;white-space:pre-wrap">${(e.stack||'').slice(0,800)}</pre><br/><a href="#/" class="btn secondary">Kembali Home</a></div>`;
      return;
    }
  }
