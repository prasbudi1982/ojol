...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    if(role==='merchant' || role==='warung'){
      ...
      if(!wLat || !wLng) return alert('Lokasi Maps Warung wajib');
    }
    st.textContent='Menyimpan...';
    try{
      // ambil field merchant untuk disimpan
      const wName=document.getElementById('profileWarungName')?.value?.trim()||null;
      const wDesa=document.getElementById('profileDesaMerchant')?.value?.trim()||null;
      const wAlamat=document.getElementById('profileAlamatMerchant')?.value?.trim()||null;
      const wLat=document.getElementById('profileMerchantLat')?.value?.trim()||null;
      const wLng=document.getElementById('profileMerchantLng')?.value?.trim()||null;
      const patch={
        name, role, status:'active',
        hp:hp,
        desa: role==='passenger'?desa : (role==='merchant'||role==='warung'?wDesa:null),
        alamat: role==='passenger'?alamat : (role==='merchant'||role==='warung'?wAlamat:null),
