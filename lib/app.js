...
// ===== STORE ISOLASI - dynamic agar tidak bikin blank =====
...
async function loadStoreRouter(){
  ...
  try{
    ...
    return mod;
  }catch(e){ console.warn('Store gagal load:', e.message); return null; }
}
async function loadStoreEvents(){
  ...
  }catch(e){ console.warn('Store events gagal:', e.message); }
}
...
async function render(){
  ...
  if(hash.startsWith('#/store')){
    try{
      const m = await loadStoreRouter();
      if(!m || !m.router) html = `<div class="card"><b>⚠️ Modul Warung Error</b><div>Food gagal load, ojol tetap jalan</div><a href="#/" class="btn primary">Home</a></div>`;
      else html = await m.router(hash, currentProfile);
    }catch(e){ html = `<div class="card"><b>❌ Error Warung</b><div>${e.message}</div><a href="#/" class="btn primary">Home</a></div>`; }
...
window.addEventListener('error', (e)=>{ const r=document.getElementById('router'); if(r && !r.innerHTML.trim()){ r.innerHTML=`<div class="card"><h3>⚠️ Error tapi tidak blank</h3><div>${e.message}</div><a href="#/login" class="btn primary">Login</a></div>`; }});
