...
// ===== STORE ROUTER TERPISAH TOTAL - TIDAK BIKIN BLANK OJOL =====
let __storeRouter = null;
let __storeEventsLoaded = false;

async function __loadStoreRouter(){
  if(__storeRouter) return __storeRouter;
  // Coba 2 lokasi: ./store/ dan ./app/store/
  const tries = ['./store/index.js', './app/store/index.js'];
  for(const path of tries){
    try{
      const mod = await import(path);
      if(mod && mod.router){
        __storeRouter = mod;
        console.log('✅ Store router loaded from', path);
        return mod;
      }
    }catch(e){
      console.warn('Gagal load', path, e.message);
    }
  }
  return null;
}

async function __loadStoreEvents(){
  if(__storeEventsLoaded) return;
  const tries = ['./store/events.js', './app/store/events.js'];
  for(const path of tries){
    try{
      await import(path);
      __storeEventsLoaded = true;
      console.log('✅ Store events loaded from', path);
      return;
    }catch(e){ console.warn('Gagal load events', path, e.message); }
  }
}
// Load events di background, tidak block render ojol
setTimeout(()=>{ __loadStoreEvents(); }, 2000);

async function __renderStorePage(hash, currentProfile){
  // Semua error store ditangkap di sini, tidak pernah lempar ke ojol
  try{
    const mod = await __loadStoreRouter();
    if(!mod || !mod.router){
      return `
        <div class="card" style="border:1px solid #f59e0b;background:var(--card)">
          <h3 style="color:#f59e0b">⚠️ Modul Warung Error</h3>
          <div style="font-size:12px;margin-top:6px">File store tidak bisa diload, tapi ojol tetap jalan normal.</div>
          <div style="font-size:11px;color:var(--muted);margin-top:6px">Cek console untuk detail error store.</div>
          <div style="margin-top:12px;display:flex;gap:8px">
            <a href="#/" class="btn primary">🏠 Home Ojol</a>
            <a href="#/login" class="btn secondary">🔑 Login</a>
          </div>
        </div>`;
    }
    // Router store sendiri, semua error di dalamnya di-catch
    const html = await mod.router(hash, currentProfile);
    return html;
  }catch(e){
    console.error('Store page error (isolasi):', e);
    return `
      <div class="card" style="border:2px solid #ef4444;background:var(--card)">
        <h3 style="color:#ef4444">❌ Error Halaman Warung</h3>
        <div style="font-size:12px;margin-top:6px">Pesan: ${e.message}</div>
        <pre style="font-size:10px;background:var(--card2);padding:8px;border-radius:8px;margin-top:8px;overflow:auto;max-height:120px">${e.stack ? e.stack.slice(0,500) : ''}</pre>
        <div style="margin-top:12px;display:flex;gap:8px">
          <a href="#/" class="btn primary">🏠 Kembali ke Ojol (tetap jalan)</a>
...
async function render(){
  ...
  if(isAdmin){ if(navHomeG) navHomeG.style.display='none'; if(navProfG) navProfG.style.display='none'; }
    ...
    let html='';
  // === STORE ROUTER TERPISAH - cek paling awal ===
  if(hash.startsWith('#/store')){
    html = await __renderStorePage(hash, currentProfile);
...
// ===== ANTI-BLANK GLOBAL - ojol tidak pernah blank karena store =====
window.addEventListener('error', (e)=>{
  console.error('Global error:', e.error||e.message);
  const r=document.getElementById('router');
  // Jika router masih kosong, tampilkan login, jangan blank
  if(r && !r.innerHTML.trim()){
    r.innerHTML=`<div class="card"><h3>⚠️ Ada error tapi tidak blank</h3><div style="font-size:12px">${e.message}</div><div style="margin-top:10px"><a href="#/login" class="btn primary">🔑 Login</a> <a href="#/" class="btn secondary">🏠 Home</a></div></div>`;
  }
});
