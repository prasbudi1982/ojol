  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    try{
      ...
      if(cur && cur.driver_id === currentProfile.id){
        // Order sudah di-assign ke saya -> kembalikan ke searching, kosongkan driver_id, info ke penumpang SAMPAI
        console.log('Reject assigned order', orderId, '-> notify passenger');
        // Update status searching + driver_id null + simpan info penolakan untuk realtime
        const { error } = await supabase.from('orders').update({ status: 'searching', driver_id: null, accepted_at: null }).eq('id', orderId);
        if(error) throw error;
        // Trigger tambahan: simpan nama driver yang menolak di localStorage penumpang via order update (akan dibaca di tracking.js dari last_driver_name yang sudah disimpan sebelumnya)
