...
async function getStoreMods(){
  ...
  try{
    ...
    storeMods = { ...idx, ...views };
    try{ idx.initStores && idx.initStores(); }catch(e){ lastStoreError = e.message; }
    return storeMods;
  }catch(e){
    // fallback: coba lokasi lama /lib/app/storeViews.js
    try{
      const [idx2, views2] = await Promise.all([
        import('./app/store/index.js'),
        import('./app/storeViews.js')
      ]);
      storeMods = { ...idx2, ...views2 };
      try{ idx2.initStores && idx2.initStores(); }catch(e2){}
      return storeMods;
    }catch(e2){}
    lastStoreError = (e.message||e.toString());
...
async function render(){
  ...
  } else { if(navHome) navHome.style.display=''; if(navPass) navPass.style.display=''; if(navDriver) navDriver.style.display=''; if(navProfile) navProfile.style.display=''; if(navHist) navHist.style.display='none'; const ng=document.querySelector('a[href="#/store"]'); if(ng) ng.style.display=''; }
  ...
  if(hash.startsWith('#/store')){
    ...
    if(!mods){
      const errTxt = lastStoreError || (typeof localStorage!=='undefined' && localStorage.getItem('last_store_error')) || 'File store belum ada atau schema belum dijalankan';
      html = `<div class="card"><h2>🍔 Food</h2><p class="muted">Modul Food belum terpasang atau tabel stores belum ada. Login tetap jalan.</p><div style="background:#1f1f1f;color:#ff8a8a;padding:10px;border-radius:8px;font-size:11px;white-space:pre-wrap;max-height:200px;overflow:auto;margin:10px 0">Error: ${errTxt}</div><p class="muted" style="font-size:11px">1. lib/app/store/index.js harus ada (pakai store-index-final-fixed.js)<br>2. lib/app/storeViews.js harus ada<br>3. Jalankan schema.sql di Supabase</p><a href="#/" class="btn primary">Home</a></div>`;
    } else {
  ...
  if(hash.startsWith('#/profile')){
    ...
    if(roleSel){ roleSel.addEventListener('change', e=>{ const df=document.getElementById('profileDriverFields'); const pf=document.getElementById('profilePassengerFields'); const mf=document.getElementById('profileMerchantFields'); if(df) df.style.display=e.target.value==='driver'?'block':'none'; if(pf) pf.style.display=e.target.value==='passenger'?'block':'none'; if(mf) mf.style.display=e.target.value==='merchant'?'block':'none'; }); }
    const btnPickMerchant=document.getElementById('btnPickMerchantMap');
    if(btnPickMerchant){ btnPickMerchant.addEventListener('click', async()=>{ try{ const m=await import('./app/map.js'); await m.openMapPicker('merchant'); }catch(e){ alert('Gagal buka peta: '+e.message); } }); }
    const btnPickWarung=document.getElementById('btnPickWarungLoc');
    if(btnPickWarung){ btnPickWarung.addEventListener('click', async()=>{ try{ const m=await import('./app/map.js'); await m.openMapPicker('warung'); }catch(e){ alert('Gagal buka peta: '+e.message); } }); }
    const btnPickWarungCreate=document.getElementById('btnPickWarungLocCreate');
    if(btnPickWarungCreate){ btnPickWarungCreate.addEventListener('click', async()=>{ try{ const m=await import('./app/map.js'); await m.openMapPicker('warung'); }catch(e){ alert('Gagal buka peta: '+e.message); } }); }
...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    const nopol=document.getElementById('profileNopol')?.value?.trim()||null;
    const tipeSim=document.getElementById('profileSim')?.value||null;
    const tipe=document.getElementById('profileTipe')?.value?.trim()||null;
    const st=document.getElementById('profileStatus');
    const wName=document.getElementById('profileWarungName')?.value?.trim()||null;
    const desaM=document.getElementById('profileDesaMerchant')?.value?.trim()||null;
    const alamatM=document.getElementById('profileAlamatMerchant')?.value?.trim()||null;
    const latM=document.getElementById('profileMerchantLat')?.value?.trim()||null;
    const lngM=document.getElementById('profileMerchantLng')?.value?.trim()||null;
    ...
    if(role==='merchant'){
      ...
      if(!desaM) return alert('Desa warung wajib');
      if(!alamatM) return alert('Alamat warung wajib');
      if(!latM || !lngM) return alert('Lokasi maps warung wajib - tap Pilih Lokasi di Peta');
    }

    const finalDesa = role==='merchant' ? desaM : (role==='passenger' ? desa : null);
    const finalAlamat = role==='merchant' ? alamatM : (role==='passenger' ? alamat : null);
    ...
    try{
      ...
      const {data,error}=await supabase.from('users').update(patch).eq('id',currentProfile.id).select().single();
      if(error) throw error;
      ...
      if(role==='merchant'){
        try{
          let storePayload={
            ...
            name: wName || data.name || 'Warung '+(data.name||''),
            alamat_text: finalAlamat,
            desa: finalDesa,
            lat: latM ? parseFloat(latM) : null,
            lng: lngM ? parseFloat(lngM) : null,
            wa_number: hp,
            is_open: true
          };
          const { data: existingStore } = await supabase.from('stores').select('id').eq('owner_id', currentProfile.id).maybeSingle();
          if(existingStore && existingStore.id){
            let { error: upErr } = await supabase.from('stores').update(storePayload).eq('id', existingStore.id);
            if(upErr && upErr.message.includes('desa')){
              const { desa, ...p2 } = storePayload;
              await supabase.from('stores').update(p2).eq('id', existingStore.id);
            }
          } else {
            let { error: insErr } = await supabase.from('stores').insert(storePayload).select().single();
            if(insErr && insErr.message.includes('desa')){
              const { desa, ...p2 } = storePayload;
              await supabase.from('stores').insert(p2).select().single();
            }
          }
          st.textContent='✅ Profil & Warung tersinkron! Warung langsung BUKA';
        }catch(eSync){
          console.warn('sync fail', eSync.message);
          st.textContent='Profil tersimpan, sync warung gagal: '+eSync.message;
        }
      } else {
        st.textContent='✅ Profil disimpan';
      }
  ...
  // ===== STORE / WARUNG HANDLERS - FIX btnCreateStore tidak berfungsi =====
  if(e.target.closest('#btnCreateStore')){
    e.preventDefault();
    const btn = e.target.closest('#btnCreateStore');
    const orig = btn.textContent;
    try{
      btn.disabled = true; btn.textContent = '⏳ Membuat...';
      const name = document.getElementById('sName')?.value?.trim();
      const alamat = document.getElementById('sAlamat')?.value?.trim();
      const wa = document.getElementById('sWa')?.value?.trim() || currentProfile?.hp || null;
      const latStr = document.getElementById('sLat')?.value?.trim();
      const lngStr = document.getElementById('sLng')?.value?.trim();
      if(!name) throw new Error('Nama warung wajib');
      if(!alamat) throw new Error('Alamat wajib');
      if(!latStr || !lngStr) throw new Error('Lat/Lng wajib - pilih dari peta 📍');
      const lat = parseFloat(latStr); const lng = parseFloat(lngStr);
      if(isNaN(lat)||isNaN(lng)) throw new Error('Lat/Lng tidak valid');
      // cek bbox
      try{
        const { isInSuruhBbox } = await import('./app/geofence.js');
        if(!isInSuruhBbox(lat,lng)) throw new Error('Lokasi harus di dalam area aktif (Suruh)');
      }catch(e){ if(e.message.includes('Lokasi harus')) throw e; }

      let payload = {
        owner_id: currentProfile.id,
        name,
        alamat_text: alamat,
        desa: currentProfile?.desa || null,
        lat, lng,
        wa_number: wa,
        is_open: true
      };
      let { data, error } = await supabase.from('stores').insert(payload).select().single();
      if(error && error.message.includes('desa')){
        // retry tanpa desa jika kolom belum ada
        const { desa, ...payload2 } = payload;
        const r2 = await supabase.from('stores').insert(payload2).select().single();
        data = r2.data; error = r2.error;
      }
      if(error) throw error;
      alert('✅ Warung berhasil dibuat! ID: '+data.id.slice(0,8)+' - Sekarang BUKA di Food');
      location.hash = '#/store/my';
      setTimeout(()=>location.reload(), 500);
    }catch(err){
      alert('❌ Gagal buat warung: '+err.message);
      btn.disabled = false; btn.textContent = orig;
    }
    return;
  }

  if(e.target.closest('#btnUpdateStore')){
    e.preventDefault();
    const btn = e.target.closest('#btnUpdateStore');
    const orig = btn.textContent;
    try{
      btn.disabled = true; btn.textContent = '⏳ Menyimpan...';
      const name = document.getElementById('sName')?.value?.trim();
      const alamat = document.getElementById('sAlamat')?.value?.trim();
      const wa = document.getElementById('sWa')?.value?.trim() || null;
      const latStr = document.getElementById('sLat')?.value?.trim();
      const lngStr = document.getElementById('sLng')?.value?.trim();
      const isOpenStr = document.getElementById('sOpen')?.value;
      if(!name) throw new Error('Nama warung wajib');
      ...
      const payload = {
        ...
        wa_number: wa,
        lat, lng,
        is_open: isOpenStr === 'true'
      };
      // cari store milik user
      const { data: existing } = await supabase.from('stores').select('id').eq('owner_id', currentProfile.id).maybeSingle();
      if(!existing) throw new Error('Warung tidak ditemukan');
      const { error } = await supabase.from('stores').update(payload).eq('id', existing.id);
      if(error) throw error;
      alert('✅ Warung diupdate! '+(payload.is_open?'BUKA':'TUTUP'));
      location.reload();
    }catch(err){
  ...
  if(e.target.closest('#btnAddProduct')){
    e.preventDefault();
    const btn = e.target.closest('#btnAddProduct');
    const statusEl = document.getElementById('pStatus');
    const orig = btn.textContent;
    try{
      btn.disabled = true; btn.textContent = '⏳ Menyimpan...';
      if(statusEl) statusEl.textContent = 'Menyimpan menu...';
      ...
      const stokStr = document.getElementById('pStok')?.value?.trim() || '100';
      const variasiStr = document.getElementById('pVariasi')?.value?.trim() || '';
      const addonsStr = document.getElementById('pAddons')?.value?.trim() || '';
      const deskripsi = document.getElementById('pDesc')?.value?.trim() || null;
      if(!name) throw new Error('Nama menu wajib');
      if(!hargaStr) throw new Error('Harga wajib');
      const harga = parseInt(hargaStr);
      if(isNaN(harga) || harga<=0) throw new Error('Harga tidak valid');
      const stok = parseInt(stokStr)||0;

      // parse variasi: "Biasa|0, Jumbo|5000, Seafood|10000"
      const parseVariasi = (str)=>{
        if(!str) return [];
      ...
      const variants = parseVariasi(variasiStr);
      const addons = parseAddons(addonsStr);

      // cari my store id
      let storeId = null;
      try{
        const { data: s } = await supabase.from('stores').select('id').eq('owner_id', currentProfile.id).maybeSingle();
        if(s) storeId = s.id;
      }catch(e){}
      if(!storeId){
        try{
          const mods = await getStoreMods();
          if(mods && mods.warungStore && mods.warungStore._actions){
            const my = await mods.warungStore._actions.fetchMyStore(currentProfile.id);
            if(my) storeId = my.id;
          }
        }catch(e){}
      }
      if(!storeId) throw new Error('Warung tidak ditemukan - buat warung dulu di Warungku');
      ...
      // coba insert via productStore jika ada, fallback supabase langsung
      let created = null;
      try{
        const mods = await getStoreMods();
        if(mods && mods.productStore && mods.productStore._actions && mods.productStore._actions.createProduct){
          created = await mods.productStore._actions.createProduct(payload);
        } else {
          throw new Error('fallback');
        }
      }catch(e){
        // fallback supabase - coba dengan variants/addons, jika kolom belum ada, coba tanpa
        let { data, error } = await supabase.from('store_products').insert(payload).select().single();
        if(error && (error.message.includes('variants') || error.message.includes('addons') || error.message.includes('variasi'))){
          const { variants: v, addons: a, ...payload2 } = payload;
          const r2 = await supabase.from('store_products').insert(payload2).select().single();
          data = r2.data; error = r2.error;
          if(!error && statusEl) statusEl.textContent = '⚠️ Kolom variants/addons belum ada di DB, menu tersimpan tanpa varian. Jalankan migration SQL.';
        }
        if(error) throw error;
        created = data;
      }

      if(statusEl) statusEl.textContent = '✅ Menu '+created.name+' berhasil ditambah! Varian: '+(variants.length||0)+' Addon: '+(addons.length||0);
      alert('✅ Menu ditambah: '+name+'\nVarian: '+(variants.map(v=>v.name).join(', ')||'-')+'\nAddon: '+(addons.map(a=>a.name).join(', ')||'-'));
      // reset form
      const pNameEl = document.getElementById('pName'); if(pNameEl) pNameEl.value='';
      const pHargaEl = document.getElementById('pHarga'); if(pHargaEl) pHargaEl.value='';
      const pVariEl = document.getElementById('pVariasi'); if(pVariEl) pVariEl.value='';
      const pAddonEl = document.getElementById('pAddons'); if(pAddonEl) pAddonEl.value='';
      const pDescEl = document.getElementById('pDesc'); if(pDescEl) pDescEl.value='';
      // reload list
