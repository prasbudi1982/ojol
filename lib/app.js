...
async function render(){
  ...
  if(hash.startsWith('#/profile')){
    ...
    if(btnPickMerchant){
      btnPickMerchant.addEventListener('click', async ()=>{
        try{
          const { openMapPicker } = await import('./app/map.js');
          const { isInSuruhBbox } = await import('./app/geofence.js');
          // override confirm behavior for merchant
          window._isMerchantMapPick = true;
          await openMapPicker();
          // hook confirm button to set merchant location
          const originalConfirm = window.confirmMapLocation;
          window.confirmMapLocation = (fn)=>{
            const lat = window.mapModule?.mapSelected?.lat || mapMod?.mapSelected?.lat;
            const lng = window.mapModule?.mapSelected?.lng || mapMod?.mapSelected?.lng;
            const addr = window.mapModule?.mapSelected?.address || '';
            if(window._isMerchantMapPick){
              if(lat && lng){
                if(!isInSuruhBbox(lat,lng)){ alert('⛔ Lokasi warung harus di dalam area aktif'); return; }
                window.setMerchantMapLocation && window.setMerchantMapLocation(lat,lng,addr);
                const { closeMapPicker } = mapMod || window.mapModule || {};
                if(closeMapPicker) closeMapPicker(); else document.getElementById('mapModal').style.display='none';
                window._isMerchantMapPick = false;
                window.confirmMapLocation = originalConfirm;
                return;
              }
            }
            originalConfirm(fn);
          };
        }catch(e){ alert('Gagal buka peta: '+e.message); }
      });
    }
...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    if(!hp) return alert('HP WA wajib diisi');
    ...
    if(role==='merchant'){
      ...
      const desaM=document.getElementById('profileDesaMerchant')?.value?.trim();
      const alamatM=document.getElementById('profileAlamatMerchant')?.value?.trim();
      const latM=document.getElementById('profileMerchantLat')?.value?.trim();
      const lngM=document.getElementById('profileMerchantLng')?.value?.trim();
      if(!wName) return alert('Nama warung wajib untuk merchant');
      if(!desaM) return alert('Desa wajib untuk merchant');
      if(!alamatM) return alert('Alamat warung wajib untuk merchant');
      if(!latM || !lngM) return alert('Lokasi maps warung wajib - tap Pilih Lokasi Warung di Peta');
      const latNum=parseFloat(latM); const lngNum=parseFloat(lngM);
      if(isNaN(latNum)||isNaN(lngNum)) return alert('Koordinat maps tidak valid');
      try{
        const { isInSuruhBbox: checkBbox } = await import('./app/geofence.js');
        if(!checkBbox(latNum,lngNum)){
          const { getActiveKecamatanName: getName } = await import('./app/geofence.js');
          return alert('⛔ Lokasi warung harus di dalam '+getName()+' - pilih lagi di peta');
        }
      }catch(e){}
    }
    ...
    try{
      ...
      const patch={
        ...
        warung_name: role==='merchant'?wName:null,
        lat: role==='merchant'? (latM? parseFloat(latM):null) : null,
        lng: role==='merchant'? (lngM? parseFloat(lngM):null) : null,
        merchant_lat: role==='merchant'? (latM? parseFloat(latM):null) : null,
        merchant_lng: role==='merchant'? (lngM? parseFloat(lngM):null) : null,
      ...
      // AUTO SYNC WARUNG - biar gak bolak-balik isi data
      if(role==='merchant'){
        try{
          const { isInSuruhBbox: checkBbox2 } = await import('./app/geofence.js');
          // upsert stores - jika sudah ada update, jika belum create
          const storePayload = {
          ...
          // cek dulu ada store milik user ini?
          ...
          if(existingStore && existingStore.id){
            const { error: upErr } = await supabase.from('stores').update(storePayload).eq('id', existingStore.id);
            if(upErr) console.warn('sync warung update fail', upErr.message);
            else console.log('✅ Warung auto-sync update', existingStore.id);
          } else {
            const { data: newStore, error: insErr } = await supabase.from('stores').insert(storePayload).select().single();
            if(insErr) console.warn('sync warung insert fail', insErr.message);
            else console.log('✅ Warung auto-sync create', newStore.id);
          }
          st.textContent = '✅ Profil & Warung tersinkron! Warung langsung BUKA di Food';
        }catch(eSync){
          console.warn('Auto sync warung fail', eSync.message);
          st.textContent = 'Profil tersimpan, tapi sync warung gagal: '+eSync.message+' - cek di Warungku';
        }
      }
