...
// ===== STORE MODULE ISOLASI ANTI-BLANK =====
let storeRouterCache = null;
let storeEventsLoaded = false;

async function loadStoreRouter(){
  if(storeRouterCache) return storeRouterCache;
  try{
    let mod = null;
    try{ mod = await import('./store/index.js'); }
    catch(e1){ try{ mod = await import('./app/store/index.js'); }catch(e2){ throw e1; } }
    storeRouterCache = mod;
    console.log('✅ Store router loaded');
    return mod;
  }catch(e){
    console.warn('⚠️ Store module gagal load (isolasi, ojol tetap jalan):', e.message);
    return null;
  }
}

async function loadStoreEvents(){
  if(storeEventsLoaded) return;
  try{
    try{ await import('./store/events.js'); storeEventsLoaded = true; }
    catch(e1){ try{ await import('./app/store/events.js'); storeEventsLoaded = true; }catch(e2){ throw e1; } }
    console.log('✅ Store events loaded');
  }catch(e){
    console.warn('⚠️ Store events gagal load (isolasi):', e.message);
  }
}
setTimeout(()=>{ loadStoreEvents(); }, 1500);
...
async function render(){
  ...
  if(isAdmin){ if(navHomeG) navHomeG.style.display='none'; if(navProfG) navProfG.style.display='none'; }
    ...
    let html='';
  // STORE ROUTE ISOLASI - cek dulu agar tidak blank
  if(hash.startsWith('#/store')){
    try{
      const storeMod = await loadStoreRouter();
      if(!storeMod || !storeMod.router){
        html = `<div class="card" style="border:1px solid #f59e0b"><b>⚠️ Modul Warung Error</b><div style="font-size:12px;margin-top:6px">Food gagal load, tapi ojol tetap jalan.</div><a href="#/" class="btn primary" style="margin-top:10px">🏠 Home</a></div>`;
      }else{
        html = await storeMod.router(hash, currentProfile);
      }
    }catch(e){
      console.error('Store router error (isolasi):', e);
      html = `<div class="card" style="border:1px solid #ef4444"><b>❌ Error Warung</b><div style="font-size:12px">${e.message}</div><a href="#/" class="btn primary" style="margin-top:10px">🏠 Home (ojol tetap jalan)</a></div>`;
    }
...
window.addEventListener('error', (e)=>{
  ...
  if(r && !r.innerHTML.trim()){ r.innerHTML=`<div class="card"><h3>⚠️ Ada error tapi tidak blank</h3><div style="font-size:12px">${e.message}</div><a href="#/login" class="btn primary">🔑 Login</a></div>`; }
});
