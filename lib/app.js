...
// --- STORE: lazy only, tidak pernah import di top-level, dengan error UI untuk HP ---
let lastStoreError = '';
let storeMods = null;
async function getStoreMods(){
  ...
  try{
    ...
    try{
      ...
      console.warn('fallback storeViews inside', e1.message);
...
document.addEventListener('click', async (e)=>{
  ...
  // ===== FOOD FIX ONLY - Peta + 5 driver + Batal Selesai (tidak ubah logic Ojol) =====
  ...
  if(e.target.closest('#btnUseMyLocation')){
    ...
    navigator.geolocation.getCurrentPosition((pos)=>{
      ...
      const raw=localStorage.getItem('ojol_cart_v2_food'); 
      if(raw){ const s=JSON.parse(raw); s.dest={lat:lat,lng:lng,text:s.dest.text||''}; localStorage.setItem('ojol_cart_v2_food', JSON.stringify(s)); location.reload(); }
  ...
  if(e.target.closest('#btnCheckout')){
    ...
    try{
      ...
      if(!state.dest.lat || !state.dest.lng) return alert('📍 Pilih lokasi antar di peta dulu!');
      const destText = document.getElementById('destText')?.value?.trim() || state.dest.text||'';
      if(destText.length<5) return alert('Alamat harus lengkap');
      const btn=document.getElementById('btnCheckout'); if(btn){ btn.disabled=true; btn.textContent='⏳ Membuat...'; }
      ...
      // Modal 5 driver terdekat
      let old=document.getElementById('foodCheckoutDriversModal'); if(old) old.remove();
      const div=document.createElement('div'); div.id='foodCheckoutDriversModal'; div.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:flex-end;justify-content:center';
      div.innerHTML='<div style="background:var(--card);width:100%;max-width:520px;max-height:85vh;overflow:auto;border-radius:20px 20px 0 0;padding:16px"><div style="display:flex;justify-content:space-between"><b>🍔 Pesanan #' + orderData.id.slice(0,6) + '</b><button id="btnCloseFoodCheckout">✕</button></div><div style="margin-top:8px;font-size:11px" class="muted">Rp ' + Number(orderData.total||0).toLocaleString() + ' • Timer 5 menit auto batal</div><div style="display:flex;gap:8px;margin-top:12px"><button id="btnFoodCheckoutTracking" class="btn primary" style="flex:1">📍 Tracking</button><button id="btnFoodCheckoutCancel" class="btn secondary" style="flex:1;color:#ef4444">❌ Batalkan Pesanan</button></div><button id="btnFoodCheckoutComplete" class="btn secondary" style="width:100%;margin-top:8px;display:none">✅ Selesai Pesanan</button><div style="font-weight:800;margin:12px 0 8px;font-size:13px">🏍️ 5 Driver Terdekat dari Warung</div><div id="foodNearbyDriversList" class="muted" style="padding:12px">🔍 Mencari...</div></div>';
      ...
      document.getElementById('btnFoodCheckoutCancel').onclick=async()=>{ if(!confirm('Batalkan?')) return; await supabase.from('food_orders').update({status:'cancelled'}).eq('id',orderData.id); trackingFood.clearFoodTracking(); div.remove(); };
      document.getElementById('btnFoodCheckoutComplete').onclick=async()=>{ if(!confirm('Selesai?')) return; await supabase.from('food_orders').update({status:'completed'}).eq('id',orderData.id); trackingFood.clearFoodTracking(); div.remove(); };
      // Load 5 driver
      try{
        ...
        if(!locs || !locs.length){ if(listEl) listEl.innerHTML='Tidak ada driver online. Menunggu driver di dashboard.'; }
        else{
          ...
          const drivers=locs.map(loc=>{
            const u=users?users.find(x=>x.id===loc.driver_id):null; if(!u || u.role!=='driver') return null;
          ...
          if(listEl) listEl.innerHTML=drivers.map(d=>'<div style="border:1px solid var(--border);border-radius:12px;padding:10px;margin-bottom:8px;display:flex;justify-content:space-between"><div><b>' + d.name + '</b> • ' + d.distance_km.toFixed(2) + ' km<br/><span class="muted" style="font-size:10px">' + (d.nopol||'') + '</span></div><button data-order-food-driver=\'' + d.driver_id + '\' data-order-id=\'' + orderData.id + '\' class="btn primary" style="width:auto;padding:8px 12px">✅ Pilih</button></div>').join('') || 'Tidak ada driver terdekat';
        }
  ...
  if(e.target.closest('[data-food-accept]')){
    ...
    try{ await supabase.from('food_orders').update({ driver_id: currentProfile.id, status: 'driver_assigned' }).eq('id', id).eq('status','searching_driver'); alert('✅ Diterima! Warung dapat notif.'); trackingFood.openFoodTrackingDetailModal(id); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('[data-food-picked]')){
    e.preventDefault();
    const id=e.target.closest('[data-food-picked]').getAttribute('data-food-picked');
    if(!confirm('Ambil di warung?')) return;
    try{ await supabase.from('food_orders').update({ status:'picked' }).eq('id', id); trackingFood.openFoodTrackingDetailModal(id); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('[data-food-complete]')){
    e.preventDefault();
    const id=e.target.closest('[data-food-complete]').getAttribute('data-food-complete');
    if(!confirm('Selesai?')) return;
    try{ await supabase.from('food_orders').update({ status:'completed' }).eq('id', id); alert('✅ Selesai'); trackingFood.clearFoodTracking(); const m=document.getElementById('foodTrackingDetailModal'); if(m) m.remove(); }catch(err){ alert(err.message); }
    return;
  }
  if(e.target.closest('[data-order-food-driver]')){
    ...
    try{ await supabase.from('food_orders').update({ driver_id: driverId, status:'driver_assigned' }).eq('id', orderId); trackingFood.startFoodTracking(orderId); trackingFood.openFoodTrackingDetailModal(orderId); }catch(err){ alert(err.message); }
    return;
  }
