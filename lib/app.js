...
async function render(){
  ...
  if(hash.startsWith('#/store')){
    ...
    if(!mods){
      html = `<div class="card"><h2>🍔 Food</h2><p class="muted">Modul Food belum siap: ${lastStoreError||'file tidak ada'}</p><p class="muted" style="font-size:10px">Pastikan lib/app/store/index.js dan lib/app/store/storeViews.js ada</p><a href="#/" class="btn primary">Home</a></div>`;
    } else {
      try{
        ...
        html = `<div class="card"><h2>🍔 Food Error</h2><p class="muted">${e.message}</p><pre style="font-size:10px;white-space:pre-wrap">${e.stack||''}</pre></div>`;
      }
    }
  ...
  if(e.target.closest('#btnPickMerchantMap')){
    ...
    try{
      if(window.mapModule && window.mapModule.openMapPicker){
        window.mapModule.openMapPicker('merchant');
      } else if(window.openMapPicker){
        window.openMapPicker('merchant');
      } else if(window.openMerchantMapPicker){
        window.openMerchantMapPicker();
      }
