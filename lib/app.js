...
async function render(){
  ...
  // ===== ROLE NAVIGASI FINAL - MINI OJOL =====
  function applyNavByRole(role){
    const nav = document.getElementById('bottomNav');
    if(!nav) return;
    const links = nav.querySelectorAll('a[data-role]');
    links.forEach(a=>{
      const allowed = (a.dataset.role||'').split(' ').map(s=>s.trim());
      let visible = false;
      if(!role){
        // GUEST: hanya home, ojek, driver
        visible = ['home','passenger','driver'].includes(a.dataset.nav);
      } else {
        visible = allowed.includes('all') || allowed.includes(role);
        // Khusus admin: hanya admin & adminSettings (sesuai logic lama)
        if(role==='admin'){
          visible = ['admin','adminSettings'].includes(a.dataset.nav);
        }
        // Khusus merchant: sembunyikan passenger & driver & store customer
        if(role==='merchant'){
          if(['passenger','driver'].includes(a.dataset.nav)) visible = false;
          // store & cart adalah untuk passenger/driver, bukan merchant
          if(['store','cart'].includes(a.dataset.nav)) visible = false;
        }
        // Khusus passenger: sembunyikan driver & merchant
        if(role==='passenger'){
          if(['driver','myStore','products','storeOrders','admin','adminSettings'].includes(a.dataset.nav)) visible = false;
        }
        // Khusus driver: sembunyikan passenger & merchant & admin
        if(role==='driver'){
          if(['passenger','myStore','products','storeOrders','admin','adminSettings'].includes(a.dataset.nav)) visible = false;
        }
      }
    ...
    const active = nav.querySelector(`a[href="${currentHash}"]`);
    if(active && active.style.display !== 'none') active.classList.add('active');
  }

  if(currentProfile){
    applyNavByRole(currentProfile.role);
    const hash = location.hash || '#/';
    const role = currentProfile.role;
    // Guard redirect (pertahankan tanpa kasus baru)
    if(role==='admin' && !hash.startsWith('#/admin')){
      location.hash = '#/admin';
      return;
    }
    if(role==='driver' && (hash.startsWith('#/passenger') || hash.startsWith('#/admin') || hash.startsWith('#/store/my') || hash.startsWith('#/store/products') || hash.startsWith('#/store/orders'))){
      location.hash = '#/';
      return;
    }
    if(role==='passenger' && (hash.startsWith('#/driver') || hash.startsWith('#/admin') || hash.startsWith('#/store/my') || hash.startsWith('#/store/products') || hash.startsWith('#/store/orders'))){
      location.hash = '#/';
      return;
    }
    if(role==='merchant' && (hash.startsWith('#/passenger') || hash.startsWith('#/driver') || hash.startsWith('#/admin') || hash==='/' || hash.startsWith('#/store/cart'))){
      // merchant boleh di #/ , #/history, #/profile, #/store/my, #/store/products, #/store/orders
      // jika nyasar ke halaman yang disembunyikan, redirect ke warungku
