...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('[data-order-driver]')){ 
    ...
    const distAttr = btn.getAttribute('data-distance');
    const distVal = parseFloat(distAttr)||0;
    console.log('🛵 Pesan driver', driverId, 'jarak', distVal, 'km');
    // Simpan jarak jemput sebelum createOrder agar pickup fee kehitung & tracking bisa pakai
    try{
      order.currentOrderMeta.driverDistanceKm = distVal;
      order.currentOrderMeta.driverId = driverId;
      if(window.order){ window.order.currentOrderMeta.driverDistanceKm = distVal; window.order.currentOrderMeta.driverId = driverId; }
    }catch(e){ console.warn('set meta fail', e); }
