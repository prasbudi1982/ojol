  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    try{
      ...
      if(cur && cur.driver_id === currentProfile.id){
        console.log('Reject assigned order', orderId, '-> notify passenger via status searching + broadcast (RLS safe)');
        // FIX RLS: driver TIDAK boleh set driver_id=null. Cukup set status=searching
        let statusError = null;
        try{
          const { error } = await supabase.from('orders').update({ status: 'searching' }).eq('id', orderId);
          if(error) throw error;
        }catch(e){ statusError = e; console.warn('status update failed', e.message); }
        // Broadcast fallback - PASTI sampai ke kartu menunggu driver penumpang meski postgres_changes diblok RLS
        try{
          ...
          console.log('Broadcast driver_rejected sent');
        }catch(e){ console.warn('broadcast failed', e.message); }
        if(statusError){
          // Jika status update gagal karena RLS, tetap anggap tolak berhasil via broadcast, penumpang yang akan null-kan
          console.log('Proceed via broadcast only');
        }
        try{
        ...
        alert('✅ Order ditolak - penumpang akan otomatis dapat info di kartu menunggu driver.');
      } else {
