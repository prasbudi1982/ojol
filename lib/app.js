...
import { viewDriver } from './app/tracking.js?v=17-fix-tujuan-driver';
...
import * as tracking from './app/tracking.js?v=17-fix-tujuan-driver';
...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('[data-driver-reject]')){
    ...
    try{
      ...
      // Simpan bahwa driver ini menolak order ini - tidak akan terlist lagi untuk ID yang sama
      const isAssigned = cur && cur.driver_id === currentProfile.id;
      
      // Broadcast penolakan dulu agar penumpang langsung tahu
      try{
        ...
        setTimeout(async()=>{ try{ await ch.send({ type: 'broadcast', event: 'driver_rejected', payload: { driverName: currentProfile.name||'Driver', driverId: currentProfile.id, orderId } }); }catch(e){} }, 300);
      }catch(e){ console.warn('broadcast failed', e.message); }

      if(isAssigned){
        // Order sudah di-assign ke driver ini -> AUTO ASSIGN ke driver terdekat selanjutnya, ID ORDER TETAP SAMA
        console.log('Reject assigned order', orderId, '-> auto assign next driver');
        try{
          // Panggil logic auto assign dari order.js
          if(order.assignToNextDriver){
            await order.assignToNextDriver(orderId, `Driver ${currentProfile.name||''} menolak`);
          } else {
            // Fallback kalau function tidak ada
            await supabase.from('orders').update({ status: 'searching', driver_id: null, accepted_at: null }).eq('id', orderId);
          }
        }catch(e){
          console.warn('Auto assign on reject failed, fallback to null', e.message);
          try{ await supabase.from('orders').update({ status: 'searching', driver_id: null }).eq('id', orderId); }catch(e2){}
        }
      } else {
        // Order broadcast (driver_id null) -> cukup sembunyikan lokal untuk driver ini
        console.log('Reject broadcast order', orderId, '- hide for this driver only, ID tetap sama');
        // Tidak perlu update order di DB, cukup localStorage
      }

      alert('✅ Order ditolak. Kamu tidak akan menerima order ini lagi. Sistem otomatis mencoba driver terdekat selanjutnya (ID order tetap sama).');
      await tracking.loadDriverOrders(currentProfile);
    }catch(e){ 
