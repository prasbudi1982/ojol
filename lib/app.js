...
document.addEventListener('click', async (e)=>{
  ...
  if(e.target.closest('#btnSaveProfileRole')){
    ...
    // merchant fields - untuk validasi saja, tidak disimpan ke users jika kolom tidak ada
    ...
    if(role==='merchant' || role==='warung'){
      ...
      if(!wLat || !wLng) return alert('Lokasi Maps Warung wajib - tap Pilih Lokasi');
    }
    st.textContent='Menyimpan...';
    try{
      // HANYA kolom yang pasti ada di users (tanpa lat/merchant_lat biar tidak error schema cache)
      const patch={
      ...
      // coba simpan warung_name jika kolomnya ada (ignore jika error)
      try{ patch.warung_name = (role==='merchant'||role==='warung')?wName:null; }catch(e){}
      const {data,error}=await supabase.from('users').update(patch).eq('id',currentProfile.id).select().single();
      if(error){
        // jika error karena warung_name tidak ada, coba lagi tanpa warung_name
        if(error.message && error.message.includes('warung_name')){
          delete patch.warung_name;
          const retry = await supabase.from('users').update(patch).eq('id',currentProfile.id).select().single();
      ...
      // simpan lat/lng ke tabel stores (yang memang punya lat/lng sesuai screenshot kamu)
      if(role==='merchant' || role==='warung'){
        try{
          const {data:existing} = await supabase.from('stores').select('id').eq('owner_id', currentProfile.id).maybeSingle();
          const storePayload={ owner_id: currentProfile.id, name: wName, desa: wDesa, alamat: wAlamat, lat: parseFloat(wLat), lng: parseFloat(wLng), is_active: true };
          if(existing) await supabase.from('stores').update(storePayload).eq('id', existing.id);
          else await supabase.from('stores').insert(storePayload);
        }catch(eS){ console.warn('stores sync', eS.message); }
      }
      userBadge.textContent=`${currentProfile.name} • ${currentProfile.role}`; st.textContent='✅ Profil disimpan'; setTimeout(()=>{ location.hash='#/'; }, 600);
