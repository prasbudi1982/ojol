...
async function render(){
  ...
  if(hash.startsWith('#/profile')){
    ...
    // Merchant map picker - FIXED pakai target merchant
    const btnPickMerchant = document.getElementById('btnPickMerchantMap');
    if(btnPickMerchant){
      btnPickMerchant.addEventListener('click', async ()=>{
        try{
          const mapMod = await import('./app/map.js');
          await mapMod.openMapPicker('merchant');
        }catch(e){ alert('Gagal buka peta merchant: '+e.message); }
      });
    }
    // Warungku map picker (di #/store/my)
    const btnPickWarungLoc = document.getElementById('btnPickWarungLoc');
    if(btnPickWarungLoc){
      btnPickWarungLoc.addEventListener('click', async ()=>{
        try{
          const mapMod = await import('./app/map.js');
          await mapMod.openMapPicker('warung');
        }catch(e){ alert('Gagal buka peta warung: '+e.message); }
      });
    }
    const btnPickWarungLocCreate = document.getElementById('btnPickWarungLocCreate');
    if(btnPickWarungLocCreate){
      btnPickWarungLocCreate.addEventListener('click', async ()=>{
        try{
          const mapMod = await import('./app/map.js');
          await mapMod.openMapPicker('warung');
        }catch(e){ alert('Gagal buka peta warung: '+e.message); }
