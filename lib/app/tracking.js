...
export function showTrackingLockModal(order){
  ...
  div.style.cssText = 'position:fixed;inset:0;background:var(--overlay);backdrop-filter:blur(8px);z-index:9998;display:flex;align-items:center;justify-content:center;padding:16px';
    ...
    <div style="background:var(--card);border-radius:18px;max-width:360px;width:100%;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,0.5);border:2px solid ${theme.primary}">
      ...
      <div style="padding:16px">
        ...
        <div style="margin-top:12px;display:flex;flex-direction:column;gap:8px">
          ...
          <button id="btnCancelFromLock" style="background:var(--card2);border:1px solid var(--border);color:var(--danger);padding:10px;border-radius:10px;font-weight:700;font-size:12px">❌ Batalkan Order</button>
        </div>
      </div>
    </div>`;
...
export async function loadActiveTracking(){
  ...
  try{
    ...
    if(error || !order){ clearActiveTracking(); hideTrackingUI(); return null; }
    // FIX: jika order sudah cancelled/completed/rejected -> auto tutup modal
    if(['completed','cancelled','rejected'].includes(order.status)){
      clearActiveTracking();
      hideTrackingUI();
      // Jika cancelled karena driver tolak atau timeout, beri info dan redirect ke passenger
  ...
  if(driver){
    bodyHtml = `
      <div style="margin:10px;background:#0f172a;border:1px solid #22c55e;border-radius:14px;padding:12px">
        ...
        <div style="margin-top:10px;background:#020617;border-radius:10px;padding:8px"><div id="trackingDistance" style="font-weight:700;color:#4ade80;font-size:12px">📍 Menghitung jarak...</div><div style="font-size:10px;color:#64748b;margin-top:2px">Auto batal jika 5 menit tidak respon • <span id="singleTimeout">${TIMER_TIMEOUT/60}:00</span> sisa</div></div>
...
function subscribeOrderUpdates(orderId){
  ...
  orderChannel = supabase.channel('order-'+orderId)
    .on('postgres_changes', { event:'UPDATE', schema:'public', table:'orders', filter:`id=eq.${orderId}` }, payload=>{
      ...
      if(!newOrder) return;
      // Jika cancelled/completed/rejected -> auto tutup modal
