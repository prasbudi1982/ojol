...
export function clearActiveTracking(){
  if(typeof orderPollInterval !== 'undefined' && orderPollInterval){ clearInterval(orderPollInterval); orderPollInterval=null; }
...
// ===== RENDER SINGLE CARD - DENGAN BANNER PENOLAKAN =====
function renderActiveOrder(order, driver, opts={}){
  ...
  card.innerHTML = `
    ${isRejected ? `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:12px;margin:12px 12px 0;display:flex;gap:10px;align-items:center"><div style="width:36px;height:36px;background:#ef4444;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white">⚠️</div><div style="flex:1"><div style="font-weight:800;color:#991b1b;font-size:13px">Driver Menolak Order</div><div style="font-size:11px;color:#7f1d1d;margin-top:2px">${rejectedName} tidak bisa melanjutkan. Mencari driver lain di ${ACTIVE_KECAMATAN_NAME||'Suruh'}...</div></div></div>` : ''}
    <div style="border:1px solid #263240;border-radius:20px;overflow:hidden;background:#151c25;box-shadow:0 12px 40px rgba(0,0,0,0.5);color:#e6edf5;margin-top:${isRejected?'12px':'0'}">
...
function handleOrderRejection(newOrder){
  if(newOrder.driver_id){
    // Ada driver - simpan sebagai lastKnown
    ...
    // Tidak ada driver - cek apakah sebelumnya ada driver (penolakan)
    if(lastKnownDriverId || localStorage.getItem('last_driver_id')){
      if(newOrder.status==='searching'){
        ...
        const lastName = localStorage.getItem('last_driver_name')||'Driver';
        console.log('REJECT DETECTED via polling/realtime - show to passenger', lastName);
        renderActiveOrder(newOrder, null, {rejected:true, driverName:lastName});
        // Reset setelah 8 detik agar tidak loop banner terus
        setTimeout(()=>{ 
          try{ localStorage.removeItem('last_driver_name'); localStorage.removeItem('last_driver_id'); lastKnownDriverId=null; }catch(e){}
        },8000);
        const si = document.getElementById('searchInfo'); if(si) si.textContent = `⚠️ ${lastName} menolak - mencari driver lain...`;
        return;
      }
    }
    // Murni masih searching tanpa pernah dapat driver
