...
function handleOrderRejection(newOrder){
  if(newOrder.driver_id){
    ...
    if(newOrder.status==='searching' && lastKnownDriverId && newOrder.driver_id===lastKnownDriverId){
      ...
      const lastName = localStorage.getItem('last_driver_name')||'Driver';
      console.log('REJECT SIGNAL from driver - passenger will nullify', lastName);
      ...
      // Penumpang sendiri yang null-kan driver_id (passenger owns order, lolos RLS)
      (async ()=>{
        try{
          ...
          else console.log('passenger nullified driver_id after rejection');
    ...
    // driver_id null = sudah di-null-kan (oleh penumpang setelah rejection, atau awal searching)
