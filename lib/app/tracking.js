  ...
  orderChannel = supabase.channel('order-tracking-'+orderId)
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `id=eq.${orderId}` }, async payload=>{
      ...
      if(newOrder.driver_id){
        ...
        // Driver menolak / driver_id dikosongkan -> info ke penumpang SAMPAI KE MODAL
        if(newOrder.status==='searching'){
          ...
          // Auto clear banner after 8 detik, kembali ke waiting normal
          setTimeout(()=>{
            localStorage.removeItem('last_driver_name');
            localStorage.removeItem('last_assigned_driver_name');
          }, 8000);
        } else {
