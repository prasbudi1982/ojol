...
function renderActiveOrder(order, driver){
  ...
  if(!card) return;
  // Pastikan activeOrderCard lama tidak pernah tampil (hapus double)
  ...
  card.innerHTML = `
    <div style="border:1px solid #263240;border-radius:20px;padding:0;overflow:hidden;background:#151c25;box-shadow:0 12px 40px rgba(0,0,0,0.5);color:#e6edf5">
      <!-- Header dark + primary accent -->
      ...
      <div style="padding:14px 16px 0;display:flex;gap:6px;align-items:center">
        <div style="flex:1;height:6px;border-radius:3px;background:${step>=1?getAppTheme().primary:'#1d2633'};transition:all 0.3s"></div>
      ...
      <div style="margin:12px;background:#1d2633;border:1px solid #263240;border-radius:16px;padding:14px">
        <div style="display:flex;gap:12px;align-items:center">
          ...
          <div style="flex:1;min-width:0">
            <div style="font-weight:800;font-size:14px;color:#e6edf5;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${driver.name} ${driver.jenis_kendaraan==='mobil'?'🚗':'🏍️'}</div>
      ...
      ` : `<div style="margin:12px;background:rgba(245,158,11,0.08);border:1px dashed #f59e0b;padding:18px;border-radius:16px;text-align:center">
        ...
        <div style="margin-top:10px;font-size:10px;color:#f59e0b;background:#0b0f14;border:1px solid #263240;padding:6px 10px;border-radius:8px;display:inline-block">ID ${order.id.slice(0,8).toUpperCase()}</div>
      </div>`}
  ...
  orderChannel = supabase.channel('order-tracking-'+orderId)
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `id=eq.${orderId}` }, async payload=>{
      ...
      if(newOrder.driver_id){
        ...
        // Driver menolak / driver_id dikosongkan -> info ke penumpang
        if(newOrder.status==='searching'){
          ...
          // Alert ringan tanpa block
          console.log('Driver rejected, back to searching');
        }
