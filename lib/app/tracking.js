...
export async function loadActiveTracking(){
  ...
  currentOrderId = orderId;
  try{
    // Tampilkan loading dulu di HP
    if(card){
      card.style.display = 'block';
      card.innerHTML = `<div style="padding:12px;text-align:center"><div>⏳ Memuat tracking...</div><div style="font-size:10px;margin-top:4px">Order ID: ${orderId.slice(0,8)}</div></div>`;
    }
...
function showTrackingUI(order){
  ...
  // Tampilkan banner lock jika ada
  const banner = document.getElementById('activeOrderLockBanner');
  if(banner) banner.style.display='flex';
  // Jika status masih aktif, tampilkan modal lock kalau user coba interaksi (kita tidak auto popup mengganggu, tapi sediakan fungsi)
  if(['searching','pending','accepted','picked'].includes(order.status)){
    // Auto show lock modal ringan setelah 1 detik pertama saja biar jelas
    if(!sessionStorage.getItem('lockShown_'+order.id)){
      setTimeout(()=>{ 
        // Hanya show jika masih di halaman passenger
        if(location.hash.includes('passenger') && document.getElementById('activeOrderCard')){
          showTrackingLockModal(order);
          sessionStorage.setItem('lockShown_'+order.id, '1');
        }
      }, 800);
    }
  }
