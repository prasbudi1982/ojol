// tracking.js - PRODUKSI LENGKAP - fix penolakan driver di dalam kartu menunggu driver
...
function startDistanceUpdater(order){
  ...
  trackingInterval = setInterval(async ()=>{
    ...
    try{ const { data } = await supabase.from('driver_locations').select('*').eq('id', order.driver_id).single(); if(data) updateDriverDistance(data); }catch(e){}
  },5000);
}
