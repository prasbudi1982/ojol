// tracking.js - AUTO ASSIGN driver terdekat berurutan + timeout + auto cancel - single card
...
export async function loadActiveTracking(){
  ...
  try{
    ...
    if(order.driver_id){
      ...
      startAutoAssignTimeout(orderId);
    } else {
...
// ===== RENDER SINGLE CARD - AUTO ASSIGN INFO DI DALAM CARD =====
function renderActiveOrder(order, driver, opts={}){
  ...
  try{
    ...
    if(q.length>0){
      queueInfo = `<div style="margin-top:8px;background:#0b1220;border:1px solid #1e3a5f;border-radius:10px;padding:8px 10px;font-size:10px;color:#8aa0b8">
        <div>🔄 Auto-assign: Driver ${idx+1}/${q.length} • ID order tetap <b>${order.id.slice(0,6).toUpperCase()}</b></div>
        <div style="margin-top:4px;display:flex;gap:4px;flex-wrap:wrap">${q.map((d,i)=>`<span style="padding:2px 6px;border-radius:6px;font-size:9px;background:${i<idx?'#1f2937':i===idx?'#16a34a':'#1d2633'};color:${i===idx?'white':'#8aa0b8'};border:1px solid ${i===idx?'#22c55e':'#263240'}">${i+1}. ${d.name?.split(' ')[0]||'Driver'} ${i<idx?'❌':i===idx?'⏳':''}</span>`).join('')}</div>
      </div>`;
    }
  }catch(e){}
  ...
  const waitingCardInner = `
    <div style="margin:12px;background:rgba(245,158,11,0.08);border:1px dashed #f59e0b;padding:18px;border-radius:16px;text-align:center">
      ${isRejected ? `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:10px;display:flex;gap:8px;align-items:center;text-align:left;margin-bottom:12px"><div style="width:32px;height:32px;background:#ef4444;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:14px">⚠️</div><div style="flex:1"><div style="font-weight:800;color:#991b1b;font-size:12px">Driver Menolak</div><div style="font-size:11px;color:#7f1d1d;margin-top:1px">${rejectedName} tidak bisa melanjutkan. ${autoInfo?autoInfo:'Mencari driver terdekat selanjutnya...'}</div></div></div>` : ''}
      ${autoAssign ? `<div style="background:#0f172a;border:1px solid #22c55e;border-radius:10px;padding:10px;display:flex;gap:8px;align-items:center;text-align:left;margin-bottom:12px"><div style="width:32px;height:32px;background:#16a34a;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white">🔄</div><div style="flex:1"><div style="font-weight:800;color:#bbf7d0;font-size:12px">Mencoba Driver Selanjutnya</div><div style="font-size:11px;color:#86efac;margin-top:1px">${autoInfo||'Menghubungkan ke driver terdekat berikutnya...'}</div></div></div>` : ''}
      ${noDriver ? `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:10px;display:flex;gap:8px;align-items:center;text-align:left;margin-bottom:12px"><div style="width:32px;height:32px;background:#991b1b;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white">😔</div><div style="flex:1"><div style="font-weight:800;color:#991b1b;font-size:12px">Tidak Ada Driver Aktif</div><div style="font-size:11px;color:#7f1d1d;margin-top:1px">Tidak ada driver online di ${ACTIVE_KECAMATAN_NAME||'Suruh'} saat ini.</div></div></div>` : ''}
      ${!isRejected && !noDriver && !autoAssign ? `<div style="font-size:28px">⏳</div><div style="font-size:13px;color:#fbbf24;font-weight:700;margin-top:8px">Menunggu driver menerima order...</div><div style="font-size:11px;color:#8aa0b8;margin-top:4px">Order terkirim ke driver terdekat di ${ACTIVE_KECAMATAN_NAME||'Suruh'}</div><div style="font-size:10px;color:#f59e0b;margin-top:6px">⏱️ Auto pindah ke driver selanjutnya dalam <span id="autoTimeout">40</span> detik jika tidak merespon</div>` : ''}
      ${isRejected || autoAssign ? `<div style="font-size:13px;color:#fbbf24;font-weight:700;margin-top:4px">⏳ Mencari driver pengganti...</div>` : ''}
      <div style="margin-top:10px;font-size:10px;color:#f59e0b;background:#0b0f14;border:1px solid #263240;padding:6px 10px;border-radius:8px;display:inline-block">ID ${order.id.slice(0,8).toUpperCase()} • <span id="searchTimer">00:00</span> • <span id="autoTimeout2">${DRIVER_TIMEOUT}</span>s timeout</div>
      ${queueInfo}
    </div>`;
...
function subscribeOrderUpdates(orderId){
  ...
  orderChannel = supabase.channel('order-tracking-'+orderId)
    ...
    .on('broadcast', { event: 'driver_rejected' }, async payload=>{
      ...
      console.log('Broadcast driver_rejected diterima', driverName);
      ...
      try{
        ...
        if(cur){
          renderActiveOrder(cur, null, {rejected:true, driverName, autoInfo:`${driverName} menolak, mencoba driver selanjutnya...`});
          // Auto assign ke driver selanjutnya - ID ORDER TETAP SAMA
    ...
    .on('broadcast', { event: 'auto_assigned' }, async payload=>{
      ...
      console.log('Auto assigned broadcast', p);
      try{
        const { data: cur } = await supabase.from('orders').select('*').eq('id', orderId).single();
        if(cur){
...
function handleOrderRejection(newOrder){
  if(newOrder.driver_id){
    ...
    if(newOrder.status==='searching' && lastKnownDriverId && newOrder.driver_id===lastKnownDriverId){
      ...
      startAutoAssignTimeout(newOrder.id);
      return;
    }
    ...
    if(lastKnownDriverId || localStorage.getItem('last_driver_id')){
      if(newOrder.status==='searching'){
        ...
        // Auto assign sudah dipicu dari broadcast, tapi backup panggil lagi jika belum
        (async ()=>{ try{ const orderMod = await import('./order.js'); const q = orderMod.getAutoQueue(newOrder.id); if(q && q.length>0){ /* sudah ada queue, jangan double */ } }catch(e){} })();
        startNoDriverCheck(newOrder.id);
        return;
      }
    }
...
function startAutoAssignTimeout(orderId){
  if(autoAssignTimer){ clearInterval(autoAssignTimer); autoAssignTimer=null; }
  let elapsed = 0;
  try{
  ...
  autoAssignTimer = setInterval(async ()=>{
    elapsed++;
    const t1 = document.getElementById('autoTimeout');
    const t2 = document.getElementById('autoTimeout2');
    const t3 = document.getElementById('autoTimeoutDriver');
    const remain = Math.max(0, DRIVER_TIMEOUT - (elapsed % (DRIVER_TIMEOUT+1)));
    if(t1) t1.textContent = remain;
    if(t2) t2.textContent = remain;
    if(t3) t3.textContent = remain;
    
    if(elapsed % (DRIVER_TIMEOUT+1) === 0 && elapsed>0){
      // Timeout untuk driver ini habis, pindah ke selanjutnya
      try{
        ...
        if(cur && cur.status==='searching' && cur.driver_id){
          console.log(`Driver ${cur.driver_id} timeout ${DRIVER_TIMEOUT}s, auto assign next`);
          const orderMod = await import('./order.js');
          const queue = orderMod.getAutoQueue(orderId);
          const idx = orderMod.getAutoIndex(orderId);
          const driverName = queue[idx]?.name || 'Driver';
          renderActiveOrder(cur, null, {autoAssign:true, autoInfo:`${driverName} tidak merespon ${DRIVER_TIMEOUT}s, mencoba driver selanjutnya...`});
      ...
      }catch(e){ console.warn('auto timeout check fail', e.message); }
    }
  },1000);
}
      ...
      try{
        ...
        if(!error && (!onlineDrivers || onlineDrivers.length===0)){
          if(searchSeconds >= 30){
            const { data: cur } = await supabase.from('orders').select('status,driver_id').eq('id', orderId).single();
            if(cur && cur.status==='searching' && !cur.driver_id){
              // Cek apakah masih ada queue auto assign
              try{
                const orderMod = await import('./order.js');
                const q = orderMod.getAutoQueue(orderId);
                const idx = orderMod.getAutoIndex(orderId);
                if(q.length>0 && idx < q.length-1){
                  // Masih ada driver lain, jangan tampil noDriver dulu, lanjut auto assign
                  await orderMod.assignToNextDriver(orderId, 'Tidak ada driver online - coba queue');
                  return;
                }
              }catch(e){}
              ...
              setTimeout(async ()=>{
                try{
                  ...
                  try{ localStorage.removeItem('auto_queue_'+orderId); localStorage.removeItem('auto_index_'+orderId); }catch(e){}
      ...
      try{
        ...
        if(cur && cur.status==='searching'){
          try{
            ...
            const idx = orderMod.getAutoIndex(orderId);
            if(q.length>0 && idx < q.length-1){
              await orderMod.assignToNextDriver(orderId, 'Timeout 90s - coba driver selanjutnya');
              searchSeconds = 0;
              return;
            }
          }catch(e){}
