...
// ===== TRACKING DETAIL MODAL - modal mengunci halaman order - DARK THEME =====
function ensureTrackingDetailModal(){
  let modal = document.getElementById('trackingDetailModal');
  if(modal) return modal;
  const div = document.createElement('div');
  div.id = 'trackingDetailModal';
  div.style.cssText = 'display:none;position:fixed;inset:0;background:rgba(11,15,20,0.88);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);z-index:9997;overflow-y:auto;padding:16px;align-items:flex-start;justify-content:center';
  div.innerHTML = `
    <div style="width:100%;max-width:440px;margin:0 auto;padding-top:4px;padding-bottom:24px">
      <div id="trackingDetailContent"></div>
      <div style="text-align:center;margin-top:14px;font-size:10px;color:#64748b;display:flex;align-items:center;justify-content:center;gap:6px"><span style="width:32px;height:1px;background:#263240"></span> 🔒 Order terkunci sampai selesai <span style="width:32px;height:1px;background:#263240"></span></div>
    </div>
  `;
  document.body.appendChild(div);
  // close prevention - klik backdrop tidak tutup
...
function renderActiveOrder(order, driver, opts={}){
  ...
  const isRejected = opts && opts.rejected;
  const rejectedDriverName = opts && opts.driverName ? opts.driverName : (localStorage.getItem('last_driver_name')||'Driver');
  ...
  card.innerHTML = `
    ${isRejected ? `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:12px;margin:12px 12px 0;display:flex;gap:10px;align-items:center"><div style="width:36px;height:36px;background:#ef4444;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:18px">⚠️</div><div style="flex:1"><div style="font-weight:800;color:#991b1b;font-size:13px">Driver Menolak Order</div><div style="font-size:11px;color:#7f1d1d;margin-top:2px">${rejectedDriverName} tidak bisa melanjutkan. Mencari driver lain di Suruh...</div></div></div>` : ''}
    <div style="border:1px solid #263240;border-radius:20px;padding:0;overflow:hidden;background:#151c25;box-shadow:0 12px 40px rgba(0,0,0,0.5);color:#e6edf5;margin-top:${isRejected?'12px':'0'}">
      <div style="background:linear-gradient(135deg, ${getAppTheme().primary}, #16a34a);color:#052e16;padding:14px 16px;display:flex;justify-content:space-between;align-items:center">
        <div style="display:flex;align-items:center;gap:10px">
          <span style="width:36px;height:36px;background:rgba(0,0,0,0.2);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px">${vehicleIcon}</span>
          <div>
            <div style="font-size:13px;font-weight:800;letter-spacing:0.3px;color:#052e16">${order.vehicle_type?.toUpperCase()||'MOTOR'} • ${order.trip_type==='roundtrip'?'PP':'Sekali'}</div>
            <div style="font-size:10px;color:rgba(5,46,22,0.7);font-weight:600">ID ${order.id.slice(0,6).toUpperCase()} • ${new Date(order.created_at).toLocaleTimeString('id-ID')}</div>
          </div>
        </div>
        <span style="background:rgba(0,0,0,0.25);color:white;padding:6px 12px;border-radius:20px;font-size:10px;font-weight:800;backdrop-filter:blur(4px)">${statusText}</span>
      </div>
      
      <div style="padding:14px 16px 0;display:flex;gap:6px;align-items:center">
        <div style="flex:1;height:6px;border-radius:3px;background:${step>=1?getAppTheme().primary:'#1d2633'}"></div>
        <div style="flex:1;height:6px;border-radius:3px;background:${step>=2?getAppTheme().primary:'#1d2633'}"></div>
        <div style="flex:1;height:6px;border-radius:3px;background:${step>=3?getAppTheme().primary:'#1d2633'}"></div>
        <div style="flex:1;height:6px;border-radius:3px;background:${step>=4?getAppTheme().primary:'#1d2633'}"></div>
        <div style="flex:1;height:6px;border-radius:3px;background:${step>=5?getAppTheme().primary:'#1d2633'}"></div>
      </div>
      <div style="padding:6px 16px 12px;display:flex;justify-content:space-between;font-size:9px;color:#8aa0b8;font-weight:600;letter-spacing:0.3px">
      ...
      <div style="margin:0 12px;background:#1d2633;border:1px solid #263240;padding:12px;border-radius:14px">
        <div style="font-size:13px;line-height:1.5;color:#e6edf5">
          <div style="display:flex;gap:8px"><span>📍</span><div><b style="color:#8aa0b8;font-size:11px">PICKUP</b><br/><span style="color:#e6edf5;font-weight:600">${order.pickup_text||order.pickup||'-'}</span></div></div>
          <div style="display:flex;gap:8px;margin-top:10px"><span>🎯</span><div><b style="color:#8aa0b8;font-size:11px">TUJUAN</b><br/><span style="color:#e6edf5;font-weight:600">${order.dest_text||order.destination||'-'}</span></div></div>
        </div>
        <div style="margin-top:12px;padding-top:10px;border-top:1px dashed #263240;display:flex;justify-content:space-between;align-items:center">
          <span style="font-size:11px;color:#8aa0b8;font-weight:600">📏 ${order.distance_km?.toFixed? order.distance_km.toFixed(2) : order.distance_km||'-'} km</span>
          <span style="font-size:13px;font-weight:800;color:#e6edf5;background:#0b0f14;border:1px solid #263240;padding:6px 12px;border-radius:10px">💰 Rp ${order.estimated_cost?.toLocaleString('id-ID')||'-'}</span>
        </div>
      </div>

      ${driver ? `
      <div style="margin:12px;background:#1d2633;border:1px solid #263240;border-radius:16px;padding:14px">
        <div style="display:flex;gap:12px;align-items:center">
          <div style="width:48px;height:48px;background:linear-gradient(135deg, #22c55e, #16a34a);border-radius:14px;display:flex;align-items:center;justify-content:center;color:#052e16;font-size:20px;font-weight:800;flex-shrink:0">${driver.name?.charAt(0)||'D'}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:800;font-size:14px;color:#e6edf5">${driver.name} ${driver.jenis_kendaraan==='mobil'?'🚗':'🏍️'}</div>
            <div style="font-size:11px;color:#8aa0b8;margin-top:2px">${driver.nopol||''} • ${driver.tipe_motor||''} • ${driver.jenis_kendaraan||'motor'}</div>
          </div>
          <button data-report-user="${driver.id}" data-report-name="${driver.name}" data-report-role="driver" data-report-order="${order.id}" style="background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.25);color:#f87171;border-radius:10px;padding:8px 10px;font-size:14px">🚩</button>
        </div>
        <div style="margin-top:12px;background:#0b0f14;border:1px solid #263240;border-radius:12px;padding:12px">
          <div id="trackingDistance" style="font-weight:800;color:#4ade80;font-size:13px">📍 Menghitung jarak driver...</div>
          <div id="trackingETA" style="font-size:11px;color:#8aa0b8;margin-top:4px">⏱️ Menunggu lokasi driver...</div>
        </div>
        <div style="margin-top:12px;display:flex;gap:8px">
          ${driver.hp ? `<a href="https://wa.me/${driver.hp.replace(/[^0-9]/g,'').replace(/^0/,'62')}?text=Halo%20${encodeURIComponent(driver.name)}%20order%20${order.id}" target="_blank" style="flex:1;background:#25D366;color:white;padding:12px;text-align:center;border-radius:12px;text-decoration:none;font-size:13px;font-weight:800">💬 WA Driver</a>` : ''}
          <a id="btnOpenDriverMap" href="#" style="${driver.hp?'':'flex:1'};background:#151c25;border:1px solid #263240;color:#e6edf5;padding:12px;border-radius:12px;text-align:center;text-decoration:none;font-size:12px;font-weight:700">🗺️ Maps</a>
        </div>
      </div>
      ` : `<div style="margin:12px;background:rgba(245,158,11,0.08);border:1px dashed #f59e0b;padding:18px;border-radius:16px;text-align:center">
        ...
        <div style="font-size:13px;color:#fbbf24;font-weight:700;margin-top:8px">Menunggu driver menerima order...</div>
        <div style="font-size:11px;color:#8aa0b8;margin-top:4px">Order terkirim ke driver terdekat di ${ACTIVE_KECAMATAN_NAME||'Suruh'}</div>
        <div style="margin-top:10px;font-size:10px;color:#f59e0b;background:#0b0f14;border:1px solid #263240;padding:6px 10px;border-radius:8px;display:inline-block">ID ${order.id.slice(0,8).toUpperCase()} • ${new Date(order.created_at).toLocaleTimeString('id-ID')}</div>
      </div>`}
      
      <div style="padding:12px;background:#0b0f14;border-top:1px solid #263240;display:flex;gap:8px;margin-top:12px;border-radius:0 0 20px 20px">
        <button id="btnCancelTracking" style="flex:1;background:#1d2633;border:1px solid #263240;color:#f87171;padding:12px;border-radius:12px;font-size:13px;font-weight:700">❌ Batalkan</button>
        <button id="btnCompleteOrder" style="flex:1;background:#22c55e;color:#052e16;border:none;padding:12px;border-radius:12px;font-size:13px;font-weight:800">✅ Selesai</button>
      </div>
    </div>
  `;
  ...
  orderChannel = supabase.channel('order-tracking-'+orderId)
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `id=eq.${orderId}` }, async payload=>{
      ...
      if(['completed','cancelled','rejected'].includes(newOrder.status)){
        ...
        if(newOrder.status==='completed' && newOrder.driver_id){
          try{
            ...
            clearActiveTracking();
            hideTrackingUI();
            const tm = document.getElementById('trackingDetailModal');
            if(tm) tm.style.display='none';
            document.body.style.overflow='';
            const mod = await import('./rating.js');
            mod.openRatingModal(newOrder.driver_id, drv?.name||'Driver', newOrder.id);
            return;
          }catch(e){
            console.error('auto rating error', e);
          }
        }
        alert(`Order ${newOrder.status}: ${newOrder.status==='completed'?'Terima kasih!':'Dibatalkan'}`);
        clearActiveTracking();
        hideTrackingUI();
        const tm2 = document.getElementById('trackingDetailModal');
        if(tm2) tm2.style.display='none';
        document.body.style.overflow='';
        location.hash = '#/';
        return;
      }
      // Jika baru dapat driver - driver berangkat jemput
      if(newOrder.driver_id){
        ...
        try{ localStorage.setItem('last_driver_name', driver?.name||'Driver'); localStorage.setItem('last_assigned_driver_name', driver?.name||'Driver'); }catch(e){}
        ...
        // Driver menolak / driver_id dikosongkan -> INFO SAMPAI KE PENUMPANG DI DALAM MODAL
        if(newOrder.status==='searching'){
          try{ if(navigator.vibrate) navigator.vibrate([200,100,200,100,200]); }catch(e){}
          const si = document.getElementById('searchInfo');
          if(si) si.textContent = `⚠️ Driver menolak - mencari driver lain...`;
          const lastName = localStorage.getItem('last_driver_name') || localStorage.getItem('last_assigned_driver_name') || 'Driver';
          console.log('Driver rejected, back to searching - show banner', lastName);
          renderActiveOrder(newOrder, null, {rejected:true, driverName:lastName});
          setTimeout(()=>{
            try{ localStorage.removeItem('last_driver_name'); localStorage.removeItem('last_assigned_driver_name'); }catch(e){}
          }, 8000);
        } else {
...
export async function loadDriverOrders(driverProfile){
  ...
  try{
    ...
    let rejectedIds = [];
    try{ rejectedIds = JSON.parse(localStorage.getItem('rejected_orders_'+driverProfile.id)||'[]'); }catch(e){}
    let orders = allOrders.filter(o => (!o.driver_id || o.driver_id === driverProfile.id) && !rejectedIds.includes(o.id));
