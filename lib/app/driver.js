...
let _locationPoolInterval = null;
let _watchId = null;

function startLocationPooling(driverProfile){
  if(_locationPoolInterval) clearInterval(_locationPoolInterval);
  if(_watchId !== null){ try{ navigator.geolocation.clearWatch(_watchId); }catch(e){} _watchId=null; }

  // update langsung sekali
  const updateLoc = async (lat,lng,heading,speed)=>{
    try{
      ...
      console.warn('pool update fail', e.message);
    }
  };

  // watchPosition = lebih hemat baterai & akurat saat jalan
  try{
    _watchId = navigator.geolocation.watchPosition(
      (pos)=>{
        ...
        const speed = (pos.coords.speed||0)*3.6; // m/s ke km/h
        updateLoc(lat,lng,heading,speed);
      },
  ...
  // fallback pooling tiap 10 detik kalau watch tidak jalan
  _locationPoolInterval = setInterval(async ()=>{
    try{
      ...
      await updateLoc(lat,lng,heading,speed);
    }catch(e){}
  }, 10000);
}

function stopLocationPooling(){
  if(_locationPoolInterval){ clearInterval(_locationPoolInterval); _locationPoolInterval=null; }
  if(_watchId !== null){ try{ navigator.geolocation.clearWatch(_watchId); }catch(e){} _watchId=null; }
}
...
export function initDriverPage(driverProfile){ 
  ...
  // === PATCH POOLING: set lokasi + update terus saat online ===
  setTimeout(()=>{
    const isOnline = (driverProfile.status||'').toLowerCase()==='online' || (driverProfile.status||'').toLowerCase()==='active';
    if(isOnline){
      startLocationPooling(driverProfile);
    }

    const btnOn = document.getElementById('btnOnline');
    const btnOff = document.getElementById('btnOffline');
    if(btnOn && !btnOn.dataset.poolPatched){
      btnOn.dataset.poolPatched='1';
      btnOn.addEventListener('click', async ()=>{
        const origText = btnOn.textContent;
        btnOn.disabled=true; btnOn.textContent='⏳ GPS...';
        try{
          ...
          try{ await supabase.from('users').update({ status: 'online' }).eq('id', driverProfile.id); }catch(e){}
          startLocationPooling(driverProfile);
          btnOn.textContent='✅ Online - Pooling Aktif';
          setTimeout(()=>location.reload(), 600);
        }catch(e){
          alert('❌ Gagal set lokasi: '+(e.message||e));
          btnOn.disabled=false; btnOn.textContent=origText;
        }
      });
    }
    if(btnOff && !btnOff.dataset.poolPatched){
      btnOff.dataset.poolPatched='1';
      btnOff.addEventListener('click', async ()=>{
        stopLocationPooling();
        try{ await supabase.from('users').update({ status: 'offline' }).eq('id', driverProfile.id); }catch(e){}
        location.reload();
      });
    }
  }, 900);
...
function subscribeDriverOrders(driverProfile){
  ...
  // FIX BERULANG: filter hanya searching + dedup cancelled
  let _lastCancelledIds = new Set();
  driverOrdersChannel = supabase.channel('driver-orders-'+driverProfile.id)
    .on('postgres_changes', { event:'INSERT', schema:'public', table:'orders' }, payload=>{
      const o = payload.new;
      // hanya broadcast / untuk saya yang status searching
      if(o && ['searching','pending','new','open'].includes(o.status)){
        loadDriverOrders(driverProfile, false);
      }
    })
    .on('postgres_changes', { event:'UPDATE', schema:'public', table:'orders', filter: `driver_id=eq.${driverProfile.id}` }, payload=>{
      const o = payload.new;
      if(o && o.status==='cancelled' && !_lastCancelledIds.has(o.id)){
        _lastCancelledIds.add(o.id);
        try{ notifyOjol('❌ Order Dibatalkan Penumpang', `${o.pickup_text||''} → ${o.dest_text||''} dibatalkan penumpang`, '/#/driver', o.id); }catch(e){}
    ...
    .on('postgres_changes', { event:'UPDATE', schema:'public', table:'orders' }, payload=>{
      const o = payload.new;
      if(o && o.old_record && o.old_record?.status !== 'searching' && o.status === 'searching'){
        loadDriverOrders(driverProfile, false);
      }
    })
    .subscribe();
  let _lastFoodCancelledIds = new Set();
  driverFoodChannel = supabase.channel('driver-food-'+driverProfile.id)
    ...
    .on('postgres_changes', { event:'UPDATE', schema:'public', table:'food_orders', filter: `driver_id=eq.${driverProfile.id}` }, payload=>{
      const o = payload.new;
      if(o && o.status==='cancelled' && !_lastFoodCancelledIds.has(o.id)){
        _lastFoodCancelledIds.add(o.id);
        try{ notifyFood('❌ Food Dibatalkan Pembeli', `${o.store_name||'Warung'} dibatalkan pembeli`, '/#/driver', o.id); }catch(e){}
