...
function buildWaLink(o){
  const raw = extractCustomerPhone(o);
  ...
  return { link:`https://wa.me/${norm}?text=${encodeURIComponent(msg)}`, raw, norm };
}
async function enrichOrdersWithUserPhone(orders){
  try{
    if(!orders||!orders.length) return orders;
    // kumpulkan semua kemungkinan user id
    const userIds = [...new Set(orders.map(o=> o.passenger_id || o.customer_id || o.user_id || o.buyer_id || o.customer || o.passenger).filter(Boolean))];
    if(!userIds.length) return orders;
    // fetch dari tabel users - coba ambil semua field phone yang mungkin ada
    const {data:users, error} = await supabase.from('users').select('id, phone, wa, whatsapp, phone_number, wa_number').in('id', userIds);
    if(error || !users) return orders;
    const map={};
    users.forEach(u=>{
      const phone = u.phone || u.wa || u.whatsapp || u.phone_number || u.wa_number;
...
export async function loadDriverFoodOrders(driverProfile, silent=false){
  ...
  try{
    ...
    // ENRICH WA dari tabel users (customer_id / user_id)
    orders = await enrichOrdersWithUserPhone(orders);
  ...
  try{
    ...
    card.innerHTML=`<h4>🚚 Order Aktif (${all.length})</h4>`+all.map(o=>{
      if(o._type==='food'){
        ...
        const links=buildMapLinks(o);
        const wa=buildWaLink(o);
        const waBtn = wa ? `<a href="${wa.link}" target="_blank" class="btn secondary" style="font-size:10px;background:#25D366;color:white;border:none;display:flex;align-items:center;justify-content:center;gap:3px">💬 WA Pembeli</a>` : '';
        return `<div style="border:1px solid var(--border);border-radius:12px;padding:12px;margin:8px 0;background:var(--card2)"><div style="font-size:12px;font-weight:800;color:var(--text)">🍔 ${o.pickup_text||''} • ${map[o.status]||o.status}</div><div style="font-size:11px;color:var(--muted)">${o.dest_text||''}</div>
          <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">
            <a href="${links.pickupLink}" target="_blank" class="btn secondary" style="font-size:10px">🏪 Warung</a>
            <a href="${links.destLink}" target="_blank" class="btn secondary" style="font-size:10px">📍 Pembeli</a>
        ...
        const links=buildMapLinks(o);
        const wa=buildWaLink(o);
        const waBtn = wa ? `<a href="${wa.link}" target="_blank" class="btn secondary" style="font-size:10px;background:#25D366;color:white;border:none">💬 WA Penumpang</a>` : '';
        return `<div style="border:1px solid var(--border);border-radius:12px;padding:12px;margin:8px 0;background:var(--card2)"><div style="font-size:12px;font-weight:800;color:var(--text)">🏍️ Ojek ${o.status}</div><div style="font-size:11px;color:var(--muted)">${o.pickup_text||''} → ${o.dest_text||''}</div>
          <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">
            <a href="${links.pickupLink}" target="_blank" class="btn secondary" style="font-size:10px">📍 Jemput</a>
            <a href="${links.destLink}" target="_blank" class="btn secondary" style="font-size:10px">🎯 Antar</a>
            <a href="${links.routeLink}" target="_blank" class="btn secondary" style="font-size:10px">🗺️ Rute</a>
            ${waBtn}
          </div>
