...
async function enrichOrdersWithUserPhone(orders){
  try{
    ...
    const {data:users, error} = await supabase.from('users').select('id, phone, wa, whatsapp, phone_number, wa_number, no_hp').in('id', userIds);
    if(error || !users) return orders;
    const map={};
    users.forEach(u=>{
      const phone = u.phone || u.wa || u.whatsapp || u.phone_number || u.wa_number || u.no_hp;
...
async function enrichOrdersWithStorePhone(orders){
  try{
    ...
    // fetch stores
    const {data:stores, error} = await supabase.from('stores').select('id, phone, wa, phone_number, wa_number, owner_id, user_id, name').in('id', storeIds);
    if(error || !stores) return orders;
    const storeMap={};
    const ownerIds=[];
    stores.forEach(s=>{
      const phone = s.phone || s.wa || s.phone_number || s.wa_number;
    ...
    if(ownerIds.length){
      try{
        const {data:owners} = await supabase.from('users').select('id, phone, wa, whatsapp, phone_number, wa_number, no_hp').in('id', [...new Set(ownerIds)]);
        const ownerPhoneMap={};
        (owners||[]).forEach(u=>{
          const ph = u.phone || u.wa || u.whatsapp || u.phone_number || u.wa_number || u.no_hp;
