...
import { notifyOjol, notifyFood, showLocalNotification } from './push.js';

// ===== FINAL WORKING - DIRECT NOTIF TANPA FILTER (JAMINAN BUNYI) =====
function driverShowNotif(title, body, url='/#/driver', tag='driver-notif'){
  try{
    console.log('🔔 DRIVER NOTIF:', title, body);
    // 1. Coba via push.js dulu (kalau setting enabled)
    try{
      if(title.includes('🍔') || title.toLowerCase().includes('food') || body.toLowerCase().includes('warung')){
        notifyFood(title, body, url);
      } else {
        notifyOjol(title, body, url);
      }
    }catch(e){}
    // 2. DIRECT - bypass filter, paksa muncul walau setting disabled
    if('Notification' in window && Notification.permission==='granted'){
      navigator.serviceWorker.ready.then(reg=>{
        reg.showNotification(title, {
          ...
          data: { url: url }
        }).catch(e=> console.warn('Direct notif fail', e));
      }).catch(e=>{
        // Fallback ke Notification API biasa
        try{ new Notification(title, { body: body, icon: '/icons/icon-192.png' }); }catch(e2){}
      });
      // Suara + getar - JAMINAN
      try{ if(navigator.vibrate) navigator.vibrate([300,100,300,100,300]); }catch(e){}
      try{ const audio = new Audio('/sounds/order.mp3'); audio.volume=1.0; audio.play().catch(()=>{ const a2=new Audio('/sounds/notif.mp3'); a2.play().catch(()=>{}); }); }catch(e){}
    } else {
      console.warn('Permission not granted:', Notification.permission);
    }
  }catch(e){ console.error('driverShowNotif fail', e); }
}
// ===== END DIRECT NOTIF =====
...
export function driverRejectOrder(orderId){
  try{ notifyOjol('❌ Kamu Tolak Ojol', `Order ${orderId.slice(0,6)} ditolak`, '/#/driver'); }catch(e){}
  return supabase.from('orders').update({ status:'cancelled' }).eq('id', orderId).select().single();
}
export function driverRejectOrderToSearch(orderId){
  try{ notifyOjol('❌ Tolak Ojol', `Order ${orderId.slice(0,6)} ditolak - cari driver lain`, '/#/driver'); }catch(e){}
  return supabase.from('orders').update({ status:'searching', driver_id: null }).eq('id', orderId).select().single();
}
...
export async function loadDriverOrders(driverProfile, silent=false){
  ...
  try{
    ...
    // FIX FINAL WORKING - NOTIF OJOL TERMASUK ORDER PERTAMA
    try{
      const k = '_driver_ojol_ids_'+driverProfile.id;
      const prev = JSON.parse(localStorage.getItem(k)||'[]');
      const curr = orders.map(o=>o.id);
      const baru = curr.filter(id=> !prev.includes(id));
      if(baru.length>0){
        // Selalu notif walau silent, kecuali load pertama kali yang memang kosong sebelumnya dan ini polling awal
        const isFirstLoad = prev.length===0 && !localStorage.getItem(k+'_initialized');
        if(!isFirstLoad || baru.length>1 || !silent){
          const first = orders.find(x=> baru.includes(x.id));
          if(first){
            const title = `📦 ${baru.length} Order Ojol Baru!`;
            const body = `${first.pickup_text||'-'} → ${first.dest_text||'-'} • Rp ${Number(first.estimated_cost||0).toLocaleString('id-ID')} - KLIK TERIMA`;
            driverShowNotif(title, body, '/#/driver', 'ojol-'+first.id);
          }
        }
        if(prev.length===0) localStorage.setItem(k+'_initialized','1');
      }
      localStorage.setItem(k, JSON.stringify(curr));
      lastOjolCount = orders.length;
    }catch(e){ console.warn('ojol notif fail', e); }
    if(orders.length===0){ box.innerHTML=`<div style="padding:12px;text-align:center;color:var(--muted);font-size:11px">Tidak ada order ojek</div>`; return; }
    ...
    box.innerHTML=orders.map(o=>{
      ...
      return `<div class="card" style="margin:8px 0;padding:12px;border:1px solid var(--border);border-radius:16px;background:var(--card);box-shadow:0 2px 12px var(--shadow)">
        ...
        <div style="margin-top:8px;display:flex;gap:6px"><button onclick="window._driverAccept('${o.id}')" class="btn primary" style="flex:1;font-size:11px;border-radius:10px">✅ Terima Ojek</button><button onclick="window._driverReject('${o.id}')" class="btn secondary" style="flex:1;font-size:11px;border-radius:10px;opacity:1;border:1px solid #ef4444;color:#ef4444;background:var(--card)">❌ Tolak</button></div>
      </div>`;
    }).join('');
...
export async function loadDriverFoodOrders(driverProfile, silent=false){
  ...
  try{
    ...
    // === TAMBAHAN HANYA NOTIF FOOD (TIDAK UBAH LAIN) ===
    try{
      const k = '_driver_food_ids_'+driverProfile.id;
      const prev = JSON.parse(localStorage.getItem(k)||'[]');
      const curr = orders.map(o=>o.id);
      const baru = curr.filter(id=> !prev.includes(id));
      if(baru.length>0 && (!silent || prev.length>0)){
        const o = orders.find(x=> baru.includes(x.id));
        if(o){
          const nm = o.store_name || o.pickup_text || 'Warung';
          const tot = o.total || o.total_price || 0;
          driverShowNotif(`🍔 ${baru.length} Order Food Baru!`, `${nm} • Rp ${Number(tot).toLocaleString('id-ID')} • ${o.dest_text||''} - KLIK TERIMA`, '/#/driver', 'food-'+o.id);
          try{ if(navigator.vibrate) navigator.vibrate([200,100,200]); }catch(e){}
          try{ new Audio('/sounds/order.mp3').play().catch(()=>{}); }catch(e){}
        }
      }
      localStorage.setItem(k, JSON.stringify(curr));
    }catch(e){ console.warn('food notif fail', e); }
    // === END TAMBAHAN ===
    if(orders.length===0){ box.innerHTML=`<div style="padding:12px;text-align:center;color:var(--muted);font-size:11px">Tidak ada order makanan</div>`; return; }
...
function subscribeDriverOrders(driverProfile){
  ...
  driverOrdersChannel = supabase.channel('driver-orders-'+driverProfile.id)
    .on('postgres_changes', { event:'INSERT', schema:'public', table:'orders' }, payload=>{
      const o = payload.new;
      if(o && ['searching','pending','new','open','created','waiting'].includes(o.status)){
        if(!o.driver_id || o.driver_id===driverProfile.id){
          driverShowNotif(`📦 Order Ojol Baru! ${o.vehicle_type||''}`, `${o.pickup_text||''} → ${o.dest_text||''} • Rp ${Number(o.estimated_cost||0).toLocaleString('id-ID')} - KLIK TERIMA`, '/#/driver', 'ojol-insert-'+o.id);
        }
      }
      loadDriverOrders(driverProfile, false);
      loadDriverActiveOrders(driverProfile);
    })
    .on('postgres_changes', { event:'*', schema:'public', table:'orders' }, payload=>{
      const o = payload.new;
      if(o && o.status==='cancelled'){
        try{ driverShowNotif('❌ Order Dibatalkan Penumpang', `${o.pickup_text||''} → ${o.dest_text||''} dibatalkan`, '/#/driver', 'cancel-'+o.id); }catch(e){}
        try{ setTimeout(()=>{ alert('❌ Order ojek dibatalkan penumpang!\n'+(o.pickup_text||'')+' → '+(o.dest_text||'')); }, 300); }catch(e){}
      }
      loadDriverOrders(driverProfile, false); 
      loadDriverActiveOrders(driverProfile);
    })
  ...
  driverFoodChannel = supabase.channel('driver-food-'+driverProfile.id)
    .on('postgres_changes', { event:'INSERT', schema:'public', table:'food_orders' }, payload=>{
      const o = payload.new;
      if(o && ['searching_driver','driver_assigned'].includes(o.status)){
        const isForMe = o.driver_id && o.driver_id === driverProfile.id;
        const isBroadcast = !o.driver_id;
        if(isForMe || isBroadcast){
          // === TAMBAHAN NOTIF FOOD INSERT ===
          try{ notifyFood(`🍔 Order Food Baru! ${o.store_name||'Warung'}`, `Rp ${Number(o.total||0).toLocaleString('id-ID')} • ${o.dest_text||''} - KLIK TERIMA`, '/#/driver', o.id); }catch(e){}
          // ===
          loadDriverFoodOrders(driverProfile, false);
          loadDriverActiveOrders(driverProfile);
        }
      }
    })
    .on('postgres_changes', { event:'UPDATE', schema:'public', table:'food_orders' }, payload=>{
      const o = payload.new;
      if(o && o.status==='cancelled'){
        try{ notifyFood('❌ Food Dibatalkan Pembeli', `${o.store_name||'Warung'} dibatalkan pembeli`, '/#/driver'); }catch(e){}
        try{ setTimeout(()=>{ alert('❌ Order makanan dibatalkan pembeli!\n'+(o.store_name||'')); }, 300); }catch(e){}
      }
      loadDriverFoodOrders(driverProfile, false); 
      loadDriverActiveOrders(driverProfile);
    })
  ...
  driverActiveChannel = supabase.channel('driver-active-'+driverProfile.id)
    .on('postgres_changes', { event:'*', schema:'public', table:'food_orders', filter:'driver_id=eq.'+driverProfile.id }, payload=>{
      const o = payload.new;
      if(o && o.status==='cancelled'){
        try{ notifyFood('❌ Food Dibatalkan Pembeli', 'Order AKTIF dibatalkan pembeli', '/#/driver'); }catch(e){}
        try{ alert('❌ Order makanan AKTIF dibatalkan pembeli!'); }catch(e){}
      }
      loadDriverActiveOrders(driverProfile);
    })
    .on('postgres_changes', { event:'*', schema:'public', table:'orders', filter:'driver_id=eq.'+driverProfile.id }, payload=>{
      const o = payload.new;
      if(o && o.status==='cancelled'){
        try{ notifyOjol('❌ Order Dibatalkan Penumpang', 'Order AKTIF dibatalkan penumpang', '/#/driver'); }catch(e){}
        try{ alert('❌ Order ojek AKTIF dibatalkan penumpang!'); }catch(e){}
      }
      loadDriverActiveOrders(driverProfile);
    })
    .subscribe();
  ...
  driverOrdersInterval = setInterval(()=>{ loadDriverOrders(driverProfile, false); loadDriverFoodOrders(driverProfile, false); loadDriverActiveOrders(driverProfile); }, 3000);
}
...
if(typeof window !== 'undefined'){
  ...
  window._driverReject = async (orderId)=>{
    try{
      if(!confirm('Tolak order ojol ini? Penumpang akan dapat info pembatalan.')) return;
      try{
        const { data: { user } } = await supabase.auth.getUser();
        const { data: profile } = await supabase.from('users').select('id').eq('google_id', user.id).single();
        const key = 'rejected_orders_'+profile.id;
        let list = []; try{ list = JSON.parse(localStorage.getItem(key)||'[]'); }catch(e){}
        if(!list.includes(orderId)){ list.push(orderId); localStorage.setItem(key, JSON.stringify(list)); }
      }catch(e){}
      const res = await driverRejectOrder(orderId);
      if(res.error) throw res.error;
      alert('❌ Order ojol ditolak. Penumpang akan dapat notifikasi pembatalan.');
      try{
        const { data: { user } } = await supabase.auth.getUser();
        const { data: profile } = await supabase.from('users').select('*').eq('google_id', user.id).single();
        await loadDriverOrders(profile, false);
      }catch(e){}
    }catch(e){ alert('Gagal tolak: '+e.message); }
  };
  window._driverRejectToSearch = async (orderId)=>{
    try{
      if(!confirm('Tolak dan kembalikan ke pencarian driver lain?')) return;
      try{
        const { data: { user } } = await supabase.auth.getUser();
        const { data: profile } = await supabase.from('users').select('id').eq('google_id', user.id).single();
        const key = 'rejected_orders_'+profile.id;
      ...
      const res = await driverRejectOrderToSearch(orderId);
      if(res.error) throw res.error;
      alert('↩️ Order dikembalikan ke pencarian driver lain.');
      location.reload();
    }catch(e){ alert('Gagal: '+e.message); }
  };
...
// Hook lama dihapus - sudah ada direct notif di loadDriverFoodOrders


// === DEBUG DRIVER NOTIF - PANGGIL DI CONSOLE ===
if(typeof window !== 'undefined'){
  window._testDriverNotif = async function(){
    console.log('🧪 Test driver notif...');
    console.log('Permission:', Notification.permission);
    console.log('SW ready:', 'serviceWorker' in navigator);
    console.log('push-enabled:', localStorage.getItem('push-enabled'));
    console.log('push-auto-active:', localStorage.getItem('push-auto-active'));
    
    if(Notification.permission!=='granted'){
      const p = await Notification.requestPermission();
      console.log('Permission result:', p);
      if(p!=='granted'){ alert('Harus Allow dulu! Permission: '+p); return; }
    }
    
    // Test 1: Direct SW
    try{
      const reg = await navigator.serviceWorker.ready;
      await reg.showNotification('🧪 Test 1 - Direct SW', {
        body: 'Kalau ini muncul, SW jalan!',
        icon: '/icons/icon-192.png',
        tag: 'test-direct-'+Date.now(),
        vibrate: [200,100,200]
      });
      console.log('✅ Test 1 Direct SW OK');
    }catch(e){ console.error('❌ Test 1 fail', e); }
    
    // Test 2: Via driverShowNotif
    setTimeout(()=>{
      driverShowNotif('🧪 Test 2 - Ojol', 'Test order ojol baru masuk - KLIK TERIMA', '/#/driver', 'test-ojol');
    }, 1000);
    
    // Test 3: Food
    setTimeout(()=>{
      driverShowNotif('🧪 Test 3 - Food', 'Test order food baru - Warung Sego - KLIK TERIMA', '/#/driver', 'test-food');
    }, 2000);
    
    alert('3 test notif dikirim! Cek HP: harus muncul 3 notif dalam 2 detik. Kalau gak muncul, SW belum aktif atau permission denied.');
  };
  
  window._clearDriverNotifCache = function(){
    const keys = ['_driver_ojol_ids_','_driver_food_ids_','_food_ids','rejected_orders_','_driver_ojol_ids','_driver_food_ids'];
    Object.keys(localStorage).forEach(k=>{
      if(keys.some(x=> k.includes(x))) localStorage.removeItem(k);
    });
    localStorage.removeItem('_driver_ojol_ids_'+(window._currentDriverId||'')+'_initialized');
    console.log('Cache notif driver di-clear, reload halaman driver');
    location.reload();
  };
}
