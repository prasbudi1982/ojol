...
function subscribeDriverOrders(driverProfile){
  ...
  // FIX PENTING: penumpang batal -> driver auto update + popup
  driverOrdersChannel = supabase.channel('driver-orders-'+driverProfile.id)
    .on('postgres_changes', { event:'*', schema:'public', table:'orders' }, payload=>{
      console.log('[driver] realtime orders', payload.eventType, payload.new?.id, payload.new?.status);
      if(payload.eventType==='UPDATE' && payload.new && payload.new.status==='cancelled'){
        try{ 
          notifyOjol('❌ Order Dibatalkan Penumpang', `${payload.new.pickup_text||''} → ${payload.new.dest_text||''} dibatalkan`, '/#/driver');
          // Popup langsung - ini yang diminta
          setTimeout(()=> alert('❌ Order ojek dibatalkan penumpang!
'+(payload.new.pickup_text||'')+' → '+(payload.new.dest_text||'')), 300);
        }catch(e){}
      }
      // Selalu reload, tanpa silent check yang bikin box gak hilang
      loadDriverOrders(driverProfile, false); 
      loadDriverActiveOrders(driverProfile);
    })
    .on('postgres_changes', { event:'DELETE', schema:'public', table:'orders' }, payload=>{
      console.log('[driver] order deleted', payload.old?.id);
      loadDriverOrders(driverProfile, false);
      loadDriverActiveOrders(driverProfile);
    })
    .subscribe((status)=>{ console.log('driver-orders channel', status); });
  ...
  driverFoodChannel = supabase.channel('driver-food-'+driverProfile.id)
    .on('postgres_changes', { event:'*', schema:'public', table:'food_orders' }, payload=>{
      console.log('[driver] realtime food', payload.eventType, payload.new?.id, payload.new?.status);
      if(payload.eventType==='UPDATE' && payload.new && payload.new.status==='cancelled'){
        try{ 
          notifyFood('❌ Food Dibatalkan Pembeli', `${payload.new.store_name||'Warung'} dibatalkan pembeli`, '/#/driver');
          setTimeout(()=> alert('❌ Order makanan dibatalkan pembeli!
'+(payload.new.store_name||'')), 300);
        }catch(e){}
      }
      loadDriverFoodOrders(driverProfile, false); 
      loadDriverActiveOrders(driverProfile);
    })
    .on('postgres_changes', { event:'DELETE', schema:'public', table:'food_orders' }, payload=>{
      loadDriverFoodOrders(driverProfile, false);
      loadDriverActiveOrders(driverProfile);
    })
    .subscribe((status)=>{ console.log('driver-food channel', status); });
  ...
  driverActiveChannel = supabase.channel('driver-active-'+driverProfile.id)
    .on('postgres_changes', { event:'*', schema:'public', table:'food_orders', filter:'driver_id=eq.'+driverProfile.id }, payload=>{
      console.log('[driver] active food', payload.eventType, payload.new?.status);
      if(payload.new && payload.new.status==='cancelled'){
        try{ notifyFood('❌ Food Dibatalkan Pembeli', 'Order dibatalkan pembeli', '/#/driver'); }catch(e){}
    ...
    .on('postgres_changes', { event:'*', schema:'public', table:'orders', filter:'driver_id=eq.'+driverProfile.id }, payload=>{
      console.log('[driver] active ojol', payload.eventType, payload.new?.status);
      if(payload.new && payload.new.status==='cancelled'){
    ...
    .subscribe((status)=>{ console.log('driver-active channel', status); });

  if(driverOrdersInterval) clearInterval(driverOrdersInterval);
  // Polling lebih cepat 3 detik biar auto hilang walau realtime RLS ke-block
  driverOrdersInterval = setInterval(()=>{ 
    loadDriverOrders(driverProfile, false); 
    loadDriverFoodOrders(driverProfile, false); 
    loadDriverActiveOrders(driverProfile); 
  }, 3000);
...
if(typeof window !== 'undefined'){
  ...
  window._driverReject = async (orderId)=>{
    try{
      if(!confirm('Tolak order ojol ini?')) return;
      // Ambil order dulu untuk cek driver_id
      let orderData = null;
      try{
        const { data } = await supabase.from('orders').select('driver_id, passenger_id, status').eq('id', orderId).single();
        orderData = data;
      }catch(e){}

      // Simpan ke rejected list lokal agar tidak muncul lagi di driver ini (untuk broadcast & assigned)
      try{
        const { data: { user } } = await supabase.auth.getUser();
      ...
      // Jika order broadcast (driver_id null) atau driver_id != saya -> jangan update DB (RLS akan block), cukup hide lokal
      const isBroadcast = !orderData || !orderData.driver_id;
      const isAssignedToMe = orderData && orderData.driver_id;

      if(isBroadcast){
        // Broadcast reject: cukup hide di driver ini, order tetap searching untuk driver lain
        alert('❌ Order diabaikan. Tidak akan muncul lagi untuk kamu, tapi tetap mencari driver lain.');
        try{
          const { data: { user } } = await supabase.auth.getUser();
          const { data: profile } = await supabase.from('users').select('*').eq('google_id', user.id).single();
      ...
      // Jika sudah assigned ke saya, baru update DB ke cancelled (ini RLS allowed karena driver_id = saya)
      if(isAssignedToMe){
        const res = await driverRejectOrder(orderId);
        if(res.error) throw res.error;
        alert('❌ Order ojol dibatalkan. Penumpang akan dapat notifikasi.');
        try{
          const { data: { user } } = await supabase.auth.getUser();
          const { data: profile } = await supabase.from('users').select('*').eq('google_id', user.id).single();
          await loadDriverOrders(profile, false);
      ...
      alert('Gagal tolak: '+e.message + ' (Jika broadcast, order tetap akan hilang dari list kamu)'); 
      // tetap refresh list
      try{
        const { data: { user } } = await supabase.auth.getUser();
        const { data: profile } = await supabase.from('users').select('*').eq('google_id', user.id).single();
  ...
  window._driverRejectFood = async (orderId)=>{
    try{
      ...
      let orderData = null;
      try{
        const { data } = await supabase.from('food_orders').select('driver_id, status').eq('id', orderId).single();
        orderData = data;
      }catch(e){}
      const isBroadcast = !orderData || !orderData.driver_id;
      
      // Simpan rejected lokal untuk food juga
      try{
        const { data: { user } } = await supabase.auth.getUser();
        const { data: profile } = await supabase.from('users').select('id').eq('google_id', user.id).single();
        const key = 'rejected_food_orders_'+profile.id;
        let list = []; try{ list = JSON.parse(localStorage.getItem(key)||'[]'); }catch(e){}
        if(!list.includes(orderId)){ list.push(orderId); localStorage.setItem(key, JSON.stringify(list)); }
      }catch(e){}

      if(isBroadcast){
        alert('❌ Order food diabaikan untuk kamu. Tetap mencari driver lain.');
        try{
          const { data: { user } } = await supabase.auth.getUser();
          const { data: profile } = await supabase.from('users').select('*').eq('google_id', user.id).single();
      ...
      if(res.error) throw res.error;
      alert('❌ Order food ditolak & dibatalkan. Pembeli akan dapat notifikasi.'); 
      location.reload();
    }catch(e){ 
      console.error('reject food fail', e);
      alert('Gagal tolak: '+e.message + ' (Order tetap hilang dari list kamu)'); 
      try{
        const { data: { user } } = await supabase.auth.getUser();
        const { data: profile } = await supabase.from('users').select('*').eq('google_id', user.id).single();
        await loadDriverFoodOrders(profile, false);
