...
export async function loadDriverOrders(driverProfile, silent=false){
  ...
  try{
    ...
    // FIX: Notif order baru - termasuk order pertama & anti spam
    try{
      const prevKey = '_driver_ojol_ids_'+driverProfile.id;
      const prevIds = JSON.parse(localStorage.getItem(prevKey)||'[]');
      const currIds = orders.map(o=>o.id);
      const baru = currIds.filter(id=> !prevIds.includes(id));
      if(baru.length>0){
        // Jika silent=false (realtime / polling) ATAU ini order baru setelah load awal
        if(!silent || prevIds.length>0){
          const first = orders.find(o=> baru.includes(o.id));
          if(first){
            notifyOjol(`📦 ${baru.length} Order Baru Masuk!`, `${first.pickup_text||'-'} → ${first.dest_text||'-'} • Rp ${first.estimated_cost?.toLocaleString('id-ID')||'-'} • KLIK TERIMA`, '/#/driver', first.id);
            try{ if(navigator.vibrate) navigator.vibrate([200,100,200]); }catch(e){}
            try{ new Audio('/sounds/order.mp3').play().catch(()=>{}); }catch(e){}
          }
        }
      }
      localStorage.setItem(prevKey, JSON.stringify(currIds));
    ...
    try{
      ...
      if(baru.length>0 && (!silent || prev.length>0)){
        ...
        if(o){
          ...
          notifyFood(`🍔 ${baru.length} Order Food Baru!`, `${nm} • Rp ${Number(tot).toLocaleString('id-ID')} • ${o.dest_text||''} - KLIK TERIMA`, '/#/driver', o.id);
          try{ if(navigator.vibrate) navigator.vibrate([200,100,200]); }catch(e){}
          try{ new Audio('/sounds/order.mp3').play().catch(()=>{}); }catch(e){}
        }
      }
  ...
  driverOrdersChannel = supabase.channel('driver-orders-'+driverProfile.id)
    .on('postgres_changes', { event:'*', schema:'public', table:'orders' }, payload=>{
      ...
      if(o && o.status==='cancelled'){
        try{ notifyOjol('❌ Order Dibatalkan Penumpang', `${o.pickup_text||''} → ${o.dest_text||''} dibatalkan penumpang`, '/#/driver'); }catch(e){}
...
// Hook lama dihapus - sudah ada di loadDriverFoodOrders langsung
