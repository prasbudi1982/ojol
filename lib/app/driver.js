...
export async function loadDriverActiveOrders(driverProfile){
  ...
  try{
    // Cek active ojol
    const qOjek=await supabase.from('orders').select('*').eq('driver_id', driverProfile.id).in('status', ['accepted','picked']).order('created_at',{ascending:false}).limit(1).maybeSingle();
    const qFood=await supabase.from('food_orders').select('*').eq('driver_id', driverProfile.id).in('status', ['accepted','preparing','ready','picked']).order('created_at',{ascending:false}).limit(1).maybeSingle();
    const ojek = qOjek.data;
    const food = qFood.data;
...
if(typeof window !== 'undefined'){
  ...
  window._driverAcceptFood = async (orderId)=>{
    try{
      ...
      loadDriverActiveOrders(profile); loadDriverFoodOrders(profile, false);
    }catch(e){ alert('Gagal: '+e.message); }
  };
