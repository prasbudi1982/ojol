...
export function driverRejectOrder(orderId){
  ...
  return supabase.from('orders').update({ status:'cancelled', cancel_reason:'Driver menolak order' }).eq('id', orderId).select().single();
}
export function driverRejectOrderToSearch(orderId){
  ...
  // Simpan ke rejected local agar tidak muncul lagi untuk driver ini, tapi tetap searching untuk driver lain
