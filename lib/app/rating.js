// rating.js - Rating driver setelah order selesai
...
export function openRatingModal(driverId, driverName, orderId){
  ...
  div.innerHTML = `
    <div style="background:var(--card);border-radius:20px;max-width:380px;width:100%;overflow:hidden;border:2px solid ${theme.primary};box-shadow:0 20px 50px rgba(0,0,0,0.4)">
      ...
      <div style="padding:20px;text-align:center">
        ...
        <div id="ratingLabel" style="font-size:13px;font-weight:700;color:var(--card);height:18px">Tap bintang</div>
        <textarea id="ratingComment" placeholder="Komentar (opsional)..." style="width:100%;margin-top:12px;padding:10px;border-radius:10px;border:1px solid var(--border);background:var(--card2);font-size:12px;height:70px;resize:none"></textarea>
        ...
        <div style="display:flex;gap:8px;margin-top:16px">
          ...
          <button id="btnSkipRating" style="background:var(--card);border:1px solid var(--border);padding:12px 16px;border-radius:12px;font-weight:700">Lewati</button>
        </div>
  ...
  document.getElementById('btnSkipRating').onclick = ()=> { try{ localStorage.setItem('rated_'+orderId, '1'); }catch(e){} div.remove(); };
  document.getElementById('btnSubmitRating').onclick = async ()=>{
    ...
    try{
      ...
      try{
        ...
        if(user){
          ...
          if(u) passengerId = u.id;
            ...
            // fallback coba ambil dari currentProfile atau localStorage
      ...
      try{ localStorage.setItem('rated_'+orderId, '1'); }catch(e){}
      try{ document.dispatchEvent(new CustomEvent('rating:completed', {detail:{orderId: orderId, driverId: driverId, rating: selected}})); }catch(e){}
      // Auto refresh history jika sedang di halaman history
      try{ if(location.hash==='#/history'){ setTimeout(()=>{ const ev = document.getElementById('historyPaginatedContainer'); if(ev){ /* trigger re-render via custom event sudah handle */ } }, 200); } }catch(e){}
      setTimeout(()=>div.remove(), 1000);
    }catch(e){
      // Fallback localStorage kalau tabel belum ada
      ...
      document.getElementById('ratingStatus').textContent='✅ Disimpan lokal (buat tabel ratings)';
      try{ localStorage.setItem('rated_'+orderId, '1'); }catch(e){}
      try{ document.dispatchEvent(new CustomEvent('rating:completed', {detail:{orderId: orderId, driverId: driverId, rating: selected}})); }catch(e){}
