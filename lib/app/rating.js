...
export function openRatingModal(driverId, driverName, orderId){
  ...
  div.innerHTML = `
    <div style="background:white;border-radius:20px;max-width:380px;width:100%;overflow:hidden;border:2px solid ${theme.primary};box-shadow:0 20px 50px rgba(0,0,0,0.4)">
      ...
      <div style="padding:20px;text-align:center">
        <div style="font-size:12px;color:#64748b;margin-bottom:12px">Bagaimana pelayanan driver?</div>
        ...
        <div id="ratingLabel" style="font-size:13px;font-weight:700;color:#0f172a;height:18px">Tap bintang</div>
        <textarea id="ratingComment" placeholder="Komentar (opsional)..." style="width:100%;margin-top:12px;padding:10px;border-radius:10px;border:1px solid #e2e8f0;background:#f8fafc;font-size:12px;height:70px;resize:none"></textarea>
        <div id="ratingStatus" style="font-size:11px;margin-top:8px;min-height:14px;color:#64748b"></div>
        <div style="display:flex;gap:8px;margin-top:16px">
          ...
          <button id="btnSkipRating" style="background:white;border:1px solid #e2e8f0;padding:12px 16px;border-radius:12px;font-weight:700">Lewati</button>
        </div>
  ...
  document.getElementById('btnSubmitRating').onclick = async ()=>{
    ...
    try{
      ...
      const payload = {
        driver_id: driverId,
        order_id: orderId,
        passenger_id: (await supabase.auth.getUser()).data.user ? (await supabase.from('users').select('id').eq('google_id',(await supabase.auth.getUser()).data.user.id).single()).data?.id : null,
        rating: selected,
