// admin.js - FINAL v3 - SETTING 100% ORIGINAL - VERSION MARKER 3.0
// If you still see 2 buttons, this file is NOT loaded.
import { supabase } from './supabase.js';
import { APP_SETTINGS_DEFAULT, KECAMATAN_DATA, getActiveKecamatanLive } from './config.js';

console.log('ADMIN.JS v3.0 LOADED - 1 button only');
...
function injectModalCSS(){
  ...
  style.textContent=`
  #adminUserModal{position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.75);display:none;align-items:center;justify-content:center;padding:16px}
  #adminUserModal.show{display:flex !important}
  #adminUserModal .modal-card{width:100%;max-width:480px;max-height:88vh;overflow:auto;background:#1f2937;border:1px solid #334155;border-radius:16px;padding:16px;position:relative;color:#fff}
  `;
...
function ensureModalExists(){
  ...
  if(!modal){
    ...
    modal.innerHTML=`<div class="modal-card"><button onclick="closeAdminModal()" style="position:absolute;right:10px;top:10px;background:#334155;color:#fff;border:0;padding:6px 12px;border-radius:8px">✕ CLOSE</button><div id="adminUserModalContent" style="margin-top:10px"></div></div>`;
...
export function viewAdminDashboard(stats=null){
  ...
  <div class="admin-page">
    <div style="background:#22c55e;color:#000;padding:4px 8px;border-radius:6px;font-size:10px;margin-bottom:8px;text-align:center">ADMIN v3.0 - 1 BUTTON MODE</div>
...
// SETTING - ORIGINAL 100% - DO NOT EDIT
  ...
  <div class="admin-page">
    ...
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div><label style="font-size:12px">🏍️ Motor - Base Fare</label><input id="setMotorBase" type="number" class="input" value="${s.tarifMotorBase||3000}" placeholder="3000"></div>
        <div><label style="font-size:12px">🏍️ Motor - Per Km</label><input id="setMotorPerKm" type="number" class="input" value="${s.tarifMotorPerKm||2500}" placeholder="2500"></div>
        <div><label style="font-size:12px">🏍️ Motor - Minimal</label><input id="setMotorMin" type="number" class="input" value="${s.tarifMotorMin||5000}" placeholder="5000"></div>
        <div><label style="font-size:12px">🔄 PP Multiplier</label><input id="setPpMultiplier" type="number" step="0.1" class="input" value="${s.ppMultiplier||1.6}" placeholder="1.6"></div>
        <div><label style="font-size:12px">🚗 Mobil - Base Fare</label><input id="setMobilBase" type="number" class="input" value="${s.tarifMobilBase||8000}" placeholder="8000"></div>
        <div><label style="font-size:12px">🚗 Mobil - Per Km</label><input id="setMobilPerKm" type="number" class="input" value="${s.tarifMobilPerKm||5500}" placeholder="5500"></div>
        <div style="grid-column:1 / -1"><label style="font-size:12px">🚗 Mobil - Minimal</label><input id="setMobilMin" type="number" class="input" value="${s.tarifMobilMin||15000}" placeholder="15000"></div>
      </div>
...
function renderUserList(users){
  ...
  el.innerHTML=users.map(u=>{
    ...
    const dot=isB?'err':(['active','online'].includes(u.status)?'ok':''); const icon=u.role==='driver'?'🏍️':u.role==='admin'?'👑':'👤';
    // ONLY 1 BUTTON
    return `<div class="admin-list-item"><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:13px;display:flex;align-items:center;gap:6px"><span class="dot ${dot}" ${dot?'':`style="background:var(--muted)"`}></span>${icon} <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(u.name||'Tanpa Nama')}</span> <span class="admin-badge ${badge}">${isB?'BANNED':escapeHtml(u.role)}</span></div><div class="muted" style="font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(u.email||'-')} ${u.hp?'• '+escapeHtml(u.hp):''}</div></div><div class="admin-actions"><button class="btn primary" style="width:auto;padding:8px 14px;font-size:12px" onclick="adminViewUser('${escapeAttr(u.id)}')">Detail</button></div></div>`;
  }).join('');
}
...
function renderReports(reports){
  ...
  el.innerHTML=reports.map(r=>{
    const status=(r.status||'pending').toLowerCase(); const reported=r.reported||{}; const reporter=r.reporter||{};
    const reportedName=escapeHtml(reported.name||r.reported_id?.toString().slice(0,8)||'User');
    return `<div style="border-radius:12px;padding:12px;margin-bottom:10px;background:var(--card);border:1px solid var(--border);border-left:4px solid ${status==='pending'?'var(--warning)':status==='banned'?'var(--danger)':'var(--primary)'}">
      <div style="display:flex;justify-content:space-between"><b>🚩 ${reportedName}</b><span class="admin-badge">${escapeHtml(status)}</span></div>
      <div style="font-size:11px" class="muted">Oleh ${escapeHtml(reporter.name||'Anonim')} • ${new Date(r.created_at).toLocaleString('id-ID')}</div>
      <div style="margin-top:6px;font-size:12px"><b>Alasan:</b> ${escapeHtml(r.reason||'')} ${escapeHtml(r.description||'')}</div>
      ${status==='pending'?`<div class="row" style="margin-top:8px"><button class="btn secondary" style="flex:1;font-size:11px" onclick="adminViewUser('${escapeAttr(r.reported_id)}')">👁️ Lihat</button><button class="btn secondary" style="flex:1;font-size:11px" onclick="adminBanFromReport('${escapeAttr(r.reported_id)}','${escapeAttr(r.id)}')">🚫 Ban</button><button class="btn secondary" style="flex:1;font-size:11px" onclick="adminReviewReport('${escapeAttr(r.id)}')">✅ Tolak</button></div>`:''}
    </div>`;
  }).join('');
}
...
window.adminViewUser=async function(id){
  console.log('adminViewUser called',id);
  ...
  box.innerHTML='<div style="padding:20px;text-align:center">⏳ Loading...</div>';
  ...
  box.innerHTML=`
    <h3 style="margin:0 0 12px">👤 ${escapeHtml(u.name||'-')} ${isB?'<span style="background:#ef4444;color:#fff;padding:2px 6px;border-radius:4px;font-size:10px">BANNED</span>':''}</h3>
    <div style="font-size:13px;display:grid;gap:6px">
      <div>Email: <b>${escapeHtml(u.email||'-')}</b></div>
      <div>HP: <b>${escapeHtml(u.hp||'-')}</b></div>
      <div>Status: <b>${escapeHtml(u.status||'-')}</b> • Role: <b>${escapeHtml(u.role||'-')}</b></div>
      <div>Daftar: ${new Date(u.created_at).toLocaleString('id-ID')}</div>
      ${u.banned_reason?`<div style="background:#450a0a;padding:8px;border-radius:8px">Alasan ban: ${escapeHtml(u.banned_reason)}</div>`:''}
    </div>
    ...
    <div style="margin-top:16px;display:flex;gap:8px">
      ${isB?`<button onclick="adminUnbanUser('${escapeAttr(u.id)}')" style="flex:1;background:#22c55e;color:#000;padding:12px;border:0;border-radius:10px;font-weight:700">✅ UNBAN</button>`:`<button onclick="adminBanUser('${escapeAttr(u.id)}')" style="flex:1;background:#ef4444;color:#fff;padding:12px;border:0;border-radius:10px;font-weight:700">🚫 BAN USER</button>`}
    ...
    <button onclick="closeAdminModal()" style="width:100%;margin-top:8px;background:#1e293b;color:#fff;padding:10px;border:1px solid #334155;border-radius:10px">Tutup</button>
  `;
};
