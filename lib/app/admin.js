// admin.js - FINAL v4 - FORCE 1 BUTTON + MODAL FIX
import { supabase } from './supabase.js';
import { APP_SETTINGS_DEFAULT, KECAMATAN_DATA, getActiveKecamatanLive } from './config.js';

console.log('ADMIN v4.0 - FORCE 1 BUTTON');

function escapeHtml(str){ if(str==null) return ''; return String(str).replace(/[&<>"']/g, s=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[s])); }
function escapeAttr(str){ return escapeHtml(str).replace(/"/g,'&quot;'); }

function injectFix(){
  if(document.getElementById('adminFixCSS')) return;
  const style=document.createElement('style'); style.id='adminFixCSS';
  style.textContent=`
  /* FORCE ONLY 1 BUTTON IN LIST - HIDE SECOND */
  .admin-list-item .admin-actions button:nth-child(2){display:none !important;}
  .admin-list-item .admin-actions{display:flex !important; flex-direction:column !important; gap:0 !important;}
  #adminUserModal{position:fixed !important;inset:0 !important;z-index:999999 !important;background:rgba(0,0,0,.8) !important;display:none !important;align-items:center !important;justify-content:center !important;padding:16px !important}
  #adminUserModal.show{display:flex !important}
  #adminUserModal .modal-card{width:100% !important;max-width:480px !important;max-height:90vh !important;overflow:auto !important;background:#1e293b !important;border-radius:16px !important;padding:16px !important;color:#fff !important}
  .report-new{border-radius:12px;padding:12px;margin-bottom:10px;background:#1e293b;border:1px solid #334155;border-left:4px solid #f59e0b}
  `;
  document.head.appendChild(style);
  // Also inject modal element immediately
...
function renderUserList(users){
  ...
  if(!users.length){ el.innerHTML=`<div class="admin-list-item">Tidak ada user</div>`; return; }
  el.innerHTML=users.map(u=>{
    ...
    return `<div class="admin-list-item" style="display:flex;justify-content:space-between;align-items:center;padding:10px;border-bottom:1px solid #334155"><div><div style="font-weight:700">${escapeHtml(u.name||'Tanpa Nama')} <span class="admin-badge ${badge}">${isB?'BANNED':escapeHtml(u.role)}</span></div><div class="muted" style="font-size:11px">${escapeHtml(u.email||'-')} • ${escapeHtml(u.hp||'')} • ${escapeHtml(u.status||'')}</div></div><button class="btn primary" style="width:auto;padding:8px 14px" onclick="adminViewUser('${escapeAttr(u.id)}')">Detail</button></div>`;
  }).join('');
  // Force remove any second button that old cache injected
  setTimeout(()=>{ document.querySelectorAll('.admin-list-item .admin-actions button:nth-child(2)').forEach(b=>b.remove()); },50);
}
function renderReports(reports){
  ...
  el.innerHTML=reports.map(r=>{
    ...
    return `<div class="report-new" style="border-left-color:${status==='pending'?'#f59e0b':status==='banned'?'#ef4444':'#22c55e'}"><div style="display:flex;justify-content:space-between"><b>🚩 ${escapeHtml(rep.name||r.reported_id?.toString().slice(0,8)||'User')}</b><span class="admin-badge">${escapeHtml(status)}</span></div><div style="font-size:11px" class="muted">Oleh ${escapeHtml(reporter.name||'Anonim')} • ${new Date(r.created_at).toLocaleString('id-ID')}</div><div style="font-size:12px;margin-top:6px"><b>${escapeHtml(r.reason||'')}</b> ${escapeHtml(r.description||'')}</div><div class="row" style="margin-top:8px"><button class="btn secondary" style="flex:1" onclick="adminViewUser('${escapeAttr(r.reported_id)}')">👁️ Lihat</button><button class="btn secondary" style="flex:1" onclick="adminBanFromReport('${escapeAttr(r.reported_id)}','${escapeAttr(r.id)}')">🚫 Ban</button><button class="btn secondary" style="flex:1" onclick="adminReviewReport('${escapeAttr(r.id)}')">✅ Selesai</button></div></div>`;
  }).join('');
}

window.closeAdminModal=function(){ const m=document.getElementById('adminUserModal'); if(m){ m.classList.remove('show'); m.style.display='none'; } };
window.adminViewUser=async function(id){
  injectFix();
  const modal=document.getElementById('adminUserModal'); const box=document.getElementById('adminUserModalContent');
  if(!modal||!box){ alert('Modal not found'); return; }
  modal.style.display='flex'; modal.classList.add('show');
  box.innerHTML='<div style="padding:20px;text-align:center">⏳ Loading user...</div>';
  const res=await getUserDetail(id); if(!res){ box.innerHTML='User tidak ditemukan'; return; }
  ...
  box.innerHTML=`
    ...
    <div style="font-size:13px;display:grid;gap:6px;margin-top:10px">
      <div>Email: <b>${escapeHtml(u.email||'-')}</b></div>
      <div>HP: <b>${escapeHtml(u.hp||'-')}</b></div>
      <div>Role: <b>${escapeHtml(u.role||'-')}</b> • Status: <b>${escapeHtml(u.status||'-')}</b></div>
      <div>Daftar: ${new Date(u.created_at).toLocaleString('id-ID')}</div>
      ${u.banned_reason?`<div style="background:#450a0a;padding:8px;border-radius:8px">Ban: ${escapeHtml(u.banned_reason)}</div>`:''}
    </div>
    <div style="margin-top:12px"><b>📦 ${res.orders.length} Order</b><div style="max-height:150px;overflow:auto">${res.orders.map(o=>`<div style="padding:6px 0;border-bottom:1px solid #334155;font-size:12px;display:flex;justify-content:space-between"><span>${new Date(o.created_at).toLocaleDateString('id-ID')} ${escapeHtml(o.status)}</span><span>Rp${(o.total_fare||0).toLocaleString('id-ID')}</span></div>`).join('')||'Tidak ada'}</div></div>
    <div style="margin-top:16px;display:flex;gap:8px">
      ${isB?`<button onclick="adminUnbanUser('${escapeAttr(u.id)}')" style="flex:1;background:#22c55e;color:#000;padding:12px;border:0;border-radius:10px;font-weight:800">✅ UNBAN</button>`:`<button onclick="adminBanUser('${escapeAttr(u.id)}')" style="flex:1;background:#ef4444;color:#fff;padding:12px;border:0;border-radius:10px;font-weight:800">🚫 BAN</button>`}
      <button onclick="window.open('https://wa.me/${cleanHp}','_blank')" style="flex:1;background:#334155;color:#fff;padding:12px;border:0;border-radius:10px">💬 WA</button>
    </div>
    <button onclick="closeAdminModal()" style="width:100%;margin-top:8px;background:#111827;color:#fff;padding:10px;border:1px solid #334155;border-radius:10px">Tutup</button>
  `;
};
...
window.adminReviewReport=async function(rid){ if(!confirm('Tolak?')) return; try{ await supabase.from('reports').update({status:'reviewed'}).eq('id',rid); const rep=await getReports(); renderReports(rep); }catch(e){ alert(e.message); } };

export async function initAdminPage(){
  injectFix(); ensureModalExists();
