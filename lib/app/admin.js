...
import { APP_SETTINGS_DEFAULT, KECAMATAN_DATA } from './config.js';
...
export function viewAdminSettings(settings, profile=null){
  ...
  <div class="admin-page">
    ...
    <div class="card">
      <h4 style="margin:0 0 10px">📍 Wilayah Aktif (Dropdown Hemat)</h4>
      <label>Kecamatan Aktif - Pickup Area<select id="setKecamatan" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);background:var(--card2);color:var(--text)">${kecOptions}</select></label>
      <p class="muted" style="font-size:11px">Ganti wilayah tanpa edit file config.js - langsung simpan, reload otomatis. Semua validasi pickup & map akan ikut kecamatan baru.</p>
    </div>
    <div class="card">
      ...
      <label>Isi Disclaimer (konek ke Home)</label><textarea id="setDiscText" style="min-height:140px">${s.disclaimerText||''}</textarea>
      <p class="muted" style="font-size:11px">Teks di sini akan langsung muncul di Home user setelah disimpan - support ganti judul & isi lengkap 4 poin.</p>
