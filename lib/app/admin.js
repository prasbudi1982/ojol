// admin_v8_inline.js - FINAL - 2 tombol, detail inline (bukan modal), report card rapi, tidak pink semua
import { supabase } from './supabase.js';
import { APP_SETTINGS_DEFAULT, KECAMATAN_DATA, getActiveKecamatanLive } from './config.js';

console.log('ADMIN v8.0 - INLINE DETAIL - 2 BUTTONS');
...
export function viewAdminDashboard(stats=null){
  ...
  <div class="admin-page">
    <div style="background:#1e293b;border:1px solid #334155;padding:6px;border-radius:8px;font-size:11px;text-align:center;color:#94a3b8;margin-bottom:8px">ADMIN v8.0 - INLINE DETAIL - 2 TOMBOL</div>
    ...
    <div style="margin-top:12px">
      <h3 style="font-size:13px;margin:8px 0">👥 List User (<span id="userCount">0</span>)</h3>
...
function renderUserList(users){
  ...
  if(!users.length){ el.innerHTML=`<div class="muted" style="padding:12px">Tidak ada user</div>`; return; }
  ...
  el.innerHTML=reports.map(r=>{
    ...
    <div style="background:#0f172a;border:1px solid #1e293b;border-left:4px solid ${borderColor};border-radius:12px;padding:12px;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <div>
            ...
            <div style="font-size:11px;color:#64748b">Dilaporkan oleh <b style="color:#94a3b8">${escapeHtml(reporterName)}</b> • ${dateStr}</div>
          </div>
        </div>
      ...
      <div style="display:flex;gap:8px;margin-top:10px">
        <button class="btn secondary" style="flex:1;padding:8px;font-size:12px;background:#1e293b;color:#e2e8f0;border:1px solid #334155" onclick="toggleDetailInline('${escapeAttr(r.reported_id)}')">👁️ Lihat User</button>
...
// Keep old modal functions for compatibility but do nothing
window.closeAdminModal=function(){};
window.adminViewUser=async function(id){ window.toggleDetailInline(id); };
