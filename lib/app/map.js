...
export function confirmMapLocation(updateEstimateFn){
  ...
  if(target==='merchant'){
    ...
    if(window.setMerchantMapLocation) window.setMerchantMapLocation(finalLat, finalLng, mapSelected.address);
    closeMapPicker();
    return;
  }
  if(target==='warung'){
    ...
    if(addrEl && mapSelected.address && !addrEl.value) addrEl.value=mapSelected.address.slice(0,180);
    closeMapPicker();
    return;
  }
  const destInput=document.getElementById('dest');
