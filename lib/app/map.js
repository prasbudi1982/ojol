// map.js - Map picker Google tiles + Leaflet + Polygon Akurat Kecamatan Suruh
import { SURUH_CENTER, SURUH_BBOX, TRENGGALEK_BBOX } from './config.js';
import { isInTrenggalekKab, loadPolygons, getSuruhPolygon, getTrenggalekPolygon, isPointInPolygon } from './geofence.js';

export let mapInstance=null;
export let mapMarker=null;
export let mapSelected={lat:null,lng:null,address:null,inTrenggalek:false};
let suruhLayer = null;
let trenggalekLayer = null;
...
function drawPolygons(){
  ...
  const suruhPoly = getSuruhPolygon();
  const trengPoly = getTrenggalekPolygon();

  // Hapus layer lama
  if(suruhLayer) try{ mapInstance.removeLayer(suruhLayer); }catch(e){}
  if(trenggalekLayer) try{ mapInstance.removeLayer(trenggalekLayer); }catch(e){}

  // Gambar Trenggalek (hijau) - jika ada polygon, pakai geoJSON, jika tidak pakai rectangle
  if(trengPoly){
    trenggalekLayer = L.geoJSON(trengPoly, {
      style: { color:'#22c55e', weight:2, dashArray:'6,6', fillOpacity:0.06, fillColor:'#22c55e' }
    }).addTo(mapInstance).bindPopup('Batas Akurat Kabupaten Trenggalek (GeoJSON)');
  } else {
    trenggalekLayer = L.rectangle([[TRENGGALEK_BBOX.minLat, TRENGGALEK_BBOX.minLng],[TRENGGALEK_BBOX.maxLat, TRENGGALEK_BBOX.maxLng]], {color:'#22c55e', weight:1, dashArray:'5,5', fillOpacity:0.05}).addTo(mapInstance).bindPopup('Batas Kabupaten Trenggalek (BBOX - loading polygon...)');
  }

  // Gambar Suruh (biru) - PICKUP AREA
  if(suruhPoly){
    suruhLayer = L.geoJSON(suruhPoly, {
      style: { color:'#3b82f6', weight:3, fillOpacity:0.12, fillColor:'#3b82f6' }
    }).addTo(mapInstance).bindPopup('Kecamatan Suruh - Pickup Area (Akurat)');
  } else {
    suruhLayer = L.rectangle([[SURUH_BBOX.minLat, SURUH_BBOX.minLng],[SURUH_BBOX.maxLat, SURUH_BBOX.maxLng]], {color:'#3b82f6', weight:2, fillOpacity:0.08}).addTo(mapInstance).bindPopup('Kecamatan Suruh - Pickup Area (BBOX)');
  }
}
...
export async function openMapPicker(){
  ...
  await ensureLeaflet();
  await loadPolygons(); // Pastikan polygon ke-load
...
export async function setMapLocation(lat,lng, shouldPan=true){
  ...
  coordsEl.textContent=`${preciseLat.toFixed(7)}, ${preciseLng.toFixed(7)} • Akurat Polygon`;
}
...
export function confirmMapLocation(updateEstimateFn){
  ...
  const badge=document.getElementById('destLive'); if(badge) badge.textContent='📍 dari Map Google (Polygon Akurat)';
  closeMapPicker();
