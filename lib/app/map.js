...
export async function searchMapLocation(query){
  ...
  const badge = document.getElementById('mapSearchStatus');
  if(!resultsBox) return;
  const q = (query||'').trim();
  if(!q || q.length<3){
    resultsBox.style.display='none';
    resultsBox.innerHTML='';
    if(badge) badge.textContent='🔍 ketik minimal 3 huruf';
    return;
  }
  if(badge) badge.textContent='⏳ mencari...';
  try{
    // Batasi pencarian hanya di Kabupaten Trenggalek via viewbox + bounded
    const viewbox = `${TRENGGALEK_BBOX.minLng},${TRENGGALEK_BBOX.maxLat},${TRENGGALEK_BBOX.maxLng},${TRENGGALEK_BBOX.minLat}`;
    const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&viewbox=${viewbox}&bounded=1&limit=8&countrycodes=id&addressdetails=1`;
    const resp = await fetch(url);
    const data = await resp.json();
    // Filter hanya yang masih dalam Trenggalek
    const filtered = (data||[]).filter(d=>{
      const lat = parseFloat(d.lat); const lon = parseFloat(d.lon);
      return isInTrenggalekKab(lat, lon);
    }).slice(0,6);
    lastSearchResults = filtered;
    if(!filtered.length){
      resultsBox.style.display='block';
      resultsBox.innerHTML = `<div style="padding:10px;font-size:11px;color:var(--muted)">❌ Tidak ditemukan di Trenggalek. Coba kata kunci lain atau klik langsung di peta.</div>`;
      if(badge) badge.textContent='❌ tidak ada hasil di Trenggalek';
      return;
    }
    resultsBox.style.display='block';
    resultsBox.innerHTML = filtered.map((d,i)=>{
      const label = d.display_name.split(',').slice(0,3).join(', ');
      return `<div class="map-search-item" data-idx="${i}" style="padding:10px 12px;border-bottom:1px solid var(--border);cursor:pointer;display:flex;gap:8px;align-items:flex-start">
        <span style="font-size:14px">📍</span>
        <div style="flex:1"><div style="font-size:12px;font-weight:600;color:var(--text);line-height:1.3">${label}</div><div style="font-size:10px;color:var(--muted);margin-top:2px">${parseFloat(d.lat).toFixed(5)}, ${parseFloat(d.lon).toFixed(5)}</div></div>
      </div>`;
    }).join('');
    if(badge) badge.textContent=`✅ ${filtered.length} hasil`;
    // bind click
    resultsBox.querySelectorAll('.map-search-item').forEach(el=>{
      el.addEventListener('click', async ()=>{
        ...
        await setMapLocation(lat, lon, true);
        resultsBox.style.display='none';
        if(input) input.value = item.display_name.split(',').slice(0,2).join(', ');
      });
    });
  }catch(e){
    if(badge) badge.textContent='❌ gagal cari';
    resultsBox.style.display='block';
    resultsBox.innerHTML = `<div style="padding:10px;font-size:11px;color:#ef4444">Error: ${e.message}</div>`;
  }
}
...
function initMapSearch(){
  ...
  input.addEventListener('input', (e)=>{
    ...
    mapSearchDebounce = setTimeout(()=> searchMapLocation(e.target.value), 400);
  });
  input.addEventListener('keydown', (e)=>{
    if(e.key==='Enter'){
      e.preventDefault();
      clearTimeout(mapSearchDebounce);
      searchMapLocation(input.value);
    }
  });
  if(clearBtn){
    clearBtn.addEventListener('click', ()=>{
      input.value=''; const box=document.getElementById('mapSearchResults'); if(box){ box.style.display='none'; box.innerHTML=''; }
      const badge=document.getElementById('mapSearchStatus'); if(badge) badge.textContent='🔍 ketik lokasi';
      input.focus();
    });
  }
}
...
export async function openMapPicker(){
  ...
  if(searchInput){ searchInput.value=''; const rb=document.getElementById('mapSearchResults'); if(rb){ rb.style.display='none'; rb.innerHTML=''; } const sb=document.getElementById('mapSearchStatus'); if(sb) sb.textContent='🔍 cari desa / jalan di Trenggalek'; }
