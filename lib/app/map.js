...
export function confirmMapLocation(updateEstimateFn){
  ...
  const badge=document.getElementById('destLive'); if(badge) badge.textContent=`📍 dari Map - ${ACTIVE_KECAMATAN_NAME}`;
  closeMapPicker();
...
// ===== VIEW & STYLE MAP - di dalam modul map (penting) =====
export const MAP_MODAL_STYLE = `
  #mapModal .map-header { background: var(--card); padding:12px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border); }
  #leafletMap { background:#0f172a; }
`;

export function viewMapModal(){
  return `<div id="mapModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:1000;align-items:center;justify-content:center;padding:12px">
    <div style="background:white;border-radius:16px;width:100%;max-width:500px;max-height:90vh;display:flex;flex-direction:column">
      <div class="map-header"><b>📍 Pilih Lokasi</b><button id="btnCloseMap" style="border:none;background:#eee;border-radius:50%;width:32px;height:32px">✕</button></div>
      <div id="leafletMap" style="height:350px;width:100%"></div>
      <div style="padding:12px">
        <div id="mapSelectedAddress" style="font-size:12px">Pilih titik di peta</div>
        <div id="mapSelectedCoords" style="font-size:10px;color:#666"></div>
        <div id="mapTrenggalekBadge" style="margin-top:6px;padding:4px 8px;border-radius:8px;font-size:11px;text-align:center;background:#16a34a;color:white"></div>
        <div style="display:flex;gap:8px;margin-top:10px"><button id="btnConfirmMap" class="btn primary" style="flex:1">✅ Pakai Lokasi</button><button id="btnOpenGoogle" class="btn secondary" style="flex:1">🗺️ Google Maps</button></div>
      </div>
    </div>
  </div>`;
}
