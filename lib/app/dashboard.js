// admin.js - v5 FINAL - Setting 100% ORIGINAL, Force delete Ban button, Modal fixed
import { supabase } from './supabase.js';
import { APP_SETTINGS_DEFAULT, KECAMATAN_DATA, getActiveKecamatanLive } from './config.js';

console.log('ADMIN v5.0 - FORCE 1 BUTTON - MODAL FIXED');
function escapeHtml(s){ if(s==null) return ''; return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
function escapeAttr(s){ return escapeHtml(s).replace(/"/g,'&quot;'); }

// Force inject modal + CSS immediately
(function inject(){
  if(document.getElementById('adminV5CSS')) return;
  const css=document.createElement('style'); css.id='adminV5CSS';
  css.textContent=`
  #adminUserModal{position:fixed!important;inset:0!important;z-index:9999999!important;background:rgba(0,0,0,.8)!important;display:none!important;align-items:center!important;justify-content:center!important;padding:16px!important}
  ...
  if(!document.getElementById('adminUserModal')){
    ...
    m.innerHTML=`<div class="modal-card"><button onclick="closeAdminModal()" style="position:absolute;right:10px;top:10px;background:#334155;color:#fff;border:0;padding:8px 12px;border-radius:8px;z-index:2">✕</button><div id="adminUserModalContent"></div></div>`;
    document.body.appendChild(m);
    m.addEventListener('click',e=>{ if(e.target.id==='adminUserModal') closeAdminModal(); });
  }
  // Observer to delete Ban buttons whenever list changes
  const obs=new MutationObserver(()=>{
    document.querySelectorAll('#adminUserList .admin-list-item').forEach(item=>{
      const btns=item.querySelectorAll('button');
      if(btns.length>1){
        btns.forEach((b,i)=>{
          if(i>0) b.remove(); // keep only first (Detail)
        });
      }
      // also remove if text contains Ban
      item.querySelectorAll('button').forEach(b=>{
        if(b.textContent.toLowerCase().includes('ban')){ 
          // keep if inside modal, delete if inside list
          if(b.closest('#adminUserList')) b.remove();
        }
      });
    });
  });
...
window.adminViewUser=async function(id){
  ...
  if(!modal||!box) return alert('Modal error');
  ...
  box.innerHTML=`
    ...
    <div style="margin-top:10px;font-size:13px;display:grid;gap:6px">
