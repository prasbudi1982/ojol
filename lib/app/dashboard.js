// admin_v6.js - FIX MODAL MUNCUL - Detail only + modal 100% muncul
import { supabase } from './supabase.js';
import { APP_SETTINGS_DEFAULT, KECAMATAN_DATA, getActiveKecamatanLive } from './config.js';

console.log('ADMIN v6.0 - MODAL FIX');
...
function ensureModal(){
  ...
  if(!modal){
    ...
    modal.style.cssText='position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,0.85);display:none;align-items:center;justify-content:center;padding:16px;';
    modal.innerHTML=`<div class="modal-card" style="width:100%;max-width:480px;max-height:90vh;overflow:auto;background:#1e293b;border:1px solid #334155;border-radius:16px;padding:16px;color:#fff;position:relative"><button onclick="closeAdminModal()" style="position:absolute;right:10px;top:10px;background:#334155;color:#fff;border:0;padding:8px 12px;border-radius:8px;z-index:10">✕</button><div id="adminUserModalContent"></div></div>`;
...
export async function getUserDetail(userId){
  try{
    if(!userId) throw new Error('ID kosong');
  ...
  }catch(e){ console.error('getUserDetail error',e); return {error:e.message}; }
}
export async function banUser(id,r='Pelanggaran'){ const {error,data}=await supabase.from('users').update({banned:true,status:'banned',banned_reason:r,banned_at:new Date().toISOString()}).eq('id',id).select(); if(error) throw error; if(!data?.length) throw new Error('RLS blokir ban'); try{await supabase.from('banned_logs').insert({user_id:id,reason:r});}catch(e){} return true; }
export async function unbanUser(id){ const {error,data}=await supabase.from('users').update({banned:false,status:'active',banned_reason:null,banned_at:null}).eq('id',id).select(); if(error) throw error; if(!data?.length) throw new Error('RLS blokir unban'); return true; }
...
window.adminViewUser=async function(id){
  console.log('adminViewUser CLICKED',id);
  const modal=ensureModal(); const box=document.getElementById('adminUserModalContent');
  if(!modal||!box){ alert('Modal element hilang'); return; }
  // SHOW MODAL FIRST - sebelum fetch
  modal.style.display='flex';
  document.body.style.overflow='hidden';
  box.innerHTML='<div style="padding:20px;text-align:center">⏳ Loading user...<br><small>'+escapeHtml(id)+'</small></div>';

  try{
    const res=await getUserDetail(id);
    if(!res){ box.innerHTML='<div style="color:#ef4444">User tidak ditemukan di DB</div><button onclick="closeAdminModal()" style="margin-top:10px;width:100%;padding:10px;background:#334155;color:#fff;border:0;border-radius:10px">Tutup</button>'; return; }
    if(res.error){ box.innerHTML='<div style="color:#ef4444">Error: '+escapeHtml(res.error)+'<br>ID: '+escapeHtml(id)+'</div><div style="margin-top:10px"><b>Coba cek RLS policy users SELECT</b></div><button onclick="closeAdminModal()" style="margin-top:10px;width:100%;padding:10px;background:#334155;color:#fff;border:0;border-radius:10px">Tutup</button>'; return; }
    ...
    box.innerHTML=`
      ...
      <div style="margin-top:10px;font-size:13px;display:grid;gap:6px">
        ...
        <div>ID: <small style="color:#94a3b8">${escapeHtml(u.id)}</small></div>
      ...
      <div style="margin-top:12px"><b>📦 ${res.orders.length} Order terakhir</b><div style="max-height:150px;overflow:auto;margin-top:6px">${res.orders.map(o=>`<div style="padding:6px 0;border-bottom:1px solid #334155;display:flex;justify-content:space-between;font-size:12px"><span>${new Date(o.created_at).toLocaleDateString('id-ID')} ${escapeHtml(o.status)}</span><span>Rp${(o.total_fare||0).toLocaleString('id-ID')}</span></div>`).join('')||'Tidak ada order'}</div></div>
    ...
    box.innerHTML='<div style="color:#ef4444">Exception: '+escapeHtml(e.message)+'</div><button onclick="closeAdminModal()" style="margin-top:10px;width:100%;padding:10px;background:#334155;color:#fff;border:0;border-radius:10px">Tutup</button>';
  }
};
