...
import { TRENGGALEK_BBOX } from './config.js';
import { isInSuruhBbox, isInTrenggalekKab, haversineKm } from './geofence.js';

export let currentNearby = [];
export let currentOrderMeta = {distanceKm:0, cost:0, tripType:'oneway', pickupLat:null, pickupLng:null, destLat:null, destLng:null};
...
export function renderDriverList(drivers){
  ...
  if(!drivers?.length){ list.innerHTML = '<p class="muted">Tidak ada driver online.</p>'; return; }
  list.innerHTML = drivers.map(d=>{
    ...
    return `<div class="driver-card" style="border:1px solid var(--border);border-radius:12px;padding:10px;display:flex;gap:10px;align-items:center;justify-content:space-between">
      <div><b>${d.name}</b> • ${d.nopol||''} • ${(d.distance_km||d.distance||0).toFixed? (d.distance_km||0).toFixed(2)+' km' : ''}<br/><span class="muted" style="font-size:11px">${d.tipe_motor||''}</span></div>
      <div class="row" style="gap:6px;flex-wrap:wrap"><button class="btn primary" data-order-driver="${d.driver_id||d.id}">✅ Pesan</button>${wa?`<a class="btn secondary" href="${wa}" target="_blank">💬 WA</a>`:''}<button class="btn secondary" style="padding:8px 10px;font-size:12px" data-report-user="${d.driver_id||d.id}" data-report-name="${(d.name||'').replace(/"/g,'')}" data-report-google="${d.google_id||''}" data-report-email="${d.email||''}" title="Laporkan">🚩</button></div>
    </div>`;
  }).join('');
}
...
export async function updateOrderEstimate(){
  ...
  const card=document.getElementById('estimateCard'); const distEl=document.getElementById('estDistance'); const costEl=document.getElementById('estCost'); const typeEl=document.getElementById('estTripType'); const btnOrder=document.getElementById('btnOrder');
  ...
  currentOrderMeta.pickupLat=pLat; currentOrderMeta.pickupLng=pLng; currentOrderMeta.distanceKm=distance; currentOrderMeta.tripType=tripType;
  const base=3000, perKm=2500; let cost=base+(distance*perKm); if(tripType==='roundtrip') cost=cost*1.6; cost=Math.round(cost/500)*500; currentOrderMeta.cost=cost;
  distEl.textContent=distance.toFixed(2)+' km'; typeEl.textContent=tripType==='roundtrip'?'Pulang-Pergi (x1.6)':'Antar Saja'; costEl.textContent='Rp '+cost.toLocaleString('id-ID');
  if(btnOrder) btnOrder.style.display='block';
  const si=document.getElementById('searchInfo'); if(si) si.textContent=`Estimasi: ${distance.toFixed(2)} km • ${typeEl.textContent} • Rp ${cost.toLocaleString('id-ID')}`;
}
...
export async function createOrder(driverId=null, currentProfile){
  ...
  const {tripType, cost, distanceKm}=currentOrderMeta;
  try{
    const payload={ passenger_id:currentProfile.id, driver_id:driverId, pickup_text:pickup, dest_text:dest, pickup_lat:currentOrderMeta.pickupLat, pickup_lng:currentOrderMeta.pickupLng, dest_lat:currentOrderMeta.destLat, dest_lng:currentOrderMeta.destLng, distance_km:distanceKm, trip_type:tripType, estimated_cost:cost, status: driverId?'pending':'searching' };
    const { data, error }=await supabase.from('orders').insert(payload).select().single(); if(error) throw error; alert(`Order dibuat! ${pickup} -> ${dest}`);
  }catch(err){ alert(`Estimasi: ${currentOrderMeta.distanceKm.toFixed(2)}km • ${currentOrderMeta.tripType} • Rp ${currentOrderMeta.cost.toLocaleString('id-ID')}`); }
}
