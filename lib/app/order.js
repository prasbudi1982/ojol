...
export async function renderDriverList(drivers){
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:16px;padding:14px;margin-bottom:10px;display:flex;gap:12px">
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          <span style="background:#1d2633;border:1px solid var(--border);padding:3px 8px;border-radius:8px;font-size:10px;color:var(--muted)">${vehIcon} ${vehLabel.toUpperCase()}</span>
        </div>
      ...
      <div style="display:flex;flex-direction:column;gap:8px;min-width:112px;justify-content:center">
        ...
        <div style="display:flex;gap:6px">
          ${wa?`<a href="${wa}" target="_blank" style="flex:1;background:#1d2633;border:1px solid var(--border);color:var(--text);padding:8px;border-radius:10px;text-align:center;font-size:11px;font-weight:700;text-decoration:none">💬 WA</a>`:''}
  ...
  if(!filtered?.length){
    ...
    list.innerHTML = '<div style="background:#1d2633;border:1px dashed #263240;border-radius:12px;padding:16px;text-align:center;color:var(--muted);font-size:12px">Tidak ada driver online</div>'; return;
  }
...
export async function searchNearby(){
  ...
  try{
    ...
    if(drivers.length===0){
      ...
      if(list) list.innerHTML='<div style="background:#1d2633;border:1px dashed #263240;border-radius:12px;padding:16px;text-align:center;color:var(--muted);font-size:12px">Tidak ada driver online saat ini<br/><span style="font-size:10px">Driver harus Go Online di halaman Driver</span></div>';
    }
...
export function viewPassenger(currentProfile){
  ...
  <label>Tujuan <span class="muted" style="font-size:10px" id="destLive">🔍 ketik 1 huruf</span>
    ...
    <div id="destSuggestions" style="display:none;max-height:220px;overflow-y:auto;border:1px solid #334155;border-radius:8px;margin-top:6px;background:var(--card2)"></div>
  </label>
