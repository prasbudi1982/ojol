...
export async function renderDriverList(drivers){
  ...
  if(!filtered?.length){
    if(drivers.length>0){
      list.innerHTML = `<p style="font-size:11px;color:#fbbf24;margin-bottom:8px">⚠️ Tidak ada driver ${selectedVehicle} online di Suruh, menampilkan semua:</p>` + drivers.map(renderCard).join('');
      return;
    }
