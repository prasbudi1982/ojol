...
export async function renderDriverList(drivers){
  ...
  // FIX: tolak admin & passenger, hanya driver
...
export async function searchNearby(){
  ...
  try{
    ...
    if(!error && data && data.length>0){ 
      // FIX: hanya role driver, tolak admin & passenger (case-insensitive)
      drivers=data.filter(d=>{ const r=(d.role||'').toLowerCase(); return r==='driver'; });
      if(info) info.textContent=`✅ ${drivers.length} driver terdekat`; 
    }
    else {
      ...
      if(locs && locs.length>0){
        ...
        const { data: users } = await supabase.from('users').select('*').in('id', ids).ilike('status','online').ilike('role','driver');
        const filteredUsers = (users||[]).filter(u=> (u.role||'').toLowerCase()==='driver');
        drivers=locs.map(l=>{ const u=filteredUsers.find(x=>x.id===l.driver_id); return u? {...u,...l,driver_id:l.driver_id}:null; }).filter(Boolean);
        if(info) info.textContent=`✅ ${drivers.length} driver online`;
      } else { if(info) info.textContent='❌ Tidak ada driver online'; }
    }
    // Final guard - pastikan tidak ada admin
