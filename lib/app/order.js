...
import { TRENGGALEK_BBOX, TARIF, VEHICLE_TYPES, hitungTarif, hitungRincianTarif, ACTIVE_KECAMATAN_NAME } from './config.js';
...
// ===== AUTO FOCUS SCROLL FLOW =====
export function autoScrollTo(targetId, delay = 120){
  setTimeout(()=>{
    ...
    el.style.boxShadow='0 0 0 3px rgba(34,197,94,0.45)';
    setTimeout(()=>{ el.style.boxShadow=prev; }, 900);
  }, delay);
}
export function onMapInserted(){ autoScrollTo('vehicleSection', 180); }
export function onVehicleSelected(){ autoScrollTo('tripTypeSection', 100); }
export function onTripTypeSelected(){
  autoScrollTo('estimateCard', 100);
  // lanjut ke driver list setelah biaya terlihat
  autoScrollTo('driverList', 650);
}
export function onCostFocused(){ autoScrollTo('driverList', 200); }

let _autoFocusBound = false;
export function initAutoFocusListeners(){
  if(_autoFocusBound) return;
  _autoFocusBound = true;
  document.addEventListener('change', (e)=>{
    if(e.target.name === 'vehicleType'){
      document.querySelectorAll('#labelMotor, #labelMobil').forEach(l=>{
        l.style.border='1px solid var(--border)'; l.style.background='transparent';
      });
      const label = document.getElementById(e.target.value==='motor'?'labelMotor':'labelMobil');
      if(label){ label.style.border='2px solid #22c55e'; label.style.background='rgba(34,197,94,0.1)'; }
      onVehicleSelected();
      if(window.updateEstimate) window.updateEstimate();
    }
  ...
  window.addEventListener('map:selected', ()=> onMapInserted());
  window.addEventListener('dest:selected', ()=> onMapInserted());
  const estCard = document.getElementById('estimateCard');
  if(estCard){
    const obs = new MutationObserver(()=>{
      if(estCard.style.display !== 'none' && estCard.style.display !== ''){
        onCostFocused();
      }
    });
    obs.observe(estCard, {attributes:true, attributeFilter:['style']});
  }
}
...
export async function renderDriverList(drivers){
  ...
  const renderCard = (d)=>{
    ...
    const ratingBg = r ? (parseFloat(r.avg)>=4.5 ? '#22c55e' : parseFloat(r.avg)>=4.0 ? '#f59e0b' : '#475569') : 'var(--border)';
...
export async function searchNearby(){
  ...
  try{
    ...
    if(drivers.length===0){
      ...
      if(list) list.innerHTML='<div style="background:var(--card2);border:1px dashed var(--border);border-radius:12px;padding:16px;text-align:center;color:var(--muted);font-size:12px">Tidak ada driver online saat ini<br/><span style="font-size:10px">Driver harus Go Online di halaman Driver</span></div>';
    }
...
export async function geocodeAddress(q){
  ...
  // Jika sudah koordinat, langsung pakai
  const coordMatch = trimmed.match(/^(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)\s*$/);
  if(coordMatch){
    const lat=parseFloat(coordMatch[1]); const lng=parseFloat(coordMatch[2]);
    if(!isNaN(lat) && !isNaN(lng)){
      return {lat, lng, display_name: `${lat.toFixed(6)}, ${lng.toFixed(6)} (Koordinat)`};
    }
  }
  // Jangan geocode jika query terlalu generik seperti "Suruh", "Suruh Trenggalek" saja - itu akan return pusat kecamatan dan bikin biaya 0
  const lower = trimmed.toLowerCase();
  if(lower === 'suruh' || lower === 'suruh trenggalek' || lower === 'kecamatan suruh' || lower === 'kec suruh'){
    console.warn('geocodeAddress blocked generic query:', q, '- harus pakai Map Picker untuk tujuan spesifik');
    return null;
  }
  try{
    ...
    if(data && data[0]){
      ...
      // FIX: jika hasil geocode terlalu dekat dengan pusat Suruh (<500m) padahal query tidak mengandung alamat spesifik, anggap tidak akurat
      try{
        const { SURUH_CENTER } = await import('./config.js');
        const { haversineKm } = await import('./geofence.js');
        const distToSuruh = haversineKm(lat, lng, SURUH_CENTER.lat, SURUH_CENTER.lng);
        if(distToSuruh < 0.5 && trimmed.length < 15){
          console.warn('geocode result too close to SURUH_CENTER for generic query', q, '-> block, pakai Map Picker');
          return null;
        }
      }catch(e){}
...
export async function searchDestLive(q, mapModule){
  ...
  const coordMatch = raw.match(/^(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)\s*$/);
  if(coordMatch){
    const lat=parseFloat(coordMatch[1]); const lng=parseFloat(coordMatch[2]);
    if(!isNaN(lat) && !isNaN(lng)){
      if(!isInTrenggalekKab(lat,lng)){
        if(box){ box.style.display='block'; box.innerHTML='<div style="padding:10px;color:#ef4444;font-size:11px">⛔ Koordinat di luar Kabupaten Trenggalek</div>'; }
        if(badge) badge.textContent='⛔ Koordinat luar Trenggalek';
        return;
      }
      if(box){
        box.style.display='block';
        box.innerHTML = `<div class="suggestion" data-lat="${lat}" data-lon="${lng}" data-label="${lat.toFixed(6)}, ${lng.toFixed(6)} (Koordinat)" style="padding:10px;border-bottom:1px solid var(--card2);cursor:pointer;color:var(--border)">📍 ${lat.toFixed(5)}, ${lng.toFixed(5)} (Koordinat) <span style="font-size:9px;background:#dcfce7;color:#166534;padding:2px 6px;border-radius:6px">Valid</span></div>`;
      }
      if(badge) badge.textContent='📍 Koordinat terdeteksi ✅';
      box.querySelectorAll('.suggestion').forEach(el=>{
        el.onclick=(e)=>{
          e.preventDefault(); e.stopPropagation();
          const lat=parseFloat(el.dataset.lat); const lon=parseFloat(el.dataset.lon);
          const di=document.getElementById('dest'); if(!di) return;
          di.value=`${lat.toFixed(6)}, ${lon.toFixed(6)}`; di.dataset.lat=lat; di.dataset.lng=lon; di.dataset.justSelected='true';
          box.style.display='none'; box.innerHTML='';
          if(badge) badge.textContent='📍 Koordinat dipilih ✅';
          if(mapModule){ mapModule.mapSelected.lat=lat; mapModule.mapSelected.lng=lon; mapModule.mapSelected.address=di.value; mapModule.mapSelected.inTrenggalek=true; }
  ...
  try{
    ...
    if(!results.length){ 
      if(box){ box.style.display='block'; box.innerHTML='<div style="padding:10px;color:#fbbf24;font-size:11px">⚠️ Lokasi tidak terdeteksi<br/>Coba ketik ulang atau gunakan 🗺️ Map Picker</div>'; } 
      if(badge) badge.textContent='⚠️ Lokasi tidak terdeteksi'; 
      return; 
    }
    ...
    if(!filtered.length){
      if(box){ box.style.display='block'; box.innerHTML='<div style="padding:10px;color:#ef4444;font-size:11px">⛔ Hasil di luar Kabupaten Trenggalek<br/>Tujuan hanya boleh di wilayah Trenggalek</div>'; }
    ...
    if(box){
      ...
      box.innerHTML = displayResults.map(r=>{
        ...
        return `<div class="suggestion" data-lat="${lat}" data-lon="${lon}" data-label="${safeLabel}" style="padding:10px;border-bottom:1px solid var(--card2);cursor:pointer;color:var(--border)">📍 ${short}</div>`;
      }).join('');
    }
    ...
    if(box){
      box.querySelectorAll('.suggestion').forEach(el=>{
        el.onclick=(e)=>{
          ...
          const di=document.getElementById('dest'); if(!di) return;
          if(!isInTrenggalekKab(lat, lon)){ alert('⛔ Tujuan di luar Kabupaten Trenggalek - pilih yang di Trenggalek'); return; }
          const clean=label.split(',').slice(0,4).join(',').slice(0,140);
          di.value=clean; di.dataset.lat=lat; di.dataset.lng=lon; di.dataset.justSelected='true';
          box.style.display='none'; box.innerHTML='';
    ...
    if(badge) badge.textContent='⚠️ Lokasi tidak terdeteksi';
    if(box){ box.style.display='block'; box.innerHTML='<div style="padding:8px;color:#ef4444;font-size:11px">⚠️ Lokasi tidak terdeteksi<br/>Coba ketik ulang atau gunakan 🗺️ Map Picker<br/><small>'+e.message+'</small></div>'; }
  }
}
...
export async function updateOrderEstimate(){
  ...
  if(dLat&&dLng && !isNaN(dLat) && !isNaN(dLng)){ 
    ...
    const coordMatch = destInput.value.trim().match(/^(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)\s*$/);
    if(coordMatch){
      const lat=parseFloat(coordMatch[1]); const lng=parseFloat(coordMatch[2]);
      if(!isNaN(lat) && !isNaN(lng)){
        geoDest={lat, lng};
        destInput.dataset.lat=lat; destInput.dataset.lng=lng;
        if(badge) badge.textContent='📍 Koordinat terdeteksi ✅';
      }
    } else {
      ...
      if(geoDest){
        ...
        if(badge) badge.textContent='⚠️ Lokasi tidak terdeteksi';
        alert('⚠️ Lokasi tidak terdeteksi - coba ketik ulang atau gunakan 🗺️ Map Picker. Tujuan harus di Trenggalek.');
        if(card) card.style.display='none'; if(btnOrder) btnOrder.style.display='none';
        return;
      }
    }
  ...
  console.log(`📏 Jarak dihitung dari koordinat: pickup ${pLat.toFixed(5)},${pLng.toFixed(5)} -> dest ${geoDest.lat.toFixed(5)},${geoDest.lng.toFixed(5)} = ${distance.toFixed(2)} km`);
  ...
  const rincian = hitungRincianTarif(distance, vehicleType, tripType);
  currentOrderMeta.cost=rincian.total;
  ...
  if(costEl) costEl.textContent='Rp '+rincian.total.toLocaleString('id-ID');
  if(btnOrder) btnOrder.style.display='block';
  const si=document.getElementById('searchInfo'); 
  if(si) si.textContent=`Estimasi: ${distance.toFixed(2)} km • ${typeEl?.textContent||''} • ${vehInfo.icon} ${vehInfo.label} • Rp ${rincian.total.toLocaleString('id-ID')} • Tujuan Trenggalek ✅`;
...
export async function createOrder(driverId, currentProfile){
  ...
  try{
    ...
    const tripType = document.querySelector('input[name="tripType"]:checked')?.value || 'oneway';
    const vehicleType = document.querySelector('input[name="vehicleType"]:checked')?.value || 'motor';
    const cost = hitungTarif(distanceKm, vehicleType, tripType);
    ...
    if(error){
      ...
      const isGeometryError = (error.message||'').toLowerCase().includes('geometry');
      const isStatusCheckError = (error.message||'').toLowerCase().includes('status_check');
      ...
      if(isGeometryError){
        // tanpa lat/lng kalau geometry error, tapi tetap ada cost & distance
        minimal = {
          passenger_id: currentProfile.id,
          driver_id: driverId||null,
          pickup: pickupVal,
    ...
    try{ 
      console.log('🚀 Starting tracking for order', data.id, 'payload', basePayload);
      ...
      localStorage.setItem('distance_km', distanceKm.toString());
      ...
      localStorage.setItem('pickup_val', pickupVal);
      localStorage.setItem('dest_val', destVal);
      ...
      // FIX: simpan queue driver untuk auto assign - id order tetap, index berubah
      try{
        const queue = (currentNearby||[]).map(d=>({id: d.driver_id||d.id, name: d.name||'Driver', distance_km: d.distance_km||0}));
        if(queue.length>0){
          ...
          console.log('Auto queue saved', queue.length, 'untuk order', data.id);
        }
      }catch(e){ console.warn('save queue fail', e); }
      // FIX TRACKING TIDAK TERPANGGIL - panggil dengan aman + fallback ke window.tracking
      if(tracking && typeof tracking.startTracking === 'function'){
        tracking.startTracking(data.id);
      } else if(window.tracking && typeof window.tracking.startTracking === 'function'){
        console.warn('tracking import gagal, pakai window.tracking');
        window.tracking.startTracking(data.id);
      } else {
        console.warn('tracking.startTracking tidak ada, coba dynamic import');
        import('./tracking.js').then(m=> m.startTracking(data.id)).catch(e=> console.error('dynamic tracking fail', e));
      }
    ...
    }catch(e){ console.error('tracking start fail', e); }
    return data;
  }catch(err){ 
...
// ===== AUTO QUEUE - untuk tracking timer 40s =====
export function getAutoQueue(orderId){
  try{
    return JSON.parse(localStorage.getItem('auto_queue_'+orderId)||'[]');
  }catch(e){ return []; }
}
export function getAutoIndex(orderId){
  try{
    return parseInt(localStorage.getItem('auto_index_'+orderId)||'0');
  }catch(e){ return 0; }
}
export function setAutoQueue(orderId, drivers){
  try{
    localStorage.setItem('auto_queue_'+orderId, JSON.stringify(drivers||[]));
    localStorage.setItem('auto_index_'+orderId, '0');
...
export async function assignToNextDriver(orderId, reason){
  try{
    ...
    idx++;
    if(idx >= queue.length){
      console.log('Auto queue habis untuk', orderId);
      // Jangan langsung cancel, biarkan tracking timer cek driver online
      try{ localStorage.setItem('auto_index_'+orderId, idx.toString()); }catch(e){}
      return null;
    }
    const nextDriver = queue[idx];
    if(!nextDriver) return null;
    console.log(`Assign next driver ${idx}: ${nextDriver.name} - ${reason}`);
    const { data, error } = await supabase.from('orders').update({ driver_id: nextDriver.id || nextDriver.driver_id, status: 'searching' }).eq('id', orderId).select().single();
    if(error){
      console.warn('assignToNextDriver error', error.message);
      // coba tanpa status
      await supabase.from('orders').update({ driver_id: nextDriver.id || nextDriver.driver_id }).eq('id', orderId);
    }
    try{
      localStorage.setItem('auto_index_'+orderId, idx.toString());
      localStorage.setItem('auto_start_'+orderId, Date.now().toString());
      localStorage.setItem('last_driver_id', nextDriver.id || nextDriver.driver_id);
      localStorage.setItem('last_driver_name', nextDriver.name||'Driver');
    }catch(e){}
    return nextDriver;
  }catch(e){
    console.error('assignToNextDriver fail', e);
    return null;
  }
}
...
export function viewPassenger(currentProfile){
  ...
  <div id="vehicleSection" style="scroll-margin-top:90px">
  ...
  <div id="tripTypeSection" style="scroll-margin-top:90px;margin-top:12px"><div class="row"><label style="flex:1"><input type="radio" name="tripType" value="oneway" checked> Antar Saja</label><label style="flex:1"><input type="radio" name="tripType" value="roundtrip"> PP x1.6</label></div></div>
  <div id="estimateCard" style="display:none;margin-top:12px;scroll-margin-top:90px;border:1px dashed #475569;padding:10px;border-radius:8px"><p style="font-size:12px">Jarak: <b id="estDistance">-</b> • <span id="estTripType">-</span> • <span id="estVehicle">-</span> • Biaya: <b id="estCost">-</b></p><p class="muted" id="searchInfo" style="font-size:11px"></p></div>
  <button id="btnOrder" class="btn primary" style="display:none;margin-top:12px">🚀 Order Sekarang</button>
  <div id="driverList" style="margin-top:12px;scroll-margin-top:90px"></div>
  </div>
  </div>`;
}

// ===== AUTO TRIGGER after dest map selection (fallback) =====
(function(){
  const _origInit = window.initPassengerOrder || null;
  // auto init listeners when DOM ready
  const tryInit = ()=> {
    if(document.getElementById('vehicleSection')) initAutoFocusListeners();
  };
  document.addEventListener('DOMContentLoaded', tryInit);
  setTimeout(tryInit, 800);
  // observe body for orderFormCard insertion
  const bodyObs = new MutationObserver(()=> tryInit());
  bodyObs.observe(document.body, {childList:true, subtree:true});

  // intercept dest input change to trigger map flow
  document.addEventListener('change', (e)=>{
    if(e.target && e.target.id === 'dest' && e.target.dataset && e.target.dataset.lat){
      onMapInserted();
...
// expose to window for map.js
if(typeof window!=='undefined'){
  window.autoScrollTo = autoScrollTo;
  window.onMapInserted = onMapInserted;
  window.onVehicleSelected = onVehicleSelected;
  window.onTripTypeSelected = onTripTypeSelected;
  window.onCostFocused = onCostFocused;
  window.initAutoFocusListeners = initAutoFocusListeners;
}
