...
// ===== VIEW PASSENGER - dengan lock modal tracking =====
export function viewPassenger(currentProfile){
  const activeId = typeof localStorage !== 'undefined' ? localStorage.getItem('active_order_id') : null;
  const lockBanner = activeId ? `<div id="activeOrderLockBanner" style="background:#0f172a;border:2px solid #f59e0b;border-radius:12px;padding:12px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;color:white"><div><div style="font-weight:800;font-size:13px">🔒 Order aktif berjalan</div><div style="font-size:10px;color:#94a3b8;margin-top:2px">ID ${activeId.slice(0,6).toUpperCase()} - selesaikan dulu</div></div><button onclick="document.getElementById('activeOrderCard')?.scrollIntoView({behavior:'smooth'})" style="background:#f59e0b;color:#0f172a;border:none;padding:8px 12px;border-radius:8px;font-weight:800;font-size:11px">Lihat</button></div>` : '';
  return `<div>
  <div id="activeOrderCard" style="display:none;margin-bottom:12px" class="card tracking-card"></div>
  ${lockBanner}
