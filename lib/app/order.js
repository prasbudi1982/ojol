...
const TRENGGALEK_FALLBACK_PLACES = [
  {name:'Pasar Pon Trenggalek', lat:-8.05, lng:111.71},
  {name:'Alun-alun Trenggalek', lat:-8.0505, lng:111.71},
  {name:'RSUD Trenggalek', lat:-8.055, lng:111.715},
  {name:'Terminal Trenggalek', lat:-8.06, lng:111.72},
  {name:'Stadion Menak Sopal', lat:-8.045, lng:111.705},
  {name:'Pasar Suruh', lat:-8.11, lng:111.60},
  {name:'Kecamatan Suruh', lat:-8.111, lng:111.606},
  {name:'Kecamatan Durenan', lat:-8.08, lng:111.78},
  {name:'Kecamatan Pogalan', lat:-8.07, lng:111.73},
  {name:'Kecamatan Trenggalek Kota', lat:-8.05, lng:111.71},
  {name:'Kecamatan Gandusari', lat:-8.05, lng:111.68},
  {name:'Kecamatan Karangan', lat:-8.07, lng:111.62},
  {name:'Kecamatan Pule', lat:-8.05, lng:111.55},
  {name:'Kecamatan Bendungan', lat:-7.98, lng:111.70},
  {name:'Kecamatan Tugu', lat:-8.02, lng:111.62},
  {name:'Watulimo - Pantai Prigi', lat:-8.28, lng:111.72},
  {name:'Munjungan', lat:-8.25, lng:111.58},
  {name:'Panggul', lat:-8.20, lng:111.45},
  {name:'Dongko', lat:-8.18, lng:111.58},
  {name:'Kampak', lat:-8.20, lng:111.58},
];
...
export async function searchDestLive(q, mapModule){
  ...
  try{
    ...
    if(!results.length){ 
      ...
      const localMatches = TRENGGALEK_FALLBACK_PLACES.filter(p=> p.name.toLowerCase().includes(low)).slice(0,8);
      if(localMatches.length){
        if(box){
          box.style.display='block';
          box.innerHTML = localMatches.map(p=>`<div class="suggestion" data-lat="${p.lat}" data-lon="${p.lng}" data-label="${p.name}" style="padding:10px;border-bottom:1px solid #1e293b;cursor:pointer;color:#e2e8f0">📍 ${p.name} <span style="font-size:9px;background:#fef3c7;padding:2px 6px;border-radius:6px">Offline</span></div>`).join('');
        }
        if(badge) badge.textContent=`${localMatches.length} hasil offline di Trenggalek`;
        box.querySelectorAll('.suggestion').forEach(el=>{
          el.onclick=(e)=>{
            e.preventDefault(); e.stopPropagation();
            const lat=parseFloat(el.dataset.lat); const lon=parseFloat(el.dataset.lon); const label=el.dataset.label;
            const di=document.getElementById('dest'); if(!di) return;
            if(!isInTrenggalekKab(lat, lon)){ alert('⛔ Tujuan di luar Trenggalek'); return; }
            di.value=label; di.dataset.lat=lat; di.dataset.lng=lon; di.dataset.justSelected='true';
            box.style.display='none'; box.innerHTML='';
            if(badge) badge.textContent='📍 Tujuan offline dipilih ✅';
            if(mapModule){ mapModule.mapSelected.lat=lat; mapModule.mapSelected.lng=lon; mapModule.mapSelected.address=label; mapModule.mapSelected.inTrenggalek=true; }
            currentOrderMeta.destLat=lat; currentOrderMeta.destLng=lon;
            updateOrderEstimate();
            setTimeout(()=>{ di.dataset.justSelected='false'; }, 500);
          };
      ...
      if(box){ box.style.display='block'; box.innerHTML='<div style="padding:10px;color:#fbbf24;font-size:11px">❌ Tujuan tidak ditemukan di Trenggalek<br/>Coba kata kunci lain, atau input koordinat lat,lng seperti -8.05, 111.71</div>'; } 
    ...
    if(box){
      box.querySelectorAll('.suggestion').forEach(el=>{
        el.onclick=(e)=>{
          ...
          const lat=parseFloat(el.dataset.lat); const lon=parseFloat(el.dataset.lon); const label=el.dataset.label;
          const di=document.getElementById('dest'); if(!di) return;
          if(!isInTrenggalekKab(lat, lon)){ alert('⛔ Tujuan di luar Kabupaten Trenggalek'); return; }
    ...
    if(localMatches.length){
      ...
      if(badge) badge.textContent=`${localMatches.length} hasil offline`;
      box.querySelectorAll('.suggestion').forEach(el=>{
        el.onclick=(e)=>{
          e.preventDefault(); const lat=parseFloat(el.dataset.lat); const lon=parseFloat(el.dataset.lon); const label=el.dataset.label;
          const di=document.getElementById('dest'); if(!di) return;
          di.value=label; di.dataset.lat=lat; di.dataset.lng=lon; di.dataset.justSelected='true';
          box.style.display='none'; box.innerHTML='';
          if(badge) badge.textContent='📍 Tujuan offline ✅';
          currentOrderMeta.destLat=lat; currentOrderMeta.destLng=lon;
          updateOrderEstimate();
          setTimeout(()=>{ di.dataset.justSelected='false'; }, 500);
        };
      ...
      if(badge) badge.textContent='❌ Error jaringan - coba koordinat lat,lng';
      if(box){ box.style.display='block'; box.innerHTML='<div style="padding:8px;color:#ef4444;font-size:11px">❌ Gagal cari tujuan - cek internet<br/>Tapi bisa input koordinat langsung seperti<br/><b>-8.05, 111.71</b><br/><small>'+e.message+'</small></div>'; }
    }
  }
}
...
export async function updateOrderEstimate(){
  ...
  if(!isInSuruhBbox(pLat,pLng)){
    const kecName = getActiveKecamatanName ? getActiveKecamatanName() : (ACTIVE_KECAMATAN_NAME||'Suruh');
    alert('⛔ Kamu di luar Kecamatan '+kecName);
    if(card) card.style.display='none'; if(btnOrder) btnOrder.style.display='none';
    return;
  }
  ...
  if(dLat&&dLng && !isNaN(dLat) && !isNaN(dLng)){ 
    ...
    if(coordMatch){
      ...
      if(geoDest){
        ...
        if(badge) badge.textContent='❌ Gagal deteksi - pilih dari daftar atau input koordinat';
        alert('❌ Gagal deteksi lokasi tujuan. Input koordinat seperti -8.05, 111.71 atau pilih dari daftar. Tujuan harus di Trenggalek.');
        if(card) card.style.display='none'; if(btnOrder) btnOrder.style.display='none';
        return;
      }
    }
  ...
  if(!isInTrenggalekKab(geoDest.lat, geoDest.lng)){
    alert('⛔ Tujuan di luar Kabupaten Trenggalek');
...
export function viewPassenger(currentProfile){
  ...
  <label>Tujuan <span class="muted" style="font-size:10px" id="destLive">🔍 ketik 1 huruf</span>
    <div class="row" style="gap:8px;align-items:center"><input id="dest" placeholder="Ketik tujuan di Trenggalek..." style="flex:1;min-width:0"></div>
