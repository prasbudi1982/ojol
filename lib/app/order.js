...
export async function createOrder(driverId=null, currentProfile){
  ...
  try{
    ...
    // Simpan order aktif & trigger tracking live
    try{
      let driverInfo = null;
      if(driverId){
        const {data: drv} = await supabase.from('users').select('*').eq('id', driverId).single();
        driverInfo = drv;
      }
      await tracking.startTracking(driverId || data.driver_id, data.id, currentOrderMeta.pickupLat, currentOrderMeta.pickupLng, driverInfo);
      // Update GMaps link
      const gmapsBtn = document.getElementById('trackGmaps');
      if(gmapsBtn && currentOrderMeta.pickupLat){
        gmapsBtn.href = `https://www.google.com/maps/dir/?api=1&destination=${currentOrderMeta.pickupLat},${currentOrderMeta.pickupLng}`;
      }
      // Show tracking immediately
      const activeCard = document.getElementById('activeOrderCard');
      if(activeCard){ activeCard.scrollIntoView({behavior:'smooth'}); }
    }catch(trackErr){ console.warn('tracking start fail', trackErr); }

    alert(`✅ Order dibuat! Driver menuju pickup - tracking aktif di bawah`);
