  ...
  try{
    ...
    if(!error && data && data.length>0){ 
      ...
      if(locs && locs.length>0){
        ...
        // Hitung jarak manual jika tidak ada dari RPC
  ...
  try{
    ...
    if(box){
      ...
      box.innerHTML = displayResults.map(r=>{
        ...
        return `<div data-lat="${lat}" data-lng="${lon}" data-label="${short.replace(/"/g,'&quot;')}" style="padding:8px 10px;border-bottom:1px solid #334155;cursor:pointer;font-size:12px;color:#e6edf5">${short}</div>`;
      }).join('');
      box.querySelectorAll('div[data-lat]').forEach(el=>{
        el.addEventListener('click', ()=>{
          const lat=el.getAttribute('data-lat'); const lng=el.getAttribute('data-lng'); const label=el.getAttribute('data-label');
          const destInput=document.getElementById('dest');
          if(destInput){ destInput.value=label; destInput.dataset.lat=lat; destInput.dataset.lng=lng; }
