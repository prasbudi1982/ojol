...
export async function fetchDriverRatings(driverIds){
  ...
  try{
    ...
    Object.keys(localGrouped).forEach(id=>{
      if(ratingsMap[id]) return; // sudah ada dari DB
...
export async function renderDriverList(drivers){
  ...
  const renderCard = (d)=>{
    ...
    const ratingBadge = r ? `⭐ ${r.avg} <span style="font-size:10px;color:#64748b">(${r.count})</span>` : `⭐ Baru`;
    const ratingColor = r ? (parseFloat(r.avg)>=4.5 ? '#16a34a' : parseFloat(r.avg)>=4.0 ? '#f59e0b' : '#64748b') : '#94a3b8';
    return `<div class="driver-card" style="border:1px solid var(--border);border-radius:12px;padding:10px;display:flex;gap:10px;align-items:center;justify-content:space-between;background:white">
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap"><b style="font-size:13px">${d.name}</b> <span style="font-size:11px">${vehIcon} ${vehLabel}</span> <span style="background:${ratingColor};color:white;padding:2px 6px;border-radius:6px;font-size:10px;font-weight:700">${ratingBadge}</span></div>
        <div style="font-size:11px;color:#334155;margin-top:2px">${d.nopol||''} • ${(d.distance_km||d.distance||0).toFixed? (d.distance_km||0).toFixed(2)+' km' : ''} • ${d.tipe_motor||''}</div>
      </div>
      <div class="row" style="gap:6px;flex-wrap:wrap;flex-shrink:0"><button class="btn primary" data-order-driver="${d.driver_id||d.id}" data-vehicle="${vehLabel}" style="font-size:11px;padding:8px 10px">✅ Pesan</button>${wa?`<a class="btn secondary" href="${wa}" target="_blank" style="font-size:10px;padding:6px 8px">💬 WA</a>`:''}<button data-report-user="${d.driver_id||d.id}" data-report-name="${d.name}" data-report-role="driver" class="btn secondary" style="background:#fef2f2;border-color:#fecaca;font-size:10px;padding:6px 8px">🚩</button></div>
    </div>`;
  };

  if(!filtered?.length){
    if(drivers.length>0){
      list.innerHTML = `<p class="muted" style="font-size:12px">⚠️ Tidak ada driver ${selectedVehicle} online di ${ACTIVE_KECAMATAN_NAME||'Suruh'}, menampilkan semua:</p>` + drivers.map(renderCard).join('');
      return;
    }
...
export async function searchDestLive(q, mapModule){
  ...
  try{
    ...
    // Jangan filter ketat di awal, tampilkan semua tapi beri badge
    ...
    if(box){
      ...
      box.innerHTML = displayResults.map(r=>{
        ...
        const isIn = isInTrenggalekKab(lat, lon);
        const badgeText = isIn ? '' : ' <span style="font-size:9px;background:#fef3c7;padding:2px 6px;border-radius:6px">Luar Trenggalek</span>';
        return `<div class="suggestion" data-lat="${lat}" data-lon="${lon}" data-label="${label.replace(/"/g,'&quot;')}" style="padding:10px;border-bottom:1px solid #1e293b;cursor:pointer;color:#e2e8f0">📍 ${short}${badgeText}</div>`;
      }).join('');
      if(!displayResults.length){
        box.innerHTML = '<div style="padding:10px;color:#fbbf24;font-size:11px">⚠️ Tidak ada lokasi di Trenggalek - coba tambah kata "Trenggalek"</div>';
      }
    }
    if(badge) badge.textContent=`${displayResults.length} hasil`;
    if(box){
    ...
    if(badge) badge.textContent='Error jaringan';
    if(box){ box.style.display='block'; box.innerHTML='<div style="padding:8px;color:#ef4444;font-size:11px">❌ Gagal cari lokasi - coba lagi atau pakai tombol Map</div>'; }
  }
}
