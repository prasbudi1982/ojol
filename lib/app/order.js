...
export let currentOrderMeta = {distanceKm:0, cost:0, tripType:'oneway', vehicleType:'motor', pickupLat:null, pickupLng:null, destLat:null, destLng:null, driverDistanceKm:0, driverId:null, pickupFee:0};
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:16px;padding:14px;margin-bottom:10px;display:flex;gap:12px">
      <div style="flex:1;min-width:0">
        ...
        <div style="margin-top:6px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          <span style="font-size:11px;color:var(--muted)">${d.nopol||''} • jemput ${dist} km ${parseFloat(dist)>2 ? `(+Rp ${Math.round((parseFloat(dist)-2)*(vehLabel==='mobil'?1500:1000)).toLocaleString('id-ID')})` : '(gratis jemput)'} • ${d.tipe_motor||'Beat'}</span>
        </div>
      </div>
      <div style="display:flex;flex-direction:column;gap:8px;min-width:112px;justify-content:center">
        <button data-order-driver="${d.driver_id||d.id}" data-distance="${(d.distance_km||0).toFixed(2)}" data-vehicle="${vehLabel}" style="background:#22c55e;color:#052e16;border:none;padding:10px 14px;border-radius:12px;font-weight:800;font-size:13px;cursor:pointer;box-shadow:0 4px 12px rgba(34,197,94,0.3)">✅ Pesan</button>
...
export async function geocodeAddress(q){
  const coordMatch = q.trim().match(/^(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)\s*$/);
  if(coordMatch){
...
export async function updateOrderEstimate(){
  ...
  // Estimasi tanpa driver dulu, tapi kalau sudah ada driver terpilih pakai jarak jemputnya
  const driverDistForEst = currentOrderMeta.driverDistanceKm || 0;
  const rincian = hitungRincianTarif(distance, vehicleType, tripType, driverDistForEst);
  currentOrderMeta.cost=rincian.total;
  currentOrderMeta.pickupFee=rincian.pickupFee;
  ...
  if(costEl) {
    if(rincian.pickupFee>0){
      costEl.innerHTML = `Rp ${rincian.total.toLocaleString('id-ID')} <span style="font-size:10px;color:var(--muted)">(trip Rp ${Math.round(rincian.tripCost).toLocaleString('id-ID')} + jemput Rp ${rincian.pickupFee.toLocaleString('id-ID')})</span>`;
    } else {
      costEl.textContent='Rp '+rincian.total.toLocaleString('id-ID');
    }
  }
  ...
  if(si){
    if(rincian.pickupFee>0){
      si.textContent=`Estimasi: ${distance.toFixed(2)} km • jemput ${rincian.driverDistanceKm.toFixed(2)} km (gratis ${rincian.freeKm}km, +Rp ${rincian.perKmPickup}/km) • ${typeEl?.textContent||''} • ${vehInfo.icon} ${vehInfo.label} • Total Rp ${rincian.total.toLocaleString('id-ID')} • Tujuan Trenggalek ✅`;
    } else {
      si.textContent=`Estimasi: ${distance.toFixed(2)} km • jemput ${driverDistForEst>0?driverDistForEst.toFixed(2)+' km gratis':'gratis (driver <2km)'} • ${typeEl?.textContent||''} • ${vehInfo.icon} ${vehInfo.label} • Rp ${rincian.total.toLocaleString('id-ID')} • Tujuan Trenggalek ✅`;
    }
  }
...
export async function createOrder(driverId, currentProfile){
  ...
  try{
    ...
    const tripType = document.querySelector('input[name="tripType"]:checked')?.value || 'oneway';
    const vehicleType = document.querySelector('input[name="vehicleType"]:checked')?.value || 'motor';
    // Cari jarak driver -> penumpang dari currentNearby
    let driverDistanceKm = 0;
    if(driverId){
      const drv = (currentNearby||[]).find(d=> (d.driver_id||d.id)===driverId);
      if(drv && (drv.distance_km||drv.distance)){
        driverDistanceKm = parseFloat(drv.distance_km||drv.distance)||0;
      }
    }
    // Gunakan jarak dari meta jika ada (dari tombol Pesan)
    if(currentOrderMeta.driverDistanceKm && currentOrderMeta.driverId===driverId){
      driverDistanceKm = currentOrderMeta.driverDistanceKm;
    }
    const rincian = hitungRincianTarif(distanceKm, vehicleType, tripType, driverDistanceKm);
    const cost = rincian.total;
    currentOrderMeta.driverDistanceKm = driverDistanceKm;
    currentOrderMeta.driverId = driverId;
    currentOrderMeta.pickupFee = rincian.pickupFee;
    ...
    try{ 
      console.log('🚀 Starting tracking for order', data.id);
...
export function viewPassenger(currentProfile){
  ...
  <label>Pickup <span class="muted" style="font-size:10px" id="pickupLive">📡 live (jemput gratis 2km)</span>
