...
export async function createOrder(driverId, currentProfile){
  ...
  try{
    ...
    const basePayload = {
      ...
      dest_text: destVal,
      // kolom lama yang error di screenshot kamu
      pickup: pickupVal,
      destination: destVal,
      dest: destVal,
    ...
    if(error){
      ...
      // Jika error karena status check, coba status yang umum diizinkan constraint lama
      ...
      if(error){
        console.warn('Minimal failed:', error.message, 'coba hanya pickup_text/dest_text dengan status new');
        const fallbackStatuses = ['new', 'open', 'created', 'waiting'];
