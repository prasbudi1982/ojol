...
export async function searchDestLive(q, mapModule){
  ...
  try{
    ...
    if(!results.length){ 
      ...
      if(box){ box.style.display='block'; box.innerHTML='<div style="padding:10px;color:#fbbf24;font-size:11px">❌ Tujuan tidak ditemukan di Trenggalek<br/>Coba kata kunci lain, atau gunakan Map Picker -8.05, 111.71</div>'; } 
    ...
    const localMatches = [].filter(p=> p.name.toLowerCase().includes(low)).slice(0,8);
    if(localMatches.length){
      ...
      if(badge) badge.textContent='⚠️ Lokasi tidak terdeteksi';
      if(box){ box.style.display='block'; box.innerHTML='<div style="padding:8px;color:#ef4444;font-size:11px">⚠️ Lokasi tidak terdeteksi<br/>Coba ketik nama tempat di Trenggalek<br/><b>-8.05, 111.71</b><br/><small>'+e.message+'</small></div>'; }
    }
  }
}
