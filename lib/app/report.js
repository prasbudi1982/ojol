...
// report.js - Fitur Laporan - ADAPTIF TEMA HP
...
function createReportModal(){
  ...
  div.id = 'reportModal';
  div.style.cssText = 'display:none;position:fixed;inset:0;background:rgba(0,0,0,0.55);backdrop-filter:blur(6px);z-index:9999;align-items:center;justify-content:center;padding:16px';
  div.innerHTML = `
    <div style="background:var(--card);border:2px solid ${theme.primary};border-radius:18px;max-width:440px;width:100%;padding:0;overflow:hidden;box-shadow:0 20px 40px var(--shadow);color:var(--text)">
      ...
      <div style="padding:16px;background:var(--card)">
        ...
        <div style="font-size:13px;margin-bottom:12px;background:var(--card2);padding:10px;border-radius:10px;border:1px solid var(--border)">Melaporkan: <b id="reportReportedName" style="color:var(--text)"></b> <span style="background:${theme.secondary};color:#0f172a;padding:2px 8px;border-radius:8px;font-size:10px;font-weight:800" id="reportReportedRole"></span></div>
        <label style="font-size:11px;font-weight:700;color:var(--muted);letter-spacing:0.5px">ALASAN LAPORAN*</label>
        <select id="reportReason" style="width:100%;padding:12px;border-radius:10px;border:1px solid var(--border);background:var(--card2);color:var(--text);margin:6px 0 12px;font-size:13px">
        ...
        <label style="font-size:11px;font-weight:700;color:var(--muted);letter-spacing:0.5px">DESKRIPSI DETAIL</label>
        <textarea id="reportDesc" placeholder="Ceritakan kejadian dengan jujur..." style="width:100%;height:90px;padding:10px;border-radius:10px;border:1px solid var(--border);background:var(--card2);color:var(--text);margin:6px 0;font-size:13px;resize:none"></textarea>
        <div id="reportStatus" style="font-size:12px;margin:8px 0;min-height:16px;color:var(--muted)"></div>
        <div style="display:flex;gap:8px;margin-top:14px">
          ...
          <button id="btnCancelReport" class="btn secondary" style="flex:1;background:var(--card2);border:1px solid var(--border);color:var(--text);padding:12px;border-radius:12px;font-weight:700">Batal</button>
        </div>
      </div>
    </div>
  `;
...
export async function submitReport(currentProfile){
  ...
  try{
    ...
    if(statusEl) statusEl.textContent='✅ Disimpan lokal';
