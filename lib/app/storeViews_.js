  ...
  setTimeout(()=>{
    ...
    if(addBtn) addBtn.onclick = ()=>{
      try{
        ...
        const existing = state.items.find(it=>it.cartKey===cartKey);
        if(existing){ cartStore._actions.updateQty(cartKey, existing.qty + _builderState.qty); }
        else { cartStore._actions.addItem(item, _builderState.qty); }
  ...
  window._cartQty = (cartKey, qty)=>{
    try{
      if(qty<=0){
        ...
        const filtered = st.items.filter(it=> (it.cartKey||it.product_id||it.id) !== cartKey);
        cartStore.setState({ items: filtered });
        location.reload();
      } else {
        cartStore._actions.updateQty(cartKey, qty);
