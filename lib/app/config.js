...
export const TARIF = {
  motor: { base: 3000, perKm: 2500, min: 5000, label: 'Motor', pickupFreeKm: 2, pickupPerKm: 1000 },
  mobil: { base: 8000, perKm: 5500, min: 15000, label: 'Mobil', pickupFreeKm: 2, pickupPerKm: 1500 },
  ppMultiplier: 1.6,
  pickup: { freeKm: 2, motorPerKm: 1000, mobilPerKm: 1500 }
};
...
export function hitungTarif(distanceKm, vehicleType='motor', tripType='oneway', driverDistanceKm=0){
  const t = TARIF[vehicleType] || TARIF.motor;
  let tripCost = t.base + (distanceKm * t.perKm);
  if(tripType==='roundtrip') tripCost *= TARIF.ppMultiplier;

  // Biaya jemput driver -> penumpang, gratis 2km pertama
  let pickupFee = 0;
  const freeKm = t.pickupFreeKm ?? TARIF.pickup.freeKm ?? 2;
  const perKmPickup = t.pickupPerKm ?? (vehicleType==='mobil' ? TARIF.pickup.mobilPerKm : TARIF.pickup.motorPerKm);
  if(driverDistanceKm && driverDistanceKm > freeKm){
    pickupFee = (driverDistanceKm - freeKm) * perKmPickup;
  }

  let cost = tripCost + pickupFee;
  cost = Math.max(cost, t.min);
  cost = Math.round(cost/500)*500;
  return cost;
}

export function hitungRincianTarif(distanceKm, vehicleType='motor', tripType='oneway', driverDistanceKm=0){
  const t = TARIF[vehicleType] || TARIF.motor;
  let tripCost = t.base + (distanceKm * t.perKm);
  ...
  let total = tripCost + pickupFee;
  total = Math.max(total, t.min);
  total = Math.round(total/500)*500;
  return { base: t.base, perKm: t.perKm, distanceKm, tripCost: Math.round(baseTrip), tripType, multiplier: tripType==='roundtrip'?TARIF.ppMultiplier:1, driverDistanceKm, freeKm, perKmPickup, pickupFee: Math.round(pickupFee), total, min: t.min };
}
