// config.js - VERSI FLEKSIBEL + TARIF EDITABLE VIA ADMIN
// Based on config asli kamu
export const ACTIVE_KECAMATAN_CODE_DEFAULT = '3503071';
...
export function getActiveKecamatanCodeLive(){ return getActiveCode(); }
...
// TARIF DEFAULT - akan di-override oleh app_settings jika ada
...
Dengan menekan tombol Pesan Ojol / Go Online, Anda menyatakan telah membaca, memahami, dan menyetujui seluruh poin di atas tanpa paksaan.`,
  ...
  // TARIF EDITABLE
  tarifMotorBase: 3000,
  tarifMotorPerKm: 2500,
  tarifMotorMin: 5000,
  tarifMobilBase: 8000,
  tarifMobilPerKm: 5500,
  tarifMobilMin: 15000,
  ppMultiplier: 1.6
};
...
// Ambil tarif live dari settings (bukan hardcode)
export function getTarifLive(){
  const s = typeof window !== 'undefined' ? loadAppSettings() : APP_SETTINGS_DEFAULT;
  return {
    motor: { base: Number(s.tarifMotorBase)||3000, perKm: Number(s.tarifMotorPerKm)||2500, min: Number(s.tarifMotorMin)||5000, label: 'Motor' },
    mobil: { base: Number(s.tarifMobilBase)||8000, perKm: Number(s.tarifMobilPerKm)||5500, min: Number(s.tarifMobilMin)||15000, label: 'Mobil' },
    ppMultiplier: Number(s.ppMultiplier)||1.6
  };
}

// Untuk kompatibilitas lama, TARIF baca dari live
export const TARIF = getTarifLive();

// SECURITY FIX: JANGAN taruh VAPID_PRIVATE_KEY di frontend!
export const VAPID_PUBLIC_KEY = 'BK-mwXYFVsevqzL8YicpB5I6bwlheTO7sDyGOWn7cHKr_uh-9AS1di-LZpgHtvm7zfqaHxkZQCwM9DLO4KrS7zE';

export function hitungTarif(distanceKm, vehicleType='motor', tripType='oneway'){
  const live = getTarifLive();
  const t = live[vehicleType] || live.motor;
  let cost = t.base + (distanceKm * t.perKm);
  if(tripType==='roundtrip') cost *= live.ppMultiplier;
  cost = Math.max(cost, t.min);
  cost = Math.round(cost/500)*500;
  return cost;
}

export function hitungRincianTarif(distanceKm, vehicleType='motor', tripType='oneway'){
  const live = getTarifLive();
  const t = live[vehicleType] || live.motor;
  let tripCost = t.base + (distanceKm * t.perKm);
  if(tripType==='roundtrip') tripCost *= live.ppMultiplier;
  let total = tripCost;
  total = Math.max(total, t.min);
  total = Math.round(total/500)*500;
  return { base: t.base, perKm: t.perKm, distanceKm, tripCost: Math.round(tripCost), tripType, multiplier: tripType==='roundtrip'?live.ppMultiplier:1, total, min: t.min };
}
