...
export function viewHome(p){
  ...
  // Disclaimer fully from admin settings - editable
  ...
  // Split first paragraph for preview
  const discPreview = discText.split('

')[0].slice(0,140);
  return `<div class="card"><h2>${greet}, ${firstName} 👋</h2><p class="muted">Mau kemana hari ini?</p><p class="muted" style="font-size:11px">Kamu login sebagai <b>${p.role}</b> • Wilayah: ${ACTIVE_KECAMATAN_NAME||'Suruh'}</p><div class="row" style="margin-top:12px">${p.role==='passenger' ? `<a href="#/passenger" class="btn primary" style="flex:1;text-align:center">🧍 Pesan Ojol</a>` : p.role==='driver' ? `<a href="#/driver" class="btn primary" style="flex:1;text-align:center">🏍️ Go Online</a>` : `<a href="#/admin" class="btn primary" style="flex:1;text-align:center">👑 Dashboard Admin</a>`}<a href="#/profile" class="btn secondary" style="flex:1;text-align:center">👤 Profil</a></div></div>
  <div class="card" style="border-left:5px solid #f59e0b;background:rgba(245,158,11,0.09);border-color:#f59e0b">
  <h4 style="margin:0 0 10px;color:#fbbf24">📜 ${discTitle}</h4>
  <div style="font-size:12.5px;line-height:1.7;white-space:pre-wrap" class="muted">${discPreview}${discText.length>140?'...':''}</div>
  <details id="disclaimerDetails" style="margin-top:10px">
    ...
    <div style="font-size:12.5px;line-height:1.8;margin-top:12px;white-space:pre-wrap" class="muted">${discText}</div>
  </details>
</div>
