...
export function viewHome(p){
  ...
  // FIX duplikasi: preview 350 char, selengkapnya hanya sisa (tidak mengulang dari atas)
  const PREVIEW_LEN = 350;
  const isLong = discHtml.length > PREVIEW_LEN;
  const discPreview = isLong ? discHtml.slice(0, PREVIEW_LEN) : discHtml;
  const discRemaining = isLong ? discHtml.slice(PREVIEW_LEN) : '';
  ...
  <div style="font-size:12.5px;line-height:1.7" class="muted" id="discShort">
    ${discPreview}${isLong?'...':''}
  </div>
  ${isLong ? `<details id="disclaimerDetails" style="margin-top:10px">
    <summary style="cursor:pointer;color:#fbbf24;font-weight:700;font-size:12.5px;list-style:none;display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border:1px solid #f59e0b;border-radius:8px;background:rgba(245,158,11,0.15)">📖 Baca selengkapnya <span style="font-size:10px">▼</span></summary>
  ...
  </details>` : `<span style="font-size:11px;color:#8aa0b8;display:block;margin-top:14px;border-top:1px solid #334155;padding-top:8px">Dengan menekan tombol Pesan Ojol / Go Online, Anda menyatakan telah membaca, memahami, dan menyetujui seluruh poin di atas tanpa paksaan.</span>`}
</div>`;
}
