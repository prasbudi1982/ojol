...
export function viewAdminReports(reports=[], bannedList=[]){
  const repHtml = reports.length ? reports.map(r=>`<div class="card" style="padding:10px;margin:8px 0"><b>${r.reason}</b> <span class="muted" style="font-size:10px">${new Date(r.created_at).toLocaleString('id-ID')}</span><p style="font-size:12px">${r.description||''}</p><p class="muted" style="font-size:10px">${r.reporter?.name||r.reporter_id} → ${r.reported?.name||r.reported_email||r.reported_google_id} • ${r.status}</p><div class="row"><button class="btn" style="background:#ef4444;color:white" data-ban-google="${r.reported_google_id||r.reported?.google_id||''}" data-ban-email="${r.reported_email||r.reported?.email||''}" data-ban-report="${r.id}" data-ban-reason="${r.reason}">🚫 Banned Google</button><button class="btn secondary" data-mark-reviewed="${r.id}">Reviewed</button></div></div>`).join('') : '<p class="muted">Belum ada laporan.</p>';
  const bannedHtml = bannedList.length ? bannedList.map(b=>`<div class="card" style="border:1px solid #ef4444;padding:10px;margin:8px 0"><b>${b.email||b.google_id}</b> <span class="muted" style="font-size:10px">${new Date(b.banned_at).toLocaleString('id-ID')}</span><br/><span class="muted" style="font-size:11px">${b.reason}</span><br/><button class="btn secondary" data-unban-google="${b.google_id}">Unban</button></div>`).join('') : '<p class="muted">Tidak ada banned.</p>';
  return `<div class="card"><h3>🛡️ Admin Panel</h3></div><div class="card"><h4>🚩 Laporan (${reports.length})</h4>${repHtml}</div><div class="card"><h4>🚫 Banned (${bannedList.length})</h4>${bannedHtml}</div>`;
}
