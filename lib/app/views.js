...
// Edit aman, tapi disclaimer sekarang konek ke Admin Setting via config.js APP_SETTINGS
...
export function viewHome(p){
  ...
  // Render disclaimer text: support multiline, auto convert newline to <br> and bold for numbered sections
  let discHtml = '';
  if(discText && discText.trim().length > 20){
    // Escape HTML basic then convert
    const esc = discText.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    discHtml = esc.replace(/\n/g,'\n').split('\n').map(line=>{
      const t = line.trim();
      if(!t) return '<br/>';
  ...
  <div style="font-size:12.5px;line-height:1.7" class="muted" id="discShort">
    ${discHtml.slice(0,600)}${discHtml.length>600?'...':''}
  </div>
