...
export function viewHome(p){
  ...
  if(discText && discText.trim().length > 20){
    ...
    discHtml = esc.split('\n').map(line=>{
      const t = line.trim();
      if(!t) return '<br/>';
      if(/^\d+\.\s/.test(t)) return `<b style="color:var(--text);display:block;margin:10px 0 4px">${t}</b>`;
      return t + '<br/>';
    }).join('');
  } else {
    discHtml = `<b style="color:var(--text)">1. Sifat Layanan Non-Komersial</b><br/>
    Platform ini murni berfungsi sebagai media informasi dan direktori komunitas sosial...<br/><br/>
    <b style="color:var(--text);display:block;margin:10px 0 4px">2. Pembatasan Tanggung Jawab Hukum</b>
      Pengelola platform bukanlah perusahaan penyedia jasa transportasi...<br/>
      <b style="color:var(--text);display:block;margin:12px 0 4px">3. Persetujuan Publikasi Data Pribadi</b>
      Seluruh data identitas dan nomor kontak WhatsApp pengemudi...<br/>
      <b style="color:var(--text);display:block;margin:12px 0 4px">4. Pernyataan Setuju Pengguna</b>
      Dengan mengakses, menggunakan, atau menghubungi kontak yang tertera...`;
  }

  const isLong = discHtml.length > 300;
  ...
  <div id="discWrapper" data-collapsed="true" style="position:relative;max-height:110px;overflow:hidden;transition:max-height 0.35s ease">
    <div id="discContent" style="font-size:12.5px;line-height:1.7" class="muted">
      ${discHtml}
    </div>
    <div id="discFade" style="position:absolute;bottom:0;left:0;right:0;height:50px;background:linear-gradient(to bottom, rgba(0,0,0,0), var(--card));pointer-events:none"></div>
  </div>

  ${isLong ? `
  <div style="margin-top:12px;display:flex;justify-content:flex-start">
    <button id="btnToggleDisc" type="button" data-expanded="false" style="cursor:pointer;color:#fbbf24;font-weight:700;font-size:12.5px;display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border:1px solid #f59e0b;border-radius:8px;background:rgba(245,158,11,0.15)">
      <span id="btnToggleDiscText">📖 Baca selengkapnya</span> <span id="btnToggleDiscIcon" style="font-size:10px">▼</span>
    </button>
  </div>` : ``}

  <span style="font-size:11px;color:var(--muted);display:block;margin-top:14px;border-top:1px solid var(--border);padding-top:8px">Dengan menekan tombol Pesan Ojol / Go Online, Anda menyatakan telah membaca, memahami, dan menyetujui seluruh poin di atas tanpa paksaan.</span>
</div>`;
}
