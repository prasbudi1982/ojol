...
export function viewProfile(p){
  ...
  return `<div class="card"><h3>👤 Profil - Pengaturan Role</h3><label>Nama<input id="editName" value="${p.name||''}"></label><p class="muted" style="font-size:12px">${p.email}</p><label>Peran / Role (hanya di sini)<select id="profileRole"><option value="passenger" ${!isDriver?'selected':''}>🧍 Penumpang</option><option value="driver" ${isDriver?'selected':''}>🏍️ Driver</option></select></label><div id="profileDriverFields" style="display:${isDriver?'block':'none'};border:1px solid #334155;padding:10px;border-radius:8px;margin-top:8px"><label>Nopol<input id="profileNopol" value="${p.nopol||''}"></label><label>Tipe SIM<select id="profileSim"><option value="">Pilih SIM</option><option value="A" ${p.tipe_sim==='A'?'selected':''}>A</option><option value="C" ${p.tipe_sim==='C'?'selected':''}>C</option><option value="B1" ${p.tipe_sim==='B1'?'selected':''}>B1</option></select></label><label>Tipe Motor<input id="profileTipe" value="${p.tipe_motor||''}"></label><label>HP WA<input id="profileHp" value="${p.hp||''}"></label></div><div class="row" style="margin-top:12px"><button id="btnSaveProfileRole" class="btn primary">💾 Simpan</button><button id="btnLogout" class="btn secondary">Logout</button></div><p id="profileStatus" class="muted"></p></div>`;
}
