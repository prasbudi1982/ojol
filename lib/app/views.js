...
export function viewProfile(p){
  ...
  return `<div class="card"><h3>👤 Edit Profil</h3>
    <label>Nama Lengkap<input id="editName" value="${p.name||''}" placeholder="Nama sesuai KTP"></label>
    <p class="muted" style="font-size:11px">📧 ${p.email} • ID: ${String(p.id).slice(0,8)}</p>
    <label>Peran / Role (hanya di sini)<select id="profileRole"><option value="passenger" ${!isDriver?'selected':''}>🧍 Penumpang</option><option value="driver" ${isDriver?'selected':''}>🏍️ Driver</option></select></label>
    ...
    <div id="profilePassengerFields" style="display:${!isDriver?'block':'none'};border:1px solid #22c55e;padding:12px;border-radius:12px;margin-top:12px;background:rgba(34,197,94,0.06)">
      <b style="font-size:13px">🧍 Detail Penumpang</b><p class="muted" style="font-size:11px;margin:4px 0 8px">Lengkapi biar driver mudah jemput</p>
