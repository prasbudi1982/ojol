    ...
    try{
      ...
      if(['completed','cancelled'].includes(o.status)){
        ...
        if(o.status==='cancelled'){
          try{ alert('❌ Pesanan dibatalkan'); }catch(e){}
        }
...
function handleFoodOrderRejection(newOrder){
  if(newOrder.driver_id){
    ...
    if(foodLastKnownDriverId && newOrder.status==='searching_driver'){
      // Driver menolak
      (async ()=>{
        try{
          await supabase.from('food_orders').update({ status:'searching_driver' }).eq('id', newOrder.id);
        }catch(e){}
      })();
      return;
    }
