...
import { notifyFood } from './push.js';
import { haversineKm } from './geofence.js';


// ===== SIMPLE AUTO REFRESH CART - HANYA CLEAR CART, TRACKING TETAP JALAN =====
function _simpleClearCartAfterCheckout(){
  const empty={items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null}};
  try{
    localStorage.setItem('ojol_cart_v2_food', JSON.stringify(empty));
    localStorage.setItem('cart_just_cleared', Date.now().toString());
    try{ localStorage.removeItem('ojol_cart_v2'); }catch(e){}
    try{ localStorage.removeItem('food_cart'); }catch(e){}
    try{ localStorage.removeItem('cart'); }catch(e){}
  }catch(e){}
  try{ sessionStorage.removeItem('cart'); sessionStorage.removeItem('food_cart'); }catch(e){}
  try{
    [typeof cartStore!=='undefined'?cartStore:null, window.cartStore?window.cartStore:null].filter(Boolean).forEach(cs=>{
      try{ cs._actions?.clearCart?.(); }catch(e){}
      try{ if(cs.setState) cs.setState({items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null},store:null}); }catch(e){}
      try{ const s=cs.getState?cs.getState():cs; if(s){ s.items=[]; if(s.cart) s.cart.items=[]; } }catch(e){}
    });
  }catch(e){}
  try{ if(window._updateCartBadge) window._updateCartBadge(); }catch(e){}
  try{
    const b=document.getElementById('cartBadge'); if(b){ b.textContent='0'; b.style.display='none'; }
  }catch(e){}
  // AUTO REFRESH BLOK CART ITEM SAJA - JANGAN HIDE YANG LAIN DULU
  try{
    const cont=document.getElementById('cartItemsContainer');
    if(cont){
      cont.innerHTML=`<div style="text-align:center;padding:32px 24px;background:var(--card2);border:1px dashed var(--border);border-radius:20px">
        <div style="font-size:48px">✅</div>
        <div style="font-weight:800;font-size:16px;margin-top:12px">Pesanan Dikirim!</div>
        <div class="muted" style="font-size:12px;margin-top:6px">Keranjang otomatis dikosongkan</div>
        <a href="#/store" class="btn primary" style="margin-top:16px;display:inline-block;padding:10px 20px;border-radius:99px">+ Cari Makanan Lain</a>
      </div>`;
    }
    // subtotal 0
    const s1=document.getElementById('cartSubtotal'); if(s1) s1.textContent='Rp 0';
    const s2=document.getElementById('cartOngkir'); if(s2) s2.textContent='Rp 0';
    const s3=document.getElementById('cartTotal'); if(s3) s3.textContent='Rp 0';
    const live=document.getElementById('ongkirLiveStatus'); if(live) live.textContent='Total ongkir Rp 0 untuk 0.00 km';
    // HIDE DRIVER + JARAK + ALAMAT + SUMMARY - HANYA KALAU SUDAH CHECKOUT, DAN HANYA DI HALAMAN CART
    if(location.hash.includes('/store/cart')){
      const driver=document.getElementById('driverSearchContainer'); if(driver) driver.style.display='none';
      const jarak=document.getElementById('detailJarakContainer'); if(jarak) jarak.style.display='none';
      const alamat=document.getElementById('alamatAntarContainer'); if(alamat) alamat.style.display='none';
      const summary=document.getElementById('cartSummaryContainer'); if(summary) summary.style.display='none';
    }
  }catch(e){ console.warn('simple clear fail',e); }
  ...
  console.log('[trackingFood] simple clear DONE - tracking tetap jalan');
}
if(typeof window!=='undefined'){
  window._simpleClearCartAfterCheckout=_simpleClearCartAfterCheckout;
  window._forceClearCartFromTracking=_simpleClearCartAfterCheckout;
  window._refreshCartUI=_simpleClearCartAfterCheckout;
}
...
async function openRatingForCompletedFoodOrder(order){
  try{
    if(!order) return;
    try{ notifyFood('✅ Pesanan Selesai', 'Makanan sampai! Rating driver & warung yuk', '/#/store'); }catch(e){}
...
let FOOD_IS_LOCKED = false; // UPDATE: kunci popup sampai driver selesai

function lockFoodPopup(){ FOOD_IS_LOCKED = true; try{ document.body.style.overflow='hidden'; }catch(e){} }
function unlockFoodPopup(){ FOOD_IS_LOCKED = false; try{ document.body.style.overflow=''; }catch(e){} }
...
export function showFoodTrackingLockModal(order){
  ...
  div.innerHTML = `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:18px;max-width:360px;width:100%;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,0.5);border:2px solid ${theme.primary}">
      ...
      <div style="padding:16px">
        ...
        <div style="margin-top:12px;display:flex;flex-direction:column;gap:8px">
          ...
          <button id="btnCancelFoodFromLock" style="background:var(--card);border:1px solid var(--border);border:1px solid var(--border);color:#ef4444;padding:10px;border-radius:10px;font-weight:700;font-size:12px">❌ Batalkan Pesanan</button>
        </div>
      </div>
    </div>`;
  document.body.appendChild(div);
...
export function startFoodTracking(orderId){
  if(!orderId) return;
  // CLEAR CART DULU - SIMPLE
  try{ _simpleClearCartAfterCheckout(); }catch(e){}
  // JANGAN HIDE TRACKING MODAL - BIARKAN LOAD
  try{
    localStorage.removeItem('last_food_driver_id');
    localStorage.removeItem('last_food_driver_name');
...
export async function openFoodTrackingDetailModal(orderId){
  ...
  try{
    ...
    div.innerHTML=`
      <div style="background:var(--card);border:1px solid var(--border);width:100%;max-width:520px;max-height:90vh;border-radius:24px 24px 0 0;overflow:auto;box-shadow:0 -4px 24px rgba(0,0,0,0.3)">
        <div style="padding:16px 20px;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--card);border:1px solid var(--border);z-index:2;border-radius:24px 24px 0 0">
          <div style="width:40px;height:4px;background:var(--border);border-radius:99px;margin:0 auto 12px"></div>
          <div style="display:flex;justify-content:space-between;align-items:center"><h3 style="margin:0;font-size:16px">🍔 Tracking Makanan</h3><div style="width:36px;height:36px"></div></div> <!-- UPDATE: tombol X dihilangkan -->
          <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap"><span id="foodDetailStatus" style="background:${theme.primary};color:white;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:800">${statusText.toUpperCase()}</span><span style="font-size:11px;color:var(--muted)">#${order.id.slice(0,6).toUpperCase()}</span><span id="singleSearchTimer" style="background:var(--card2);border:1px solid var(--border);padding:4px 10px;border-radius:20px;font-size:11px">00:00</span><span id="singleTimeout" style="background:var(--card2);border:1px solid #fcd34d;padding:4px 10px;border-radius:20px;font-size:11px">05:00</span></div>
        </div>
        <div style="padding:16px">
          ...
          <div style="margin-top:12px;display:flex;gap:8px"><button id="btnCancelFoodDetail" style="flex:1;background:var(--card);border:1px solid var(--border);border:1px solid var(--border);color:#ef4444;padding:12px;border-radius:12px;font-weight:700">❌ Batalkan</button></div> <!-- UPDATE: tombol Selesai dihilangkan -->
        </div>
      </div>
    `;
    document.body.appendChild(div);
    // ===== UPDATE: KUNCI POPUP & DISABLE BATALKAN JIKA DRIVER SUDAH TERIMA =====
    (function applyLockLogic(){
      const acceptedStatuses = ['accepted','preparing','ready','picked'];
      ...
      if(isAccepted){
        ...
        try{
          ...
          const t2=document.getElementById('singleTimeout'); if(t2){ t2.style.display='none'; t2.textContent='00:00'; }
          const t3=document.getElementById('singleTimeout2'); if(t3){ t3.textContent='00:00'; }
          const tAuto=document.getElementById('foodAutoTimerText'); if(tAuto){ tAuto.textContent='🔒 Terkunci - Driver sudah terima'; tAuto.style.color='#16a34a'; }
        }catch(e){}
        // disable tombol batalkan
        const btnCancel = document.getElementById('btnCancelFoodDetail');
        if(btnCancel){
          btnCancel.disabled = true;
          btnCancel.style.opacity='0.4';
          btnCancel.style.pointerEvents='none';
          btnCancel.textContent='🔒 Terkunci - Driver OTW';
          btnCancel.title='Tidak bisa dibatalkan, order sudah diterima driver';
        }
      } else {
        unlockFoodPopup();
      }
    })();
    // Block klik backdrop & ESC saat terkunci
    div.addEventListener('click', function(e){
      if(e.target===div && FOOD_IS_LOCKED){
        e.preventDefault(); e.stopPropagation();
        const box=div.querySelector('div > div');
        if(box){ try{ box.animate([{transform:'translateX(0)'},{transform:'translateX(-6px)'},{transform:'translateX(6px)'},{transform:'translateX(0)'}],{duration:300}); }catch(e){} }
        return;
      }
    }, true);
    if(!window._foodEscBlocker){
      window._foodEscBlocker = function(e){ if(FOOD_IS_LOCKED && e.key==='Escape'){ e.preventDefault(); e.stopPropagation(); } };
      window.addEventListener('keydown', window._foodEscBlocker, true);
    }
    function renderDetailActions(o){
      const box=document.getElementById('foodDetailActions');
      if(!box) return;
      if(o.status==='searching_driver'){
        box.innerHTML=`<div style="background:var(--card2);border:1px solid #fcd34d;border-radius:10px;padding:10px;font-size:11px;color:#92400e">🔍 Broadcast ke semua driver... Menunggu driver terdekat terima order.<br><small>Timer 5 menit auto batal jika tidak ada respon.</small></div>`;
      } else if(o.status==='driver_assigned'){
        box.innerHTML=`<div style="background:var(--card2);border:1px solid #fde047;border-radius:10px;padding:10px;font-size:11px;color:#854d0e">⏳ Driver dipilih, menunggu konfirmasi driver...<br><small>Driver belum klik Terima di aplikasi driver. Jika menolak, order kembali mencari.</small></div>`;
        ...
        // UPDATE: tidak ada lagi tombol Selesai, semua selesai dari sisi driver
      }
    }
    ...
    // UPDATE: listener btnCompleteFoodDetail dihapus karena tombol Selesai dihilangkan
...
function handleFoodOrderRejection(newOrder){
  ...
  if(newOrder.driver_id){
    // UPDATE: Driver ada - driver terima order -> matikan timer & kunci popup & disable batalkan
    if(foodNoDriverTimer){ clearInterval(foodNoDriverTimer); foodNoDriverTimer=null; }
    const acceptedForLock = ['accepted','preparing','ready','picked'];
    if(acceptedForLock.includes(newOrder.status)){
      lockFoodPopup();
      if(foodAutoTimer){ clearInterval(foodAutoTimer); foodAutoTimer=null; }
      try{
        localStorage.removeItem('food_auto_start_'+newOrder.id);
        const btnCancelDetail=document.getElementById('btnCancelFoodDetail');
        if(btnCancelDetail){ btnCancelDetail.disabled=true; btnCancelDetail.style.opacity='0.4'; btnCancelDetail.style.pointerEvents='none'; btnCancelDetail.textContent='🔒 Terkunci'; }
        const t1=document.getElementById('singleSearchTimer'); if(t1) t1.style.display='none';
    ...
    fetchDriverProfileFood(newOrder.driver_id).then(driver=>{
      ...
      // FIX BERULANG: dedup per order+status untuk food
      try{
        const notifKey = 'food_notif_'+newOrder.id+'_'+newOrder.status;
        if(sessionStorage.getItem(notifKey)) {
          // sudah notif, skip
        } else {
          const statusMapNotif = {
            accepted: ['✅ Driver Terima Food', `Driver ${driver?.name||''} OTW ke warung ${newOrder.store_name||''}`],
            preparing: ['🏪 Warung Masak', `Warung sedang masak - ${newOrder.store_name||''}`],
            ready: ['🍱 Makanan Siap', `Makanan siap diantar - ${newOrder.store_name||''}`],
            picked: ['🚚 Driver OTW', `Driver mengantar makanan ke kamu`]
          };
          const mn = statusMapNotif[newOrder.status];
          if(mn){
            sessionStorage.setItem(notifKey,'1');
            notifyFood(mn[0], mn[1], '/#/store/cart', newOrder.id);
          }
        }
      }catch(e){}
...
function startFoodUnifiedTimer(orderId){
  // UPDATE: cek apakah order sudah diterima, jika ya matikan timer
  try{
    const curStatus = document.getElementById('foodDetailStatus')?.textContent?.toLowerCase() || '';
    if(FOOD_IS_LOCKED || curStatus.includes('terima') || curStatus.includes('masak') || curStatus.includes('siap') || curStatus.includes('otw')){
      if(foodAutoTimer){ clearInterval(foodAutoTimer); foodAutoTimer=null; }
      return;
    }
  }catch(e){}
  ...
  foodAutoTimer = setInterval(async ()=>{
    // UPDATE: jika sudah locked (driver terima), hentikan timer
    if(FOOD_IS_LOCKED){
      ...
      try{
        ...
        const tAuto=document.getElementById('foodAutoTimerText'); if(tAuto) tAuto.textContent='✅ Driver sudah terima - timer dimatikan';
      }catch(e){}
      return;
    }
    ...
    if(elapsed >= FOOD_TIMEOUT){
      ...
      try{
        ...
        if(cur && ['searching_driver','driver_assigned'].includes(cur.status)){
          ...
          try{ notifyFood('⏰ Timeout Food', '5 menit tidak ada respon driver - Food dibatalkan', '/#/store/cart'); }catch(e){}
        }
