...
function _forceClearCartFromTracking(){
  ...
  try{
    // cari container cart - support banyak selector
    const containers=[
    ...
    containers.forEach(cont=>{
      // hanya refresh kalau ini memang container cart (ada cartKey atau _cartQty)
      if(cont.innerHTML.includes('cartKey') || cont.innerHTML.includes('_cartQty') || cont.id==='cartItemsContainer' || location.hash.includes('/store/cart')){
        cont.style.transition='all 0.3s ease';
        cont.style.opacity='0';
        setTimeout(()=>{
          cont.innerHTML=`<div style="text-align:center;padding:32px 24px;background:var(--card2, #f8fafc);border:2px dashed var(--border, #e2e8f0);border-radius:20px;animation:fadeIn 0.4s">
            ...
            <div style="font-size:11px;color:#94a3b8;margin-top:4px">Kamu bisa pantau di tracking</div>
            <a href="#/store" class="btn primary" style="margin-top:16px;display:inline-block;padding:10px 20px;font-size:13px;border-radius:99px;text-decoration:none">+ Cari Makanan Lain</a>
          </div>`;
          cont.style.opacity='1';
        },200);
      }
    });
    // subtotal / total jadi 0
    document.querySelectorAll('[id*=subtotal],[id*=Subtotal],[id*=ongkir],[id*=Ongkir],[id*=total]').forEach(el=>{
      if(el.textContent.includes('Rp')) el.textContent='Rp 0';
    });
    // disable tombol checkout
    document.querySelectorAll('#btnCheckoutBroadcast, [id*=Checkout], [id*=checkout]').forEach(btn=>{
