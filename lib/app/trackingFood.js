...
export async function openFoodTrackingDetailModal(orderId){
  try{
    ...
    div.innerHTML = `
      <div style="background:var(--card);border-radius:18px;max-width:380px;width:100%;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,0.5);border:2px solid ${theme.primary};max-height:90vh;overflow:auto">
        ...
        <div style="padding:14px">
          ...
          <button id="btnFoodDetailComplete" style="width:100%;margin-top:8px;background:#0ea5e9;color:white;border:none;padding:12px;border-radius:10px;font-weight:800;font-size:13px;display:none">✅ Selesai - Pesanan Sudah Diterima</button>
    ...
    function renderDetailActions(o){
      const completeBtn = document.getElementById('btnFoodDetailComplete');
      if(completeBtn){
        if(o.status==='picked'){ completeBtn.style.display='block'; completeBtn.onclick = async ()=>{ if(!confirm('Selesaikan pesanan makanan?')) return; try{ await supabase.from('food_orders').update({status:'completed'}).eq('id', o.id); }catch(e){} div.remove(); clearFoodTracking(); alert('✅ Pesanan selesai!'); }; } else { completeBtn.style.display='none'; }
      }
