...
// ====== FIX ITEM CART BELUM CLEAR - AUTO REFRESH KERANJANG DI TRACKING ======
function _forceClearCartFromTracking(){
  const empty={items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null}};
  console.log('[trackingFood] force clear cart START - ONLY AFTER CHECKOUT');
  // GUARD: jangan hide kalau belum checkout (tidak ada active order dan tidak ada flag cleared)
  try{
    const hasActive = localStorage.getItem('active_food_order_id');
    const hasCleared = localStorage.getItem('cart_just_cleared');
    const raw = localStorage.getItem('ojol_cart_v2_food');
    let hasItems = false;
    try{ const p=JSON.parse(raw||'{}'); hasItems = p.items && p.items.length>0; }catch(e){}
    // Kalau tidak ada active order dan tidak ada flag cleared dan masih ada item, JANGAN hide container
    // Tapi tetap lanjut clear jika dipanggil dari startFoodTracking (ada hasActive)
    if(!hasActive && !hasCleared && hasItems){
      console.log('[trackingFood] skip hide - belum checkout, cart masih ada item');
      // hanya update badge, jangan hide container
  ...
  try{
    // 1. localStorage semua key cart
    const allKeys=[]; for(let i=0;i<localStorage.length;i++){ try{ allKeys.push(localStorage.key(i)); }catch(e){} }
    allKeys.forEach(k=>{
      if(!k) return; const low=k.toLowerCase();
      if(low.includes('cart')||low.includes('food')||low.includes('warung')||low.includes('ojol')){
        if(low.includes('active_food')||low.includes('last_food')||low.includes('food_auto')||low.includes('food_pickup')) return;
        try{ localStorage.setItem(k, JSON.stringify(empty)); }catch(e){}
        try{ if(k!=='active_food_order_id' && k!=='ojol_cart_v2_food' && !k.includes('food_auto')) localStorage.removeItem(k); }catch(e){}
      }
    });
    // paksa set kosong
    ['ojol_cart_v2_food','ojol_cart_v2','food_cart','cart','warung_cart','cart-storage','cartStore','__cart'].forEach(k=>{
      try{ localStorage.setItem(k, JSON.stringify(empty)); }catch(e){}
  ...
  // 2. cartStore memory - hajar semua API
  try{
    const stores=[typeof cartStore!=='undefined'?cartStore:null, window.cartStore?window.cartStore:null, window.warungStore?window.warungStore:null].filter(Boolean);
    stores.forEach(cs=>{
      try{ cs._actions?.clearCart?.(); }catch(e){}
      try{ cs._actions?.clear?.(); }catch(e){}
      try{ cs._actions?.reset?.(); }catch(e){}
      try{ cs._actions?.setItems?.([]); }catch(e){}
      try{ cs._actions?.setCart?.(empty); }catch(e){}
      try{ cs._actions?.setState?.(empty); }catch(e){}
      try{ if(cs.setState) cs.setState({items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null},store:null}); }catch(e){}
      try{ const s=cs.getState?cs.getState():cs; if(s){ s.items=[]; if(s.cart) s.cart.items=[]; s.storeName=''; s.storeId=null; s.pickup={}; s.dest={text:'',lat:null,lng:null}; } }catch(e){}
      // zustand persist
      try{ if(cs.persist?.clearStorage) cs.persist.clearStorage(); }catch(e){}
    });
  }catch(e){ console.warn('cartStore clear fail',e); }

  // 3. DOM - auto refresh blok cart item tanpa reload
  try{
    if(window._updateCartBadge) window._updateCartBadge();
    const badge=document.getElementById('cartBadge');
    if(badge){ badge.textContent='0'; badge.style.display='none'; badge.style.visibility='hidden'; }
    document.querySelectorAll('[id*=cartBadge], .cart-badge, .badge-cart, #cartCount').forEach(el=>{ el.textContent='0'; el.style.display='none'; });
  }catch(e){}
  ...
  try{
    ...
    const containers=[
      ...
      document.querySelector('#cartItemsContainer'),
    ...
    if(containers.length){
      containers.forEach(cont=>{
        ...
        setTimeout(()=>{
          cont.innerHTML=`<div style="text-align:center;padding:32px 24px;background:var(--card2, #f8fafc);border:2px dashed var(--border, #e2e8f0);border-radius:20px;">
            ...
            <a href="#/store" class="btn primary" style="margin-top:16px;display:inline-block;padding:10px 20px;border-radius:99px;text-decoration:none">+ Cari Makanan Lain</a>
          </div>`;
          cont.style.opacity='1';
        },200);
      });
    } else {
      // fallback kalau id tidak ada - cari list yang ada cartKey
      document.querySelectorAll('.list').forEach(cont=>{
        if(cont.innerHTML.includes('cartKey') || cont.innerHTML.includes('_cartQty')){
          cont.innerHTML=`<div style="text-align:center;padding:24px;border:1px dashed var(--border);border-radius:16px">✅ Pesanan Dikirim - Keranjang Kosong</div>`;
        }
      });
    }

    // 3B. CLEAR SUBTOTAL + ONGKIR + TOTAL - PAKAI ID BARU + fallback query
    try{
      const sub=document.getElementById('cartSubtotal'); if(sub) sub.textContent='Rp 0';
      const ong=document.getElementById('cartOngkir'); if(ong) ong.textContent='Rp 0';
      const tot=document.getElementById('cartTotal'); if(tot) tot.textContent='Rp 0';
      const summary=document.getElementById('cartSummaryContainer');
      if(summary){
        // set 0 item text juga
        summary.querySelectorAll('span').forEach(sp=>{
          if(sp.textContent.includes('item)')) sp.textContent='Subtotal (0 item)';
          if(sp.textContent.includes('km)') && sp.textContent.includes('Ongkir')) sp.textContent='Ongkir (0.00 km)';
        });
        // kasih efek
      ...
      // fallback brutal - semua yang ada Rp di dalam card summary jadi 0
      document.querySelectorAll('#cartSummaryContainer [id*=subtotal], #cartSummaryContainer [id*=Subtotal], #cartSummaryContainer [id*=ongkir], #cartSummaryContainer [id*=Ongkir], #cartSummaryContainer [id*=total], #cartSummaryContainer span').forEach(el=>{
        if(el.textContent.trim().startsWith('Rp')) el.textContent='Rp 0';
      });
      // paling brutal - cari div yang ada tulisan Subtotal/Ongkir/Total dan masih ada angka
      document.querySelectorAll('div').forEach(div=>{
        const t=div.textContent||'';
        if((t.includes('Subtotal') && t.includes('Rp') && !t.includes('0 item')) || (t.includes('Ongkir') && t.includes('Rp') && t.includes('km'))){
          // jangan ubah div wrapper besar, hanya span Rp nya
          div.querySelectorAll('span').forEach(s=>{ if(s.textContent.includes('Rp')) s.textContent='Rp 0'; });
        }
      });
      // update juga ongkirLiveStatus
      const live=document.getElementById('ongkirLiveStatus'); if(live) live.textContent='Total ongkir Rp 0 untuk 0.00 km';
    }catch(e){ console.warn('clear totals fail',e); }

    // 3C. disable tombol checkout & hide driver list biar gak bisa broadcast lagi
    document.querySelectorAll('#btnCheckoutBroadcast, #btnCheckoutDirect, [id*=Checkout], [id*=checkout]').forEach(btn=>{
      btn.disabled=true; btn.style.opacity='0.5'; btn.style.pointerEvents='none'; btn.textContent='✅ Sudah Dipesan';
    });
    // HIDE SEMUA DI BAWAH ITEM - BENAR-BENAR HILANG, BUKAN SAMAR
    const toHideIds=['driverSearchContainer','cartSummaryContainer','detailJarakContainer','alamatAntarContainer'];
    ...
    // fallback - cari card yang mengandung text Detail Jarak / Alamat Antar
    document.querySelectorAll('.card, div').forEach(el=>{
      const t=(el.textContent||'').trim();
      if(t.startsWith('📍 Detail Jarak Warung') || t.includes('Detail Jarak Warung') && el.querySelector && el.querySelector('span')){
        const wrap=el.closest('#detailJarakContainer') || el.closest('div[style*=background]') || el;
        if(wrap && wrap.id!=='cartItemsContainer') { wrap.style.display='none'; }
      }
      if(t.startsWith('📍 Alamat Antar') || t.includes('Alamat Antar (Wajib Isi)')){
        const wrap=el.closest('#alamatAntarContainer') || el.parentElement;
        if(wrap && wrap.id!=='cartItemsContainer') { 
          // cari parent card
          let p=wrap;
          for(let i=0;i<4;i++){ if(p && p.textContent.includes('Alamat Antar')) { p.style.display='none'; p=p.parentElement; } }
          wrap.style.display='none'; 
        }
      }
    });
    // juga hide parent besar yang punya Detail Jarak + Alamat di dalamnya
    const bgContainer=document.getElementById('detailJarakContainer');
    if(bgContainer) bgContainer.style.display='none';
    const alamatContainer=document.getElementById('alamatAntarContainer');
    if(alamatContainer) alamatContainer.style.display='none';

    // PASTIKAN container item TETAP MUNCUL DAN CLEAN
    const keep=document.getElementById('cartItemsContainer');
    if(keep){
      keep.style.display='block';
      keep.style.visibility='visible';
      keep.style.opacity='1';
      keep.style.height='auto';
      keep.style.marginTop='10px';
    }
    // pastikan parent card dari keep tidak ke-hide
    if(keep){
      let parent=keep.parentElement;
      if(parent){ parent.style.display='block'; parent.style.opacity='1'; parent.style.visibility='visible'; }
    }
  }catch(e){ console.warn('DOM refresh fail',e); }
  ...
  console.log('[trackingFood] force clear cart DONE');
  return true;
}

if(typeof window!=='undefined'){
  window._forceClearCartFromTracking = _forceClearCartFromTracking;
  window._refreshCartUI = _forceClearCartFromTracking; // alias biar storeViews bisa panggil
  window._clearAllCartKeys = _forceClearCartFromTracking;
}
...
export function startFoodTracking(orderId){
  if(!orderId) return;
  // AUTO CLEAR CART DISINI - PASTI BERSIH SETELAH CHECKOUT
  try{ _forceClearCartFromTracking(); }catch(e){ console.warn('force clear in start fail',e); }
  try{
    // delay 300ms lagi untuk jaga-jaga zustand restore
    setTimeout(()=>{ try{ _forceClearCartFromTracking(); }catch(e){} }, 300);
    setTimeout(()=>{ try{ _forceClearCartFromTracking(); }catch(e){} }, 1000);
  }catch(e){}
  try{
    localStorage.removeItem('last_food_driver_id');
    localStorage.removeItem('last_food_driver_name');
...
export function clearFoodTracking(){
  ...
  localStorage.removeItem('active_food_order_id'); localStorage.removeItem('food_pickup_lat');
  try{ _forceClearCartFromTracking(); }catch(e){} localStorage.removeItem('food_pickup_lng');
  try{
    localStorage.removeItem('last_food_driver_id');
    localStorage.removeItem('last_food_driver_name');
    if(orderId){
