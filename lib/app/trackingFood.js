...
async function openRatingForCompletedFoodOrder(order){
  try{
    ...
    // 1. Auto tutup semua modal tracking dulu (fix bug modal tidak auto tutup)
    try{
    ...
    // 2. Auto kosongkan keranjang (fix bug keranjang tidak kosong)
    try{
    ...
    try{
      ...
      localStorage.setItem('last_rated_food_order', order.id);
      ...
      setTimeout(()=>{
        try{ 
          ...
          // Setelah rating driver, coba buka rating warung jika ada
          setTimeout(async ()=>{
            try{
              if(order.store_id){
                const { openStoreRatingModal } = await import('./rating.js');
                if(openStoreRatingModal) openStoreRatingModal(order.store_id, order.id);
              }
            }catch(e){}
          }, 1500);
        }catch(err){ 
          console.error('openRatingModal food fail', err);
          // Fallback alert jika rating modal gagal load
          alert('✅ Pesanan selesai! Terima kasih sudah order. Jangan lupa rating driver '+ (driver?.name||'') +' di halaman riwayat.');
        }
      }, 800);
    }catch(e){
      console.warn('rating.js tidak ada, fallback', e);
      alert('✅ Pesanan selesai! Order #'+order.id.slice(0,6).toUpperCase()+' selesai. Keranjang sudah dikosongkan.');
    }
