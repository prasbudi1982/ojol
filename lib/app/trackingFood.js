...
function _forceClearCartFromTracking(){
  ...
  try{
    ...
    const driverList=document.getElementById('nearbyDriversList'); if(driverList) driverList.style.opacity='0.5';
    const driverContainer=document.getElementById('driverSearchContainer'); if(driverContainer){ driverContainer.style.transition='all 0.3s'; driverContainer.style.opacity='0'; setTimeout(()=>{ driverContainer.style.display='none'; },300); }
    const summaryContainer=document.getElementById('cartSummaryContainer'); if(summaryContainer){ summaryContainer.style.transition='all 0.3s'; summaryContainer.style.opacity='0'; setTimeout(()=>{ summaryContainer.style.display='none'; },300); }
    const jarakCard=document.querySelectorAll('.card').forEach(card=>{ if(card.textContent.includes('Detail Jarak Warung')){ card.style.display='none'; } });
    const alamatCard=document.querySelectorAll('div').forEach(div=>{ if(div.textContent.includes('Alamat Antar (Wajib Isi)')){ let p=div.closest('[style*=background]'); if(p) p.style.display='none'; } });
