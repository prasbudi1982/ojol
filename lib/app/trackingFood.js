...
async function openRatingForCompletedFoodOrder(order){
  try{
    ...
    // 2. AUTO REFRESH KERANJANG - kosongkan + dispatch + UI
    try{
      ...
      if(window._refreshCartUI) window._refreshCartUI();
      if(window._clearAllCartKeys) window._clearAllCartKeys();
      var badge=document.getElementById('cartBadge'); if(badge){ badge.textContent='0'; badge.style.display='none'; }
...
export function startFoodTracking(orderId){
  ...
  // pastikan cart UI ke-refresh setelah checkout broadcast/direct
  try{ if(window._refreshCartUI) setTimeout(()=>window._refreshCartUI(), 300); }catch(e){}
  try{ document.dispatchEvent(new CustomEvent('food_tracking_started',{detail:{orderId}})); }catch(e){}
...
export function clearFoodTracking(){
  ...
  try{ if(window._refreshCartUI) window._refreshCartUI(); }catch(e){}
  try{ document.dispatchEvent(new CustomEvent('cart:cleared')); }catch(e){} localStorage.removeItem('food_pickup_lng');
  try{
    localStorage.removeItem('last_food_driver_id');
    localStorage.removeItem('last_food_driver_name');
    if(orderId){
