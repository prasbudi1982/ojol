...
function _forceClearCartFromTracking(){
  ...
  try{
    ...
    // HIDE DRIVER SAJA - JANGAN HIDE CONTAINER ITEM
    const driverContainer=document.getElementById('driverSearchContainer');
    if(driverContainer){
      driverContainer.style.transition='all 0.3s';
      driverContainer.style.opacity='0';
      setTimeout(()=>{ driverContainer.style.display='none'; driverContainer.style.visibility='hidden'; },300);
    }
    const driverList=document.getElementById('nearbyDriversList'); 
    if(driverList){ const p=driverList.closest('#driverSearchContainer') || driverList.closest('.card'); if(p) { p.style.display='none'; } }
    
    // Summary tetap hide kalau mau bersih, tapi jangan hide cartItemsContainer
    const summaryContainer=document.getElementById('cartSummaryContainer');
    if(summaryContainer){
      summaryContainer.style.transition='all 0.3s';
      summaryContainer.style.opacity='0';
      setTimeout(()=>{ summaryContainer.style.display='none'; },300);
    }
    const ongkirStatus=document.getElementById('ongkirLiveStatus');
    if(ongkirStatus){ const c=ongkirStatus.closest('div[style*=background]'); if(c && !c.id.includes('cartItems')) c.style.opacity='0.3'; }

    // PASTIKAN container item TETAP MUNCUL
