...
// ===== TAMBAHAN: REDIRECT KE HALAMAN FOOD di storeViews.js (#/store) + TIMER FIX =====
...
function handleFoodOrderRejection(newOrder){
  ...
  if(newOrder.driver_id){
    ...
    if(newOrder.status==='cancelled'){
      ...
      clearFoodTracking();
      hideFoodTrackingUI();
      showCancelPopupFood(newOrder.id, 'dibatalkan sistem/driver');
      return;
    }
...
function showCancelPopupFood(orderId, reason='dibatalkan'){
  ...
  div.innerHTML=`
    <div style="background:white;border-radius:20px;max-width:360px;width:100%;padding:20px;text-align:center">
      ...
      <button id="btnCloseCancelPopup" onclick="location.hash='#/store'" style="margin-top:16px;width:100%;padding:12px;border-radius:12px;border:none;background:${theme.primary};color:white;font-weight:800;cursor:pointer">Kembali ke Menu Makanan</button>
    </div>
  `;
  document.body.appendChild(div);
...
if(typeof window !== 'undefined'){
  ...
  window.isStillSearching = isStillSearching;
}
