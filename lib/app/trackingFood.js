...
// ===== MODIFIKASI: Nonaktifkan timer jika driver terima order =====
function stopFoodTimerOnDriverAccepted(status, orderIdForCleanup){
  if(['accepted','preparing','ready','picked','completed'].includes(status)){
    if(foodAutoTimer){ clearInterval(foodAutoTimer); foodAutoTimer=null; }
    if(foodNoDriverTimer){ clearInterval(foodNoDriverTimer); foodNoDriverTimer=null; }
    try{
      const oid = orderIdForCleanup || foodCurrentOrderId || localStorage.getItem('active_food_order_id');
      if(oid) localStorage.removeItem('food_auto_start_'+oid);
    }catch(e){}
    try{
      const tAuto = document.getElementById('foodAutoTimerText');
      if(tAuto) tAuto.textContent = '✅ Driver diterima - timer berhenti';
      ...
      if(t2){ t2.textContent='Driver OK'; t2.style.background='#dcfce7'; t2.style.borderColor='#86efac'; }
      const t3 = document.getElementById('singleTimeout2');
      if(t3){ t3.textContent='Driver OK'; }
    }catch(e){}
    console.log('[food] Timer dimatikan karena driver diterima, status:', status);
  }
}
...
export function showFoodTrackingLockModal(order){
  ...
  document.body.appendChild(div);
  // MODIFIKASI: Kunci modal lock - cegah tutup via backdrop
  div.addEventListener('click', function(e){
    if(e.target === div){
      e.preventDefault();
      e.stopPropagation();
      console.log('[food] Lock modal terkunci');
    }
  });
...
export async function loadFoodActiveTracking(){
  ...
  try{
    ...
    if(['searching_driver','driver_assigned'].includes(order.status)){
      ...
      stopFoodTimerOnDriverAccepted(order.status, orderId);
    }
  ...
  try{
    ...
    div.innerHTML=`
      <div style="background:var(--card);border:1px solid var(--border);width:100%;max-width:520px;max-height:90vh;border-radius:24px 24px 0 0;overflow:auto;box-shadow:0 -4px 24px rgba(0,0,0,0.3)">
        <div style="padding:16px 20px;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--card);border:1px solid var(--border);z-index:2;border-radius:24px 24px 0 0">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center"><h3 style="margin:0;font-size:16px">🍔 Tracking Makanan</h3></div>
        ...
        <div style="padding:16px">
          ...
          <div style="margin-top:12px;display:flex;gap:8px"><button id="btnCancelFoodDetail" style="flex:1;background:var(--card);border:1px solid var(--border);border:1px solid var(--border);color:#ef4444;padding:12px;border-radius:12px;font-weight:700">❌ Batalkan</button><button id="btnCompleteFoodDetail" style="flex:1;background:#0ea5e9;color:white;border:none;padding:12px;border-radius:12px;font-weight:700;display:none">✅ Selesai</button></div>
        </div>
      </div>
    `;
    document.body.appendChild(div);
    // MODIFIKASI: Kunci modal detail - cegah tutup via backdrop sebelum selesai/dibatalkan
    div.addEventListener('click', function(e){
      if(e.target === div){
        e.preventDefault();
        e.stopPropagation();
        console.log('[food] Modal terkunci - selesaikan/batalkan dulu');
      }
    });
    ...
    foodOrderChannel = supabase.channel('food-detail-'+orderId)
      .on('postgres_changes', { event:'*', schema:'public', table:'food_orders', filter:'id=eq.'+orderId }, payload=>{
        ...
        stopFoodTimerOnDriverAccepted(o.status, o.id);
...
function handleFoodOrderRejection(newOrder){
  ...
  if(newOrder.driver_id){
    ...
    if(foodNoDriverTimer){ clearInterval(foodNoDriverTimer); foodNoDriverTimer=null; }
    // MODIFIKASI: matikan timer saat driver terima
    stopFoodTimerOnDriverAccepted(newOrder.status, newOrder.id);
    ...
    fetchDriverProfileFood(newOrder.driver_id).then(driver=>{
      ...
      // STEP2: Notif buyer sesuai status
      try{
        ...
        if(mn) notifyFood(mn[0], mn[1], '/#/store/cart');
      }catch(e){}
