...
export async function viewStoreList(){
  try{
    ...
    try{
      ...
      ratingsMap=await fetchStoreRatings(ids);
    }catch(e){}
    const cards=stores.map(s=>{
      const r=ratingsMap[s.id];
      const ratingBadge = r 
        ? `<span style="display:inline-flex;align-items:center;gap:3px;background:#fef3c7;border:1px solid #fde68a;color:#92400e;padding:3px 8px;border-radius:99px;font-size:11px;font-weight:800">⭐ ${r.avg} <span style="font-weight:400;color:#b45309">(${r.count})</span></span>`
        : `<span style="display:inline-flex;align-items:center;gap:3px;background:#f1f5f9;border:1px solid #e2e8f0;color:#64748b;padding:3px 8px;border-radius:99px;font-size:10px">⭐ Baru</span>`;
      return `<div class="driver-card" style="flex-direction:column;gap:8px;border:1px solid var(--border);border-radius:16px;padding:12px;background:linear-gradient(135deg,var(--card) 0%,var(--card2) 100%)">
        ...
        <a href="#/store/detail/${s.id}" class="btn primary" style="text-align:center;padding:10px;border-radius:12px;margin-top:4px">Lihat Menu</a>
      </div>`;
    }).join('');
    return `<div class="card" style="border:none;box-shadow:none;background:transparent;padding:0"><h2 style="font-size:18px">🍔 Warung Buka</h2><div class="list" style="margin-top:12px">${cards||'<div class="muted">Belum ada warung</div>'}</div></div>`;
