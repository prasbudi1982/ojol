...
export async function viewStoreCart(){
  ...
  // FIX PICKUP: jika pickup kosong, ambil dari store table agar tidak 0.00km dan tidak error "Isi pickup & tujuan"
  if((!state.pickup||!state.pickup.lat) && state.storeId){
    try{
      const {data:storeFix}=await supabase.from('stores').select('id,name,lat,lng,latitude,longitude,alamat_text').eq('id', state.storeId).single();
      if(storeFix){
        var sLat=storeFix.lat||storeFix.latitude;
        var sLng=storeFix.lng||storeFix.longitude;
        if(sLat&&sLng){
          state.pickup={lat:Number(sLat), lng:Number(sLng), text:storeFix.name, alamat_text:storeFix.alamat_text};
