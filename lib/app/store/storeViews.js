...
export async function viewStoreOrders(storeId){
  try{
    ...
    return `<div class="card"><a href="#/store/my" class="muted">← Warungku</a><div style="margin-bottom:12px">\${getMerchantNotifBtnHtml()}</div>
      <h2 style="margin:0 0 12px 0">📦 Order Masuk - \${allOrders.length} order (max 100)</h2>
      <div id="warungOrderList" class="list"></div>
    ...
    <script>
      (function(){
        ...
        window._renderWarungOrdersPage=function(page){
          ...
          if(!data.length){
            if(listEl) listEl.innerHTML='<div class="muted" style="text-align:center;padding:24px;border:1px dashed var(--border);border-radius:12px">Belum ada order</div>';
          ...
          var html=slice.map(function(o){
            ...
            return '<div class="card" style="margin:0 0 10px 0;border-radius:12px;padding:10px;border:1px solid var(--border)">'
        ...
        setTimeout(function(){ window._renderWarungOrdersPage(1); },0);
      })();
    </script>`;
  }catch(e){ return `<div class="card">Error: \${e.message}</div>`; }
}
