...
export async function viewMyStore(){
  ...
  if(!myStore){
    // Prefill dari profile merchant biar sinkron
    const preName = profile.warung_name || profile.name || '';
    const preAlamat = profile.alamat || profile.alamat_text || '';
    const preDesa = profile.desa || '';
    const preLat = profile.lat || profile.merchant_lat || '';
    const preLng = profile.lng || profile.merchant_lng || '';
    const preWa = profile.hp || '';
    return `
      <div class="card">
        <h2>🏪 Buat Warung ${preName ? '- Sinkron dari Profil ✅' : ''}</h2>
        <p class="muted">${preName ? 'Data sudah terisi otomatis dari profil merchant. Tinggal Simpan kalau mau buka.' : 'Buat warung pertama kamu. Data sesuai tabel stores.'}</p>
        <label>Nama Warung</label><input id="sName" value="${preName}" placeholder="Warung Sego Bu Sri"/>
