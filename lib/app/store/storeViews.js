...
if(typeof window !== 'undefined'){
  ...
  window._checkoutFood = async function(){
    try{
      ...
      // Build payload - FIX customer_id harus users.id bukan auth.user.id
      let total = 0; 
      let subtotal = 0;
      try{ subtotal = state.items.reduce((a,b)=>a+Number(b.harga||0)*Number(b.qty||0),0); total = subtotal; }catch(e){}
      let delivery_fee = 0;
      ...
      // Ambil users.id yang benar (bukan auth id)
      let customerId = null;
      try{
        ...
        if(user){
          ...
          else customerId = user.id; // fallback jika tabel users pakai auth id langsung
        }
      }catch(e){}
      const payload = {
      ...
      if(error){
        console.warn('food_orders insert full payload failed, coba minimal', error.message);
        // Fallback minimal payload kalau kolom baru belum ada di tabel
        if(error.message.includes('column') || error.message.includes('subtotal') || error.message.includes('delivery_fee') || error.message.includes('distance_km') || error.message.includes('pickup_text')){
          const minimal = {
