...
window._checkoutFoodDirect = async function(driverId){
  try{
    ...
    if(window.trackingFood?.openFoodTrackingDetailModal) window.trackingFood.openFoodTrackingDetailModal(od.id); else if(window.openFoodTrackingDetailModal) window.openFoodTrackingDetailModal(od.id); else { localStorage.setItem('active_food_order_id', od.id); location.hash='#/store/tracking/'+od.id; }
