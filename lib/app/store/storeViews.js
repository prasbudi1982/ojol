...
/**
 * _updateCartBadge - Update badge counter di navigasi keranjang
 */
function _updateCartBadge(){
  try{
    ...
    var count = 0;
    for(var i=0;i<(state.items||[]).length;i++){ count += Number(state.items[i].qty||0); }
    var badgeEls = document.querySelectorAll('#cartBadge, #cartCount, .cart-badge, [data-cart-badge]');
    for(var b=0;b<badgeEls.length;b++){ badgeEls[b].textContent = count > 0 ? count : ''; badgeEls[b].style.display = count > 0 ? 'inline-block' : 'none'; }
    // Update nav keranjang text
    var navCart = document.querySelector('a[href="#/store/cart"]');
    if(navCart){
      var existing = navCart.querySelector('.badge');
      if(count>0){
        if(!existing){
          var span = document.createElement('span');
          span.className = 'badge';
          span.style.cssText = 'background:#ef4444;color:white;font-size:10px;padding:2px 6px;border-radius:99px;margin-left:6px';
    ...
    // Also update foodActiveOrderCard if needed
    try{ if(window.foodOrderStore && window.foodOrderStore._actions && window.foodOrderStore._actions.getActive) window.foodOrderStore._actions.getActive(); }catch(e){}
  }catch(e){}
}
...
export async function viewStoreCart(){
  ...
  try{ totals = cartStore._actions.getTotals(); }catch(e){ 
    ...
    totals = { subtotal: subtotal, delivery_fee: delivery_fee, total: subtotal+delivery_fee, distance_km: distance_km.toFixed(2), count: (state.items||[]).length };
  }
  // Fallback hitung ulang jika delivery_fee 0 tapi ada koordinat
  try{
    if((!totals.delivery_fee || totals.delivery_fee==0) && state.pickup && state.pickup.lat && state.dest && state.dest.lat){
      var R=6371;
      var dLat=(state.dest.lat - state.pickup.lat)*3.1415926535/180;
      var dLng=(state.dest.lng - state.pickup.lng)*3.1415926535/180;
      var a=Math.sin(dLat/2)*Math.sin(dLat/2) + Math.cos(state.pickup.lat*3.1415926535/180)*Math.cos(state.dest.lat*3.1415926535/180)*Math.sin(dLng/2)*Math.sin(dLng/2);
      var dist=2*R*Math.asin(Math.sqrt(a));
      var fee = dist<=2 ? 8000 : Math.max(5000, Math.round(dist*3000));
      totals.distance_km = dist.toFixed(2);
      totals.delivery_fee = fee;
      totals.total = Number(totals.subtotal||0) + fee;
    }
  }catch(e){}
...
(function(){
  ...
  // Update badge saat hash change dan saat load
  window.addEventListener('hashchange', function(){ setTimeout(function(){ try{ _updateCartBadge(); }catch(e){} }, 300); });
  setTimeout(function(){ try{ _updateCartBadge(); }catch(e){} }, 500);
