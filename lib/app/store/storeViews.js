...
// lib/app/store/storeViews.js - FINAL CLEAN V7 - fix Invalid token, fix cart 3x & hapus, 1 order = varian+addon gabungan
...
export async function viewStoreList(){
  ...
  return `
    <div class="card">
      <h2>🍔 Warung Buka</h2>
      <p class="muted">Pilih warung, susun varian + addon jadi 1 order.</p>
      <div class="list">
  ...
  return `
          ...
          <div style="padding:12px 14px;background:var(--card2);border-bottom:1px solid var(--border)">
            <div style="display:flex;justify-content:space-between;gap:10px">
              ...
              <div style="text-align:right">
                ...
                <div class="muted" style="font-size:10px">dasar</div>
              </div>
            </div>
          </div>
    ...
    <div id="menuBuilderModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);align-items:flex-end;justify-content:center">
      <div style="background:var(--card);width:100%;max-width:520px;max-height:85vh;overflow:auto;border-radius:20px 20px 0 0">
        <div style="padding:16px 16px 8px;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--card);z-index:2">
        ...
        <div style="padding:16px">
          ...
          <div class="muted" style="font-size:10px;text-align:center;margin-top:8px">1 order = 1 baris di keranjang</div>
        </div>
      </div>
    </div>
  ...
  return `
    <div class="card">
      ...
      <div class="list">
        ${state.items.length ? state.items.map(i=>{
          ...
          <div class="driver-card" style="flex-direction:column;align-items:stretch;gap:8px;border-left:3px solid var(--primary)">
            ...
            <button onclick="window._removeCartItem('${i.cartKey||i.product_id||i.id}')" class="btn secondary" style="width:100%;padding:8px;font-size:11px;background:#fee2e2;color:#dc2626;border-color:#fecaca">🗑️ Hapus Order Ini</button>
          </div>
      ...
      <div style="margin-top:12px;border-top:1px solid var(--border);padding-top:12px">
        ...
        ${state.items.length ? '<button onclick="window._clearCart && window._clearCart()" class="btn secondary" style="margin-top:8px;background:#fee2e2;color:#dc2626">🗑️ Kosongkan Keranjang</button>' : ''}
      </div>
    </div>
  `;
}
...
if(typeof window !== 'undefined'){
  ...
  window._openMenuBuilder = function(pid){
    try{
      ...
      variContainer.querySelectorAll('input[name="builderVariant"]').forEach(function(r){
        r.onchange = function(){ window._builderPickVariant(this.value); };
      });
      ...
      if(!addons.length){
        ...
        addonContainer.innerHTML = addons.map(function(a){
        ...
        addonContainer.querySelectorAll('input[type="checkbox"]').forEach(function(cb){
          cb.onchange = function(){ window._builderToggleAddon(this.value); };
        });
      }
      ...
      if(minus) minus.onclick = function(){ if(_builderState.qty>1){ _builderState.qty--; document.getElementById('builderQty').textContent = _builderState.qty; window._updateBuilderTotal(); } };
      if(plus) plus.onclick = function(){ _builderState.qty++; document.getElementById('builderQty').textContent = _builderState.qty; window._updateBuilderTotal(); };
      if(addBtn){
        addBtn.onclick = function(){
          try{
            ...
            const addonList = _builderState.addons.length ? ' + ' + _builderState.addons.map(function(a){ return a.name; }).join(', ') : '';
  ...
  window._updateBuilderTotal = function(){
    ...
    const addonText = _builderState.addons.length ? ' + ' + _builderState.addons.map(function(a){ return a.name; }).join(', +') : '';
