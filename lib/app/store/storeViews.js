...
// MAP PICKER UNTUK EDIT & CREATE WARUNG
window._openWarungMapPicker=function(mode){
  // mode: 'edit' atau 'create' - FIX z-index agar tidak di belakang modal
  ...
  // Coba pakai global openMapPicker jika ada
  if(window.openMapPicker){
    const cb=(res)=>{
      try{
        const lat=res.lat||res.latitude||null;
        const lng=res.lng||res.longitude||res.long||null;
        if(lat) document.getElementById(latId).value=lat;
        if(lng) document.getElementById(lngId).value=lng;
        if(res.address||res.text){ document.getElementById(addrId).value=res.address||res.text; }
      }catch(e){}
      // setelah pilih, kembalikan z-index warung modal
      setTimeout(()=>{ const wm=document.getElementById(warungModalId); if(wm) wm.style.display='flex'; },300);
    };
    window._warungMapCb=cb;
    // Sembunyikan warung modal sementara agar map picker tidak di belakang
    const warungModalEl=document.getElementById(warungModalId);
    if(warungModalEl){ warungModalEl.style.display='none'; }
    // Inject style untuk paksa map picker di atas
    try{
      ...
      style.textContent=`
        #mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal, #map-picker-modal { z-index: 10050 !important; }
        .map-picker-container, .leaflet-container { z-index: 10051 !important; }
      `;
    }catch(e){}
    try{ window.openMapPicker({ onSelect: cb, callback: cb }); 
      setTimeout(()=>{
        document.querySelectorAll('#mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal, #map-picker-modal').forEach(el=>{
          el.style.zIndex='10050'; el.style.position='fixed';
        });
      },200);
      return; 
    }catch(e){ try{ window.openMapPicker(); 
      setTimeout(()=>{
        document.querySelectorAll('#mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal, #map-picker-modal').forEach(el=>{ el.style.zIndex='10050'; });
        // jika masih tidak terlihat, kembalikan modal warung
        if(document.querySelectorAll('#mapPickerModal, .map-picker-modal').length===0){
          if(warungModalEl) warungModalEl.style.display='flex';
        }
      },500);
      return; 
    }catch(e2){ if(warungModalEl) warungModalEl.style.display='flex'; } }
  }
  // fallback: pakai prompt manual koordinat
