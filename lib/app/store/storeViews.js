...
// MAP PICKER UNTUK EDIT & CREATE WARUNG - FIX: modal tidak hilang setelah insert map
window._openWarungMapPicker=function(mode){
  ...
  // Inject style untuk paksa map picker di atas modal warung (9999 -> map 10050)
  try{
    ...
    style.textContent=`
      #mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal, #map-picker-modal, .mapPicker, #map-picker { z-index: 10060 !important; position: fixed !important; }
      #mapPickerModal *, .map-picker-modal * { z-index: auto; }
      .leaflet-container, .maplibregl-map { z-index: 10061 !important; }
      #${warungModalId} { z-index: 9998 !important; } /* turunkan sedikit agar map di atas */
    `;
  }catch(e){}

  // Callback saat lokasi dipilih
  const cb=(res)=>{
    try{
      const lat=res?.lat||res?.latitude||res?.latlng?.lat||null;
      const lng=res?.lng||res?.longitude||res?.long||res?.lng||res?.latlng?.lng||null;
      const addr=res?.address||res?.text||res?.alamat_text||'';
    ...
    // PASTIKAN modal warung tetap tampil setelah pilih map
    setTimeout(()=>{
      if(warungModalEl){ warungModalEl.style.display='flex'; warungModalEl.style.zIndex='9999'; }
      // hapus style khusus map setelah selesai agar tidak mengganggu
      // tapi biarkan dulu agar next open tetap di atas
    },200);
  };
  window._warungMapCb=cb;

  // Simpan state modal sebelum buka map agar bisa direstore kalau cancel
  window._warungModalRestore = {
    id: warungModalId,
    display: warungModalEl?warungModalEl.style.display:'flex',
    zIndex: warungModalEl?warungModalEl.style.zIndex:'9999'
  };

  if(window.openMapPicker){
    try{
      // Coba panggil dengan callback
      window.openMapPicker({ onSelect: cb, callback: cb, onClose: ()=>{
        // kalau map ditutup tanpa pilih, kembalikan modal
        setTimeout(()=>{
          const wm=document.getElementById(window._warungModalRestore.id);
          if(wm){ wm.style.display='flex'; wm.style.zIndex='9999'; }
        },100);
      }});
      // Paksa z-index map setelah dibuka
      setTimeout(()=>{
      ...
      // Observer: kalau map modal hilang (cancel), kembalikan warung modal
      const observer = new MutationObserver(()=>{
        const mapExists = document.querySelector('#mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal');
        if(!mapExists){
          const wm=document.getElementById(window._warungModalRestore.id);
          if(wm && wm.style.display==='none'){ wm.style.display='flex'; wm.style.zIndex='9999'; }
          observer.disconnect();
        }
      });
      observer.observe(document.body, {childList:true, subtree:false});
      // Fallback timeout 5 detik kalau observer tidak kepanggil
      setTimeout(()=>{
        const mapStillExists = document.querySelector('#mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal');
        if(!mapStillExists){
          const wm=document.getElementById(window._warungModalRestore.id);
          if(wm && wm.style.display==='none'){ wm.style.display='flex'; wm.style.zIndex='9999'; }
        }
      ...
      try{
        ...
        setTimeout(()=>{
          document.querySelectorAll('#mapPickerModal, #mapPicker, .map-picker-modal').forEach(el=>el.style.zIndex='10060');
        },150);
        // restore observer juga
        const observer2 = new MutationObserver(()=>{
          if(!document.querySelector('#mapPickerModal, .map-picker-modal')){
            const wm=document.getElementById(window._warungModalRestore.id);
            if(wm && wm.style.display==='none'){ wm.style.display='flex'; wm.style.zIndex='9999'; }
  ...
  // Fallback jika tidak ada openMapPicker global -> pakai prompt
  ...
  if(inp===null){
    // cancel prompt, pastikan modal tetap ada
    if(warungModalEl){ warungModalEl.style.display='flex'; warungModalEl.style.zIndex='9999'; }
    return;
  }
  if(!inp.trim()){
    window._useWarungMyLocation(mode);
    // setelah lokasi saya, modal tetap tampil (jangan hide)
    if(warungModalEl){ warungModalEl.style.display='flex'; warungModalEl.style.zIndex='9999'; }
    return;
  }
  ...
  // pastikan modal tetap tampil setelah prompt
  if(warungModalEl){ warungModalEl.style.display='flex'; warungModalEl.style.zIndex='9999'; }
};
