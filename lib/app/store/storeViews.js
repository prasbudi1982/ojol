...
export async function viewStoreCart(){
  ...
  // FIX: Jika localStorage sudah kosong (setelah checkout), paksa kosong tanpa baca cartStore memory
  try{
    if(rawLocal){
      var parsedLocal=JSON.parse(rawLocal);
      if(!parsedLocal.items || !parsedLocal.items.length){
        // localStorage kosong -> paksa state kosong, jangan pakai cartStore memory yang masih ada
        state={items:[],storeName:'',dest:{text:'',lat:null,lng:null},pickup:{},storeId:null};
        try{ if(cartStore && cartStore.setState) cartStore.setState({items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null}}); }catch(e){}
        try{ if(cartStore && cartStore._actions && cartStore._actions.clearCart) cartStore._actions.clearCart(); }catch(e){}
        try{ if(cartStore && cartStore._actions && cartStore._actions.setItems) cartStore._actions.setItems([]); }catch(e){}
      }
    }
  }catch(e){}
  if(!state){
  ...
  // Double check: jika rawLocal kosong, tetap paksa kosong
