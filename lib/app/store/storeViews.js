...
// ===== AUTO REFRESH CART ITEM BLOCK - HELPER =====
function _clearAllCartKeys(){
  const empty={items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null}};
  try{
    const keys=[]; for(let i=0;i<localStorage.length;i++){ try{ keys.push(localStorage.key(i)); }catch(e){} }
    keys.forEach(k=>{
    ...
    ['ojol_cart_v2_food','ojol_cart_v2','food_cart','cart','warung_cart','cart-storage','cartStore'].forEach(k=>{
      try{ localStorage.setItem(k, JSON.stringify(empty)); }catch(e){}
    });
  ...
  try{
    [typeof cartStore!=='undefined'?cartStore:null, window.cartStore?window.cartStore:null].filter(Boolean).forEach(cs=>{
      try{ cs._actions?.clearCart?.(); cs._actions?.clear?.(); cs._actions?.setItems?.([]); cs._actions?.setState?.(empty); }catch(e){}
      try{ if(cs.setState) cs.setState({items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null},store:null}); }catch(e){}
      try{ const s=cs.getState?cs.getState():cs; if(s){ s.items=[]; s.storeName=''; s.storeId=null; s.pickup={}; s.dest={text:'',lat:null,lng:null}; } }catch(e){}
    });
  }catch(e){}
}
...
function _refreshCartUI(){
  ...
  // 3. AUTO REFRESH blok cart item tanpa reload
  try{
    const cartContainer = document.getElementById('cartItemsContainer') || document.querySelector('.list');
    if(cartContainer && (location.hash.includes('/store/cart') || cartContainer.innerHTML.includes('cartKey') || cartContainer.innerHTML.includes('_cartQty'))){
      cartContainer.style.opacity='0.5';
      cartContainer.style.transition='opacity 0.2s';
      // re-render kosong cantik
      setTimeout(()=>{
        cartContainer.innerHTML = `<div style="text-align:center;padding:32px 24px;background:var(--card2);border:1px dashed var(--border);border-radius:16px;animation:fadeIn 0.3s">
          <div style="font-size:42px">✅</div>
          <div style="font-weight:800;margin-top:8px">Pesanan Dikirim!</div>
          <div class="muted" style="font-size:12px;margin-top:4px">Keranjang dikosongkan otomatis</div>
          <a href="#/store" class="btn primary" style="margin-top:16px;display:inline-block;padding:10px 18px;font-size:13px;border-radius:99px">+ Cari Makanan Lain</a>
        </div>`;
        cartContainer.style.opacity='1';
        // update subtotal / total yang ada
        document.querySelectorAll('[id*=subtotal],[id*=Subtotal],[id*=total],[id*=Total]').forEach(el=>{
          if(el.textContent.includes('Rp')) el.textContent='Rp 0';
        });
        const checkoutBtns = document.querySelectorAll('#btnCheckoutBroadcast, #btnCheckoutDirect, [id*=Checkout]');
        checkoutBtns.forEach(btn=>{ btn.disabled=true; btn.style.opacity='0.5'; btn.textContent='Keranjang Kosong'; });
      },150);
    }
  }catch(e){ console.warn('refreshCartUI fail', e); }
}

window._refreshCartUI = _refreshCartUI;
window._clearAllCartKeys = _clearAllCartKeys;
...
export async function viewStoreDetail(storeId){
  try{
    ...
    return `<div class="card"><a href="#/store" class="muted">← Kembali</a><h2>${store.name}</h2><div class="muted" style="font-size:11px">${store.alamat_text||''}</div></div><div id="cartItemsContainer" class="list" style="margin-top:12px">${listHtml||'Belum ada menu'}</div>
...
window._checkoutFoodDirect = async function(driverId){
  try{
    ...
    // AUTO REFRESH CART - FIXED
    _clearAllCartKeys();
    _refreshCartUI();
    console.log('[cart] dikosongkan + auto refresh UI setelah checkout');
...
window._cartQty=async function(key,qty){
  try{ cartStore._actions.updateQty(key,qty); }catch(e){
    try{ let raw=localStorage.getItem('ojol_cart_v2_food'); let state=JSON.parse(raw);
      state.items=state.items.map(it=>{ if((it.cartKey||it.product_id||it.id)===key) it.qty=qty; return it; }).filter(it=>it.qty>0);
      localStorage.setItem('ojol_cart_v2_food',JSON.stringify(state));
    }catch(err){}
  }
  ...
  try{
    if(location.hash.includes('/store/cart')){
      const newHtml = await viewStoreCart();
      const app = document.getElementById('app') || document.querySelector('[id*=app]') || document.body;
      // kalau viewStoreCart return full page, cari container
      const container = document.getElementById('cartItemsContainer');
      if(container){
        const temp=document.createElement('div'); temp.innerHTML=newHtml;
        const newItems=temp.querySelector('#cartItemsContainer') || temp.querySelector('.list');
        if(newItems) container.innerHTML=newItems.innerHTML;
      } else {
        location.hash='#/store/cart'; setTimeout(()=>location.reload(),100);
      }
    }
  }catch(e){ location.reload(); }
};
window._removeCartItem=async function(key){
  try{ cartStore._actions.removeItem(key); }catch(e){
    let raw=localStorage.getItem('ojol_cart_v2_food'); let state=JSON.parse(raw);
    state.items=state.items.filter(it=>(it.cartKey||it.product_id||it.id)!==key);
    localStorage.setItem('ojol_cart_v2_food',JSON.stringify(state));
  }
  window._updateCartBadge();
  try{
    if(location.hash.includes('/store/cart')){
      const newHtml = await viewStoreCart();
      const temp=document.createElement('div'); temp.innerHTML=newHtml;
      const newList=temp.querySelector('#cartItemsContainer') || temp.querySelector('.list');
      const curList=document.getElementById('cartItemsContainer') || document.querySelector('.list');
      if(newList && curList) curList.innerHTML=newList.innerHTML;
      document.dispatchEvent(new CustomEvent('cart:updated',{detail:{items:JSON.parse(localStorage.getItem('ojol_cart_v2_food')||'{}').items||[]}}));
    }
  }catch(e){}
};
