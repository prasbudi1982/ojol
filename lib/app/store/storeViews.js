...
window._checkoutFoodDirect = async function(driverId){
  try{
    ...
    // HANYA SAAT CHECKOUT - kosongkan keranjang SUPER AMAN
    (function(){
      var empty={items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null}};
      try{ localStorage.setItem('ojol_cart_v2_food', JSON.stringify(empty)); }catch(e){}
      try{ localStorage.setItem('ojol_cart_v2', JSON.stringify(empty)); }catch(e){}
      try{ localStorage.setItem('food_cart', JSON.stringify(empty)); }catch(e){}
      try{ localStorage.setItem('cart', JSON.stringify(empty)); }catch(e){}
      try{ localStorage.removeItem('warung_cart'); }catch(e){}
      try{ if(typeof cartStore!=='undefined' && cartStore){
        if(cartStore._actions){
          if(cartStore._actions.clearCart) cartStore._actions.clearCart();
          if(cartStore._actions.clear) cartStore._actions.clear();
          if(cartStore._actions.reset) cartStore._actions.reset();
          if(cartStore._actions.setItems) cartStore._actions.setItems([]);
          if(cartStore._actions.setCart) cartStore._actions.setCart(empty);
          if(cartStore._actions.setState) cartStore._actions.setState({items:[]});
        }
        if(cartStore.setState) cartStore.setState({items:[],storeName:'',storeId:null});
        if(cartStore.getState){
          try{ var st=cartStore.getState(); if(st && st.items) st.items=[]; }catch(e){}
        }
      }}catch(e){}
      try{ if(window.cartStore && window.cartStore._actions){
        if(window.cartStore._actions.clearCart) window.cartStore._actions.clearCart();
        if(window.cartStore._actions.clear) window.cartStore._actions.clear();
      }}catch(e){}
      try{ if(window._updateCartBadge){ window._updateCartBadge(); } }catch(e){}
      try{ var _b=document.getElementById('cartBadge'); if(_b){ _b.textContent='0'; _b.style.display='none'; } }catch(e){}
      try{ var _b2=document.querySelectorAll('[id*=cartBadge], [class*=cartBadge], .badge-cart'); _b2.forEach(function(el){ try{ el.textContent='0'; }catch(e){} }); }catch(e){}
      ...
      console.log('[cart] dikosongkan setelah checkout');
    })();
