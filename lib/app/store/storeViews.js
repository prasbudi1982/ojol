...
export async function viewMyStore(){
  try{
    ...
    if(!stores.length) return `<div class="card"><h2>🏪 Warungku</h2><p class="muted">Belum punya warung</p><a onclick="window._createWarung()" href="javascript:void(0)" class="btn primary" style="display:block;text-align:center;padding:12px">+ Buat Warung</a></div>`;
    const cards=stores.map(s=>`<div class="card" style="margin:0 0 12px 0;border:1px solid var(--border);border-radius:16px;padding:12px;background:linear-gradient(135deg,var(--card) 0%,var(--card2) 100%)"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><div style="display:flex;gap:8px;align-items:center"><div style="width:40px;height:40px;border-radius:12px;background:var(--bg);display:flex;align-items:center;justify-content:center;font-size:20px">🏪</div><div><b style="font-size:14px">${s.name}</b><div class="muted" style="font-size:11px">${s.alamat_text||''}</div></div></div><span style="font-size:10px;background:${s.is_open?'#16a34a':'#ef4444'};color:white;padding:4px 10px;border-radius:99px;font-weight:700">${s.is_open?'BUKA':'TUTUP'}</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><button onclick="window._toggleWarung('${s.id}',${!s.is_open})" class="btn secondary" style="font-size:12px;padding:10px;border-radius:10px">${s.is_open?'🔴 Tutup':'🟢 Buka'}</button><a href="#/store/products/${s.id}" class="btn secondary" style="font-size:12px;padding:10px;border-radius:10px;text-align:center">🍱 Kelola Menu + Varian</a><button onclick="window._openWarungOrders('${s.id}')" class="btn primary" style="font-size:12px;padding:12px;border-radius:12px;background:#22c55e;color:#052e16;font-weight:800">📦 Order</button><button onclick="window._editWarung('${s.id}')" class="btn secondary" style="font-size:12px;padding:10px;border-radius:10px">✏️ Edit</button></div></div>`).join('');
    return `<div class="card" style="border:none;background:transparent;box-shadow:none;padding:0"><h2 style="font-size:22px;margin-bottom:12px">🏪 Warungku</h2><div class="list">${cards}</div><a onclick="window._createWarung()" href="javascript:void(0)" class="btn secondary" style="width:100%;margin-top:12px;text-align:center;padding:12px;border-radius:12px">+ Tambah Warung</a>
      <div id="warungEditModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);align-items:center;justify-content:center;padding:16px"><div style="background:var(--card);width:100%;max-width:440px;border-radius:16px;padding:16px;max-height:92vh;overflow:auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="margin:0">✏️ Edit Warung</h3><button onclick="window._closeWarungEditModal()" class="btn secondary" style="width:32px;height:32px;border-radius:50%;padding:0">✕</button></div><div style="display:flex;flex-direction:column;gap:10px"><div><label class="muted" style="font-size:11px">Nama Warung</label><input id="warungEditName" placeholder="Nama Warung" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div><div><label class="muted" style="font-size:11px">Alamat Lengkap</label><textarea id="warungEditAlamat" placeholder="RT/RW, Jalan, Desa" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px;min-height:60px"></textarea></div><div style="background:var(--bg);border:1px dashed var(--border);border-radius:10px;padding:10px;margin-top:4px"><div style="display:flex;justify-content:space-between;align-items:center"><b style="font-size:11px">📍 Lokasi Maps</b><span id="warungEditMapStatus" class="muted" style="font-size:10px"></span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:6px"><div><label class="muted" style="font-size:11px">Latitude</label><input id="warungEditLat" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px" readonly/></div><div><label class="muted" style="font-size:11px">Longitude</label><input id="warungEditLng" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px" readonly/></div></div><button type="button" id="btnPickWarungEditMap" class="btn secondary" style="width:100%;margin-top:8px;padding:10px;border-radius:10px;border:1px solid #16a34a;color:#16a34a">📍 Pilih Lokasi di Peta</button><div id="warungEditMapPreview" class="muted" style="font-size:10px;margin-top:6px"></div></div><label style="font-size:12px;display:flex;gap:6px"><input id="warungEditIsOpen" type="checkbox"/> Warung Buka</label><div style="display:flex;gap:8px;margin-top:8px"><button onclick="window._closeWarungEditModal()" class="btn secondary" style="flex:1;padding:12px;border-radius:10px">Batal</button><button id="btnSaveWarungEdit" class="btn primary" style="flex:1;padding:12px;border-radius:10px;font-weight:800">💾 Simpan</button></div></div></div></div></div>`;
  }catch(e){ return `<div class="card">Error Warungku: ${e.message}</div>`; }
}

</div>
      <div id="warungCreateModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);align-items:center;justify-content:center;padding:16px"><div style="background:var(--card);width:100%;max-width:440px;border-radius:16px;padding:16px;max-height:92vh;overflow:auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="margin:0">➕ Tambah Warung</h3><button onclick="window._closeWarungCreateModal()" class="btn secondary" style="width:32px;height:32px;border-radius:50%;padding:0">✕</button></div><div style="display:flex;flex-direction:column;gap:10px"><div><label class="muted" style="font-size:11px">Nama Warung *</label><input id="warungCreateName" placeholder="Warung Sego Bu Sri" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div><div><label class="muted" style="font-size:11px">Alamat Lengkap *</label><textarea id="warungCreateAlamat" placeholder="RT/RW, Jalan, Desa" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px;min-height:60px"></textarea></div><div style="background:var(--bg);border:1px dashed var(--border);border-radius:10px;padding:10px"><div style="display:flex;justify-content:space-between;align-items:center"><b style="font-size:11px">📍 Lokasi Maps *</b><span id="warungCreateMapStatus" class="muted" style="font-size:10px">Belum set</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:6px"><div><label class="muted" style="font-size:11px">Latitude</label><input id="warungCreateLat" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px" readonly/></div><div><label class="muted" style="font-size:11px">Longitude</label><input id="warungCreateLng" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px" readonly/></div></div><button type="button" id="btnPickWarungCreateMap" class="btn secondary" style="width:100%;margin-top:8px;padding:10px;border-radius:10px;border:1px solid #16a34a;color:#16a34a">📍 Pilih Lokasi di Peta</button><div id="warungCreateMapPreview" class="muted" style="font-size:10px;margin-top:6px">Tap untuk pilih lokasi warung di dalam area Suruh</div></div><div style="display:flex;gap:8px;margin-top:8px"><button onclick="window._closeWarungCreateModal()" class="btn secondary" style="flex:1;padding:12px;border-radius:10px">Batal</button><button id="btnSaveWarungCreate" onclick="window._saveWarungCreate()" class="btn primary" style="flex:1;padding:12px;border-radius:10px;font-weight:800">💾 Simpan</button></div></div></div></div></div>`;

  }catch(e){ return `<div class="card">Error Warungku: ${e.message}</div>`; }
}

export async function viewStoreCreate(){
  try{
    const profile=await getProfile(); if(!profile) return '<div class="card">Login dulu</div>';
    return `<div class="card"><a href="#/store/my" class="muted">← Warungku</a><h2>➕ Tambah Warung Baru</h2><div style="display:flex;flex-direction:column;gap:10px;margin-top:12px"><div><label class="muted" style="font-size:11px">Nama Warung *</label><input id="warungCreateName2" placeholder="Warung Sego Bu Sri" style="width:100%;padding:12px;border-radius:10px;border:1px solid var(--border)"/></div><div><label class="muted" style="font-size:11px">Alamat Lengkap *</label><textarea id="warungCreateAlamat2" placeholder="RT/RW, Jalan, Desa" style="width:100%;padding:10px;border-radius:10px;border:1px solid var(--border);min-height:70px"></textarea></div><div style="background:var(--bg);border:1px dashed var(--border);border-radius:12px;padding:12px"><b style="font-size:12px">📍 Lokasi Maps *</b><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px"><input id="warungCreateLat2" placeholder="Lat" readonly style="padding:10px;border-radius:8px;border:1px solid var(--border)"/><input id="warungCreateLng2" placeholder="Lng" readonly style="padding:10px;border-radius:8px;border:1px solid var(--border)"/></div><button type="button" id="btnPickWarungCreateMap2" class="btn secondary" style="width:100%;margin-top:8px;padding:12px;border:1px solid #16a34a;color:#16a34a">📍 Pilih Lokasi di Peta</button><div id="warungCreateMapPreview2" class="muted" style="font-size:11px;margin-top:6px"></div></div><button id="btnSaveWarungCreate2" onclick="window._saveWarungCreate2()" class="btn primary" style="width:100%;padding:14px;border-radius:12px;font-weight:800;margin-top:8px">💾 Simpan Warung</button></div></div>`;
  }catch(e){ return '<div class="card">Error: '+e.message+'</div>'; }
}
...
window._editWarung=async function(storeId){
  try{
    const {data:store}=await supabase.from('stores').select('*').eq('id',storeId).single();
    if(!store) throw new Error('Warung tidak ditemukan');
    document.getElementById('warungEditName').value=store.name||'';
    document.getElementById('warungEditAlamat').value=store.alamat_text||store.alamat||'';
    document.getElementById('warungEditLat').value=store.lat||'';
    document.getElementById('warungEditLng').value=store.lng||'';
    document.getElementById('warungEditIsOpen').checked=!!store.is_open;
    const statusEl=document.getElementById('warungEditMapStatus'); if(statusEl) statusEl.textContent=store.lat?'✅ Sudah set':'❌ Belum set';
    const previewEl=document.getElementById('warungEditMapPreview'); if(previewEl) previewEl.textContent=store.lat?`Koordinat: ${store.lat}, ${store.lng}`:'Belum ada koordinat';
    document.getElementById('warungEditModal').style.display='flex';
    currentEditWarungId=storeId;
    return;
  }catch(e){ /* fallback to old logic */ }
  try{
  try{
    const {data:store}=await supabase.from('stores').select('*').eq('id',storeId).single();
    if(!store) throw new Error('Warung tidak ditemukan');
...
window._closeWarungEditModal=function(){ document.getElementById('warungEditModal').style.display='none'; currentEditWarungId=null; };
...
window._createWarung=function(){ 
  const modal=document.getElementById('warungCreateModal'); 
  if(modal){ modal.style.display='flex'; } else { location.hash='#/store/create'; }
};
window._closeWarungCreateModal=function(){ const m=document.getElementById('warungCreateModal'); if(m) m.style.display='none'; };
window._saveWarungCreate=async function(){
  try{
    const profile=await getProfile();
    const name=document.getElementById('warungCreateName')?.value.trim() || document.getElementById('warungCreateName2')?.value.trim();
    const alamat=document.getElementById('warungCreateAlamat')?.value.trim() || document.getElementById('warungCreateAlamat2')?.value.trim();
    const lat=parseFloat(document.getElementById('warungCreateLat')?.value || document.getElementById('warungCreateLat2')?.value);
    const lng=parseFloat(document.getElementById('warungCreateLng')?.value || document.getElementById('warungCreateLng2')?.value);
    if(!name) return alert('Nama warung wajib');
    if(!alamat) return alert('Alamat wajib');
    if(!lat || !lng) return alert('Pilih lokasi maps dulu - tap Pilih Lokasi di Peta');
    const btn=document.getElementById('btnSaveWarungCreate') || document.getElementById('btnSaveWarungCreate2');
    if(btn){ btn.disabled=true; btn.textContent='Menyimpan...'; }
    const payload={ owner_id: profile.id, user_id: profile.id, name, alamat_text: alamat, alamat: alamat, lat, lng, is_open: true };
    const {data,error}=await supabase.from('stores').insert(payload).select().single();
    if(error) throw error;
    alert('✅ Warung berhasil dibuat');
    window._closeWarungCreateModal();
    location.hash='#/store/my';
    setTimeout(()=>location.reload(), 500);
  }catch(e){ alert('Gagal buat warung: '+e.message); const btn=document.getElementById('btnSaveWarungCreate') || document.getElementById('btnSaveWarungCreate2'); if(btn){ btn.disabled=false; btn.textContent='💾 Simpan'; } }
};
window._saveWarungCreate2=window._saveWarungCreate;
window._setWarungMap=function(lat,lng){
  const latE=document.getElementById('warungEditLat'); if(latE) latE.value=parseFloat(lat).toFixed(7);
  const lngE=document.getElementById('warungEditLng'); if(lngE) lngE.value=parseFloat(lng).toFixed(7);
  const latC=document.getElementById('warungCreateLat'); if(latC) latC.value=parseFloat(lat).toFixed(7);
  const lngC=document.getElementById('warungCreateLng'); if(lngC) lngC.value=parseFloat(lng).toFixed(7);
  const latC2=document.getElementById('warungCreateLat2'); if(latC2) latC2.value=parseFloat(lat).toFixed(7);
  const lngC2=document.getElementById('warungCreateLng2'); if(lngC2) lngC2.value=parseFloat(lng).toFixed(7);
  const sE=document.getElementById('warungEditMapStatus'); if(sE){ sE.textContent='✅ Sudah set'; sE.style.color='#16a34a'; }
  const sC=document.getElementById('warungCreateMapStatus'); if(sC){ sC.textContent='✅ Sudah set'; sC.style.color='#16a34a'; }
  const pE=document.getElementById('warungEditMapPreview'); if(pE) pE.textContent=`Koordinat: ${parseFloat(lat).toFixed(7)}, ${parseFloat(lng).toFixed(7)}`;
  const pC=document.getElementById('warungCreateMapPreview'); if(pC) pC.textContent=`Koordinat: ${parseFloat(lat).toFixed(7)}, ${parseFloat(lng).toFixed(7)}`;
  const pC2=document.getElementById('warungCreateMapPreview2'); if(pC2) pC2.textContent=`Koordinat: ${parseFloat(lat).toFixed(7)}, ${parseFloat(lng).toFixed(7)}`;
};
