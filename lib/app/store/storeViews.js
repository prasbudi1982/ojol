...
// === TAMBAHAN: Modul BACA Ulasan Warung (hanya baca, tidak mengulas) ===
(function(){
  if(window.openStoreReviews) return;
  function ensureStyle(){
    if(document.getElementById('read-review-style')) return;
    const s=document.createElement('style');
    s.id='read-review-style';
    s.textContent=`
    .review-read-overlay{position:fixed;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;padding:16px}
    .review-read-card{width:100%;max-width:420px;max-height:80vh;background:var(--card);border:1px solid var(--border);border-radius:20px;box-shadow:0 20px 60px rgba(0,0,0,.4);display:flex;flex-direction:column;overflow:hidden}
    .review-read-head{padding:14px 16px;display:flex;gap:10px;align-items:center;border-bottom:1px solid var(--border);flex-shrink:0}
    .review-item{padding:12px 14px;border-bottom:1px solid var(--border);display:flex;gap:10px}
  ...
  window.openStoreReviews = async function(storeId, storeName){
    ...
    ov.innerHTML=`<div class="review-read-card">
      <div class="review-read-head">
        <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--primary),var(--primary-2));display:flex;align-items:center;justify-content:center;font-size:18px">⭐</div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:900;font-size:13px;color:var(--text)">Ulasan ${safe}</div>
          <div style="font-size:11px;color:var(--muted)" id="revSummary">Memuat...</div>
        </div>
      ...
      <div class="rev-list" id="revList" style="padding:8px">
        <div style="text-align:center;padding:24px;color:var(--muted);font-size:12px">⏳ Memuat ulasan...</div>
      </div>
    </div>`;
    ...
    let reviews=[];
    try{
      const {data, error} = await supabase.from('store_ratings').select('rating, comment, created_at, user_id').eq('store_id', storeId).order('created_at', {ascending:false}).limit(50);
      if(!error && data) reviews=data;
    }catch(e){ console.warn('fetch store_ratings fail', e); }
    // fallback local
    try{
      const local=JSON.parse(localStorage.getItem('local_store_ratings')||'[]');
      const localFiltered=local.filter(r=>r.store_id===storeId);
      if(localFiltered.length){
        reviews=[...reviews, ...localFiltered.map(r=>({rating:r.rating, comment:r.comment||'', created_at:r.created_at, local:true}))];
      }
    }catch(e){}
    ...
    if(!reviews.length){
      listEl.innerHTML=`<div style="text-align:center;padding:32px 16px;color:var(--muted)">
        ...
        <div style="font-weight:700;font-size:13px">Belum ada ulasan</div>
        <div style="font-size:11px;margin-top:4px">Warung ini belum diulas pembeli</div>
      </div>`;
      if(sumEl) sumEl.textContent='Belum ada ulasan';
      return;
    }
    // hitung avg
    const avg=(reviews.reduce((a,b)=>a+Number(b.rating||0),0)/reviews.length).toFixed(1);
    if(sumEl) sumEl.textContent=`${avg} ★ dari ${reviews.length} ulasan`;
    ...
    listEl.innerHTML=reviews.map(r=>{
      ...
      const comment=r.comment ? r.comment.replace(/</g,'&lt;') : '<i style="color:var(--muted)">Tanpa komentar</i>';
      return `<div class="review-item">
        <div style="width:32px;height:32px;border-radius:99px;background:var(--card2);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0">👤</div>
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
            <span style="color:#fbbf24;font-size:12px;font-weight:800">${stars}</span>
            <span style="font-size:11px;font-weight:700;color:var(--text)">${Number(r.rating).toFixed(1)}</span>
            <span style="font-size:10px;color:var(--muted)">${date}${r.local?' • lokal':''}</span>
          </div>
          <div style="font-size:12px;color:var(--text);margin-top:4px;line-height:1.4;word-break:break-word">${comment}</div>
        </div>
      </div>`;
    }).join('');
  };
      ...
      return `<div class="store-card-adaptive" style="position:relative;overflow:hidden;background:var(--card);border:1px solid var(--border);border-radius:20px;padding:0;box-shadow:0 4px 20px var(--shadow);transition:all .25s">
        ...
        <div style="padding:14px 14px 12px 14px">
          ...
          <div style="display:flex;gap:8px;margin-top:12px">
            ...
            <button onclick="event.preventDefault(); window.openStoreReviews('${s.id}', '${s.name.replace(/'/g, "\'")}')" style="padding:12px 14px;border-radius:14px;font-weight:800;font-size:12px;background:var(--card2);border:1px solid var(--border);color:var(--text);cursor:pointer;white-space:nowrap">⭐ Ulasan</button>
          </div>
        </div>
      </div>`;
    }).join('');
