...
export async function viewStoreCart(){
  ...
  return `<div class="card"><h2>🛒 Keranjang - ${state.storeName||''}</h2><div class="list" style="margin-top:10px">${itemsHtml}</div><hr/><div style="background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:10px"><input id="destText" value="${state.dest.text||''}" placeholder="Alamat lengkap" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border)"/><div style="display:flex;gap:6px;margin-top:8px"><button id="btnPickDest" class="btn secondary" style="flex:1;font-size:11px">📍 Peta</button><button id="btnUseMyLocation" class="btn secondary" style="flex:1;font-size:11px">📌 Saya</button></div><div class="muted" style="font-size:10px;margin-top:6px">Jarak ${distance_km.toFixed(2)}km • <span id="ongkirLiveStatus">Ongkir Rp ${delivery_fee.toLocaleString()}</span></div></div><div style="margin-top:12px"><div style="display:flex;justify-content:space-between"><span>Subtotal</span><span>Rp ${totals.subtotal.toLocaleString()}</span></div><div style="display:flex;justify-content:space-between;font-weight:800"><span>Total</span><span>Rp ${total.toLocaleString()}</span></div></div></div><div class="card" style="margin-top:12px;border:2px solid #16a34a"><div style="display:flex;justify-content:space-between"><h3 style="margin:0;font-size:14px">🏍️ Driver Terdekat dari Warung</h3><button id="btnRefreshDrivers" class="btn secondary" style="width:auto;font-size:11px">🔄 Refresh</button></div><div id="foodDriverStatus" class="muted" style="font-size:11px;margin-top:6px">Memuat...</div><div id="nearbyDriversList" style="margin-top:10px"></div><div style="margin-top:12px"><button id="btnCheckoutBroadcast" class="btn secondary" style="width:100%">📢 Broadcast ke Semua Driver</button></div></div>`;
}
...
(function(){
  ...
  document.addEventListener('click',e=>{
    ...
    if(e.target.closest&&e.target.closest('#btnPickDest')){ e.preventDefault(); window._pickDestMap&&window._pickDestMap(); }
    ...
    if(e.target.closest&&e.target.closest('#builderAddBtn')){
      try{
        ...
        try{ cartStore._actions.addItem(cartItem); }catch(e){ let raw=localStorage.getItem('ojol_cart_v2_food'); let state=raw?JSON.parse(raw):{items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null}}; state.items.push(cartItem); localStorage.setItem('ojol_cart_v2_food',JSON.stringify(state)); }
        window._closeMenuBuilder(); alert('Ditambahkan ke keranjang'); location.hash='#/store/cart';
      }catch(e){ alert(e.message); }
    }
  });
