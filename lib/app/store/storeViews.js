  ...
  try{
    ...
    if(!state.pickup||!state.pickup.lat){
      try{
        ...
        if(cs.pickup && cs.pickup.lat) state.pickup=cs.pickup;
        else if(cs.store && cs.store.lat) state.pickup={lat:cs.store.lat,lng:cs.store.lng||cs.store.long,text:cs.store.name||''};
      }catch(e){}
    }
    if(!state.pickup||!state.pickup.lat){
      if(statusEl) statusEl.textContent='⚠️ Lokasi warung belum ada';
      if(listEl) listEl.innerHTML='<div class=muted style="padding:12px;text-align:center">Pickup belum ada. Buka warung yang punya koordinat.</div>';
      return;
    }
    if(statusEl) statusEl.textContent='Mencari driver di sekitar '+state.pickup.text||'...';
    if(listEl) listEl.innerHTML='<div class=muted style="padding:12px;text-align:center">🔍 Mencari driver terdekat...</div>';

    var {data:locs, error:locErr}=await supabase.from('driver_locations').select('*').limit(100);
    if(locErr){
      if(statusEl) statusEl.textContent='Error: '+locErr.message;
      if(listEl) listEl.innerHTML='<div style="padding:12px;background:#fee2e2;border-radius:8px;font-size:11px">Gagal baca driver_locations: '+locErr.message+'</div>';
      return;
    }
    if(!locs||!locs.length){
      if(statusEl) statusEl.textContent='Tidak ada driver online';
      if(listEl) listEl.innerHTML='<div style="background:var(--card2);border:1px dashed var(--border);border-radius:12px;padding:16px;text-align:center;color:var(--muted);font-size:12px">Tidak ada driver online<br><small>Driver belum update lokasi</small><br><button onclick="window._loadNearbyDriversForCart()" class="btn secondary" style="margin-top:8px;width:auto">🔄 Coba Lagi</button></div>';
      return;
    }
    ...
    // Mapping driver + hitung jarak seperti di order.js
    var drivers=locs.map(loc=>{
      ...
      if(u.role && (u.role||'').toLowerCase()!=='driver'){ /* tetap tampilkan tapi biarkan */ }
      ...
      if(loc.lat&&loc.lng){ dLat=Number(loc.lat); dLng=Number(loc.lng); }
        ...
        try{
          var m=String(loc.lokasi).match(/POINT\s*\(\s*([^ ]+)\s+([^ ]+)\s*\)/);
      ...
      return {...u, driver_id:u.id, actual_driver_id:driverIdVal, dLat, dLng, distance_km:dist, distance:dist, rawLoc:loc};
    }).filter(Boolean);

    // Filter hanya driver role seperti order.js
    var driversOnly=drivers.filter(d=>{ var role=(d.role||'').toLowerCase(); return role==='driver' || !role; });
    if(!driversOnly.length) driversOnly=drivers;

    // Sort terdekat dan ambil 10
    driversOnly.sort((a,b)=>a.distance_km-b.distance_km);
    var filtered=driversOnly.slice(0,10);

    // Fetch rating seperti order.js
    ...
    // Render card ala order.js - ada Pesan & WA
    var renderCard=(d)=>{
      var vehIcon=(d.jenis_kendaraan||'motor')==='mobil'?'🚗':'🏍️';
      var vehLabel=d.jenis_kendaraan||'motor';
      var wa=d.hp ? `https://wa.me/${String(d.hp).replace(/[^0-9]/g,'').replace(/^0/,'62')}?text=Halo%20${encodeURIComponent(d.name)}%20antar%20makanan%20dari%20${encodeURIComponent(state.storeName||state.pickup.text||'warung')}%20ke%20${encodeURIComponent(state.dest.text||'')}` : null;
      ...
      <div style="background:var(--card);border:1px solid var(--border);border-radius:16px;padding:14px;margin-bottom:10px;display:flex;gap:12px">
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            <span style="font-weight:800;font-size:14px;color:var(--text)">${d.name}</span>
          ...
          <div style="margin-top:6px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            ...
            <span style="font-size:11px;color:var(--muted)">${d.nopol||''} • ${dist} km • ${d.tipe_motor||'Beat'} • ${d.dLat.toFixed(4)},${d.dLng.toFixed(4)}</span>
          </div>
          <div class="muted" style="font-size:10px;margin-top:4px">Order: ${state.storeName||'Makanan'} → ${state.dest.text||'Tujuan'} • Total Rp ${(state.items||[]).reduce((a,b)=>a+Number(b.harga||0)*Number(b.qty||0),0).toLocaleString()}</div>
        </div>
    ...
    if(!filtered.length){
      if(statusEl) statusEl.textContent='Tidak ada driver online';
      if(listEl) listEl.innerHTML='<div style="background:var(--card2);border:1px dashed var(--border);border-radius:12px;padding:16px;text-align:center;color:var(--muted);font-size:12px">Tidak ada driver online<br><small>Menampilkan semua: '+driversOnly.length+'</small></div>';
      return;
    }

    if(statusEl) statusEl.textContent='Ditemukan '+filtered.length+' driver terdekat (dari '+locs.length+')';
    if(listEl) listEl.innerHTML=filtered.map(renderCard).join('');

  }catch(err){
    console.error('[driver-search] error', err);
    if(statusEl) statusEl.textContent='Error: '+err.message;
    if(listEl) listEl.innerHTML='<div style="padding:12px;background:#fee2e2;border-radius:8px">Error: '+err.message+'</div>';
  }
};
