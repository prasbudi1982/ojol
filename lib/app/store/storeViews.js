...
// ============================================================
// CART CHECKOUT CLEANUP - aman untuk cartStore + localStorage
// Dipanggil hanya SETELAH order berhasil dibuat.
// Tujuan: mencegah persistence/middleware cartStore menulis kembali
// item lama sesudah checkout.
// ============================================================
function clearFoodCartAfterCheckout(orderId){
  const empty={items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null},store:null};
  try{ localStorage.setItem('cart_just_cleared',String(Date.now())); }catch(e){}
  try{ if(orderId) localStorage.setItem('active_food_order_id',orderId); }catch(e){}
  const clearOnce=()=>{
    try{
      const stores=[(typeof cartStore!=='undefined'?cartStore:null),window.cartStore||null].filter(Boolean);
      stores.forEach(cs=>{
        try{ if(cs._actions){
          if(typeof cs._actions.clearCart==='function') cs._actions.clearCart();
          if(typeof cs._actions.clear==='function') cs._actions.clear();
          if(typeof cs._actions.reset==='function') cs._actions.reset();
          if(typeof cs._actions.setItems==='function') cs._actions.setItems([]);
          if(typeof cs._actions.setCart==='function') cs._actions.setCart(empty);
          if(typeof cs._actions.setStore==='function') cs._actions.setStore(null);
        }}catch(e){}
        try{ if(typeof cs.setState==='function') cs.setState({items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null},store:null}); }catch(e){}
        try{ const st=typeof cs.getState==='function'?cs.getState():null; if(st){ if(Array.isArray(st.items)) st.items.length=0; if(st.cart&&Array.isArray(st.cart.items)) st.cart.items.length=0; } }catch(e){}
      });
    }catch(e){ console.warn('[cart] clear memory failed',e); }
    try{
      const raw=JSON.stringify(empty);
      localStorage.setItem('ojol_cart_v2_food',raw);
      localStorage.setItem('ojol_cart_v2',raw);
      localStorage.setItem('food_cart',raw);
      localStorage.setItem('cart',raw);
      localStorage.setItem('warung_cart',raw);
      localStorage.setItem('cart-storage',raw);
      localStorage.setItem('cartStore',raw);
    }catch(e){ console.warn('[cart] clear localStorage failed',e); }
    ...
    try{ document.querySelectorAll('#cartBadge,#cartCount,#navCartBadge,.cart-badge,.food-cart-badge,[data-cart-count],#bottomNavCartBadge,span.cart-count,.badge-cart,#foodCartBadge').forEach(el=>{ el.textContent=''; el.style.display='none'; }); }catch(e){}
  };
  clearOnce();
  [0,50,150,350].forEach(delay=>setTimeout(()=>{
    try{
      const raw=localStorage.getItem('ojol_cart_v2_food');
      const parsed=raw?JSON.parse(raw):null;
      if(!parsed || !Array.isArray(parsed.items) || parsed.items.length!==0){ clearOnce(); }
      else { try{ const st=cartStore.getState(); if(st&&Array.isArray(st.items)&&st.items.length) clearOnce(); }catch(e){} }
    }catch(e){ clearOnce(); }
  },delay));
...
export async function viewStoreCart(){
  ...
  // CRITICAL FIX: setelah checkout, jangan biarkan state memory lama menghidupkan kembali cart.
  try{
    var justCleared=localStorage.getItem('cart_just_cleared');
    if(justCleared){
      var clearAge=Date.now()-parseInt(justCleared,10);
      if(!isNaN(clearAge) && clearAge<60000){
        ...
        try{ localStorage.setItem('ojol_cart_v2_food',JSON.stringify({items:[],storeName:'',storeId:null,pickup:{},dest:{text:'',lat:null,lng:null},store:null})); }catch(e){}
  ...
  // Jika localStorage kosong setelah checkout, LANGSUNG return keranjang kosong tanpa baca cartStore memory
  try{
    if(rawLocal){
...
window._checkoutFoodDirect = async function(driverId){
  try{
    ...
    // HANYA SETELAH ORDER BERHASIL: kosongkan cart secara atomik.
    clearFoodCartAfterCheckout(od.id);
