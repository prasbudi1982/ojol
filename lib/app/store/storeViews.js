...
// === TAMBAHAN: Modul Ulasan Warung (inline, tanpa ubah fungsi lain) ===
(function(){
  if(window.openStoreReview) return;
  function ensureStyle(){
    if(document.getElementById('review-warung-style')) return;
    const s=document.createElement('style');
    s.id='review-warung-style';
    s.textContent=`.review-overlay{position:fixed;inset:0;background:rgba(0,0,0,.5);backdrop-filter:blur(6px);z-index:99999;display:flex;align-items:center;justify-content:center;padding:16px}.review-card{width:100%;max-width:380px;background:var(--card);border:1px solid var(--border);border-radius:20px;box-shadow:0 16px 40px rgba(0,0,0,.3)}.rev-stars{display:flex;gap:6px;justify-content:center;padding:14px 0 4px}.rev-star{font-size:34px;cursor:pointer;opacity:.3}.rev-star.active{opacity:1;color:#fbbf24}`;
    document.head.appendChild(s);
  }
  window._revRating=0;
  window._setRevStar=function(v){
    window._revRating=v;
    document.querySelectorAll('.rev-star').forEach(el=> el.classList.toggle('active', Number(el.dataset.v)<=v));
    const l=document.getElementById('revLabel'); if(l) l.textContent=['','Buruk','Cukup','Baik','Bagus','Mantap'][v];
  };
  window.openStoreReview=function(storeId, storeName){
    ensureStyle(); window._revRating=0;
    const safe=(storeName||'Warung').replace(/</g,'');
    const ov=document.createElement('div'); ov.className='review-overlay';
    ov.innerHTML=`<div class="review-card">
      <div style="padding:14px 16px;display:flex;gap:10px;align-items:center;border-bottom:1px solid var(--border)">
        <div style="width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,var(--primary),var(--primary-2));display:flex;align-items:center;justify-content:center">🏪</div>
        <div style="flex:1"><div style="font-weight:900;font-size:13px">Ulas ${safe}</div><div style="font-size:11px;color:var(--muted)">Beri rating warung ini</div></div>
        <button onclick="this.closest('.review-overlay').remove()" style="width:28px;height:28px;border-radius:99px;border:1px solid var(--border);background:var(--card2)">✕</button>
      </div>
      <div class="rev-stars">${[1,2,3,4,5].map(i=>`<span class="rev-star" data-v="${i}" onclick="window._setRevStar(${i})">★</span>`).join('')}</div>
      <div id="revLabel" style="text-align:center;font-size:12px;font-weight:700;color:var(--muted);min-height:14px">Tap bintang</div>
      <div style="padding:10px 14px"><textarea id="revComment" placeholder="Rasanya gimana?" style="width:100%;background:var(--card2);border:1px solid var(--border);border-radius:10px;padding:10px;color:var(--text);min-height:70px;font-size:12px"></textarea></div>
      <div style="display:flex;gap:8px;padding:0 14px 14px">
        <button onclick="this.closest('.review-overlay').remove()" style="flex:1;padding:10px;border-radius:10px;border:1px solid var(--border);background:var(--card2);font-weight:700;font-size:12px">Nanti</button>
        <button onclick="window._submitStoreReview('${storeId}')" style="flex:1;padding:10px;border-radius:10px;border:none;background:linear-gradient(135deg,var(--primary),var(--primary-2));font-weight:800;font-size:12px;color:#052e16">Kirim</button>
      </div>
    </div>`;
    document.body.appendChild(ov);
    ov.addEventListener('click',e=>{ if(e.target===ov) ov.remove(); });
    // fix id after template
    ov.querySelector('[onclick^="window._submitStoreReview"]').setAttribute('onclick', `window._submitStoreReview('${storeId}')`);
  };
  window._submitStoreReview=async function(storeId){
    if(!window._revRating) return alert('Pilih bintang dulu');
    const comment=document.getElementById('revComment')?.value||'';
    try{
      try{ const {error}=await supabase.from('store_ratings').insert({store_id:storeId, rating:window._revRating, comment}); if(error) throw error; }
      catch(e){ const arr=JSON.parse(localStorage.getItem('local_store_ratings')||'[]'); arr.push({store_id:storeId, rating:window._revRating, comment, created_at:new Date().toISOString()}); localStorage.setItem('local_store_ratings', JSON.stringify(arr)); }
      document.querySelectorAll('.review-overlay').forEach(o=>o.remove());
      const t=document.createElement('div'); t.textContent='✓ Makasih ulasannya!'; t.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:var(--text);color:var(--card);padding:8px 14px;border-radius:99px;font-size:11px;font-weight:700;z-index:100000'; document.body.appendChild(t); setTimeout(()=>t.remove(),2000);
      ...
      return `<div class="store-card-adaptive" style="position:relative;overflow:hidden;background:var(--card);border:1px solid var(--border);border-radius:20px;padding:0;box-shadow:0 4px 20px var(--shadow);transition:all .25s">
        ...
        <div style="padding:14px 14px 12px 14px">
          ...
          <div style="display:flex;gap:8px;margin-top:12px">
            ...
            <button onclick="event.preventDefault(); window.openStoreReview('${s.id}', '${s.name.replace(/'/g, "\'")}')" style="padding:12px 14px;border-radius:14px;font-weight:800;font-size:12px;background:var(--card2);border:1px solid rgba(34,197,94,.35);color:var(--primary);cursor:pointer">⭐ Ulas</button>
          </div>
        </div>
      </div>`;
    }).join('');
