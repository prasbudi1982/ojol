...
export async function viewMyStore(){
  try{
    ...
    if(!stores.length) return `<div class="card"><h2>🏪 Warungku</h2><p class="muted">Belum punya warung</p><button onclick="window._addWarung()" class="btn primary" style="display:block;text-align:center;padding:12px;width:100%">+ Buat Warung</button></div>`;
    ...
    return `<div class="card" style="border:none;background:transparent;box-shadow:none;padding:0"><h2 style="font-size:22px;margin-bottom:12px">🏪 Warungku</h2><div class="list">${cards}</div><button onclick="window._addWarung()" class="btn secondary" style="width:100%;margin-top:12px;text-align:center;padding:12px;border-radius:12px">+ Tambah Warung</button>
      <div id="warungEditModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);align-items:center;justify-content:center;padding:16px"><div style="background:var(--card);width:100%;max-width:440px;border-radius:16px;padding:16px;max-height:92vh;overflow:auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="margin:0">✏️ Edit Warung</h3><button onclick="window._closeWarungEditModal()" class="btn secondary" style="width:32px;height:32px;border-radius:50%;padding:0">✕</button></div><div style="display:flex;flex-direction:column;gap:10px"><div><label class="muted" style="font-size:11px">Nama Warung</label><input id="warungEditName" placeholder="Nama Warung" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div><div><label class="muted" style="font-size:11px">Alamat Lengkap</label><textarea id="warungEditAlamat" placeholder="RT/RW, Jalan, Desa" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px;min-height:60px"></textarea></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div><label class="muted" style="font-size:11px">Latitude</label><input id="warungEditLat" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div><div><label class="muted" style="font-size:11px">Longitude</label><input id="warungEditLng" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div></div>
            <div style="display:flex;gap:6px">
              <button id="btnWarungEditMapPicker" type="button" class="btn secondary" style="flex:1;font-size:11px;padding:8px;border-radius:8px">🗺️ Pilih di Peta</button>
              <button id="btnWarungEditMyLoc" type="button" class="btn secondary" style="flex:1;font-size:11px;padding:8px;border-radius:8px">📌 Lokasi Saya</button>
            </div>
            <label style="font-size:12px;display:flex;gap:6px"><input id="warungEditIsOpen" type="checkbox"/> Warung Buka</label><div style="display:flex;gap:8px;margin-top:8px"><button onclick="window._closeWarungEditModal()" class="btn secondary" style="flex:1;padding:12px;border-radius:10px">Batal</button><button id="btnSaveWarungEdit" class="btn primary" style="flex:1;padding:12px;border-radius:10px;font-weight:800">💾 Simpan</button></div></div></div></div>
      <!-- Modal Tambah Warung -->
      <div id="warungCreateModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);align-items:center;justify-content:center;padding:16px"><div style="background:var(--card);width:100%;max-width:440px;border-radius:16px;padding:16px;max-height:92vh;overflow:auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="margin:0">+ Tambah Warung</h3><button onclick="window._closeWarungCreateModal()" class="btn secondary" style="width:32px;height:32px;border-radius:50%;padding:0">✕</button></div><div style="display:flex;flex-direction:column;gap:10px"><div><label class="muted" style="font-size:11px">Nama Warung</label><input id="warungCreateName" placeholder="Nama Warung" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div><div><label class="muted" style="font-size:11px">Alamat Lengkap</label><textarea id="warungCreateAlamat" placeholder="RT/RW, Jalan, Desa" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px;min-height:60px"></textarea></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div><label class="muted" style="font-size:11px">Latitude</label><input id="warungCreateLat" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div><div><label class="muted" style="font-size:11px">Longitude</label><input id="warungCreateLng" type="number" step="0.000001" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div></div>
            <div style="display:flex;gap:6px">
              <button id="btnWarungCreateMapPicker" type="button" class="btn secondary" style="flex:1;font-size:11px;padding:8px;border-radius:8px">🗺️ Pilih di Peta</button>
              <button id="btnWarungCreateMyLoc" type="button" class="btn secondary" style="flex:1;font-size:11px;padding:8px;border-radius:8px">📌 Lokasi Saya</button>
            </div>
            <label style="font-size:12px;display:flex;gap:6px"><input id="warungCreateIsOpen" type="checkbox" checked/> Warung Buka</label><div style="display:flex;gap:8px;margin-top:8px"><button onclick="window._closeWarungCreateModal()" class="btn secondary" style="flex:1;padding:12px;border-radius:10px">Batal</button><button id="btnSaveWarungCreate" class="btn primary" style="flex:1;padding:12px;border-radius:10px;font-weight:800">+ Buat</button></div></div></div></div></div>`;
...
// TAMBAH WARUNG - FIX belum berfungsi
window._addWarung=function(){
  ...
  document.getElementById('warungCreateModal').style.display='flex';
  document.getElementById('btnSaveWarungCreate').onclick=window._saveWarungCreate;
};
window._closeWarungCreateModal=function(){ document.getElementById('warungCreateModal').style.display='none'; };
...
// MAP PICKER UNTUK EDIT & CREATE WARUNG - FIX FINAL: tidak di belakang + tidak hilang
window._openWarungMapPicker=function(mode){
  ...
  const warungModalId = isEdit?'warungEditModal':'warungCreateModal';
  const warungModalEl=document.getElementById(warungModalId);
  const prevDisplay = warungModalEl?warungModalEl.style.display:'flex';

  // Helper restore warung modal - selalu pastikan tampil kembali
  const restoreWarungModal = ()=>{
    const wm=document.getElementById(warungModalId);
    if(wm){ wm.style.display='flex'; wm.style.zIndex='9999'; wm.style.opacity='1'; }
    // bersihkan interval/observer
    if(window._warungMapCheckInterval){ clearInterval(window._warungMapCheckInterval); window._warungMapCheckInterval=null; }
    if(window._warungMapObserver){ try{ window._warungMapObserver.disconnect(); }catch(e){} window._warungMapObserver=null; }
  };

  // Inject CSS global untuk paksa map picker di atas segalanya
  try{
    ...
    style.textContent=`
      ...
      #mapPickerModal *, .map-picker-modal * { z-index: auto !important; }
      .leaflet-container, .maplibregl-map, .mapboxgl-map { z-index: 2147483646 !important; }
    `;
  }catch(e){}

  const cb=(res)=>{
    try{
      const lat=res?.lat||res?.latitude||res?.latlng?.lat||res?.location?.lat||null;
      const lng=res?.lng||res?.longitude||res?.long||res?.lng||res?.location?.lng||res?.latlng?.lng||null;
      const addr=res?.address||res?.text||res?.alamat_text||res?.alamat||res?.name||'';
      if(lat!=null) document.getElementById(latId).value=lat;
      if(lng!=null) document.getElementById(lngId).value=lng;
      if(addr) document.getElementById(addrId).value=addr;
    }catch(e){}
    restoreWarungModal();
  };

  window._warungMapCb=cb;
  window._warungModalRestore={ id: warungModalId, display: prevDisplay };

  if(window.openMapPicker){
    // Sembunyikan warung modal dulu biar map tidak ketutup
    if(warungModalEl){ warungModalEl.style.display='none'; }

    // Setup observer & interval untuk deteksi map ditutup (cancel atau pilih)
    try{
      // interval cek setiap 300ms apakah map masih ada
      if(window._warungMapCheckInterval) clearInterval(window._warungMapCheckInterval);
      window._warungMapCheckInterval=setInterval(()=>{
        const mapExists=document.querySelector('#mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal, #map-picker-modal, .mapPicker, [id*="mapPicker"], [class*="map-picker"]');
        if(!mapExists){
          // map sudah tertutup, kembalikan modal warung jika masih hidden
          const wm=document.getElementById(warungModalId);
    ...
    try{
      const observer=new MutationObserver(()=>{
        const mapExists=document.querySelector('#mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal');
        // paksa z-index tinggi untuk map jika muncul
        document.querySelectorAll('#mapPickerModal, #mapPicker, .map-picker-modal, .mapPickerModal, #map-picker-modal, .mapPicker').forEach(el=>{
        ...
        if(!mapExists){
          // map hilang -> restore
          const wm=document.getElementById(warungModalId);
          if(wm && wm.style.display==='none'){ restoreWarungModal(); observer.disconnect(); }
        }
      });
      observer.observe(document.body, {childList:true, subtree:true});
      window._warungMapObserver=observer;
    }catch(e){}

    // Wrap close function jika ada
    const origCloseMap = window.closeMapPicker || window._closeMapPicker;
    if(origCloseMap && !origCloseMap._wrapped){
      const wrappedClose=function(...args){
        try{ origCloseMap.apply(this,args); }catch(e){}
        restoreWarungModal();
      };
      wrappedClose._wrapped=true;
      wrappedClose._orig=origCloseMap;
      if(window.closeMapPicker) window.closeMapPicker=wrappedClose;
      if(window._closeMapPicker) window._closeMapPicker=wrappedClose;
    }

    try{
      // Panggil dengan berbagai format callback yang mungkin dipakai app
      const opts={
        onSelect: cb,
        onPick: cb,
        callback: cb,
        onClose: ()=>{ restoreWarungModal(); },
        onCancel: ()=>{ restoreWarungModal(); }
      };
      window.openMapPicker(opts);
      // paksa z-index setelah 100ms
      setTimeout(()=>{
      ...
      try{
        ...
        setTimeout(()=>{
          document.querySelectorAll('#mapPickerModal, #mapPicker, .map-picker-modal').forEach(el=>{ el.style.zIndex='2147483647'; });
        },100);
        return;
      }catch(e2){
        // jika gagal, restore modal
        restoreWarungModal();
      }
    }
  }
  ...
  const inp=prompt(`Masukkan koordinat Lat,Lng (contoh: ${curLat},${curLng}) untuk ${mode} warung:\nKosongkan untuk pakai Lokasi Saya`, `${curLat},${curLng}`);
  if(inp===null){
    restoreWarungModal();
    return;
  }
  if(!inp.trim()){
    window._useWarungMyLocation(mode);
    restoreWarungModal();
    return;
  }
  ...
  if(parts.length>=2){
    ...
    if(!isNaN(la)&&!isNaN(ln)){ document.getElementById(latId).value=la; document.getElementById(lngId).value=ln; }
    else alert('Format salah, pakai: lat,lng');
  }
  restoreWarungModal();
};
