...
// === PAGINATION WARUNG - GLOBAL HELPERS (tanpa auto-refresh) ===
if(typeof window !== 'undefined'){
  window._warungOrdersData=[];
  window._warungOrdersPage=1;
  window._renderWarungOrdersPage=function(page){
    try{
      ...
      if(!listEl) return;
      if(!data.length){
        listEl.innerHTML='<div class="muted" style="text-align:center;padding:24px;border:1px dashed var(--border);border-radius:12px">Belum ada order</div>';
      ...
      for(var i=0;i<slice.length;i++){
        ...
        var itemsTxt=(o.items||[]).map(function(it){return (it.name||'')+' x'+(it.qty||1);}).join(', ');
        var total=Number(o.total||0).toLocaleString('id-ID');
        var status=o.status||'-';
        var color=status==='completed'?'var(--primary)':status==='ready'?'#22c55e':status==='preparing'?'#f59e0b':'var(--warning)';
        var time=o.created_at?new Date(o.created_at).toLocaleString('id-ID',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}):'';
        html+='<div class="card" style="margin:0 0 10px 0;border-radius:12px;padding:10px;border:1px solid var(--border)">'
          +'<div style="display:flex;justify-content:space-between"><b>#'+shortId+'</b><span style="font-size:10px;background:'+color+';color:white;padding:3px 8px;border-radius:99px">'+status+'</span></div>'
          +'<div class="muted" style="font-size:11px;margin-top:4px">'+(itemsTxt||'-')+'</div>'
          +'<div style="font-size:11px">Rp '+total+' • <span class="muted">'+time+'</span></div>'
          +'<div style="display:flex;gap:6px;margin-top:8px">'
      ...
      if(pagEl){
        var b='min-width:36px;height:36px;padding:0 10px;border-radius:10px;border:1px solid var(--border);background:var(--card2);color:var(--text);font-weight:700;font-size:12px;cursor:pointer';
        var a='min-width:36px;height:36px;padding:0 10px;border-radius:10px;border:1px solid var(--primary);background:var(--primary);color:#052e16;font-weight:800;font-size:12px;cursor:pointer';
        var d='min-width:36px;height:36px;padding:0 10px;border-radius:10px;border:1px solid var(--border);background:var(--card);color:var(--muted);opacity:.5';
        var h='';
        if(page>1) h+='<button onclick="window._renderWarungOrdersPage('+(page-1)+')" style="'+b+'">‹</button>'; else h+='<button disabled style="'+d+'">‹</button>';
        var maxV=5, st=Math.max(1,page-2), en=Math.min(totalPages,st+maxV-1);
        if(en-st<maxV-1) st=Math.max(1,en-maxV+1);
        if(st>1){ h+='<button onclick="window._renderWarungOrdersPage(1)" style="'+(1===page?a:b)+'">1</button>'; if(st>2) h+='<span style="color:var(--muted)">…</span>'; }
        for(var p=st;p<=en;p++){ if(p===page) h+='<button style="'+a+'">'+p+'</button>'; else h+='<button onclick="window._renderWarungOrdersPage('+p+')" style="'+b+'">'+p+'</button>'; }
        if(en<totalPages){ if(en<totalPages-1) h+='<span style="color:var(--muted)">…</span>'; h+='<button onclick="window._renderWarungOrdersPage('+totalPages+')" style="'+(totalPages===page?a:b)+'">'+totalPages+'</button>'; }
        if(page<totalPages) h+='<button onclick="window._renderWarungOrdersPage('+(page+1)+')" style="'+b+'">›</button>'; else h+='<button disabled style="'+d+'">›</button>';
        pagEl.innerHTML=h;
      }
      if(infoEl) infoEl.textContent='Halaman '+page+' dari '+totalPages+' • '+(s+1)+'-'+Math.min(e,data.length)+' dari '+data.length;
    }catch(err){ console.error('pagination render fail',err); }
  };
}
...
export async function viewStoreOrders(storeId){
  try{
    ...
    // Simpan ke global untuk pagination (tanpa <script> tag)
    if(typeof window!=='undefined'){
      window._warungOrdersData=allOrders;
      window._warungOrdersPage=1;
    }

    // Render halaman 1 langsung di server (biar gak blank)
    var perPage=10;
    var slice=allOrders.slice(0,perPage);
    var listHtml=slice.map(function(o){
      var shortId=(o.id||'').slice(0,6).toUpperCase();
      var itemsTxt=(o.items||[]).map(function(i){return (i.name||'')+' x'+(i.qty||1);}).join(', ');
      var total=Number(o.total||0).toLocaleString();
      var status=o.status||'-';
      return `<div class="card" style="margin:0 0 10px 0;border-radius:12px;padding:10px"><div style="display:flex;justify-content:space-between"><b>#${shortId}</b><span style="font-size:10px;background:var(--warning);color:var(--card);padding:3px 8px;border-radius:99px">${status}</span></div><div class="muted" style="font-size:11px">${itemsTxt}</div><div style="font-size:11px">Rp ${total}</div><div style="display:flex;gap:6px;margin-top:8px"><button onclick="window._orderStatus('${o.id}','preparing')" class="btn secondary" style="flex:1;font-size:11px">Masak</button><button onclick="window._orderStatus('${o.id}','ready')" class="btn secondary" style="flex:1;font-size:11px">Siap</button><button onclick="window._orderStatus('${o.id}','completed')" class="btn primary" style="flex:1;font-size:11px">Selesai</button></div></div>`;
    }).join('')||'Belum ada order';

    var totalPages=Math.max(1,Math.ceil(allOrders.length/perPage));
    var pagHtml='';
    if(allOrders.length>perPage){
      var b='min-width:36px;height:36px;padding:0 10px;border-radius:10px;border:1px solid var(--border);background:var(--card2);color:var(--text);font-weight:700;font-size:12px;cursor:pointer';
      var a='min-width:36px;height:36px;padding:0 10px;border-radius:10px;border:1px solid var(--primary);background:var(--primary);color:#052e16;font-weight:800;font-size:12px;cursor:pointer';
      var d='min-width:36px;height:36px;padding:0 10px;border-radius:10px;border:1px solid var(--border);background:var(--card);color:var(--muted);opacity:.5';
      if(1>1) pagHtml+='<button onclick="window._renderWarungOrdersPage(0)" style="'+b+'">‹</button>'; else pagHtml+='<button disabled style="'+d+'">‹</button>';
      for(var p=1;p<=Math.min(totalPages,5);p++){
        if(p===1) pagHtml+='<button style="'+a+'">'+p+'</button>'; else pagHtml+='<button onclick="window._renderWarungOrdersPage('+p+')" style="'+b+'">'+p+'</button>';
      }
      if(totalPages>5){ pagHtml+='<span style="color:var(--muted)">…</span><button onclick="window._renderWarungOrdersPage('+totalPages+')" style="'+b+'">'+totalPages+'</button>'; }
      if(totalPages>1) pagHtml+='<button onclick="window._renderWarungOrdersPage(2)" style="'+b+'">›</button>'; else pagHtml+='<button disabled style="'+d+'">›</button>';
    }

    return `<div class="card"><a href="#/store/my" class="muted">← Warungku</a><div style="margin-bottom:12px">${getMerchantNotifBtnHtml()}</div><h2>📦 Order Masuk - ${allOrders.length} order (max 100)</h2><div id="warungOrderList" class="list" style="margin-top:12px">${listHtml}</div><div id="warungOrderPagination" style="display:flex;justify-content:center;align-items:center;gap:6px;margin-top:16px;flex-wrap:wrap">${pagHtml}</div><div id="warungOrderInfo" class="muted" style="text-align:center;font-size:11px;margin-top:8px">Halaman 1 dari ${totalPages} • 10 per halaman</div></div>`;
  }catch(e){ return `<div class="card">Error: ${e.message}</div>`; }
}
