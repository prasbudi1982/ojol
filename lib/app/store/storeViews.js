...
export async function viewStoreOrders(storeId){
  try{
    // FIX: fetch maksimal 100 order terbaru
    ...
    return `<div class="card"><a href="#/store/my" class="muted">← Warungku</a><div style="margin-bottom:12px">\${getMerchantNotifBtnHtml()}</div>
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        ...
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
          <span id="warungOrderCount" class="muted" style="font-size:11px">\${allOrders.length} order (max 100)</span>
          <span id="warungAutoStatus" style="font-size:10px;padding:4px 8px;border-radius:99px;background:var(--card2);border:1px solid var(--border);display:flex;align-items:center;gap:4px"><span style="width:6px;height:6px;border-radius:50%;background:#22c55e;display:inline-block;animation:warungPulse 1.5s infinite"></span> Auto 5s</span>
          <button id="warungToggleAuto" onclick="window._toggleWarungAutoRefresh()" class="btn secondary" style="width:auto;font-size:10px;padding:5px 8px;border-radius:8px">⏸️</button>
    ...
    <style>@keyframes warungPulse{0%{opacity:1}50%{opacity:.4}100%{opacity:1}}</style>
    ...
    <script>
      (function(){
        ...
        function warungHash(data){
          try{ return data.map(function(o){ return o.id+':'+o.status+':'+(o.updated_at||o.created_at); }).join('|'); }catch(e){ return String(data.length); }
        }
        window._warungLastHash = warungHash(window._warungOrdersData);
        ...
        window._renderWarungOrdersPage = function(page){
          ...
          if(countEl) countEl.textContent = data.length + ' order (max 100)';
          if(!data.length){
          ...
          var html = slice.map(function(o){
            ...
            return '<div class="card" data-order-id="'+o.id+'" style="margin:0 0 10px 0;border-radius:12px;padding:10px;border:1px solid var(--border)">'
              + '<div style="display:flex;justify-content:space-between;align-items:center"><b>#'+shortId+'</b><span style="font-size:10px;background:'+statusColor+';color:white;padding:3px 8px;border-radius:99px;font-weight:700">'+status+'</span></div>'
          ...
          if(pagEl){
            ...
            var maxV=5, s=Math.max(1,page-2), e=Math.min(totalPages,s+maxV-1);
            if(e-s<maxV-1) s=Math.max(1,e-maxV+1);
            if(s>1){ h+='<button onclick="window._renderWarungOrdersPage(1)" style="'+(1===page?a:b)+'">1</button>'; if(s>2) h+='<span style="color:var(--muted)">…</span>'; }
            for(var p=s;p<=e;p++){ if(p===page) h+='<button style="'+a+'">'+p+'</button>'; else h+='<button onclick="window._renderWarungOrdersPage('+p+')" style="'+b+'">'+p+'</button>'; }
            if(e<totalPages){ if(e<totalPages-1) h+='<span style="color:var(--muted)">…</span>'; h+='<button onclick="window._renderWarungOrdersPage('+totalPages+')" style="'+(totalPages===page?a:b)+'">'+totalPages+'</button>'; }
          ...
          if(infoEl) infoEl.textContent='Halaman '+page+' dari '+totalPages+' • '+ (startIdx+1) +'-'+ Math.min(endIdx,data.length) +' dari '+data.length;
          var banner=document.getElementById('warungNewOrderBanner');
          if(banner) setTimeout(function(){ if(!banner._keep) banner.style.display='none'; },2000);
        };

        window._refreshWarungOrdersSoft = async function(){
          if(window._warungIsFetching) return;
          window._warungIsFetching=true;
          var st=document.getElementById('warungAutoStatus');
          try{
            if(st) st.innerHTML='<span style="width:6px;height:6px;border-radius:50%;background:#f59e0b;display:inline-block"></span> Refreshing...';
            ...
            var newHash=warungHash(newData);
            if(newHash===window._warungLastHash){
              if(st) st.innerHTML='<span style="width:6px;height:6px;border-radius:50%;background:#22c55e;display:inline-block;animation:warungPulse 1.5s infinite"></span> Auto 5s • '+new Date().toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
              window._warungIsFetching=false; return;
            }
            var oldIds=new Set((window._warungOrdersData||[]).map(function(o){return o.id}));
            var hasNew=newData.some(function(o){return !oldIds.has(o.id)});
            window._warungOrdersData=newData;
            window._warungLastHash=newHash;
            if(hasNew && window._warungOrdersPage!==1){
              var bn=document.getElementById('warungNewOrderBanner');
              if(bn){ bn.style.display='block'; bn._keep=true; setTimeout(function(){bn._keep=false;},8000); }
            }
            window._renderWarungOrdersPage(window._warungOrdersPage);
            if(st) st.innerHTML='<span style="width:6px;height:6px;border-radius:50%;background:#22c55e;display:inline-block;animation:warungPulse 1.5s infinite"></span> Auto 5s • '+new Date().toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
          }catch(e){
            console.warn('soft refresh fail',e);
            if(st) st.innerHTML='<span style="width:6px;height:6px;border-radius:50%;background:#ef4444;display:inline-block"></span> Gagal';
          }
          window._warungIsFetching=false;
        };
        ...
        window._toggleWarungAutoRefresh=function(){
          ...
          if(window._warungAutoEnabled){
            ...
            if(st) st.innerHTML='<span style="width:6px;height:6px;border-radius:50%;background:#22c55e;display:inline-block;animation:warungPulse 1.5s infinite"></span> Auto 5s';
            window._startWarungAutoRefresh();
          }else{
            if(btn) btn.textContent='▶️';
            if(st) st.innerHTML='<span style="width:6px;height:6px;border-radius:50%;background:var(--muted);display:inline-block"></span> Paused';
            if(window._warungAutoInterval){ clearInterval(window._warungAutoInterval); window._warungAutoInterval=null; }
          }
        };
