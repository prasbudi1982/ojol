  ...
  return `
    <div class="card">
      ...
      <div id="cartNearbyDriversSection" style="margin-top:14px;border:1px solid #f59e0b;border-radius:14px;padding:12px;background:var(--card)">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <b style="font-size:13px">5 Driver Terdekat dari Warung (Aktif)</b>
        ...
        <div class="muted" style="font-size:10px;margin-top:4px">Driver aktif 15 menit terakhir, urut dari warung. Pilih driver atau Broadcast.</div>
        <div id="cartNearbyDriversList" style="margin-top:10px"><div class="muted" style="padding:12px;text-align:center;font-size:11px">Pilih alamat antar dulu</div></div>
        <div style="display:flex;gap:8px;margin-top:10px">
          ...
          <button id="btnCheckout" class="btn primary" style="flex:1;padding:12px;border-radius:12px;font-weight:800;font-size:12px">Pesan dan Broadcast</button>
        </div>
      </div>
      </div>
    </div>
...
if(typeof window !== 'undefined'){
  ...
  window._loadNearbyDriversForCart = async function(){
    ...
    try{
      ...
      for(var j=0;j<locs.length;j++){
        ...
        var R=6371;
        var dLat=(loc.lat - plat.lat)*Math.PI/180;
        var dLng=(loc.lng - plat.lng)*Math.PI/180;
        var a=Math.sin(dLat/2)*Math.sin(dLat/2) + Math.cos(plat.lat*Math.PI/180)*Math.cos(loc.lat*Math.PI/180)*Math.sin(dLng/2)*Math.sin(dLng/2);
      ...
      for(var d=0; d<drivers.length; d++){
        ...
        var star = d===0 ? '⭐ ' : '';
        var live = onlineAgo<=2 ? ' <span style="color:#22c55e;font-size:10px">• LIVE</span>' : '';
        html += '<div style="border:1px solid var(--border);border-radius:12px;padding:10px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;background:' + (d===0 ? '#f0fdf4' : 'var(--card2)') + '">'
          + '<div style="flex:1">'
          + '<div style="font-weight:800;font-size:12px">' + star + (usr.name||'Driver') + live + '</div>'
          + '<div style="font-size:10px" class="muted">' + (usr.jenis_kendaraan||'') + ' ' + (usr.nopol||'') + ' • ' + dr.dist.toFixed(2) + ' km dari warung • ' + onlineAgo + ' mnt lalu</div>'
          + '</div>'
      ...
      console.warn('loadNearbyDrivers fail', e);
  ...
  window._checkoutFoodDirect = async function(preferredDriverId){
    ...
    try{
      ...
      var subtotal = 0;
      try{ for(var i=0;i<state.items.length;i++){ subtotal += Number(state.items[i].harga||0)*Number(state.items[i].qty||0); } }catch(e){}
      var distance_km = state.distanceKm||0;
      var delivery_fee = 0;
      try{
        if(state.pickup && state.pickup.lat){
          var R=6371;
          var dLat=(state.dest.lat - state.pickup.lat)*Math.PI/180;
          var dLng=(state.dest.lng - state.pickup.lng)*Math.PI/180;
          var a=Math.sin(dLat/2)*Math.sin(dLat/2) + Math.cos(state.pickup.lat*Math.PI/180)*Math.cos(state.dest.lat*Math.PI/180)*Math.sin(dLng/2)*Math.sin(dLng/2);
      ...
      if(window.openFoodTrackingDetailModal) window.openFoodTrackingDetailModal(od.id);
        ...
        var tf = await import('./trackingFood.js');
        tf.openFoodTrackingDetailModal(od.id);
      }
    }catch(e){ alert('Gagal: '+e.message); }
  };

  // Checkout - Langsung ke modal detail tracking
