// lib/app/store/storeViews.js - FINAL - Food Delivery Merchant (tanpa dummy)
...
export async function viewMyStore(){
  ...
  if(profile.role!=='merchant' && profile.role!=='warung') return `<div class="card"><h2>🏪 Warungku</h2><p class="muted">Halaman ini khusus role merchant. Role kamu: ${profile.role}. Hubungi admin untuk jadi merchant.</p><p class="muted">Jalankan SQL fix-users-role-check.sql jika baru ubah role jadi merchant.</p></div>`;
  let myStore = null;
  ...
  if(!myStore){
    return `
      <div class="card">
        <h2>🏪 Buat Warung</h2>
        <p class="muted">Warung belum ada. Karena kamu sudah jadi merchant dari Profil, data warung seharusnya auto-sync. Jika belum, buat manual di sini. Wajib pakai peta.</p>
        <label>Nama Warung</label><input id="sName" placeholder="Warung Sego Bu Sri" value="${profile.warung_name||profile.name||''}"/>
        ...
        <label>Lat (wajib dari peta)</label><input id="sLat" type="number" step="any" placeholder="-8.111..."/>
        ...
        <p class="muted" style="font-size:11px;margin-top:8px">Tips: tap 📍, klik peta di Suruh, Confirm, baru Buat Warung.</p>
      </div>
    `;
  }
  let avg = { avg:0, count:0 };
