...
import { openFoodTrackingDetailModal as openFoodTrackingModal, startFoodTracking } from '../trackingFood.js';
...
window._checkoutFoodDirect = async function(driverId){
  try{
    ...
    try{
      if(typeof openFoodTrackingModal === 'function'){
        openFoodTrackingModal(od.id);
      } else if(window.trackingFood?.openFoodTrackingDetailModal){
        ...
        // fallback terakhir: start tracking dan pakai hash yang benar
        try{ startFoodTracking(od.id); }catch(e){}
        if(typeof openFoodTrackingModal === 'function') openFoodTrackingModal(od.id);
      }
      // pastikan active tracking dimulai
      try{ startFoodTracking(od.id); }catch(e){}
    }catch(err){
      console.error('open tracking fail', err);
      alert('✅ Order dibuat! ID: '+od.id.slice(0,6).toUpperCase()+' - buka Tracking untuk lihat status');
    }
