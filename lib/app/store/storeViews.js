...
export async function viewStoreCart(){
  ...
  return `<div class="card" style="border:none;box-shadow:none;background:transparent;padding:0"><h2 style="font-size:22px;margin:0 0 4px 0">🛒 Keranjang - ${state.storeName||'Nasgor Pak W'}</h2><p class="muted" style="font-size:11px;margin:0 0 12px 0">${state.items?.length||0} item • ${warungName}</p><div id="cartItemsContainer" class="list" style="display:block !important;opacity:1 !important" style="margin-top:10px">${itemsHtml}</div><div style="height:12px"></div>
    <div id="detailJarakContainer" style="background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:12px;display:${state.items&&state.items.length?"block":"none"}">
      ...
      <div id="alamatAntarContainer" style="display:${state.items&&state.items.length?"block":"none"}"><div style="font-weight:800;font-size:12px">📍 Alamat Antar (Wajib Isi)</div>
      ...
      <div class="muted" style="font-size:10px;margin-top:6px">Lat: <span id="cartLatDisplay">${state.dest.lat||'-'}</span> Lng: <span id="cartLngDisplay">${state.dest.lng||'-'}</span> • </div><div style="display:none"><span id="ongkirLiveStatus">Total ongkir Rp ${delivery_fee.toLocaleString()} untuk ${distance_km.toFixed(2)} km</span></div>
    </div>
    </div><div id="cartSummaryContainer" style="margin-top:12px;display:${state.items&&state.items.length?"block":"none"};background:var(--card2);border:1px solid var(--border);border-radius:12px;padding:12px">
