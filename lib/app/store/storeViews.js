...
window._checkoutFoodDirect = async function(driverId){
  try{
    ...
    // FIX: buka modal trackingFood langsung, jangan balik ke halaman food
    try{
      if(window.trackingFood && window.trackingFood.openFoodTrackingDetailModal){
        ...
        // dynamic import di dalam fungsi agar storeViews.js tidak gagal load
        try{
          const mod = await import('../trackingFood.js');
          if(mod.openFoodTrackingDetailModal) mod.openFoodTrackingDetailModal(od.id);
          if(mod.startFoodTracking) mod.startFoodTracking(od.id);
        }catch(e){
          console.warn('trackingFood dynamic import fail', e);
          // fallback: tetap stay di halaman, jangan redirect ke #/store
          alert('✅ Order dibuat! ID: '+od.id.slice(0,6).toUpperCase()+' - Tracking akan muncul di menu Keranjang');
        }
      }
    }catch(err){
...
Buka menu Warungku > Edit Warung > isi lat/lng via Pilih di Peta.'); }
    ...
    if(destText.length<5){ alert('⚠️ Alamat tujuan minimal 5 huruf.'); return; }
    if(!state.dest.lat||!state.dest.lng){
      alert('⚠️ Koordinat tujuan belum ada (lat/lng kosong).
Klik tombol 📌 Pakai Lokasi Saya untuk auto isi alamat + koordinat.');
      return;
    }
    ...
    try{
      if(window.trackingFood && window.trackingFood.openFoodTrackingDetailModal){
        ...
        try{
          ...
          console.warn('trackingFood import fail', e);
