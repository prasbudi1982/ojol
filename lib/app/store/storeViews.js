...
export async function viewStoreList() {
  try {
    ...
    const cards = stores
      .map(
        (s) =>
          `<div class="driver-card" style="flex-direction:column;gap:8px"><div style="display:flex;justify-content:space-between"><b>${s.name}</b><span style="font-size:10px;background:${
    ...
    return `<div class="card"><a href="#/store" class="muted">← Kembali</a><h2>${
      ...
      <div id="menuBuilderModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.7);align-items:flex-end;justify-content:center;backdrop-filter:blur(4px)">
        <div style="background:var(--bg);width:100%;max-width:520px;max-height:90vh;overflow:auto;border-radius:24px 24px 0 0;box-shadow:0 -4px 24px rgba(0,0,0,0.2)">
          ...
          <div style="position:fixed;bottom:0;left:0;right:0;max-width:520px;margin:0 auto;background:var(--card);border-top:1px solid var(--border);padding:12px 16px 20px"><button id="builderAddBtn" onclick="window._saveMenuBuilder()" class="btn primary" style="width:100%;padding:14px;font-weight:800;font-size:14px;border-radius:14px">🛒 Tambah • <span id="builderTotalBtn">Rp 0</span></button></div>
        </div>
...
export async function viewStoreCart() {
  ...
  try {
    if (!state.items || !state.items.length) {
      window._clearFoodCart();
      return `<div class="card" style="border:none;box-shadow:none;background:transparent;padding:0"><h2 style="font-size:22px;margin:0 0 4px 0">🛒 Keranjang - Kosong</h2><p class="muted" style="font-size:11px;margin:0 0 12px 0">0 item</p><div style="text-align:center;padding:24px;background:var(--card2);border:1px dashed var(--border);border-radius:16px"><div style="font-size:32px">🛒</div><div class="muted" style="font-size:13px;margin-top:8px">Keranjang masih kosong</div><a href="#/store" class="btn primary" style="margin-top:12px;display:inline-block;padding:10px 18px;font-size:12px;border-radius:12px">+ Cari Makanan Lagi</a></div></div>`;
    }
  } catch (e) {}
...
// ==========================================
// FIX: MENU BUILDER MODAL HANDLERS
// ==========================================
...
window._openMenuBuilder = function (productId) {
  try {
    const dataEl = document.getElementById('productsData');
    if (!dataEl) return alert('Data produk tidak ditemukan');
    const products = JSON.parse(dataEl.textContent || '[]');
    _currentProduct = products.find((p) => String(p.id) === String(productId));
    if (!_currentProduct) return alert('Produk tidak ditemukan');

    _builderQty = 1;
    _selectedVariant = null;
    _selectedAddons = [];

    document.getElementById('builderName').textContent = _currentProduct.name;
    document.getElementById('builderBase').textContent = `Rp ${Number(
      _currentProduct.harga || 0
    ...
    try {
      variants =
        typeof _currentProduct.variants === 'string'
          ? JSON.parse(_currentProduct.variants)
          : _currentProduct.variants || [];
    } catch (e) {}

    const varContainer = document.getElementById('builderVariasi');
    if (variants.length) {
      _selectedVariant = variants[0]; // Default pilih varian pertama
      varContainer.innerHTML = variants
        .map(
          (v, idx) => `
        <label style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:var(--card);border:1px solid var(--border);border-radius:12px;cursor:pointer">
          <div style="display:flex;align-items:center;gap:8px">
            <input type="radio" name="builder_variant" value="${idx}" ${
            idx === 0 ? 'checked' : ''
          } onchange="window._onVariantChange(${idx})"/>
            <span style="font-weight:600;font-size:13px">${v.name}</span>
          </div>
          <span class="muted" style="font-size:12px">${
            v.price_delta ? `+Rp ${Number(v.price_delta).toLocaleString('id-ID')}` : 'Gratis'
          }</span>
        </label>`
        )
        .join('');
    } else {
      varContainer.innerHTML = '<div class="muted" style="font-size:12px">Tidak ada opsi variasi</div>';
    }
    ...
    try {
      addons =
        typeof _currentProduct.addons === 'string'
          ? JSON.parse(_currentProduct.addons)
          : _currentProduct.addons || [];
    } catch (e) {}

    const addonContainer = document.getElementById('builderAddon');
    if (addons.length) {
      addonContainer.innerHTML = addons
        .map(
          (a, idx) => `
        <label style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:var(--card);border:1px solid var(--border);border-radius:12px;cursor:pointer">
          <div style="display:flex;align-items:center;gap:8px">
            <input type="checkbox" value="${idx}" onchange="window._onAddonToggle(${idx}, this.checked)"/>
            <span style="font-weight:600;font-size:13px">${a.name}</span>
          </div>
          <span class="muted" style="font-size:12px">+Rp ${Number(a.price || 0).toLocaleString('id-ID')}</span>
        </label>`
        )
        .join('');
    } else {
      addonContainer.innerHTML = '<div class="muted" style="font-size:12px">Tidak ada opsi addon</div>';
    }

    window._updateBuilderCalc();
    const modal = document.getElementById('menuBuilderModal');
    if (modal) modal.style.display = 'flex';
  } catch (err) {
    console.error('Error openMenuBuilder:', err);
  }
};
...
window._onVariantChange = function (index) {
  let variants = [];
  try {
    variants =
      typeof _currentProduct.variants === 'string'
  ...
  _selectedVariant = variants[index] || null;
  window._updateBuilderCalc();
};

window._onAddonToggle = function (index, isChecked) {
  let addons = [];
  try {
    addons =
      typeof _currentProduct.addons === 'string'
  ...
  if (isChecked) {
    _selectedAddons.push(target);
  } else {
    _selectedAddons = _selectedAddons.filter((a) => a.name !== target.name);
  }
  window._updateBuilderCalc();
};

window._changeBuilderQty = function (delta) {
  _builderQty = Math.max(1, _builderQty + delta);
  document.getElementById('builderQty').textContent = _builderQty;
  window._updateBuilderCalc();
};

window._updateBuilderCalc = function () {
  if (!_currentProduct) return;
  const basePrice = Number(_currentProduct.harga || 0);
  const variantDelta = Number(_selectedVariant?.price_delta || 0);
  const addonTotal = _selectedAddons.reduce((acc, a) => acc + Number(a.price || 0), 0);

  const unitPrice = basePrice + variantDelta + addonTotal;
  const grandTotal = unitPrice * _builderQty;

  const summaryArr = [];
  if (_selectedVariant) summaryArr.push(_selectedVariant.name);
  if (_selectedAddons.length)
    summaryArr.push(`Addon: ${_selectedAddons.map((a) => a.name).join(', ')}`);

  document.getElementById('builderSummary').textContent = summaryArr.join(' • ') || 'Standar';
  document.getElementById('builderTotal').textContent = `Rp ${grandTotal.toLocaleString('id-ID')}`;
  document.getElementById('builderTotalBtn').textContent = `Rp ${grandTotal.toLocaleString('id-ID')}`;
};

window._saveMenuBuilder = function () {
  if (!_currentProduct) return;
  const basePrice = Number(_currentProduct.harga || 0);
  const variantDelta = Number(_selectedVariant?.price_delta || 0);
  const addonTotal = _selectedAddons.reduce((acc, a) => acc + Number(a.price || 0), 0);
  const finalUnitPrice = basePrice + variantDelta + addonTotal;

  const itemData = {
    cartKey: `${_currentProduct.id}_${_selectedVariant?.name || 'base'}_${_selectedAddons
      .map((a) => a.name)
      .join('-')}`,
  ...
  try {
    if (cartStore && cartStore._actions && cartStore._actions.addItem) {
      cartStore._actions.addItem(itemData);
    } else {
      let raw = localStorage.getItem('ojol_cart_v2_food');
      let state = raw ? JSON.parse(raw) : { items: [] };
      state.items = state.items || [];
      const existingIdx = state.items.findIndex((i) => i.cartKey === itemData.cartKey);
      if (existingIdx > -1) {
        state.items[existingIdx].qty += itemData.qty;
      } else {
...
// Cart Helper Global
window._cartQty = function (cartKey, newQty) {
  try {
    let raw = localStorage.getItem('ojol_cart_v2_food');
    if (!raw) return;
    ...
    if (newQty <= 0) {
      state.items = state.items.filter(
        (i) => (i.cartKey || i.product_id || i.id) !== cartKey
      );
    } else {
