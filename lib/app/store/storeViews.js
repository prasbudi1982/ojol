...
export async function viewStoreCart(){
  ...
  if(state.items&&state.items.length){ 
    for(var i=0;i<state.items.length;i++){ 
      ...
      itemsHtml+=`
        <div class="card" style="margin:0 0 10px 0;border:1px solid var(--border);border-radius:16px;padding:12px;background:linear-gradient(135deg,var(--card) 0%,var(--card2) 100%);box-shadow:0 2px 8px rgba(0,0,0,0.08);position:relative;overflow:hidden">
          <div style="position:absolute;left:0;top:0;bottom:0;width:4px;background:var(--primary)"></div>
          <div style="display:flex;gap:12px;align-items:center">
            <div style="width:44px;height:44px;border-radius:12px;background:var(--bg);display:flex;align-items:center;justify-content:center;font-size:22px">🍱</div>
            <div style="flex:1;min-width:0">
              <div style="font-weight:800;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${it.name||''}</div>
              <div style="font-size:11px" class="muted">Rp ${Number(it.harga||0).toLocaleString()} x ${it.qty} ${variTxt?`• ${variTxt}`:''}</div>
              ${addonTxt?`<div style="font-size:10px" class="muted">+ ${addonTxt}</div>`:''}
              <div style="font-weight:700;font-size:12px;color:var(--primary);margin-top:2px">Rp ${(Number(it.harga||0)*Number(it.qty||0)).toLocaleString()}</div>
            </div>
            <div style="display:flex;flex-direction:column;align-items:center;gap:6px">
              <div style="display:flex;align-items:center;gap:6px;background:var(--bg);border:1px solid var(--border);border-radius:99px;padding:4px">
                <button onclick="window._cartQty('${key}',${it.qty-1})" class="btn secondary" style="width:28px;height:28px;border-radius:50%;padding:0;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;background:var(--card)">−</button>
                <span style="min-width:20px;text-align:center;font-weight:800;font-size:13px">${it.qty}</span>
                <button onclick="window._cartQty('${key}',${it.qty+1})" class="btn secondary" style="width:28px;height:28px;border-radius:50%;padding:0;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;background:var(--primary);color:white;border:none">+</button>
              </div>
              <button onclick="window._removeCartItem('${key}')" style="font-size:10px;background:transparent;border:none;color:#ef4444;padding:2px 6px">🗑️ Hapus</button>
            </div>
          </div>
        </div>`;
    } 
...
export async function viewMyStore(){
  try{
    ...
    const cards=stores.map(s=>`<div class="card" style="margin:0 0 12px 0;border:1px solid var(--border);border-radius:16px;padding:12px;background:linear-gradient(135deg,var(--card) 0%,var(--card2) 100%);box-shadow:0 2px 8px rgba(0,0,0,0.08)">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <div style="display:flex;gap:8px;align-items:center"><div style="width:40px;height:40px;border-radius:12px;background:var(--bg);display:flex;align-items:center;justify-content:center;font-size:20px">🏪</div><div><b style="font-size:14px">${s.name}</b><div class="muted" style="font-size:11px">${s.alamat_text||'RT12 RW05'}</div></div></div>
        <span style="font-size:10px;background:${s.is_open?'#16a34a':'#ef4444'};color:white;padding:4px 10px;border-radius:99px;font-weight:700">${s.is_open?'BUKA':'TUTUP'}</span>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        <button onclick="window._toggleWarung('${s.id}',${!s.is_open})" class="btn secondary" style="font-size:12px;padding:10px;border-radius:10px">${s.is_open?'🔴 Tutup':'🟢 Buka'}</button>
        <a href="#/store/products/${s.id}" class="btn secondary" style="font-size:12px;padding:10px;border-radius:10px;text-align:center">🍱 Kelola Menu + Varian</a>
        <button onclick="window._openWarungOrders('${s.id}')" class="btn primary" style="font-size:12px;padding:12px;border-radius:12px;background:#22c55e;color:#052e16;font-weight:800">📦 Order</button>
        <button onclick="window._editWarung('${s.id}')" class="btn secondary" style="font-size:12px;padding:10px;border-radius:10px">✏️ Edit</button>
      </div>
    </div>`).join('');
    return `<div class="card" style="border:none;background:transparent;box-shadow:none;padding:0">
      ...
      <a href="#/store/create" class="btn secondary" style="width:100%;margin-top:12px;text-align:center;padding:12px;border-radius:12px">+ Tambah Warung</a>
      
      <!-- Modal Edit Warung - FIX -->
      <div id="warungEditModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);align-items:center;justify-content:center;padding:16px">
        <div style="background:var(--card);width:100%;max-width:440px;border-radius:16px;padding:16px;max-height:92vh;overflow:auto">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="margin:0">✏️ Edit Warung</h3><button onclick="window._closeWarungEditModal()" class="btn secondary" style="width:32px;height:32px;border-radius:50%;padding:0">✕</button></div>
          <div style="display:flex;flex-direction:column;gap:10px">
            <div><label style="font-size:11px" class="muted">Nama Warung</label><input id="warungEditName" placeholder="Nama Warung" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div>
            <div><label style="font-size:11px" class="muted">Alamat Lengkap</label><textarea id="warungEditAlamat" placeholder="RT/RW, Jalan, Desa" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px;min-height:60px"></textarea></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
              <div><label style="font-size:11px" class="muted">Latitude</label><input id="warungEditLat" type="number" step="0.000001" placeholder="-8.09593" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div>
              <div><label style="font-size:11px" class="muted">Longitude</label><input id="warungEditLng" type="number" step="0.000001" placeholder="111.63510" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);margin-top:4px"/></div>
            </div>
            <label style="font-size:12px;display:flex;align-items:center;gap:6px"><input id="warungEditIsOpen" type="checkbox"/> Warung Buka</label>
            <div class="muted" style="font-size:10px">Lokasi diambil dari warung saat ini. Ganti jika pindah lokasi.</div>
            <div style="display:flex;gap:8px;margin-top:8px">
              <button onclick="window._closeWarungEditModal()" class="btn secondary" style="flex:1;padding:12px;border-radius:10px">Batal</button>
              <button id="btnSaveWarungEdit" class="btn primary" style="flex:1;padding:12px;border-radius:10px;font-weight:800">💾 Simpan</button>
            </div>
          </div>
        </div>
      </div>
...
window._editWarung=async function(storeId){
  try{
    ...
    document.getElementById('warungEditLng').value=s.lng||s.longitude||s.long||'';
...
// BADGE NAV KERANJANG - FIX hanya 1 counter, tidak berantakan
window._updateCartBadge = function(){
  try{
    ...
    // Daftar selector badge yang memang ada di layout asli - JANGAN buat baru
    ...
    selectors.forEach(sel=>{
      document.querySelectorAll(sel).forEach(el=>{
        // Cegah duplikat: jika parent sudah punya badge lain, hapus yang lama kecuali ini
    ...
    // FIX khusus bottom nav KERANJANG yang di screenshot ada 2 badge (atas & bawah)
    // Kita bersihkan semua badge di dalam bottom nav item keranjang dan sisakan 1
    document.querySelectorAll('a[href*="store/cart"], button[data-nav="cart"], [data-tab="cart"]').forEach(link=>{
      ...
      if(badges.length>1){
        ...
        badges.forEach((b,idx)=>{ if(idx>0) b.remove(); });
      }
      // Jika belum ada badge tapi count>0, baru buat 1 dengan posisi absolute di kanan atas icon
      if(badges.length===0 && count>0){
        const icon=link.querySelector('img, svg, div, span');
        ...
        // pastikan posisi tidak bentrok bawah
