...
export async function subscribeUser(userId){
  try{
    ...
    // 3. tunggu SW ready dengan timeout 8 detik
    let reg;
    try{
      reg = await withTimeout(navigator.serviceWorker.ready, 8000, 'serviceWorker.ready');
    }catch(e){
      throw new Error('Service Worker belum ready. Pastikan sw.js di root sejajar index.html dan HTTPS. Error: '+e.message);
    }
