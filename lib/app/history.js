...
function renderPaginatedHistory(){
  ...
  for(let i=1;i<=totalPages;i++){
    ...
    btns += `<button data-history-page="${i}" style="min-width:38px;padding:8px 10px;border-radius:10px;border:1px solid ${active?theme.primary:'#263240'};background:${active?theme.primary:'#151c25'};color:${active?'#052e16':'#e6edf5'};font-weight:800;font-size:12px;cursor:pointer">${i}</button>`;
  }
  pagEl.innerHTML = `
    <div style="display:flex;gap:6px;justify-content:center;align-items:center;margin-top:16px;flex-wrap:wrap">
      <button data-history-page="prev" style="padding:8px 14px;border-radius:10px;border:1px solid #263240;background:#151c25;color:#e6edf5;font-size:11px;font-weight:700;cursor:pointer" ${currentPage===1?'disabled':''}>‹ Prev</button>
      ${btns}
      <button data-history-page="next" style="padding:8px 14px;border-radius:10px;border:1px solid #263240;background:#151c25;color:#e6edf5;font-size:11px;font-weight:700;cursor:pointer" ${currentPage===totalPages?'disabled':''}>Next ›</button>
    </div>
    <div style="text-align:center;font-size:10px;color:#8aa0b8;margin-top:8px">Hal ${currentPage} dari ${totalPages} • ${total} total • 5/hal • max 20 fetch</div>
  `;
}
...
function viewHistoryPassangerInner(orders){
  ...
  if(!orders || orders.length===0){
    return `<div style="text-align:center;padding:24px;color:#8aa0b8;font-size:12px;background:#151c25;border:1px dashed #263240;border-radius:12px">Tidak ada data di halaman ini</div>`;
  }
  return orders.map(o=>{
    ...
    return `<div style="border:1px solid #263240;border-radius:14px;padding:12px;background:#151c25;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:11px;color:#8aa0b8">${date}</span>
      ...
      <div style="margin-top:8px;font-size:13px;color:#e6edf5;line-height:1.4">
        <div>📍 ${o.pickup_text||o.pickup||''}</div>
        <div style="margin-top:2px">🎯 ${o.dest_text||o.destination||''}</div>
      </div>
      <div style="margin-top:10px;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:11px;color:#8aa0b8">${o.vehicle_type==='mobil'?'🚗':'🏍️'} ${o.distance_km?.toFixed?o.distance_km.toFixed(1):o.distance_km||'-'} km • ${o.driver?.name||'Driver -'}</span>
        <span style="font-weight:800;font-size:12px;color:#e6edf5">Rp ${(o.estimated_cost||0).toLocaleString('id-ID')}</span>
      </div>
      ${o.status==='completed' && o.driver_id ? `<div style="margin-top:10px;display:flex;gap:6px">
        ...
        <a href="https://www.google.com/maps/dir/${o.pickup_lat||''},${o.pickup_lng||''}/${o.dest_lat||''},${o.dest_lng||''}" target="_blank" style="background:#1d2633;border:1px solid #263240;padding:8px 10px;border-radius:10px;font-size:11px;text-decoration:none;color:#e6edf5">🗺️</a>
      </div>` : ''}
    </div>`;
  }).join('');
}
...
function viewHistoryDriverInner(orders){
  ...
  return orders.map(o=>{
    const stColor = o.status==='completed' ? theme.primary : o.status==='cancelled' ? '#ef4444' : '#f59e0b';
    const date = new Date(o.created_at).toLocaleString('id-ID',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    return `<div style="border:1px solid #263240;border-radius:14px;padding:12px;background:${o.status==='completed'?'#0f1a14':'#151c25'};margin-bottom:10px">
      <div style="display:flex;justify-content:space-between">
        <span style="font-size:11px;color:#8aa0b8">${date} • ${o.passenger?.name||'Penumpang'}</span>
        <span style="background:${stColor};color:white;padding:3px 8px;border-radius:8px;font-size:10px;font-weight:800">${o.status.toUpperCase()}</span>
      </div>
      <div style="margin-top:8px;font-size:13px;color:#e6edf5">
        <div>📍 ${o.pickup_text||o.pickup||''}</div>
        <div style="margin-top:2px">🎯 ${o.dest_text||o.destination||''}</div>
      </div>
      <div style="margin-top:10px;display:flex;justify-content:space-between">
        <span style="font-size:11px;color:#8aa0b8">📏 ${o.distance_km?.toFixed?o.distance_km.toFixed(1):'-'} km • ${o.vehicle_type||'motor'}</span>
...
export function viewHistoryPassanger(orders){
  ...
  currentPage = 1;
  if(!orders || orders.length===0){
    return `<div class="card" style="text-align:center;padding:24px"><div style="font-size:32px">📭</div><div style="margin-top:8px;font-weight:700">Belum ada histori order</div><div style="font-size:11px;color:#8aa0b8">Order selesai akan muncul di sini</div></div>`;
  }
...
export function viewHistoryDriver(orders){
  ...
  currentPage = 1;
  if(!orders || orders.length===0){
    return `<div class="card" style="text-align:center;padding:24px"><div style="font-size:32px">📭</div><div style="margin-top:8px;font-weight:700">Belum ada pengantaran</div><div style="font-size:11px;color:#8aa0b8">Order yang kamu selesaikan muncul di sini</div></div>`;
  }
