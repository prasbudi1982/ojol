...
function renderPaginatedHistory(){
  ...
  if(!pagEl) return;
  if(total <= perPage){
    pagEl.innerHTML = `<div style="text-align:center;font-size:11px;color:#8aa0b8;margin-top:10px">${total} order • 5/halaman</div>`;
    return;
  }
  let btns = '';
  for(let i=1;i<=totalPages;i++){
    const active = i===currentPage;
    btns += `<button data-history-page="${i}" style="min-width:36px;padding:8px 10px;border-radius:10px;border:1px solid ${active?theme.primary:'#263240'};background:${active?theme.primary:'#151c25'};color:${active?'#052e16':'#e6edf5'};font-weight:800;font-size:12px">${i}</button>`;
  }
  pagEl.innerHTML = `
    <div style="display:flex;gap:6px;justify-content:center;align-items:center;margin-top:16px;flex-wrap:wrap">
      <button data-history-page="prev" style="padding:8px 14px;border-radius:10px;border:1px solid #263240;background:#151c25;color:#e6edf5;font-size:11px;font-weight:700" ${currentPage===1?'disabled style="opacity:0.4"':''}>‹ Prev</button>
      ${btns}
      <button data-history-page="next" style="padding:8px 14px;border-radius:10px;border:1px solid #263240;background:#151c25;color:#e6edf5;font-size:11px;font-weight:700" ${currentPage===totalPages?'disabled style="opacity:0.4"':''}>Next ›</button>
    </div>
    <div style="text-align:center;font-size:10px;color:#8aa0b8;margin-top:8px">Hal ${currentPage} dari ${totalPages} • ${total} total • 5/hal</div>
  `;
}
...
function viewHistoryPassangerInner(orders){
  ...
  return orders.map(o=>{
    ...
    return `<div style="border:1px solid #e2e8f0;border-radius:12px;padding:12px;background:#151c25;border:1px solid #263240;margin-bottom:10px">
