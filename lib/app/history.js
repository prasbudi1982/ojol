// history.js - Histori order + Pagination 5 per halaman, max 20 fetch + FIX rating hilang setelah rated
...
function isOrderRated(orderId){
  try{
    if(localStorage.getItem('rated_'+orderId)) return true;
    const local = JSON.parse(localStorage.getItem('local_ratings')||'[]');
    if(local.some(r=>r.order_id===orderId)) return true;
  }catch(e){}
  return false;
}

function getLocalRating(orderId){
  try{
    const local = JSON.parse(localStorage.getItem('local_ratings')||'[]');
    const found = local.find(r=>r.order_id===orderId);
    return found||null;
  }catch(e){ return null; }
}
...
export async function getPassengerHistory(passengerId, limit=20){
  ...
  // FIX: cek rating dari DB biar tombol hilang
  try{
    const orderIds = orders.map(o=>o.id);
    const { data: ratings, error: rErr } = await supabase.from('ratings').select('order_id, rating, created_at').in('order_id', orderIds);
    if(!rErr && ratings){
      const ratedMap = {};
      ratings.forEach(r=>{ ratedMap[r.order_id]=r; });
      orders.forEach(o=>{
        if(ratedMap[o.id]){
          o._rated = true;
          o._ratingData = ratedMap[o.id];
          try{ localStorage.setItem('rated_'+o.id, '1'); }catch(e){}
        } else if(isOrderRated(o.id)){
          o._rated = true;
          o._ratingData = getLocalRating(o.id);
        }
      });
    } else {
      // fallback localStorage only
      orders.forEach(o=>{
        if(isOrderRated(o.id)){
          o._rated = true;
          o._ratingData = getLocalRating(o.id);
        }
      });
...
function viewHistoryPassangerInner(orders){
  ...
  return orders.map(o=>{
    ...
    // FIX RATING: cek sudah dirating
    const alreadyRated = o._rated || isOrderRated(o.id);
    const ratingInfo = o._ratingData;
    let ratingBadge = '';
    if(alreadyRated){
      const star = ratingInfo?.rating ? `⭐ ${ratingInfo.rating}/5` : '⭐ Sudah dirating';
      ratingBadge = `<div style="margin-top:10px;display:flex;gap:6px;align-items:center"><span style="background:#dcfce7;color:#166534;border:1px solid #bbf7d0;padding:6px 10px;border-radius:10px;font-size:11px;font-weight:700">✅ ${star}</span><span style="font-size:10px;color:var(--muted)">Terima kasih!</span></div>`;
    }
    return `<div style="border:1px solid var(--border);border-radius:14px;padding:12px;background:var(--card);margin-bottom:10px">
      ...
      ${o.status==='completed' && o.driver_id ? (
        alreadyRated ? ratingBadge : `<div style="margin-top:10px;display:flex;gap:6px">
        <button data-rate-driver="${o.driver_id}" data-rate-order="${o.id}" data-rate-name="${o.driver?.name||'Driver'}" style="flex:1;background:${theme.primary};color:#052e16;border:none;padding:10px;border-radius:10px;font-size:11px;font-weight:800;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.1)">⭐ Beri Rating</button>
        <a href="https://www.google.com/maps/dir/${o.pickup_lat||''},${o.pickup_lng||''}/${o.dest_lat||''},${o.dest_lng||''}" target="_blank" style="background:var(--card2);border:1px solid var(--border);padding:10px 12px;border-radius:10px;font-size:11px;text-decoration:none;color:var(--text)">🗺️</a>
      </div>`
      ) : ''}
    </div>`;
  }).join('');
...
if(typeof document !== 'undefined'){
  ...
  // FIX: listen event rating selesai biar history auto refresh tanpa tombol
  document.addEventListener('rating:completed', (e)=>{
    const orderId = e.detail?.orderId;
    if(orderId){
      try{
        // update currentOrders cache
        currentOrders.forEach(o=>{
          if(o.id===orderId){ o._rated=true; o._ratingData={rating:e.detail.rating||5}; }
        });
      }catch(err){}
      setTimeout(()=> renderPaginatedHistory(), 300);
    }
  });
}
